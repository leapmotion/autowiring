...
TEST_F(DispatchQueueTest, PathologicalStartAndStop){
  ...
  // do so _eventually_.
  t1->Stop(true);
  EXPECT_TRUE(t1->WaitFor(boost::chrono::seconds(10)));
  t2->Stop(true);
  EXPECT_TRUE(t2->WaitFor(boost::chrono::seconds(10)));
  t3->Stop(true);
  EXPECT_TRUE(t3->WaitFor(boost::chrono::seconds(10)));
  t4->Stop(true);
  EXPECT_TRUE(t4->WaitFor(boost::chrono::seconds(10)));
}
