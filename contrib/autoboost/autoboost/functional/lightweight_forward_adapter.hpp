...
#ifndef AUTOBOOST_FUNCTIONAL_LIGHTWEIGHT_FORWARD_ADAPTER_HPP_INCLUDED
...
#   ifndef AUTOBOOST_FUNCTIONAL_LIGHTWEIGHT_FORWARD_ADAPTER_MAX_ARITY
#     define AUTOBOOST_FUNCTIONAL_LIGHTWEIGHT_FORWARD_ADAPTER_MAX_ARITY 10
#   elif AUTOBOOST_FUNCTIONAL_FORDWARD_ADAPTER_MAX_ARITY < 3
#     undef  AUTOBOOST_FUNCTIONAL_LIGHTWEIGHT_FORWARD_ADAPTER_MAX_ARITY
#     define AUTOBOOST_FUNCTIONAL_LIGHTWEIGHT_FORWARD_ADAPTER_MAX_ARITY 3
#   endif
...
namespace autoboost
{
    ...
    namespace detail
    {
        ...
        struct lightweight_forward_adapter_result
        {
            ...
            template< typename T > struct x< autoboost::reference_wrapper<T> >
            { typedef T& t; };
...
#   define AUTOBOOST_TMP_MACRO(f,fn,fc) \
    ...
    namespace detail
    {
        ...
        struct lightweight_forward_adapter_impl_zero<MD, F, FC,
            typename lightweight_forward_adapter_sfinae<typename autoboost::result_of< FC() >::type>::type>
            : lightweight_forward_adapter_result
        {
...
#       define  AUTOBOOST_PP_FILENAME_1 \
            <autoboost/functional/lightweight_forward_adapter.hpp>
#       define  AUTOBOOST_PP_ITERATION_LIMITS                                     \
            (1,AUTOBOOST_FUNCTIONAL_LIGHTWEIGHT_FORWARD_ADAPTER_MAX_ARITY)
#       include AUTOBOOST_PP_ITERATE()

    } // namespace detail

    template<class F, int A0, int A1>
    struct result_of<autoboost::lightweight_forward_adapter<F,A0,A1> const ()>
        : autoboost::detail::lightweight_forward_adapter_result::template apply<
            autoboost::lightweight_forward_adapter<F,A0,A1> const () >
    { };
    template<class F, int A0, int A1>
    struct result_of<autoboost::lightweight_forward_adapter<F,A0,A1>()>
        : autoboost::detail::lightweight_forward_adapter_result::template apply<
            autoboost::lightweight_forward_adapter<F,A0,A1>() >
    { };
    template<class F, int A0, int A1>
    struct result_of<autoboost::lightweight_forward_adapter<F,A0,A1> const& ()>
        : autoboost::detail::lightweight_forward_adapter_result::template apply<
...
#     define AUTOBOOST_FUNCTIONAL_LIGHTWEIGHT_FORWARD_ADAPTER_HPP_INCLUDED
...
#     define N AUTOBOOST_PP_ITERATION()
        ...
        struct lightweight_forward_adapter_result::apply<
            Self (AUTOBOOST_PP_ENUM_PARAMS(N,T)) >
            : autoboost::result_of<
                AUTOBOOST_DEDUCED_TYPENAME c<Self>::t (AUTOBOOST_PP_ENUM_BINARY_PARAMS(N,
                    typename x<T,>::t AUTOBOOST_PP_INTERCEPT)) >
        { };

        template< class MD, class F, class FC >
        struct lightweight_forward_adapter_impl<MD,F,FC,AUTOBOOST_PP_DEC(N),N>
            : lightweight_forward_adapter_result
        {
            template< AUTOBOOST_PP_ENUM_PARAMS(N,typename T) >
            inline typename autoboost::result_of< F(AUTOBOOST_PP_ENUM_BINARY_PARAMS(N,
                T,const& AUTOBOOST_PP_INTERCEPT)) >::type
        ...
        struct lightweight_forward_adapter_impl<MD,F,FC,N,MinArity>
            : lightweight_forward_adapter_impl<MD,F,FC,AUTOBOOST_PP_DEC(N),MinArity>
        {
            using lightweight_forward_adapter_impl<MD,F,FC,AUTOBOOST_PP_DEC(N),
                MinArity>::operator();
...
#     define M(z,i,d) \
          static_cast<typename d::template x<T##i>::t>(a##i)
            ...
            inline typename lightweight_forward_adapter_result::template apply<
                MD const (AUTOBOOST_PP_ENUM_BINARY_PARAMS(N,
                    T,const& AUTOBOOST_PP_INTERCEPT)) >::type
            operator()(AUTOBOOST_PP_ENUM_BINARY_PARAMS(N,T,const& a)) const
            {
                typedef lightweight_forward_adapter_result _;
                return static_cast<MD const*>(this)->target_function()(
                    AUTOBOOST_PP_ENUM(N,M,_));
            }
            template< AUTOBOOST_PP_ENUM_PARAMS(N,typename T) >
            inline typename lightweight_forward_adapter_result::template apply<
                MD (AUTOBOOST_PP_ENUM_BINARY_PARAMS(N,
                    T,const& AUTOBOOST_PP_INTERCEPT)) >::type
            operator()(AUTOBOOST_PP_ENUM_BINARY_PARAMS(N,T,const& a))
            {
                typedef lightweight_forward_adapter_result _;
                return static_cast<MD*>(this)->target_function()(
                    AUTOBOOST_PP_ENUM(N,M,_));
