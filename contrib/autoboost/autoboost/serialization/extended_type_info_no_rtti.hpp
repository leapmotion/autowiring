#ifndef AUTOBOOST_EXTENDED_TYPE_INFO_NO_RTTI_HPP
#define AUTOBOOST_EXTENDED_TYPE_INFO_NO_RTTI_HPP
...
#include <autoboost/serialization/factory.hpp>
...
#include <autoboost/config/abi_prefix.hpp> // must be the last header
...
class AUTOBOOST_SERIALIZATION_DECL(AUTOBOOST_PP_EMPTY()) extended_type_info_no_rtti_0 : 
...
public:
    virtual bool
    is_less_than(const autoboost::serialization::extended_type_info &rhs) const ;
    virtual bool
    is_equal(const autoboost::serialization::extended_type_info &rhs) const ;
};
    ...
    public singleton<extended_type_info_no_rtti< T > >
{
    ...
    struct action {
        ...
        struct undefined {
            ...
            // as pointers.
            AUTOBOOST_STATIC_ASSERT(0 == sizeof(T));
            static const char * invoke();
        };
        static const char * invoke(){
            typedef 
                typename autoboost::mpl::if_c<
                    tf,
                    defined,
                    undefined
                >::type type;
...
public:
    ...
    get_derived_extended_type_info(const T & t) const {
        ...
        AUTOBOOST_STATIC_WARNING(autoboost::is_polymorphic< T >::value);
        const char * derived_key = t.get_key();
        AUTOBOOST_ASSERT(NULL != derived_key);
        return autoboost::serialization::extended_type_info::find(derived_key);
    }
    ...
    virtual void * construct(unsigned int count, ...) const{
        ...
        switch(count){
        case 0:
            return factory<typename autoboost::remove_const< T >::type, 0>(ap);
        case 1:
            return factory<typename autoboost::remove_const< T >::type, 1>(ap);
        case 2:
            return factory<typename autoboost::remove_const< T >::type, 2>(ap);
        case 3:
            return factory<typename autoboost::remove_const< T >::type, 3>(ap);
        case 4:
            return factory<typename autoboost::remove_const< T >::type, 4>(ap);
        default:
            AUTOBOOST_ASSERT(false); // too many arguments
            // throw exception here?
            return NULL;
        }
    }
    virtual void destroy(void const * const p) const{
        autoboost::serialization::access::destroy(
...
// before any of the other headers.
    ...
    #define AUTOBOOST_SERIALIZATION_DEFAULT_TYPE_INFO
    namespace autoboost {
    namespace serialization {
    template<class T>
    struct extended_type_info_impl {
        typedef typename 
            autoboost::serialization::extended_type_info_no_rtti< T > type;
    };
    } // namespace serialization
    } // namespace autoboost
#endif
...
#  pragma warning(pop)
#endif
#include <autoboost/config/abi_suffix.hpp> // pops abi_suffix.hpp pragmas

#endif // AUTOBOOST_EXTENDED_TYPE_INFO_NO_RTTI_HPP
