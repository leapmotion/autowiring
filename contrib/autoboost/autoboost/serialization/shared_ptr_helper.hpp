#ifndef AUTOBOOST_SERIALIZATION_SHARED_PTR_HELPER_HPP
#define AUTOBOOST_SERIALIZATION_SHARED_PTR_HELPER_HPP
...
#include <autoboost/archive/detail/abi_prefix.hpp> // must be the last headern

namespace autoboost_132 {
...
#if defined(AUTOBOOST_NO_MEMBER_TEMPLATE_FRIENDS) || defined(AUTOBOOST_MSVC)
public:
#else
    template<class Archive, class U>
    friend void autoboost::serialization::load(
        Archive & ar,
        SPT< U > &t,
        const unsigned int file_version
    );
    ...
    std::list<autoboost_132::shared_ptr<const void> > * m_pointers_132;
    AUTOBOOST_ARCHIVE_DECL(void)
    append(const autoboost_132::shared_ptr<const void> & t){
    ...
    struct non_polymorphic {
        template<class U>
        static const autoboost::serialization::extended_type_info * 
        get_object_type(U & ){
            return & autoboost::serialization::singleton<
                typename 
                autoboost::serialization::type_info_implementation< U >::type
            >::get_const_instance();
        }
    };
    struct polymorphic {
...
public:
    ...
    void reset(SPT< T > & s, T * t){
        ...
        const autoboost::serialization::extended_type_info * this_type
        ...
        if(NULL == true_type)
            autoboost::serialization::throw_exception(
                autoboost::archive::archive_exception(
                    autoboost::archive::archive_exception::unregistered_class,
        ...
        if(NULL == oid)
            autoboost::serialization::throw_exception(
                autoboost::archive::archive_exception(
                    autoboost::archive::archive_exception::unregistered_cast,
        ...
        if(i == m_o_sp->end()){
            ...
            AUTOBOOST_ASSERT(result.second);
        }
...
#endif // AUTOBOOST_SERIALIZATION_SHARED_PTR_HELPER_HPP
