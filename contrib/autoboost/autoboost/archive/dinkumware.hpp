#ifndef AUTOBOOST_ARCHIVE_DINKUMWARE_HPP
#define AUTOBOOST_ARCHIVE_DINKUMWARE_HPP
...
operator<<(basic_ostream<CharType> & os, autoboost::uint64_t t){
...
template<class CharType>
basic_ostream<CharType> &
operator<<(basic_ostream<CharType> &os, autoboost::int64_t t){
    if(0 <= t){
        os << static_cast<autoboost::uint64_t>(t);
    }
    else{
        os.put('-');
        os << -t;
...
operator>>(basic_istream<CharType> &is, autoboost::int64_t & t){
    CharType d;
    do{
        d = is.get();
    }
...
template<class CharType>
basic_istream<CharType> &
operator>>(basic_istream<CharType> &is, autoboost::uint64_t & t){
    autoboost::int64_t it;
    is >> it;
    t = it;
    return is;
}
...
#endif //AUTOBOOST_ARCHIVE_DINKUMWARE_HPP
