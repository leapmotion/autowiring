#ifndef AUTOBOOST_ARCHIVE_ITERATORS_WCHAR_FROM_MB_HPP
#define AUTOBOOST_ARCHIVE_ITERATORS_WCHAR_FROM_MB_HPP
    ...
    >
{
    ...
    autoboost::archive::detail::utf8_codecvt_facet m_codecvt_facet;
    std::mbstate_t m_mbs;

    template<typename T>
    struct sliding_buffer {
        autoboost::array<T, 32> m_buffer;
        typename autoboost::array<T, 32>::const_iterator m_next_available;
        typename autoboost::array<T, 32>::iterator m_next;
        bool m_done;
        // default ctor
        sliding_buffer() :
...
public:
    ...
    wchar_from_mb(T start) :
        ...
        m_mbs(std::mbstate_t())
    {
        AUTOBOOST_ASSERT(std::mbsinit(&m_mbs));
        drain();
    }
...
void wchar_from_mb<Base>::drain(){
    AUTOBOOST_ASSERT(! m_input.m_done);
    for(;;){
        typename autoboost::iterators::iterator_reference<Base>::type c = *(this->base_reference());
    ...
    const typename autoboost::iterators::iterator_value<Base>::type * input_new_start;
    ...
    AUTOBOOST_ASSERT(std::codecvt_base::ok == r);
...
#endif // AUTOBOOST_ARCHIVE_ITERATORS_WCHAR_FROM_MB_HPP
