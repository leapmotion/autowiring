#ifndef AUTOBOOST_NEW_ITERATOR_TESTS_HPP
# define AUTOBOOST_NEW_ITERATOR_TESTS_HPP
...
# include <autoboost/concept_archetype.hpp> // for detail::dummy_constructor
# include <autoboost/detail/iterator.hpp>
# include <autoboost/pending/iterator_tests.hpp>
# include <autoboost/iterator/is_readable_iterator.hpp>
# include <autoboost/iterator/is_lvalue_iterator.hpp>
...
# if !AUTOBOOST_WORKAROUND(__MWERKS__, <= 0x2407)
  ...
  // the above code.
  AUTOBOOST_STATIC_ASSERT(is_readable_iterator<Iterator>::value);
# endif
}
...
void constant_lvalue_iterator_test(Iterator i, T v1)
{
  ...
  AUTOBOOST_STATIC_ASSERT((is_same<const value_type&, reference>::value));
  const T& v2 = *i2;
  AUTOBOOST_TEST(v1 == v2);
# ifndef AUTOBOOST_NO_LVALUE_RETURN_DETECTION
  AUTOBOOST_STATIC_ASSERT(is_lvalue_iterator<Iterator>::value);
  AUTOBOOST_STATIC_ASSERT(!is_non_const_lvalue_iterator<Iterator>::value);
# endif
}
...
void non_const_lvalue_iterator_test(Iterator i, T v1, T v2)
{
  ...
  typedef typename detail::iterator_traits<Iterator>::value_type value_type;
  typedef typename detail::iterator_traits<Iterator>::reference reference;
  AUTOBOOST_STATIC_ASSERT((is_same<value_type&, reference>::value));
  T& v3 = *i2;
  AUTOBOOST_TEST(v1 == v3);
...
void random_access_readable_iterator_test(Iterator i, int N, TrueVals vals)
{
  ...
  for (c = 0; c < N-1; ++c)
  {
    AUTOBOOST_TEST(i == j + c);
    AUTOBOOST_TEST(*i == vals[c]);
    typename detail::iterator_traits<Iterator>::value_type x = j[c];
    AUTOBOOST_TEST(*i == x);
    AUTOBOOST_TEST(*i == *(j + c));
    AUTOBOOST_TEST(*i == *(c + j));
    ++i;
    AUTOBOOST_TEST(i > j);
    AUTOBOOST_TEST(i >= j);
    AUTOBOOST_TEST(j <= i);
  ...
  for (c = 0; c < N-1; ++c)
  {
    AUTOBOOST_TEST(i == k - c);
    AUTOBOOST_TEST(*i == vals[N - 1 - c]);
...
#endif // AUTOBOOST_NEW_ITERATOR_TESTS_HPP
