...
#ifndef AUTOBOOST_ATOMIC_DETAIL_OPS_MSVC_X86_HPP_INCLUDED_
#define AUTOBOOST_ATOMIC_DETAIL_OPS_MSVC_X86_HPP_INCLUDED_
...
#if !defined(_M_IX86) && !(defined(AUTOBOOST_ATOMIC_INTERLOCKED_COMPARE_EXCHANGE8) && defined(AUTOBOOST_ATOMIC_INTERLOCKED_COMPARE_EXCHANGE16))
...
#if defined(AUTOBOOST_ATOMIC_DETAIL_X86_HAS_MFENCE)
...
struct msvc_x86_operations_base
{
    ...
    static AUTOBOOST_FORCEINLINE void fence_after_load(memory_order) AUTOBOOST_NOEXCEPT
    {
        AUTOBOOST_ATOMIC_DETAIL_COMPILER_BARRIER();
    ...
    public msvc_x86_operations< typename make_storage_type< 4u, Signed >::type, operations< 4u, Signed > >
{
    ...
    {
        return static_cast< storage_type >(AUTOBOOST_ATOMIC_INTERLOCKED_EXCHANGE_ADD(&storage, v));
    }
    ...
    {
        return static_cast< storage_type >(AUTOBOOST_ATOMIC_INTERLOCKED_EXCHANGE(&storage, v));
    }
    ...
    {
        return static_cast< storage_type >(AUTOBOOST_ATOMIC_INTERLOCKED_AND(&storage, v));
    }
#else
    ...
    {
        return static_cast< storage_type >(AUTOBOOST_ATOMIC_INTERLOCKED_OR(&storage, v));
    }
#else
    ...
    {
        return static_cast< storage_type >(AUTOBOOST_ATOMIC_INTERLOCKED_XOR(&storage, v));
    }
#else
...
#if defined(AUTOBOOST_ATOMIC_INTERLOCKED_COMPARE_EXCHANGE8)
    ...
    public msvc_x86_operations< typename make_storage_type< 1u, Signed >::type, operations< 1u, Signed > >
{
    ...
    static AUTOBOOST_FORCEINLINE storage_type fetch_add(storage_type volatile& storage, storage_type v, memory_order) AUTOBOOST_NOEXCEPT
    {
        return static_cast< storage_type >(AUTOBOOST_ATOMIC_INTERLOCKED_EXCHANGE_ADD8(&storage, v));
    }

    static AUTOBOOST_FORCEINLINE storage_type exchange(storage_type volatile& storage, storage_type v, memory_order) AUTOBOOST_NOEXCEPT
    {
        return static_cast< storage_type >(AUTOBOOST_ATOMIC_INTERLOCKED_EXCHANGE8(&storage, v));
    }
    ...
    {
        storage_type previous = expected;
        storage_type old_val = static_cast< storage_type >(AUTOBOOST_ATOMIC_INTERLOCKED_COMPARE_EXCHANGE8(&storage, desired, previous));
        expected = old_val;
        return (previous == old_val);
    }
    ...
    static AUTOBOOST_FORCEINLINE storage_type fetch_and(storage_type volatile& storage, storage_type v, memory_order) AUTOBOOST_NOEXCEPT
    {
        return static_cast< storage_type >(AUTOBOOST_ATOMIC_INTERLOCKED_AND8(&storage, v));
    }

    static AUTOBOOST_FORCEINLINE storage_type fetch_or(storage_type volatile& storage, storage_type v, memory_order) AUTOBOOST_NOEXCEPT
    {
        return static_cast< storage_type >(AUTOBOOST_ATOMIC_INTERLOCKED_OR8(&storage, v));
    }

    static AUTOBOOST_FORCEINLINE storage_type fetch_xor(storage_type volatile& storage, storage_type v, memory_order) AUTOBOOST_NOEXCEPT
    {
        return static_cast< storage_type >(AUTOBOOST_ATOMIC_INTERLOCKED_XOR8(&storage, v));
    }
};
    ...
    public msvc_x86_operations< typename make_storage_type< 1u, Signed >::type, operations< 1u, Signed > >
{
    ...
    static AUTOBOOST_FORCEINLINE bool compare_exchange_strong(
        storage_type volatile& storage, storage_type& expected, storage_type desired, memory_order success_order, memory_order) AUTOBOOST_NOEXCEPT
    {
...
#if defined(AUTOBOOST_ATOMIC_INTERLOCKED_COMPARE_EXCHANGE16)
    ...
    public msvc_x86_operations< typename make_storage_type< 2u, Signed >::type, operations< 2u, Signed > >
{
    ...
    static AUTOBOOST_FORCEINLINE storage_type fetch_add(storage_type volatile& storage, storage_type v, memory_order) AUTOBOOST_NOEXCEPT
    {
        return static_cast< storage_type >(AUTOBOOST_ATOMIC_INTERLOCKED_EXCHANGE_ADD16(&storage, v));
    }

    static AUTOBOOST_FORCEINLINE storage_type exchange(storage_type volatile& storage, storage_type v, memory_order) AUTOBOOST_NOEXCEPT
    {
        return static_cast< storage_type >(AUTOBOOST_ATOMIC_INTERLOCKED_EXCHANGE16(&storage, v));
    }
    ...
    {
        storage_type previous = expected;
        storage_type old_val = static_cast< storage_type >(AUTOBOOST_ATOMIC_INTERLOCKED_COMPARE_EXCHANGE16(&storage, desired, previous));
        expected = old_val;
        return (previous == old_val);
    }
    ...
    static AUTOBOOST_FORCEINLINE storage_type fetch_and(storage_type volatile& storage, storage_type v, memory_order) AUTOBOOST_NOEXCEPT
    {
        return static_cast< storage_type >(AUTOBOOST_ATOMIC_INTERLOCKED_AND16(&storage, v));
    }

    static AUTOBOOST_FORCEINLINE storage_type fetch_or(storage_type volatile& storage, storage_type v, memory_order) AUTOBOOST_NOEXCEPT
    {
        return static_cast< storage_type >(AUTOBOOST_ATOMIC_INTERLOCKED_OR16(&storage, v));
    }

    static AUTOBOOST_FORCEINLINE storage_type fetch_xor(storage_type volatile& storage, storage_type v, memory_order) AUTOBOOST_NOEXCEPT
    {
        return static_cast< storage_type >(AUTOBOOST_ATOMIC_INTERLOCKED_XOR16(&storage, v));
    }
};
...
struct msvc_dcas_x86
{
        ...
        storage_type volatile& storage, storage_type& expected, storage_type desired, memory_order, memory_order) AUTOBOOST_NOEXCEPT
    {
        ...
        storage_type volatile* p = &storage;
#if defined(AUTOBOOST_ATOMIC_INTERLOCKED_COMPARE_EXCHANGE64)
        const storage_type old_val = (storage_type)AUTOBOOST_ATOMIC_INTERLOCKED_COMPARE_EXCHANGE64(p, desired, expected);
    ...
    public msvc_x86_operations< typename make_storage_type< 8u, Signed >::type, operations< 8u, Signed > >
{
    ...
    static AUTOBOOST_FORCEINLINE storage_type fetch_add(storage_type volatile& storage, storage_type v, memory_order) AUTOBOOST_NOEXCEPT
    {
        return static_cast< storage_type >(AUTOBOOST_ATOMIC_INTERLOCKED_EXCHANGE_ADD64(&storage, v));
    }

    static AUTOBOOST_FORCEINLINE storage_type exchange(storage_type volatile& storage, storage_type v, memory_order) AUTOBOOST_NOEXCEPT
    {
        return static_cast< storage_type >(AUTOBOOST_ATOMIC_INTERLOCKED_EXCHANGE64(&storage, v));
    }
    ...
    {
        storage_type previous = expected;
        storage_type old_val = static_cast< storage_type >(AUTOBOOST_ATOMIC_INTERLOCKED_COMPARE_EXCHANGE64(&storage, desired, previous));
        expected = old_val;
        return (previous == old_val);
    }
    ...
    static AUTOBOOST_FORCEINLINE storage_type fetch_and(storage_type volatile& storage, storage_type v, memory_order) AUTOBOOST_NOEXCEPT
    {
        return static_cast< storage_type >(AUTOBOOST_ATOMIC_INTERLOCKED_AND64(&storage, v));
    }

    static AUTOBOOST_FORCEINLINE storage_type fetch_or(storage_type volatile& storage, storage_type v, memory_order) AUTOBOOST_NOEXCEPT
    {
        return static_cast< storage_type >(AUTOBOOST_ATOMIC_INTERLOCKED_OR64(&storage, v));
    }

    static AUTOBOOST_FORCEINLINE storage_type fetch_xor(storage_type volatile& storage, storage_type v, memory_order) AUTOBOOST_NOEXCEPT
    {
        return static_cast< storage_type >(AUTOBOOST_ATOMIC_INTERLOCKED_XOR64(&storage, v));
    }
};
...
struct msvc_dcas_x86_64
{
    ...
    static AUTOBOOST_FORCEINLINE void store(storage_type volatile& storage, storage_type v, memory_order) AUTOBOOST_NOEXCEPT
    {
        ...
        while (!AUTOBOOST_ATOMIC_INTERLOCKED_COMPARE_EXCHANGE128(&storage, v, &value)) {}
    }

    static AUTOBOOST_FORCEINLINE storage_type load(storage_type const volatile& storage, memory_order) AUTOBOOST_NOEXCEPT
    {
        storage_type value = storage_type();
        AUTOBOOST_ATOMIC_INTERLOCKED_COMPARE_EXCHANGE128(&storage, value, &value);
        return value;
    }
    ...
    static AUTOBOOST_FORCEINLINE bool compare_exchange_strong(
        storage_type volatile& storage, storage_type& expected, storage_type desired, memory_order, memory_order) AUTOBOOST_NOEXCEPT
    {
        return !!AUTOBOOST_ATOMIC_INTERLOCKED_COMPARE_EXCHANGE128(&storage, desired, &expected);
    }
...
#endif // AUTOBOOST_ATOMIC_DETAIL_OPS_MSVC_X86_HPP_INCLUDED_
