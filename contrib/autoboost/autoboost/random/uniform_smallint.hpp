/* autoboost random/uniform_smallint.hpp header file
 *
...
#ifndef AUTOBOOST_RANDOM_UNIFORM_SMALLINT_HPP
#define AUTOBOOST_RANDOM_UNIFORM_SMALLINT_HPP
...
public:
    ...
    /** Returns the minimum value of the distribution. */
    result_type min AUTOBOOST_PREVENT_MACRO_SUBSTITUTION () const { return _min; }
    /** Returns the maximum value of the distribution. */
    result_type max AUTOBOOST_PREVENT_MACRO_SUBSTITUTION () const { return _max; }
    ...
    result_type operator()(Engine& eng) const
    {
        ...
        return generate(eng, autoboost::random::traits::is_integral<base_result>());
    }
    ...
    AUTOBOOST_RANDOM_DETAIL_OSTREAM_OPERATOR(os, uniform_smallint, ud)
    {
        os << ud._min << " " << ud._max;
        return os;
    }
    ...
    AUTOBOOST_RANDOM_DETAIL_ISTREAM_OPERATOR(is, uniform_smallint, ud)
    {
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_EQUALITY_OPERATOR(uniform_smallint, lhs, rhs)
    { return lhs._min == rhs._min && lhs._max == rhs._max; }
     ...
     * sequences of values given equal generators.
     */
    AUTOBOOST_RANDOM_DETAIL_INEQUALITY_OPERATOR(uniform_smallint)

private:

    // \cond show_private
    template<class Engine>
    result_type generate(Engine& eng, autoboost::mpl::true_) const
    {
        // equivalent to (eng() - eng.min()) % (_max - _min + 1) + _min,
        // but guarantees no overflow.
        typedef typename Engine::result_type base_result;
        typedef typename autoboost::random::traits::make_unsigned<base_result>::type base_unsigned;
        typedef typename autoboost::random::traits::make_unsigned_or_unbounded<result_type>::type range_type;
#ifdef AUTOBOOST_NO_CXX11_EXPLICIT_CONVERSION_OPERATORS
        ...
        if(range >= base_range) {
            return autoboost::random::detail::add<range_type, result_type>()(
    ...
    template<class Engine>
    result_type generate(Engine& eng, autoboost::mpl::false_) const
    {
        typedef typename Engine::result_type base_result;
        typedef typename autoboost::random::traits::make_unsigned<result_type>::type range_type;
        range_type range = random::detail::subtract<result_type>()(_max, _min);
        base_result val = autoboost::uniform_01<base_result>()(eng);
        ...
        return autoboost::random::detail::add<range_type, result_type>()(offset , _min);
    }
    // \endcond
...
#endif // AUTOBOOST_RANDOM_UNIFORM_SMALLINT_HPP
