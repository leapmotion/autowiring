/* autoboost random/non_central_chi_squared_distribution.hpp header file
 *
...
#ifndef AUTOBOOST_RANDOM_NON_CENTRAL_CHI_SQUARED_DISTRIBUTION_HPP
#define AUTOBOOST_RANDOM_NON_CENTRAL_CHI_SQUARED_DISTRIBUTION_HPP
...
public:
    ...
    public:
        ...
        {
            AUTOBOOST_ASSERT(k_arg > RealType(0));
            AUTOBOOST_ASSERT(lambda_arg > RealType(0));
        }
    ...
    RealType operator()(URNG& eng)
    {
        using std::sqrt;
        if (_param.k() > 1) {
            autoboost::random::normal_distribution<RealType> n_dist;
            autoboost::random::chi_squared_distribution<RealType> c_dist(_param.k() - RealType(1));
            ...
            autoboost::random::poisson_distribution<> p_dist(_param.lambda()/RealType(2));
            autoboost::random::poisson_distribution<>::result_type _p = p_dist(eng);
            autoboost::random::chi_squared_distribution<RealType> c_dist(_param.k() + RealType(2)*_p);
            return c_dist(eng);
        }
    }
    ...
    RealType min AUTOBOOST_PREVENT_MACRO_SUBSTITUTION() const
    { return RealType(0); }

    /** Returns the largest value that the distribution can produce. */
    RealType max AUTOBOOST_PREVENT_MACRO_SUBSTITUTION() const
    { return (std::numeric_limits<RealType>::infinity)(); }

    /** Writes the parameters of the distribution to a @c std::ostream. */
    AUTOBOOST_RANDOM_DETAIL_OSTREAM_OPERATOR(os, non_central_chi_squared_distribution, dist)
    {
        os << dist.param();
        return os;
    }
    ...
    /** reads the parameters of the distribution from a @c std::istream. */
    AUTOBOOST_RANDOM_DETAIL_ISTREAM_OPERATOR(is, non_central_chi_squared_distribution, dist)
    {
        param_type parm;
        if(is >> parm) {
            dist.param(parm);
    ...
    AUTOBOOST_RANDOM_DETAIL_EQUALITY_OPERATOR(non_central_chi_squared_distribution, lhs, rhs)
    ...
    AUTOBOOST_RANDOM_DETAIL_INEQUALITY_OPERATOR(non_central_chi_squared_distribution)
