/* autoboost random/discrete_distribution.hpp header file
 *
...
#ifndef AUTOBOOST_RANDOM_DISCRETE_DISTRIBUTION_HPP_INCLUDED
#define AUTOBOOST_RANDOM_DISCRETE_DISTRIBUTION_HPP_INCLUDED
...
public:
    ...
    public:
        ...
        explicit param_type(const Range& range)
          : _probabilities(autoboost::begin(range), autoboost::end(range))
        {
            normalize();
        }
    ...
    {
        init(autoboost::begin(range), autoboost::end(range));
    }
    /**
    ...
    {
        AUTOBOOST_ASSERT(!_impl._alias_table.empty());
        IntType result;
        WeightType test;
        do {
    ...
    result_type min AUTOBOOST_PREVENT_MACRO_SUBSTITUTION () const { return 0; }
    ...
    AUTOBOOST_RANDOM_DETAIL_OSTREAM_OPERATOR(os, discrete_distribution, dd)
    {
        os << dd.param();
        return os;
    }
    ...
    AUTOBOOST_RANDOM_DETAIL_ISTREAM_OPERATOR(is, discrete_distribution, dd)
    {
        param_type parm;
        if(is >> parm) {
            dd.param(parm);
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_EQUALITY_OPERATOR(discrete_distribution, lhs, rhs)
    {
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_INEQUALITY_OPERATOR(discrete_distribution)
...
private:
    ...
    typedef typename detail::select_alias_table<
        (::autoboost::is_integral<WeightType>::value)
