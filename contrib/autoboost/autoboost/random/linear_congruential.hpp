...
#ifndef AUTOBOOST_RANDOM_LINEAR_CONGRUENTIAL_HPP
#define AUTOBOOST_RANDOM_LINEAR_CONGRUENTIAL_HPP
...
public:
    ...
    AUTOBOOST_STATIC_CONSTANT(IntType, multiplier = a);
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_CONSTRUCTOR(linear_congruential_engine,
                                             SeedSeq, seq)
    { seed(seq); }
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_SEED(linear_congruential_engine, IntType, x0)
    {
        ...
        AUTOBOOST_ASSERT(_x >= (min)());
        AUTOBOOST_ASSERT(_x <= (max)());
    }

    /**
     * Seeds a @c linear_congruential_engine using values from a SeedSeq.
     */
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_SEED(linear_congruential_engine, SeedSeq, seq)
    ...
    void discard(autoboost::uintmax_t z)
    {
        ...
        if(b_gcd == 1) {
            ...
            autoboost::uintmax_t exponent = z;
            while(exponent != 0) {
                if(exponent % 2 == 1) {
                    a_zm1_over_gcd =
                        mod_type::mult_add(
...
public:
     ...
     */
    static uint32_t min AUTOBOOST_PREVENT_MACRO_SUBSTITUTION () { return 0; }
    /**
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(rand48, result_type, x0)
    { seed(x0); }
    /**
     * Seeds the generator with values produced by @c seq.generate().
     */
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_CONSTRUCTOR(rand48, SeedSeq, seq)
    { seed(seq); }
    /**
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_SEED(rand48, result_type, x0)
    { lcf.seed(cnv(x0)); }
    /**
     * Seeds the generator using values from an iterator range,
     * and updates first to point one past the last value consumed.
     ...
     * Seeds the generator with values produced by @c seq.generate().
     */
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_SEED(rand48, SeedSeq, seq)
    { lcf.seed(seq); }
...
private:
    ...
    static autoboost::uint64_t cnv(autoboost::uint32_t x)
