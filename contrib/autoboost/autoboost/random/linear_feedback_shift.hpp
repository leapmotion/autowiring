/* autoboost random/linear_feedback_shift.hpp header file
 *
...
#ifndef AUTOBOOST_RANDOM_LINEAR_FEEDBACK_SHIFT_HPP
#define AUTOBOOST_RANDOM_LINEAR_FEEDBACK_SHIFT_HPP
...
public:
    ...
    AUTOBOOST_STATIC_CONSTANT(int, exponent1 = k);
    AUTOBOOST_STATIC_CONSTANT(int, exponent2 = q);
    AUTOBOOST_STATIC_CONSTANT(int, step_size = s);
    AUTOBOOST_STATIC_CONSTANT(UIntType, default_seed = 341);
    ...
    AUTOBOOST_STATIC_ASSERT(w > 0);
    AUTOBOOST_STATIC_ASSERT(q > 0);
    AUTOBOOST_STATIC_ASSERT(k < w);
    AUTOBOOST_STATIC_ASSERT(0 < 2*q && 2*q < k);
    AUTOBOOST_STATIC_ASSERT(0 < s && s <= k-q);
    ...
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(linear_feedback_shift_engine,
        UIntType, s0)
    { seed(s0); }

    /** Constructs a @c linear_feedback_shift_engine, seeding it with seq. */
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_CONSTRUCTOR(linear_feedback_shift_engine,
        SeedSeq, seq)
    { seed(seq); }
    ...
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_SEED(linear_feedback_shift_engine,
        UIntType, s0)
    {
        value = s0 & wordmask();
        if(value < (1 << (w-k))) {
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_SEED(linear_feedback_shift_engine,
        SeedSeq, seq)
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_OSTREAM_OPERATOR(os, linear_feedback_shift_engine, x)
    {
        os << x.value;
        return os;
    }
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_ISTREAM_OPERATOR(is, linear_feedback_shift_engine, x)
    {
        is >> x.value;
        return is;
    }
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_EQUALITY_OPERATOR(linear_feedback_shift_engine, x, y)
     ...
     * sequences of outputs.
     */
    AUTOBOOST_RANDOM_DETAIL_INEQUALITY_OPERATOR(linear_feedback_shift_engine)

private:
    /// \cond show_private
    static UIntType wordmask() { return autoboost::low_bits_mask_t<w>::sig_bits; }
    /// \endcond
    UIntType value;
};
...
public:
    ...
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_CONSTRUCTOR(linear_feedback_shift,
        SeedSeq, seq)
    { seed(seq); }
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(linear_feedback_shift,
        UIntType, val)
    { seed(val); }
    template<class It>
...
#endif // AUTOBOOST_RANDOM_LINEAR_FEEDBACK_SHIFT_HPP
