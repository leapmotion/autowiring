/* autoboost random/shuffle_order.hpp header file
 *
...
#ifndef AUTOBOOST_RANDOM_SHUFFLE_ORDER_HPP
#define AUTOBOOST_RANDOM_SHUFFLE_ORDER_HPP
...
public:
    ...
    AUTOBOOST_STATIC_CONSTANT(std::size_t, buffer_size = k);
    AUTOBOOST_STATIC_CONSTANT(std::size_t, table_size = k);

    AUTOBOOST_STATIC_ASSERT(std::numeric_limits<result_type>::is_integer);
    AUTOBOOST_STATIC_ASSERT(k > 0);
     ...
     * Complexity: Exactly k+1 invocations of the base generator.
     */
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(shuffle_order_engine,
                                               result_type, s)
    { _rng.seed(s); init(); }
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_CONSTRUCTOR(shuffle_order_engine, SeedSeq, seq)
    { _rng.seed(seq); init(); }
    /**
     ...
     * Complexity: Exactly k+1 invocations of the base generator.
     */
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_SEED(shuffle_order_engine,
     ...
     * Complexity: Exactly k+1 invocations of the base generator.
     */
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_SEED(shuffle_order_engine, SeedSeq, seq)
    { _rng.seed(seq); init(); }
    ...
    result_type operator()() {
        ...
        typedef typename autoboost::random::traits::make_unsigned<result_type>::type base_unsigned;
        ...
        if(k == 1) {
            ...
            autoboost::uintmax_t divisor =
                static_cast<autoboost::uintmax_t>(brange) + 1;
    ...
    AUTOBOOST_RANDOM_DETAIL_OSTREAM_OPERATOR(os, shuffle_order_engine, s)
    {
        os << s._rng;
    ...
    AUTOBOOST_RANDOM_DETAIL_ISTREAM_OPERATOR(is, shuffle_order_engine, s)
    {
        is >> s._rng;
        for(std::size_t i = 0; i < k; ++i)
            is >> std::ws >> s.v[i];
    ...
    AUTOBOOST_RANDOM_DETAIL_EQUALITY_OPERATOR(shuffle_order_engine, x, y)
    { return x._rng == y._rng && x.y == y.y && std::equal(x.v, x.v+k, y.v); }
    /** Returns true if the two generators will produce different sequences. */
    AUTOBOOST_RANDOM_DETAIL_INEQUALITY_OPERATOR(shuffle_order_engine)
...
#endif // AUTOBOOST_RANDOM_SHUFFLE_OUTPUT_HPP
