...
#ifndef AUTOBOOST_RANDOM_DETAIL_SEED_IMPL_HPP
#define AUTOBOOST_RANDOM_DETAIL_SEED_IMPL_HPP
...
#include <autoboost/random/detail/const_mod.hpp>
...
struct seed_type
{
    typedef typename autoboost::mpl::if_<autoboost::is_integral<T>,
        T,
        autoboost::uint32_t
    >::type type;
};
...
void generate_from_real(Engine& eng, Iter begin, Iter end)
{
    ...
    while(true) {
        ...
        if(Bits < 32 && available_bits < 32 - remaining_bits) {
            saved_bits |= autoboost::uint_least32_t(val) << remaining_bits;
            remaining_bits += Bits;
        } else {
            ...
            if(Bits < 32 || remaining_bits != 0) {
                autoboost::uint_least32_t divisor =
                    (autoboost::uint_least32_t(1) << (32 - remaining_bits));
                autoboost::uint_least32_t extra_bits = autoboost::uint_least32_t(fmod(val, mult32)) & (divisor - 1);
                val = val / divisor;
            ...
            if(Bits >= 32) {
                for(; available_bits >= 32; available_bits -= 32) {
                    autoboost::uint_least32_t word = autoboost::uint_least32_t(fmod(val, mult32));
                    val /= mult32;
                    *begin++ = word;
                    if(begin == end) return;
                }
            ...
            saved_bits = static_cast<autoboost::uint_least32_t>(val);
        }
    }
}
...
void generate_from_int(Engine& eng, Iter begin, Iter end)
{
    ...
    while(true) {
        unsigned_type val;
        do {
            val = autoboost::random::detail::subtract<IntType>()(eng(), (eng.min)());
        } while(limit != range && val > limit);
        val &= mask;
        int available_bits = bits;
        if(available_bits == 32) {
            *begin++ = static_cast<autoboost::uint_least32_t>(val) & 0xFFFFFFFFu;
            if(begin == end) return;
            ...
            if(bits < 32 || remaining_bits != 0) {
                autoboost::uint_least32_t extra_bits = autoboost::uint_least32_t(val) & ((autoboost::uint_least32_t(1) << (32 - remaining_bits)) - 1);
...
template<class Engine, class Iter>
void generate_impl(Engine& eng, Iter first, Iter last, autoboost::mpl::true_)
{
    return detail::generate_from_int(eng, first, last);
}

template<class Engine, class Iter>
void generate_impl(Engine& eng, Iter first, Iter last, autoboost::mpl::false_)
{
...
{
    static const int log = ::autoboost::mpl::if_c<(m == 0),
        ::autoboost::mpl::int_<(::std::numeric_limits<IntType>::digits)>,
        ::autoboost::static_log2<m> >::type::value;
    static const int k =
        (log + ((~(static_cast<IntType>(2) << (log - 1)) & m)? 32 : 31)) / 32;
    ::autoboost::uint_least32_t array[log / 32 + 4];
...
IntType get_one_int(Iter& first, Iter last)
{
    ...
    for(int j = 0; j < k; ++j) {
        if(first == last) {
            autoboost::throw_exception(::std::invalid_argument("Not enough elements in call to seed."));
        }
...
{
    autoboost::uint_least32_t storage[((w+31)/32) * n];
    ...
    for(std::size_t j = 0; j < n; j++) {
        ...
        x[j] = val & ::autoboost::low_bits_mask_t<w>::sig_bits;
    }
}
...
template<int w, std::size_t n, class SeedSeq, class IntType>
inline void seed_array_int_impl(SeedSeq& seq, IntType (&x)[n], autoboost::mpl::false_)
{
...
template<int w, std::size_t n, class Iter, class IntType>
inline void fill_array_int_impl(Iter& first, Iter last, IntType (&x)[n], autoboost::mpl::false_)
{
...
void seed_array_real_impl(const autoboost::uint_least32_t* storage, RealType (&x)[n])
{
    autoboost::uint_least32_t mask = ~((~autoboost::uint_least32_t(0)) << (w%32));
    ...
    for(j = 0; j < n; ++j) {
        ...
        AUTOBOOST_ASSERT(val >= 0);
        AUTOBOOST_ASSERT(val < 1);
        x[j] = val;
    }
}
...
void fill_array_real(Iter& first, Iter last, RealType (&x)[n])
{
    ...
    for(j = 0; j < n; ++j) {
        ...
        for(int k = 0; k < w/32; ++k, ++first) {
            if(first == last) autoboost::throw_exception(std::invalid_argument("Not enough elements in call to seed."));
            val += *first * mult;
            mult *= two32;
        }
        if(mask != 0) {
