...
#ifndef AUTOBOOST_RANDOM_CONST_MOD_HPP
#define AUTOBOOST_RANDOM_CONST_MOD_HPP

#include <autoboost/assert.hpp>
#include <autoboost/static_assert.hpp>
#include <autoboost/integer_traits.hpp>
#include <autoboost/type_traits/make_unsigned.hpp>
#include <autoboost/random/detail/large_arithmetic.hpp>
...
public:
  static IntType apply(IntType x)
  {
    if(((unsigned_m() - 1) & unsigned_m()) == 0)
      ...
      AUTOBOOST_ASSERT(supress_warnings == 0);
  ...
  static IntType pow(IntType a, autoboost::uintmax_t exponent)
  {
      IntType result = 1;
      while(exponent != 0) {
          if(exponent % 2 == 1) {
...
private:
  ...
  static IntType mult_schrage(IntType a, IntType value)
  {
    ...
    AUTOBOOST_ASSERT(r < q);        // check that overflow cannot happen
  ...
  static IntType mult_general(IntType a, IntType b)
  {
    ...
    AUTOBOOST_ASSERT(suppress_warnings == 0);
    IntType modulus = m + suppress_warnings;
    AUTOBOOST_ASSERT(modulus == m);
    if(::autoboost::uintmax_t(modulus) <=
        (::std::numeric_limits< ::autoboost::uintmax_t>::max)() / modulus)
    {
      return static_cast<IntType>(autoboost::uintmax_t(a) * b % modulus);
    } else {
  ...
  static IntType invert_euclidian0(IntType c)
  {
    ...
    IntType q = max / n;
    AUTOBOOST_ASSERT(max % n != n - 1 && "c must be relatively prime to m.");
    l1 += q * l2;
    p = max - q * n + 1;
    for(;;) {
      if(p == 0)
...
#endif // AUTOBOOST_RANDOM_CONST_MOD_HPP
