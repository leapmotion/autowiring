/* autoboost random/detail/polynomial.hpp header file
 *
...
#ifndef AUTOBOOST_RANDOM_DETAIL_POLYNOMIAL_HPP
#define AUTOBOOST_RANDOM_DETAIL_POLYNOMIAL_HPP
...
public:
    ...
    {
        const std::size_t bits = std::numeric_limits<digit_t>::digits;
        AUTOBOOST_ASSERT(shift > 0);
        AUTOBOOST_ASSERT(shift < bits);
        digit_t prev = 0;
        for(std::size_t i = 0; i < size; ++i) {
            digit_t tmp = val[i];
    ...
    struct sparse_mod {
        sparse_mod(const digit_t * divisor, std::size_t divisor_bits)
        {
            ...
            AUTOBOOST_ASSERT(_bit_indices.back() == divisor_bits - 1);
    ...
    static void mod_pow_x(autoboost::uintmax_t exponent, const digit_t * mod, std::size_t mod_bits, digit_t * out)
    {
        const std::size_t bits = std::numeric_limits<digit_t>::digits;
        ...
        autoboost::uintmax_t i = std::numeric_limits<autoboost::uintmax_t>::digits - 1;
        while(((autoboost::uintmax_t(1) << i) & exponent) == 0) {
            --i;
        }
        out[0] = 2;
        std::fill_n(out + 1, n - 1, digit_t(0));
        ...
        while(i--) {
            sqr(out, n);
            m(out, 2 * mod_bits - 1);
            if((autoboost::uintmax_t(1) << i) & exponent) {
                shift_left(out, n, 1);
...
public:
    ...
    friend polynomial mod_pow_x(autoboost::uintmax_t exponent, polynomial mod);
private:
...
inline polynomial mod_pow_x(autoboost::uintmax_t exponent, polynomial mod)
{
    polynomial result;
    mod.normalize();
...
#endif // AUTOBOOST_RANDOM_DETAIL_POLYNOMIAL_HPP
