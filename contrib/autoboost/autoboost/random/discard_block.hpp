/* autoboost random/discard_block.hpp header file
 *
...
#ifndef AUTOBOOST_RANDOM_DISCARD_BLOCK_HPP
#define AUTOBOOST_RANDOM_DISCARD_BLOCK_HPP
...
public:
    ...
    AUTOBOOST_STATIC_CONSTANT(std::size_t, block_size = p);
    AUTOBOOST_STATIC_CONSTANT(std::size_t, used_block = r);

    AUTOBOOST_STATIC_CONSTANT(bool, has_fixed_range = false);
    AUTOBOOST_STATIC_CONSTANT(std::size_t, total_block = p);
    AUTOBOOST_STATIC_CONSTANT(std::size_t, returned_block = r);

    AUTOBOOST_STATIC_ASSERT(total_block >= returned_block);
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(discard_block_engine,
                                               seed_type, value)
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_CONSTRUCTOR(discard_block_engine, SeedSeq, seq)
    { _rng.seed(seq); _n = 0; }
    ...
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_SEED(discard_block_engine, seed_type, s)
    { _rng.seed(s); _n = 0; }
    /** Seeds the underlying generator with seq. */
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_SEED(discard_block_engine, SeedSeq, seq)
    { _rng.seed(seq); _n = 0; }
...
public:
    ...
    result_type min AUTOBOOST_PREVENT_MACRO_SUBSTITUTION ()
    { return (this->base().min)(); }
    result_type max AUTOBOOST_PREVENT_MACRO_SUBSTITUTION ()
...
#endif // AUTOBOOST_RANDOM_DISCARD_BLOCK_HPP
