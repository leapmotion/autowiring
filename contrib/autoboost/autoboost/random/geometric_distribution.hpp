/* autoboost random/geometric_distribution.hpp header file
 *
...
#ifndef AUTOBOOST_RANDOM_GEOMETRIC_DISTRIBUTION_HPP
#define AUTOBOOST_RANDOM_GEOMETRIC_DISTRIBUTION_HPP

#include <autoboost/config/no_tr1/cmath.hpp>          // std::log
#include <iosfwd>
#include <ios>
 ...
 * autoboost::geometric_distribution.  A backwards compatible
 * wrapper is provided in namespace autoboost.
 * @endxmlwarning
 */
...
public:
    ...
    public:
        ...
        explicit param_type(RealType p_arg = RealType(0.5))
          : _p(p_arg)
        {
            AUTOBOOST_ASSERT(RealType(0) < _p && _p < RealType(1));
        }
    ...
    IntType min AUTOBOOST_PREVENT_MACRO_SUBSTITUTION () const { return IntType(0); }

    /** Returns the largest value that the distribution can produce. */
    IntType max AUTOBOOST_PREVENT_MACRO_SUBSTITUTION () const
    ...
    result_type operator()(Engine& eng) const
    {
        using std::log;
        using std::floor;
        RealType x = RealType(1) - autoboost::uniform_01<RealType>()(eng);
    ...
    AUTOBOOST_RANDOM_DETAIL_OSTREAM_OPERATOR(os, geometric_distribution, gd)
    {
        os << gd._p;
        return os;
    }
    ...
    AUTOBOOST_RANDOM_DETAIL_ISTREAM_OPERATOR(is, geometric_distribution, gd)
    {
        param_type parm;
        if(is >> parm) {
            gd.param(parm);
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_EQUALITY_OPERATOR(geometric_distribution, lhs, rhs)
    { return lhs._p == rhs._p; }
     ...
     * sequences of values given equal generators.
     */
    AUTOBOOST_RANDOM_DETAIL_INEQUALITY_OPERATOR(geometric_distribution)
...
#endif // AUTOBOOST_RANDOM_GEOMETRIC_DISTRIBUTION_HPP
