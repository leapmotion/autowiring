/* autoboost random/xor_combine.hpp header file
 *
...
#ifndef AUTOBOOST_RANDOM_XOR_COMBINE_HPP
#define AUTOBOOST_RANDOM_XOR_COMBINE_HPP
...
#include <autoboost/cstdint.hpp>     // uint32_t
...
public:
    ...
    AUTOBOOST_STATIC_CONSTANT(int, shift1 = s1);
    AUTOBOOST_STATIC_CONSTANT(int, shift2 = s2);
     ...
     * @endxmlwarning
     */
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(xor_combine_engine,
        result_type, v)
    { seed(v); }
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_CONSTRUCTOR(xor_combine_engine,
        SeedSeq, seq)
    { seed(seq); }
    ...
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_SEED(xor_combine_engine, result_type, v)
    { _rng1.seed(v); _rng2.seed(v); }

    /** @c seeds both base generators with values produced by @c seq. */
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_SEED(xor_combine_engine, SeedSeq, seq)
    ...
    static result_type min AUTOBOOST_PREVENT_MACRO_SUBSTITUTION () { return (std::min)((URNG1::min)(), (URNG2::min)()); }
    /** Returns the largest value that the generator can produce. */
    static result_type max AUTOBOOST_PREVENT_MACRO_SUBSTITUTION () { return (std::max)((URNG1::min)(), (URNG2::max)()); }

    /**
     * Writes the textual representation of the generator to a @c std::ostream.
     */
    AUTOBOOST_RANDOM_DETAIL_OSTREAM_OPERATOR(os, xor_combine_engine, s)
    {
        os << s._rng1 << ' ' << s._rng2;
        return os;
    }
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_ISTREAM_OPERATOR(is, xor_combine_engine, s)
    {
    ...
    AUTOBOOST_RANDOM_DETAIL_EQUALITY_OPERATOR(xor_combine_engine, x, y)
    { return x._rng1 == y._rng1 && x._rng2 == y._rng2; }

    /** Returns true if the two generators will produce different sequences. */
    AUTOBOOST_RANDOM_DETAIL_INEQUALITY_OPERATOR(xor_combine_engine)
...
public:
    ...
    result_type min AUTOBOOST_PREVENT_MACRO_SUBSTITUTION () const { return (std::min)((this->base1().min)(), (this->base2().min)()); }
    result_type max AUTOBOOST_PREVENT_MACRO_SUBSTITUTION () const { return (std::max)((this->base1().min)(), (this->base2().max)()); }
};
...
#endif // AUTOBOOST_RANDOM_XOR_COMBINE_HPP
