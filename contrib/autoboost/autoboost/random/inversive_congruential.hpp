/* autoboost random/inversive_congruential.hpp header file
 *
...
#ifndef AUTOBOOST_RANDOM_INVERSIVE_CONGRUENTIAL_HPP
#define AUTOBOOST_RANDOM_INVERSIVE_CONGRUENTIAL_HPP
...
public:
    ...
    AUTOBOOST_STATIC_CONSTANT(result_type, multiplier = a);
    AUTOBOOST_STATIC_CONSTANT(result_type, increment = b);
    AUTOBOOST_STATIC_CONSTANT(result_type, modulus = p);
    AUTOBOOST_STATIC_CONSTANT(IntType, default_seed = 1);

    static result_type min AUTOBOOST_PREVENT_MACRO_SUBSTITUTION () { return b == 0 ? 1 : 0; }
    static result_type max AUTOBOOST_PREVENT_MACRO_SUBSTITUTION () { return p-1; }
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(inversive_congruential_engine,
                                               IntType, x0)
    { seed(x0); }
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_CONSTRUCTOR(inversive_congruential_engine,
                                             SeedSeq, seq)
    { seed(seq); }
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_SEED(inversive_congruential_engine, IntType, x0)
    {
        ...
        AUTOBOOST_ASSERT(_value >= (min)());
        AUTOBOOST_ASSERT(_value <= (max)());
    }

    /**
     * Seeds an @c inversive_congruential_engine using values from a SeedSeq.
     */
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_SEED(inversive_congruential_engine, SeedSeq, seq)
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_OSTREAM_OPERATOR(os, inversive_congruential_engine, x)
    {
        os << x._value;
        return os;
    }
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_ISTREAM_OPERATOR(is, inversive_congruential_engine, x)
    {
        is >> x._value;
        return is;
    }
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_EQUALITY_OPERATOR(inversive_congruential_engine, x, y)
     ...
     * sequences of outputs.
     */
    AUTOBOOST_RANDOM_DETAIL_INEQUALITY_OPERATOR(inversive_congruential_engine)
...
#endif // AUTOBOOST_RANDOM_INVERSIVE_CONGRUENTIAL_HPP
