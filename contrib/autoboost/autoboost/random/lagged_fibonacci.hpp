/* autoboost random/lagged_fibonacci.hpp header file
 *
...
#ifndef AUTOBOOST_RANDOM_LAGGED_FIBONACCI_HPP
#define AUTOBOOST_RANDOM_LAGGED_FIBONACCI_HPP
...
public:
    ...
    AUTOBOOST_STATIC_CONSTANT(int, word_size = w);
    AUTOBOOST_STATIC_CONSTANT(unsigned int, long_lag = p);
    AUTOBOOST_STATIC_CONSTANT(unsigned int, short_lag = q);

    AUTOBOOST_STATIC_CONSTANT(UIntType, default_seed = 331u);

    /** Returns the smallest value that the generator can produce. */
    static result_type min AUTOBOOST_PREVENT_MACRO_SUBSTITUTION () { return 0; }
    ...
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(lagged_fibonacci_engine,
        UIntType, value)
    { seed(value); }

    /** Creates a new @c lagged_fibonacci_engine and calls @c seed(seq). */
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_CONSTRUCTOR(lagged_fibonacci_engine,
        SeedSeq, seq)
    { seed(seq); }
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_SEED(lagged_fibonacci_engine,
        UIntType, value)
    {
        minstd_rand0 intgen(static_cast<autoboost::uint32_t>(value));
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_SEED(lagged_fibonacci_engine, SeedSeq, seq)
    {
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_OSTREAM_OPERATOR(os, lagged_fibonacci_engine, f)
    {
        os << f.i;
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_ISTREAM_OPERATOR(is, lagged_fibonacci_engine, f)
    {
        is >> f.i >> std::ws;
        for(unsigned int j = 0; j < f.long_lag; ++j)
            is >> f.x[j] >> std::ws;
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_EQUALITY_OPERATOR(lagged_fibonacci_engine, x_, y_)
     ...
     * sequences of outputs.
     */
    AUTOBOOST_RANDOM_DETAIL_INEQUALITY_OPERATOR(lagged_fibonacci_engine)
...
public:
    lagged_fibonacci() {}
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(lagged_fibonacci, UIntType, val)
    { this->seed(val); }
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_CONSTRUCTOR(lagged_fibonacci, SeedSeq, seq)
    { this->seed(seq); }
    template<class It>
...
public:
    ...
    AUTOBOOST_STATIC_CONSTANT(autoboost::uint32_t, default_seed = 331u);
    ...
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(lagged_fibonacci_01_engine, uint32_t, value)
    { seed(value); }
    /** Constructs a @c lagged_fibonacci_01 generator and calls @c seed(gen). */
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_CONSTRUCTOR(lagged_fibonacci_01_engine, SeedSeq, seq)
    { seed(seq); }
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_SEED(lagged_fibonacci_01_engine, autoboost::uint32_t, value)
    {
     ...
     * @c seq.generate.
     */
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_SEED(lagged_fibonacci_01_engine, SeedSeq, seq)
    {
    ...
    /** Returns the smallest value that the generator can produce. */
    static result_type min AUTOBOOST_PREVENT_MACRO_SUBSTITUTION () { return result_type(0); }
    /** Returns the upper bound of the generators outputs. */
    static result_type max AUTOBOOST_PREVENT_MACRO_SUBSTITUTION () { return result_type(1); }
     ...
     * Writes the textual representation of the generator to a @c std::ostream.
     */
    AUTOBOOST_RANDOM_DETAIL_OSTREAM_OPERATOR(os, lagged_fibonacci_01_engine, f)
    {
     ...
     * Reads the textual representation of the generator from a @c std::istream.
     */
    AUTOBOOST_RANDOM_DETAIL_ISTREAM_OPERATOR(is, lagged_fibonacci_01_engine, f)
    {
        is >> f.i;
        for(unsigned int j = 0; j < f.long_lag; ++j) {
     ...
     * sequences of outputs.
     */
    AUTOBOOST_RANDOM_DETAIL_EQUALITY_OPERATOR(lagged_fibonacci_01_engine, x_, y_)
    { return x_.i == y_.i && std::equal(x_.x, x_.x+long_lag, y_.x); }
     ...
     * sequences of outputs.
     */
    AUTOBOOST_RANDOM_DETAIL_INEQUALITY_OPERATOR(lagged_fibonacci_01_engine)
...
template<class RealType, int w, unsigned int p, unsigned int q>
const autoboost::uint32_t lagged_fibonacci_01_engine<RealType,w,p,q>::default_seed;
#endif
...
public:
    lagged_fibonacci_01() {}
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(lagged_fibonacci_01, autoboost::uint32_t, val)
    { this->seed(val); }
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_CONSTRUCTOR(lagged_fibonacci_01, SeedSeq, seq)
    { this->seed(seq); }
    template<class It>
...
/** @copydoc autoboost::random::detail::lagged_fibonacci_doc */
...
#endif // AUTOBOOST_RANDOM_LAGGED_FIBONACCI_HPP
