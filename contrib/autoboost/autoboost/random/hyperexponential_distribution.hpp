/* autoboost random/hyperexponential_distribution.hpp header file
 *
 ...
 *
 * Much of the code here taken by autoboost::math::hyperexponential_distribution.
...
#ifndef AUTOBOOST_RANDOM_HYPEREXPONENTIAL_DISTRIBUTION_HPP
#define AUTOBOOST_RANDOM_HYPEREXPONENTIAL_DISTRIBUTION_HPP


#include <autoboost/config.hpp>
#include <autoboost/math/special_functions/fpclassify.hpp>
#include <autoboost/random/detail/operators.hpp>
#include <autoboost/random/detail/vector_io.hpp>
...
#include <cstddef>
#include <iterator>
#ifndef AUTOBOOST_NO_CXX11_HDR_INITIALIZER_LIST
# include <initializer_list>
#endif // AUTOBOOST_NO_CXX11_HDR_INITIALIZER_LIST
#include <iostream>
#include <limits>
#include <numeric>
#include <vector>
...
namespace autoboost { namespace random {
...
bool check_probabilities(std::vector<RealT> const& probabilities)
{
    ...
    for (std::size_t i = 0; i < n; ++i)
    {
        if (probabilities[i] < 0
            || probabilities[i] > 1
            || !(autoboost::math::isfinite)(probabilities[i]))
        {
            return false;
        }
        sum += probabilities[i];
...
bool check_rates(std::vector<RealT> const& rates)
{
    ...
    {
        if (rates[i] <= 0
            || !(autoboost::math::isfinite)(rates[i]))
        {
            return false;
        }
    }
...
class hyperexponential_distribution
{
    ...
    public: class param_type
    {
         ...
         *
         * \tparam ProbRangeT Must meet the requirements of <a href="autoboost:/libs/range/doc/html/range/concepts.html">Range</a> concept.
         * \tparam RateRangeT Must meet the requirements of <a href="autoboost:/libs/range/doc/html/range/concepts.html">Range</a> concept.
         *
        ...
        public: template <typename ProbRangeT, typename RateRangeT>
                param_type(ProbRangeT const& prob_range,
                           ...
                           typename autoboost::disable_if_c<autoboost::has_pre_increment<ProbRangeT>::value || autoboost::has_pre_increment<RateRangeT>::value>::type* = 0)
        : probs_(autoboost::begin(prob_range), autoboost::end(prob_range)),
          rates_(autoboost::begin(rate_range), autoboost::end(rate_range))
        {
            hyperexp_detail::normalize(probs_);
        ...
        public: template <typename RateIterT>
                param_type(RateIterT rate_first,
                           RateIterT rate_last,
                           typename autoboost::enable_if_c<autoboost::has_pre_increment<RateIterT>::value>::type* = 0)
        ...
        : probs_(autoboost::size(rate_range), 1), // Will be normalized below
          rates_(autoboost::begin(rate_range), autoboost::end(rate_range))
        {
            hyperexp_detail::normalize(probs_);
        ...
        public: AUTOBOOST_RANDOM_DETAIL_OSTREAM_OPERATOR(os, param_type, param)
        {
        ...
        public: AUTOBOOST_RANDOM_DETAIL_ISTREAM_OPERATOR(is, param_type, param)
        {
        ...
        public: AUTOBOOST_RANDOM_DETAIL_EQUALITY_OPERATOR(param_type, lhs, rhs)
        {
        ...
        public: AUTOBOOST_RANDOM_DETAIL_INEQUALITY_OPERATOR(param_type)
    ...
    : dd_(std::vector<RealT>(autoboost::size(rate_range), 1)),
      rates_(autoboost::begin(rate_range), autoboost::end(rate_range))
    {
        assert( hyperexp_detail::check_params(dd_.probabilities(), rates_) );
    }
            ...
            RealT operator()(URNG& urng) const
    {
        ...
        return autoboost::random::exponential_distribution<RealT>(rates_[i])(urng);
    }
    ...
    public: RealT min AUTOBOOST_PREVENT_MACRO_SUBSTITUTION () const
    {
        return 0;
    }

    /** Returns the largest value that the distribution can produce. */
    public: RealT max AUTOBOOST_PREVENT_MACRO_SUBSTITUTION () const
    {
    ...
    {
        dd_.param(typename autoboost::random::discrete_distribution<int,RealT>::param_type(param.probabilities()));
        rates_ = param.rates();
    }
    ...
    public: AUTOBOOST_RANDOM_DETAIL_OSTREAM_OPERATOR(os, hyperexponential_distribution, hd)
    {
        os << hd.param();
        return os;
    }
    ...
    public: AUTOBOOST_RANDOM_DETAIL_ISTREAM_OPERATOR(is, hyperexponential_distribution, hd)
    {
        param_type param;
        if(is >> param)
        {
     ...
     */
    public: AUTOBOOST_RANDOM_DETAIL_EQUALITY_OPERATOR(hyperexponential_distribution, lhs, rhs)
    {
     ...
     */
    public: AUTOBOOST_RANDOM_DETAIL_INEQUALITY_OPERATOR(hyperexponential_distribution)


    private: autoboost::random::discrete_distribution<int,RealT> dd_; ///< The \c discrete_distribution used to sample the phase probability and choose the rate
    private: std::vector<RealT> rates_; ///< The <em>rate vector</em> parameter of the distribution
}; // hyperexponential_distribution

}} // namespace autoboost::random


#endif // AUTOBOOST_RANDOM_HYPEREXPONENTIAL_DISTRIBUTION_HPP
