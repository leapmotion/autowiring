/* autoboost random/variate_generator.hpp header file
 *
...
#ifndef AUTOBOOST_RANDOM_RANDOM_GENERATOR_HPP
#define AUTOBOOST_RANDOM_RANDOM_GENERATOR_HPP
...
{
private:
    typedef autoboost::random::detail::ptr_helper<Engine> helper_type;
public:
     ...
     */
    result_type min AUTOBOOST_PREVENT_MACRO_SUBSTITUTION () const { return (distribution().min)(); }
    /**
     * Precondition: distribution().max() is well-formed
     *
     * Returns: distribution().max()
     */
    result_type max AUTOBOOST_PREVENT_MACRO_SUBSTITUTION () const { return (distribution().max)(); }
...
#endif // AUTOBOOST_RANDOM_RANDOM_GENERATOR_HPP
