/* autoboost random/traits.hpp header file
 *
...
#ifndef AUTOBOOST_RANDOM_TRAITS_HPP
#define AUTOBOOST_RANDOM_TRAITS_HPP
...
namespace traits {
   ...
   {
      typedef typename autoboost::make_unsigned<T>::type type;
   };
   template <class T>
   struct make_unsigned_imp<T, false>
   {
      AUTOBOOST_STATIC_ASSERT(std::numeric_limits<T>::is_specialized);
      AUTOBOOST_STATIC_ASSERT(std::numeric_limits<T>::is_signed == false);
      AUTOBOOST_STATIC_ASSERT(std::numeric_limits<T>::is_integer == true);
      typedef T type;
   };
   // \endcond
   ...
   * types this trait defaults to `autoboost::make_unsigned<T>::type`.  For user defined types it simply asserts that
   ...
   */
   ...
   // \cond show_private
      : public make_unsigned_imp < T, autoboost::is_integral<T>::value >
      // \endcond
   {};
   // \cond show_private
   template <class T, bool intrinsic>
   ...
   {
      AUTOBOOST_STATIC_ASSERT(std::numeric_limits<T>::is_specialized);
      AUTOBOOST_STATIC_ASSERT((std::numeric_limits<T>::is_signed == false) || (std::numeric_limits<T>::is_bounded == false));
      AUTOBOOST_STATIC_ASSERT(std::numeric_limits<T>::is_integer == true);
      typedef T type;
   };
   // \endcond
   ...
   */
   ...
   struct make_unsigned_or_unbounded
      // \cond show_private
      : public make_unsigned_or_unbounded_imp < T, autoboost::is_integral<T>::value >
      // \endcond
   {};
   ...
   */
   template <class T>
   struct is_integral
      : public mpl::bool_<autoboost::is_integral<T>::value || (std::numeric_limits<T>::is_integer)>
   {};
   /** \brief Traits class that indicates whether type T is a signed integer
   */
   template <class T> struct is_signed
      : public mpl::bool_ < autoboost::is_signed<T>::value || (std::numeric_limits<T>::is_specialized && std::numeric_limits<T>::is_integer && std::numeric_limits<T>::is_signed)>
   {};
