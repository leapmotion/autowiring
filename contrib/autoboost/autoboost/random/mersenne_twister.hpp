/* autoboost random/mersenne_twister.hpp header file
 *
...
#ifndef AUTOBOOST_RANDOM_MERSENNE_TWISTER_HPP
#define AUTOBOOST_RANDOM_MERSENNE_TWISTER_HPP
...
#include <autoboost/random/detail/generator_seed_seq.hpp>
#include <autoboost/random/detail/polynomial.hpp>
 ...
 *
 * @xmlnote
 * The autoboost variant has been implemented from scratch and does not
...
public:
    ...
    AUTOBOOST_STATIC_CONSTANT(std::size_t, word_size = w);
    AUTOBOOST_STATIC_CONSTANT(std::size_t, state_size = n);
    AUTOBOOST_STATIC_CONSTANT(std::size_t, shift_size = m);
    AUTOBOOST_STATIC_CONSTANT(std::size_t, mask_bits = r);
    AUTOBOOST_STATIC_CONSTANT(UIntType, xor_mask = a);
    AUTOBOOST_STATIC_CONSTANT(std::size_t, tempering_u = u);
    AUTOBOOST_STATIC_CONSTANT(UIntType, tempering_d = d);
    AUTOBOOST_STATIC_CONSTANT(std::size_t, tempering_s = s);
    AUTOBOOST_STATIC_CONSTANT(UIntType, tempering_b = b);
    AUTOBOOST_STATIC_CONSTANT(std::size_t, tempering_t = t);
    AUTOBOOST_STATIC_CONSTANT(UIntType, tempering_c = c);
    AUTOBOOST_STATIC_CONSTANT(std::size_t, tempering_l = l);
    AUTOBOOST_STATIC_CONSTANT(UIntType, initialization_multiplier = f);
    AUTOBOOST_STATIC_CONSTANT(UIntType, default_seed = 5489u);

    // backwards compatibility
    AUTOBOOST_STATIC_CONSTANT(UIntType, parameter_a = a);
    AUTOBOOST_STATIC_CONSTANT(std::size_t, output_u = u);
    AUTOBOOST_STATIC_CONSTANT(std::size_t, output_s = s);
    AUTOBOOST_STATIC_CONSTANT(UIntType, output_b = b);
    AUTOBOOST_STATIC_CONSTANT(std::size_t, output_t = t);
    AUTOBOOST_STATIC_CONSTANT(UIntType, output_c = c);
    AUTOBOOST_STATIC_CONSTANT(std::size_t, output_l = l);
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(mersenne_twister_engine,
                                               UIntType, value)
    { seed(value); }
     ...
     * @endxmlnote
     */
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_CONSTRUCTOR(mersenne_twister_engine,
                                             SeedSeq, seq)
    { seed(seq); }
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_SEED(mersenne_twister_engine, UIntType, value)
    {
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_SEED(mersenne_twister_engine, SeeqSeq, seq)
    {
    ...
    {
#ifndef AUTOBOOST_RANDOM_MERSENNE_TWISTER_DISCARD_THRESHOLD
#define AUTOBOOST_RANDOM_MERSENNE_TWISTER_DISCARD_THRESHOLD 10000000
#endif
        if(z > AUTOBOOST_RANDOM_MERSENNE_TWISTER_DISCARD_THRESHOLD) {
            discard_many(z);
        } else {
            for(autoboost::uintmax_t j = 0; j < z; ++j) {
                (*this)();
            }
        }
    }
...
private:
     ...
     */
    void discard_many(autoboost::uintmax_t z)
    {
...
#define AUTOBOOST_RANDOM_MT_DEFINE_CONSTANT(type, name)                         \
...
AUTOBOOST_RANDOM_MT_DEFINE_CONSTANT(std::size_t, word_size);
AUTOBOOST_RANDOM_MT_DEFINE_CONSTANT(std::size_t, state_size);
AUTOBOOST_RANDOM_MT_DEFINE_CONSTANT(std::size_t, shift_size);
AUTOBOOST_RANDOM_MT_DEFINE_CONSTANT(std::size_t, mask_bits);
AUTOBOOST_RANDOM_MT_DEFINE_CONSTANT(UIntType, xor_mask);
AUTOBOOST_RANDOM_MT_DEFINE_CONSTANT(std::size_t, tempering_u);
AUTOBOOST_RANDOM_MT_DEFINE_CONSTANT(UIntType, tempering_d);
AUTOBOOST_RANDOM_MT_DEFINE_CONSTANT(std::size_t, tempering_s);
AUTOBOOST_RANDOM_MT_DEFINE_CONSTANT(UIntType, tempering_b);
AUTOBOOST_RANDOM_MT_DEFINE_CONSTANT(std::size_t, tempering_t);
AUTOBOOST_RANDOM_MT_DEFINE_CONSTANT(UIntType, tempering_c);
AUTOBOOST_RANDOM_MT_DEFINE_CONSTANT(std::size_t, tempering_l);
AUTOBOOST_RANDOM_MT_DEFINE_CONSTANT(UIntType, initialization_multiplier);
AUTOBOOST_RANDOM_MT_DEFINE_CONSTANT(UIntType, default_seed);
AUTOBOOST_RANDOM_MT_DEFINE_CONSTANT(UIntType, parameter_a);
AUTOBOOST_RANDOM_MT_DEFINE_CONSTANT(std::size_t, output_u );
AUTOBOOST_RANDOM_MT_DEFINE_CONSTANT(std::size_t, output_s);
AUTOBOOST_RANDOM_MT_DEFINE_CONSTANT(UIntType, output_b);
AUTOBOOST_RANDOM_MT_DEFINE_CONSTANT(std::size_t, output_t);
AUTOBOOST_RANDOM_MT_DEFINE_CONSTANT(UIntType, output_c);
AUTOBOOST_RANDOM_MT_DEFINE_CONSTANT(std::size_t, output_l);
AUTOBOOST_RANDOM_MT_DEFINE_CONSTANT(bool, has_fixed_range);
#undef AUTOBOOST_RANDOM_MT_DEFINE_CONSTANT
#endif
...
public:
    mersenne_twister() {}
    AUTOBOOST_RANDOM_DETAIL_GENERATOR_CONSTRUCTOR(mersenne_twister, Gen, gen)
    { seed(gen); }
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(mersenne_twister, UIntType, val)
    { seed(val); }
    template<class It>
    mersenne_twister(It& first, It last) : base_type(first, last) {}
    void seed() { base_type::seed(); }
    AUTOBOOST_RANDOM_DETAIL_GENERATOR_SEED(mersenne_twister, Gen, gen)
    {
        detail::generator_seed_seq<Gen> seq(gen);
        base_type::seed(seq);
    }
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_SEED(mersenne_twister, UIntType, val)
    { base_type::seed(val); }
    template<class It>
...
AUTOBOOST_RANDOM_PTR_HELPER_SPEC(autoboost::mt11213b)
AUTOBOOST_RANDOM_PTR_HELPER_SPEC(autoboost::mt19937)
AUTOBOOST_RANDOM_PTR_HELPER_SPEC(autoboost::mt19937_64)

#include <autoboost/random/detail/enable_warnings.hpp>

#endif // AUTOBOOST_RANDOM_MERSENNE_TWISTER_HPP
