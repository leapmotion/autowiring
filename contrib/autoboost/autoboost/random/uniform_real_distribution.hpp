/* autoboost random/uniform_real_distribution.hpp header file
 *
...
#ifndef AUTOBOOST_RANDOM_UNIFORM_REAL_DISTRIBUTION_HPP
#define AUTOBOOST_RANDOM_UNIFORM_REAL_DISTRIBUTION_HPP
    ...
    autoboost::mpl::false_  /** is_integral<Engine::result_type> */)
{
    for(;;) {
        ...
        AUTOBOOST_ASSERT(divisor > 0);
        AUTOBOOST_ASSERT(numerator >= 0 && numerator <= divisor);
...
public:
    ...
    /**  Returns the minimum value of the distribution */
    RealType min AUTOBOOST_PREVENT_MACRO_SUBSTITUTION () const { return _min; }
    /**  Returns the maximum value of the distribution */
    RealType max AUTOBOOST_PREVENT_MACRO_SUBSTITUTION () const { return _max; }
    ...
    AUTOBOOST_RANDOM_DETAIL_OSTREAM_OPERATOR(os, uniform_real_distribution, ud)
    {
        os << ud.param();
        return os;
    }
    ...
    AUTOBOOST_RANDOM_DETAIL_ISTREAM_OPERATOR(is, uniform_real_distribution, ud)
    {
        param_type parm;
        if(is >> parm) {
            ud.param(parm);
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_EQUALITY_OPERATOR(uniform_real_distribution, lhs, rhs)
    { return lhs._min == rhs._min && lhs._max == rhs._max; }
     ...
     * of values given equal generators.
     */
    AUTOBOOST_RANDOM_DETAIL_INEQUALITY_OPERATOR(uniform_real_distribution)
