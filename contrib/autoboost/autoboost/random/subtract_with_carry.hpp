/* autoboost random/subtract_with_carry.hpp header file
 *
...
#ifndef AUTOBOOST_RANDOM_SUBTRACT_WITH_CARRY_HPP
#define AUTOBOOST_RANDOM_SUBTRACT_WITH_CARRY_HPP

#include <autoboost/config/no_tr1/cmath.hpp>         // std::pow
#include <iostream>
...
struct subtract_with_carry_discard
{
    ...
    static void apply(Engine& eng, autoboost::uintmax_t z)
    {
...
public:
    ...
    AUTOBOOST_STATIC_CONSTANT(std::size_t, long_lag = r);
    AUTOBOOST_STATIC_CONSTANT(std::size_t, short_lag = s);
    AUTOBOOST_STATIC_CONSTANT(uint32_t, default_seed = 19780503u);

    // Required by the old Boost.Random concepts
    AUTOBOOST_STATIC_CONSTANT(bool, has_fixed_range = false);
    // Backwards compatibility
    AUTOBOOST_STATIC_CONSTANT(result_type, modulus = (result_type(1) << w));

    AUTOBOOST_STATIC_ASSERT(std::numeric_limits<result_type>::is_integer);
     ...
     * it with @c value.
     */
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(subtract_with_carry_engine,
                                               IntType, value)
    { seed(value); }
    /**
     * Constructs a new @c subtract_with_carry_engine and seeds
     ...
     */
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_CONSTRUCTOR(subtract_with_carry_engine,
                                             SeedSeq, seq)
    { seed(seq); }
    /**
     * Constructs a new @c subtract_with_carry_engine and seeds
    ...
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_SEED(subtract_with_carry_engine,
                                        IntType, value)
    {
        typedef linear_congruential_engine<uint32_t,40014,0,2147483563> gen_t;
        gen_t intgen(static_cast<autoboost::uint32_t>(value == 0 ? default_seed : value));
        detail::generator_seed_seq<gen_t> gen(intgen);
        seed(gen);
    }

    /** Seeds the generator with values produced by @c seq.generate(). */
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_SEED(subtract_with_carry, SeedSeq, seq)
    {
    ...
    AUTOBOOST_RANDOM_DETAIL_OSTREAM_OPERATOR(os, subtract_with_carry_engine, f)
    {
    ...
    AUTOBOOST_RANDOM_DETAIL_ISTREAM_OPERATOR(is, subtract_with_carry_engine, f)
    {
        for(unsigned int j = 0; j < f.long_lag; ++j)
            is >> f.x[j] >> std::ws;
        is >> f.carry;
     ...
     * sequences of values.
     */
    AUTOBOOST_RANDOM_DETAIL_EQUALITY_OPERATOR(subtract_with_carry_engine, x, y)
    {
     ...
     * sequences of values.
     */
    AUTOBOOST_RANDOM_DETAIL_INEQUALITY_OPERATOR(subtract_with_carry_engine)
...
public:
    ...
    AUTOBOOST_STATIC_CONSTANT(std::size_t, long_lag = r);
    AUTOBOOST_STATIC_CONSTANT(std::size_t, short_lag = s);
    AUTOBOOST_STATIC_CONSTANT(autoboost::uint32_t, default_seed = 19780503u);
    ...
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(subtract_with_carry_01_engine,
                                               autoboost::uint32_t, value)
    { init_modulus(); seed(value); }
    /**
     * Creates a new \subtract_with_carry_01_engine and seeds with values
     * produced by seq.generate().
     */
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_CONSTRUCTOR(subtract_with_carry_01_engine,
                                             SeedSeq, seq)
...
private:
    ...
    void init_modulus()
    {
#ifndef AUTOBOOST_NO_STDC_NAMESPACE
...
public:
    ...
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_SEED(subtract_with_carry_01_engine,
                                        autoboost::uint32_t, value)
    {
        typedef linear_congruential_engine<uint32_t, 40014, 0, 2147483563> gen_t;
    ...
    /** Seeds the generator with values produced by @c seq.generate(). */
    AUTOBOOST_RANDOM_DETAIL_SEED_SEQ_SEED(subtract_with_carry_01_engine,
                                      SeedSeq, seq)
    {
    ...
    AUTOBOOST_RANDOM_DETAIL_OSTREAM_OPERATOR(os, subtract_with_carry_01_engine, f)
    {
    ...
    AUTOBOOST_RANDOM_DETAIL_ISTREAM_OPERATOR(is, subtract_with_carry_01_engine, f)
    {
        RealType value;
    ...
    AUTOBOOST_RANDOM_DETAIL_EQUALITY_OPERATOR(subtract_with_carry_01_engine, x, y)
    {
        for(unsigned int j = 0; j < r; ++j)
            if(x.compute(j) != y.compute(j))
                return false;
    ...
    AUTOBOOST_RANDOM_DETAIL_INEQUALITY_OPERATOR(subtract_with_carry_01_engine)
...
class subtract_with_carry :
    public subtract_with_carry_engine<IntType,
        autoboost::static_log2<m>::value, s, r>
{
    typedef subtract_with_carry_engine<IntType,
        autoboost::static_log2<m>::value, s, r> base_type;
public:
    subtract_with_carry() {}
    AUTOBOOST_RANDOM_DETAIL_GENERATOR_CONSTRUCTOR(subtract_with_carry, Gen, gen)
    { seed(gen); }
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(subtract_with_carry,
                                               IntType, val)
    { seed(val); }
    template<class It>
    subtract_with_carry(It& first, It last) : base_type(first, last) {}
    void seed() { base_type::seed(); }
    AUTOBOOST_RANDOM_DETAIL_GENERATOR_SEED(subtract_with_carry, Gen, gen)
    {
        detail::generator_seed_seq<Gen> seq(gen);
        base_type::seed(seq);
    }
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_SEED(subtract_with_carry, IntType, val)
    { base_type::seed(val); }
    template<class It>
...
public:
    ...
    AUTOBOOST_RANDOM_DETAIL_GENERATOR_CONSTRUCTOR(subtract_with_carry_01, Gen, gen)
    { seed(gen); }
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(subtract_with_carry_01,
                                               uint32_t, val)
    { seed(val); }
    template<class It>
    subtract_with_carry_01(It& first, It last) : base_type(first, last) {}
    void seed() { base_type::seed(); }
    AUTOBOOST_RANDOM_DETAIL_GENERATOR_SEED(subtract_with_carry_01, Gen, gen)
    {
        detail::generator_seed_seq<Gen> seq(gen);
        base_type::seed(seq);
    }
    AUTOBOOST_RANDOM_DETAIL_ARITHMETIC_SEED(subtract_with_carry_01, uint32_t, val)
    { base_type::seed(val); }
    template<class It>
    void seed(It& first, It last) { base_type::seed(first, last); }
};
...
#endif // AUTOBOOST_RANDOM_SUBTRACT_WITH_CARRY_HPP
