...
#include <autoboost/math_fwd.hpp>  // self include
...
#else // AUTOBOOST_NO_LIMITS_COMPILE_TIME_CONSTANTS
    template < typename T >
    struct gcd_optimal_evaluator
    {
        T  operator ()( T const &a, T const &b )
...
#define AUTOBOOST_PRIVATE_GCD_UF( Ut )                  \
    template < >  struct gcd_optimal_evaluator<Ut>  \
    {  Ut  operator ()( Ut a, Ut b ) const  { return gcd_binary( a, b ); }  }

    AUTOBOOST_PRIVATE_GCD_UF( unsigned char );
    AUTOBOOST_PRIVATE_GCD_UF( unsigned short );
    AUTOBOOST_PRIVATE_GCD_UF( unsigned );
    AUTOBOOST_PRIVATE_GCD_UF( unsigned long );

#ifdef AUTOBOOST_HAS_LONG_LONG
    AUTOBOOST_PRIVATE_GCD_UF( autoboost::ulong_long_type );
#elif defined(AUTOBOOST_HAS_MS_INT64)
    AUTOBOOST_PRIVATE_GCD_UF( unsigned __int64 );
#endif

#if CHAR_MIN == 0
    AUTOBOOST_PRIVATE_GCD_UF( char ); // char is unsigned
#endif

#undef AUTOBOOST_PRIVATE_GCD_UF

#define AUTOBOOST_PRIVATE_GCD_SF( St, Ut )                            \
    ...
    AUTOBOOST_PRIVATE_GCD_SF( signed char, unsigned char );
    AUTOBOOST_PRIVATE_GCD_SF( short, unsigned short );
    AUTOBOOST_PRIVATE_GCD_SF( int, unsigned );
    AUTOBOOST_PRIVATE_GCD_SF( long, unsigned long );

#if CHAR_MIN < 0
    AUTOBOOST_PRIVATE_GCD_SF( char, unsigned char ); // char is signed
#endif

#ifdef AUTOBOOST_HAS_LONG_LONG
    AUTOBOOST_PRIVATE_GCD_SF( autoboost::long_long_type, autoboost::ulong_long_type );
#elif defined(AUTOBOOST_HAS_MS_INT64)
    AUTOBOOST_PRIVATE_GCD_SF( __int64, unsigned __int64 );
#endif

#undef AUTOBOOST_PRIVATE_GCD_SF
