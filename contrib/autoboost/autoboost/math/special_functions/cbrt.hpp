...
#ifndef AUTOBOOST_MATH_SF_CBRT_HPP
#define AUTOBOOST_MATH_SF_CBRT_HPP
...
struct big_int_type
{
   operator autoboost::uintmax_t()const;
};
...
struct largest_cbrt_int_type
{
   typedef typename mpl::if_<
      autoboost::is_convertible<big_int_type, T>,
      autoboost::uintmax_t,
      unsigned int
   >::type type;
};
...
T cbrt_imp(T z, const Policy& pol)
{
   ...
   if((autoboost::math::isinf)(z) || (z == 0))
      return z;
   if(!(autoboost::math::isfinite)(z))
   {
      return policies::raise_domain_error("autoboost::math::cbrt<%1%>(%1%)", "Argument to function must be finite but got %1%.", z, pol);
   }
   ...
   AUTOBOOST_STATIC_ASSERT( ::std::numeric_limits<shift_type>::radix == 2);
...
#endif // AUTOBOOST_MATH_SF_CBRT_HPP
