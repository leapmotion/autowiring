...
#ifndef AUTOBOOST_MATH_SF_GAMMA_HPP
#define AUTOBOOST_MATH_SF_GAMMA_HPP
...
#include <autoboost/math/special_functions/powm1.hpp>
#include <autoboost/math/special_functions/sqrt1pm1.hpp>
#include <autoboost/math/special_functions/lanczos.hpp>
#include <autoboost/math/special_functions/fpclassify.hpp>
#include <autoboost/math/special_functions/detail/igamma_large.hpp>
#include <autoboost/math/special_functions/detail/unchecked_factorial.hpp>
#include <autoboost/math/special_functions/detail/lgamma_small.hpp>
#include <autoboost/math/special_functions/bernoulli.hpp>
#include <autoboost/math/special_functions/zeta.hpp>
...
template <class T>
inline bool is_odd(T v, const autoboost::true_type&)
{
   int i = static_cast<int>(v);
   return i&1;
}
template <class T>
inline bool is_odd(T v, const autoboost::false_type&)
{
...
{
   return is_odd(v, ::autoboost::is_convertible<T, int>());
}
...
T sinpx(T z)
{
   ...
   AUTOBOOST_ASSERT(fl >= 0);
   if(dist > 0.5)
      dist = 1 - dist;
   T result = sin(dist*autoboost::math::constants::pi<T>());
...
T gamma_imp(T z, const Policy& pol, const Lanczos& l)
{
...
#endif
   static const char* function = "autoboost::math::tgamma<%1%>(%1%)";
   ...
   if(z <= 0)
   {
      ...
      if(z <= -20)
      {
         ...
         if((fabs(result) < 1) && (tools::max_value<T>() * fabs(result) < autoboost::math::constants::pi<T>()))
            return -autoboost::math::sign(result) * policies::raise_overflow_error<T>(function, "Result of tgamma is too large to represent.", pol);
         result = -autoboost::math::constants::pi<T>() / result;
         if(result == 0)
            return policies::raise_underflow_error<T>(function, "Result of tgamma is too small to represent.", pol);
         if((autoboost::math::fpclassify)(result) == (int)FP_SUBNORMAL)
   ...
   else
   {
      ...
      T zgh = (z + static_cast<T>(Lanczos::g()) - autoboost::math::constants::half<T>());
      T lzgh = log(zgh);
      AUTOBOOST_MATH_INSTRUMENT_VARIABLE(result);
      AUTOBOOST_MATH_INSTRUMENT_VARIABLE(tools::log_max_value<T>());
      if(z * lzgh > tools::log_max_value<T>())
      {
         // we're going to overflow unless this is done with care:
         AUTOBOOST_MATH_INSTRUMENT_VARIABLE(zgh);
         if(lzgh * z / 2 > tools::log_max_value<T>())
            return autoboost::math::sign(result) * policies::raise_overflow_error<T>(function, "Result of tgamma is too large to represent.", pol);
         T hp = pow(zgh, (z / 2) - T(0.25));
         AUTOBOOST_MATH_INSTRUMENT_VARIABLE(hp);
         result *= hp / exp(zgh);
         AUTOBOOST_MATH_INSTRUMENT_VARIABLE(result);
      ...
      {
         AUTOBOOST_MATH_INSTRUMENT_VARIABLE(zgh);
         AUTOBOOST_MATH_INSTRUMENT_VARIABLE(pow(zgh, z - autoboost::math::constants::half<T>()));
         AUTOBOOST_MATH_INSTRUMENT_VARIABLE(exp(zgh));
         result *= pow(zgh, z - autoboost::math::constants::half<T>()) / exp(zgh);
         AUTOBOOST_MATH_INSTRUMENT_VARIABLE(result);
      }
   }
   return result;
...
T lgamma_imp(T z, const Policy& pol, const Lanczos& l, int* sign = 0)
{
   ...
   static const char* function = "autoboost::math::lgamma<%1%>(%1%)";
   ...
   if(z <= -tools::root_epsilon<T>())
   {
      ...
      result = log(autoboost::math::constants::pi<T>()) - lgamma_imp(z, pol, l) - log(t);
   }
   else if (z < tools::root_epsilon<T>())
   {
      if (0 == z)
   ...
   {
      // regular evaluation:
      T zgh = static_cast<T>(z + Lanczos::g() - autoboost::math::constants::half<T>());
      result = log(zgh) - 1;
      result *= z - 0.5f;
...
inline T upper_gamma_fraction(T a, T z, T eps)
{
   ...
   return 1 / (z - a + 1 + autoboost::math::tools::continued_fraction_a(f, eps));
}
...
inline T lower_gamma_series(T a, T z, const Policy& pol, T init_value = 0)
{
   ...
   T result = autoboost::math::tools::sum_series(s, factor, max_iter, init_value);
   policies::check_series_iterations<T>("autoboost::math::detail::lower_gamma_series<%1%>(%1%)", max_iter, pol);
   return result;
}
...
std::size_t highest_bernoulli_index()
{
   const float digits10_of_type = (std::numeric_limits<T>::is_specialized
                                      ...
                                      : static_cast<float>(autoboost::math::tools::digits<T>() * 0.301F));
...
T gamma_imp(T z, const Policy& pol, const lanczos::undefined_lanczos&)
{
   ...
   if((is_at_zero) || ((autoboost::math::isinf)(z) && (z < 0)))
   ...
   if((!b_neg) && floor_of_z_is_equal_to_z && (z < autoboost::math::max_factorial<T>::value))
   {
      return autoboost::math::unchecked_factorial<T>(itrunc(z) - 1);
   }
   ...
   {
      const T a0(1);
      const T a1(autoboost::math::constants::euler<T>());
      const T six_euler_squared((autoboost::math::constants::euler<T>() * autoboost::math::constants::euler<T>()) * 6);
      const T a2((six_euler_squared -  autoboost::math::constants::pi_sqr<T>()) / 12);
   ...
   {
      n_recur = autoboost::math::itrunc(min_arg_for_recursion - zz) + 1;
   ...
   if(b_neg)
   {
      ...
      AUTOBOOST_MATH_INSTRUMENT_VARIABLE(gamma_value);

      const bool result_is_too_large_to_represent = (   (abs(gamma_value) < 1)
                                                     && ((tools::max_value<T>() * abs(gamma_value)) < autoboost::math::constants::pi<T>()));

      if(result_is_too_large_to_represent)
         return policies::raise_overflow_error<T>(function, "Result of tgamma is too large to represent.", pol);

      gamma_value = -autoboost::math::constants::pi<T>() / gamma_value;
      AUTOBOOST_MATH_INSTRUMENT_VARIABLE(gamma_value);

      if(gamma_value == 0)
         return policies::raise_underflow_error<T>(function, "Result of tgamma is too small to represent.", pol);

      if((autoboost::math::fpclassify)(gamma_value) == static_cast<int>(FP_SUBNORMAL))
...
inline T log_gamma_near_1(const T& z, Policy const& pol)
{
   ...
   do
   {
      term = autoboost::math::zeta<T>(j + 2, pol) * power_term / (j + 2);
      if(j & 1)
         result -= term;
      else
         result += term;
...
T lgamma_imp(T z, const Policy& pol, const lanczos::undefined_lanczos&, int* sign)
{
   ...
   if((!b_neg) && floor_of_z_is_equal_to_z && (z < autoboost::math::max_factorial<T>::value))
   {
      return log(autoboost::math::unchecked_factorial<T>(itrunc(z) - 1));
   }
   ...
   else
   {
      ...
      const T one_over_x2                    = one_over_x_pow_two_n_minus_one * one_over_x_pow_two_n_minus_one;
            T sum                            = (autoboost::math::bernoulli_b2n<T>(1) / 2) * one_over_x_pow_two_n_minus_one;
      const T target_epsilon_to_break_loop   = (sum * autoboost::math::tools::epsilon<T>()) * T(1.0E-10F);
      ...
      for(std::size_t n = 2U; n < number_of_bernoullis_b2n; ++n)
      {
         ...
         const T term = (autoboost::math::bernoulli_b2n<T>(static_cast<int>(n)) * one_over_x_pow_two_n_minus_one) / (n2 * (n2 - 1U));
      ...
      const T half_ln_two_pi = log(autoboost::math::constants::two_pi<T>()) / 2;

      log_gamma_value = ((((zz - autoboost::math::constants::half<T>()) * log(zz)) - zz) + half_ln_two_pi) + sum;
   }
   ...
   if(b_neg)
   {
      ...
      log_gamma_value = - log_gamma_value
                        + log(autoboost::math::constants::pi<T>())
                        - log(t);
   }
...
T tgammap1m1_imp(T dz, Policy const& pol, const Lanczos& l)
{
   ...
   if(dz < 0)
   {
      if(dz < -0.5)
      {
         ...
         result = autoboost::math::tgamma(1+dz, pol) - 1;
         AUTOBOOST_MATH_INSTRUMENT_CODE(result);
      }
      else
      {
         // Use expm1 on lgamma:
         result = autoboost::math::expm1(-autoboost::math::log1p(dz, pol)
   ...
   else
   {
      ...
      {
         // Use expm1 on lgamma:
         result = autoboost::math::expm1(lgamma_small_imp<T>(dz+1, dz, dz-1, tag_type(), pol, l), pol);
         AUTOBOOST_MATH_INSTRUMENT_CODE(result);
      }
      else
      {
...
inline T tgammap1m1_imp(T z, Policy const& pol,
                 const ::autoboost::math::lanczos::undefined_lanczos&)
{
   AUTOBOOST_MATH_STD_USING // ADL of std names

   if(fabs(z) < 0.55)
   {
      return autoboost::math::expm1(log_gamma_near_1(z, pol));
   }
   return autoboost::math::expm1(autoboost::math::lgamma(1 + z, pol));
}
...
T full_igamma_prefix(T a, T z, const Policy& pol)
{
   ...
   //
   if((autoboost::math::fpclassify)(prefix) == (int)FP_INFINITE)
      return policies::raise_overflow_error<T>("autoboost::math::detail::full_igamma_prefix<%1%>(%1%, %1%)", "Result of incomplete gamma function is too large to represent.", pol);
...
T regularised_gamma_prefix(T a, T z, const Policy& pol, const Lanczos& l)
{
   ...
   else if((fabs(d*d*a) <= 100) && (a > 150))
   {
      ...
      prefix = a * autoboost::math::log1pmx(d, pol) + z * static_cast<T>(0.5 - Lanczos::g()) / agh;
      prefix = exp(prefix);
   }
   else
   {
   ...
   prefix *= sqrt(agh / autoboost::math::constants::e<T>()) / Lanczos::lanczos_sum_expG_scaled(a);
   return prefix;
}
//
...
T regularised_gamma_prefix(T a, T z, const Policy& pol, const lanczos::undefined_lanczos&)
{
   ...
   sum += detail::upper_gamma_fraction(a, limit, ::autoboost::math::policies::get_epsilon<T, Policy>());
...
inline T tgamma_small_upper_part(T a, T x, const Policy& pol, T* pgam = 0, bool invert = false, T* pderivative = 0)
{
   ...
   //
   T result;
   result = autoboost::math::tgamma1pm1(a, pol);
   if(pgam)
      *pgam = (result + 1) / a;
   T p = autoboost::math::powm1(x, a, pol);
   result -= p;
   result /= a;
   detail::small_gamma2_series<T> s(a, x);
   autoboost::uintmax_t max_iter = policies::get_max_series_iterations<Policy>() - 10;
   p += 1;
   if(pderivative)
      *pderivative = p / (*pgam * exp(x));
   T init_value = invert ? *pgam : 0;
   result = -p * tools::sum_series(s, autoboost::math::policies::get_epsilon<T, Policy>(), max_iter, (init_value - result) / p);
   policies::check_series_iterations<T>("autoboost::math::tgamma_small_upper_part<%1%>(%1%, %1%)", max_iter, pol);
   if(invert)
      result = -result;
   return result;
}
...
inline T finite_gamma_q(T a, T x, Policy const& pol, T* pderivative = 0)
{
   ...
   if(pderivative)
   {
      *pderivative = e * pow(x, a) / autoboost::math::unchecked_factorial<T>(itrunc(T(a - 1), pol));
   }
   return sum;
}
//
...
T finite_half_gamma_q(T a, T x, T* p_derivative, const Policy& pol)
{
   ...
   //
   AUTOBOOST_MATH_STD_USING
   T e = autoboost::math::erfc(sqrt(x), pol);
   if((e != 0) && (a > 1))
   {
...
{
   static const char* function = "autoboost::math::gamma_p<%1%>(%1%, %1%)";
   if(a <= 0)
      ...
      else
      {
         ...
         if(result == 0)
         {
            if(invert)
            {
               ...
               result = log(result) - a + (a - 0.5f) * log(a) + log(autoboost::math::constants::root_two_pi<T>());
               if(p_derivative)
         ...
         {
            result = log(result) + autoboost::math::lgamma(a, pol);
         }
      }
   ...
   AUTOBOOST_ASSERT((p_derivative == 0) || (normalised == true));
   ...
   case 0:
      {
         ...
         if(normalised == false)
            result *= autoboost::math::tgamma(a, pol);
         break;
      }
   case 1:
      {
   ...
   case 2:
      {
         ...
         if(result != 0)
         {
            ...
            if(invert)
            {
               init_value = (normalised ? 1 : autoboost::math::tgamma(a, pol));
   ...
   case 6:
      {
         ...
         result = !normalised ? pow(x, a) / (a) : pow(x, a) / autoboost::math::tgamma(a + 1, pol);
         result *= 1 - a * x / (a + 1);
      }
   }
   ...
   if(invert)
   {
      T gam = normalised ? 1 : autoboost::math::tgamma(a, pol);
      result = gam - result;
   }
   if(p_derivative)
   {
...
T tgamma_delta_ratio_imp_lanczos(T z, T delta, const Policy& pol, const Lanczos& l)
{
   ...
   if(z < tools::epsilon<T>())
   {
      ...
      //
      if(autoboost::math::max_factorial<T>::value < delta)
      {
         T ratio = tgamma_delta_ratio_imp_lanczos(delta, T(autoboost::math::max_factorial<T>::value - delta), pol, l);
         ratio *= z;
         ratio *= autoboost::math::unchecked_factorial<T>(autoboost::math::max_factorial<T>::value - 1);
         return 1 / ratio;
      }
      else
      {
         return 1 / (z * autoboost::math::tgamma(z + delta, pol));
      }
   }
   ...
   {
      if(fabs(delta) < 10)
         result = exp((constants::half<T>() - z) * autoboost::math::log1p(delta / zgh, pol));
      else
         result = 1;
   }
   else
...
T tgamma_delta_ratio_imp_lanczos(T z, T delta, const Policy& pol, const lanczos::undefined_lanczos&)
{
   ...
   if(delta < 10)
   {
      prefix *= exp(-z * autoboost::math::log1p(delta / z, pol));
   }
   else
   {
      prefix *= pow(z / zd, z);
   ...
   sum += detail::upper_gamma_fraction(z, z, ::autoboost::math::policies::get_epsilon<T, Policy>());
   T sumd = detail::lower_gamma_series(zd, zd, pol) / zd;
   sumd += detail::upper_gamma_fraction(zd, zd, ::autoboost::math::policies::get_epsilon<T, Policy>());
   sum /= sumd;
   if(fabs(tools::max_value<T>() / prefix) < fabs(sum))
      return policies::raise_overflow_error<T>("autoboost::math::tgamma_delta_ratio<%1%>(%1%, %1%)", "Result of tgamma is too large to represent.", pol);
   return sum * prefix;
}
...
T tgamma_delta_ratio_imp(T z, T delta, const Policy& pol)
{
   ...
   if((z <= 0) || (z + delta <= 0))
   {
      ...
      return autoboost::math::tgamma(z, pol) / autoboost::math::tgamma(z + delta, pol);
   }
...
T tgamma_ratio_imp(T x, T y, const Policy& pol)
{
   ...
   if((x <= 0) || (autoboost::math::isinf)(x))
      return policies::raise_domain_error<T>("autoboost::math::tgamma_ratio<%1%>(%1%, %1%)", "Gamma function ratios only implemented for positive arguments (got a=%1%).", x, pol);
   if((y <= 0) || (autoboost::math::isinf)(y))
      return policies::raise_domain_error<T>("autoboost::math::tgamma_ratio<%1%>(%1%, %1%)", "Gamma function ratios only implemented for positive arguments (got b=%1%).", y, pol);
   ...
   if((x < max_factorial<T>::value) && (y < max_factorial<T>::value))
   {
      ...
      return autoboost::math::tgamma(x, pol) / autoboost::math::tgamma(y, pol);
   }
   T prefix = 1;
   if(x < 1)
   {
      if(y < 2 * max_factorial<T>::value)
      {
         ...
         return prefix * autoboost::math::tgamma(x, pol) / autoboost::math::tgamma(y, pol);
      }
      //
      // result is almost certainly going to underflow to zero, try logs just in case:
      //
      return exp(autoboost::math::lgamma(x, pol) - autoboost::math::lgamma(y, pol));
   }
   if(y < 1)
   {
   ...
   //
   return autoboost::math::tgamma_delta_ratio(x, y - x, pol);
}
...
T gamma_p_derivative_imp(T a, T x, const Policy& pol)
{
   ...
   //
   if(a <= 0)
      return policies::raise_domain_error<T>("autoboost::math::gamma_p_derivative<%1%>(%1%, %1%)", "Argument a to the incomplete gamma function must be greater than zero (got a=%1%).", a, pol);
   if(x < 0)
      return policies::raise_domain_error<T>("autoboost::math::gamma_p_derivative<%1%>(%1%, %1%)", "Argument x to the incomplete gamma function must be >= 0 (got x=%1%).", x, pol);
   //
   // Now special cases:
   //
   if(x == 0)
   {
      return (a > 1) ? 0 :
         (a == 1) ? 1 : policies::raise_overflow_error<T>("autoboost::math::gamma_p_derivative<%1%>(%1%, %1%)", 0, pol);
   }
   //
   // Normal case:
   //
   ...
   {
      // overflow:
      return policies::raise_overflow_error<T>("autoboost::math::gamma_p_derivative<%1%>(%1%, %1%)", 0, pol);
   }
   if(f1 == 0)
   {
   ...
   tgamma(T z, const Policy& /* pol */, const mpl::true_)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::gamma_imp(static_cast<value_type>(z), forwarding_policy(), evaluation_type()), "autoboost::math::tgamma<%1%>(%1%)");
}
...
struct igamma_initializer
{
   struct init
   {
      ...
      static void do_init(const mpl::int_<N>&)
      {
         ...
         {
            autoboost::math::gamma_p(static_cast<T>(400), static_cast<T>(400), Policy());
         }
      }
...
struct lgamma_initializer
{
   struct init
   {
      ...
      {
         autoboost::math::lgamma(static_cast<T>(2.5), Policy());
         autoboost::math::lgamma(static_cast<T>(1.25), Policy());
         autoboost::math::lgamma(static_cast<T>(1.75), Policy());
      }
      static void do_init(const mpl::int_<113>&)
      {
         autoboost::math::lgamma(static_cast<T>(2.5), Policy());
         autoboost::math::lgamma(static_cast<T>(1.25), Policy());
         autoboost::math::lgamma(static_cast<T>(1.5), Policy());
         autoboost::math::lgamma(static_cast<T>(1.75), Policy());
      }
   ...
   tgamma(T1 a, T2 z, const Policy&, const mpl::false_)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      ...
      forwarding_policy(), static_cast<value_type*>(0)), "autoboost::math::tgamma<%1%>(%1%, %1%)");
}
   ...
   lgamma(T z, int* sign, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::lgamma_imp(static_cast<value_type>(z), forwarding_policy(), evaluation_type(), sign), "autoboost::math::lgamma<%1%>(%1%)");
}
...
{
   return ::autoboost::math::lgamma(x, 0, pol);
}
...
inline typename tools::promote_args<T>::type
   lgamma(T x)
{
   return ::autoboost::math::lgamma(x, 0, policies::policy<>());
}
   ...
   tgamma1pm1(T z, const Policy& /* pol */)
{
   ...
   return policies::checked_narrowing_cast<typename remove_cv<result_type>::type, forwarding_policy>(detail::tgammap1m1_imp(static_cast<value_type>(z), forwarding_policy(), evaluation_type()), "autoboost::math::tgamma1pm1<%!%>(%1%)");
}
   ...
   tgamma_delta_ratio(T1 z, T2 delta, const Policy& /* pol */)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::tgamma_delta_ratio_imp(static_cast<value_type>(z), static_cast<value_type>(delta), forwarding_policy()), "autoboost::math::tgamma_delta_ratio<%1%>(%1%, %1%)");
}
template <class T1, class T2>
inline typename tools::promote_args<T1, T2>::type
   ...
   tgamma_ratio(T1 a, T2 b, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::tgamma_ratio_imp(static_cast<value_type>(a), static_cast<value_type>(b), forwarding_policy()), "autoboost::math::tgamma_delta_ratio<%1%>(%1%, %1%)");
}
template <class T1, class T2>
inline typename tools::promote_args<T1, T2>::type
   ...
   gamma_p_derivative(T1 a, T2 x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::gamma_p_derivative_imp(static_cast<value_type>(a), static_cast<value_type>(x), forwarding_policy()), "autoboost::math::gamma_p_derivative<%1%>(%1%, %1%)");
}
template <class T1, class T2>
inline typename tools::promote_args<T1, T2>::type
...
#include <autoboost/math/special_functions/detail/igamma_inverse.hpp>
#include <autoboost/math/special_functions/detail/gamma_inva.hpp>
#include <autoboost/math/special_functions/erf.hpp>

#endif // AUTOBOOST_MATH_SF_GAMMA_HPP
