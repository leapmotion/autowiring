...
inline T tgammap1m1_imp(T dz, Policy const& pol,
                 const ::autoboost::math::lanczos::undefined_lanczos& l)
{
   ...
   //
   AUTOBOOST_MATH_INSTRUMENT_CODE((dz > -0.5));
   AUTOBOOST_MATH_INSTRUMENT_CODE((dz < 2));
   AUTOBOOST_MATH_INSTRUMENT_CODE((ldexp(1.0, autoboost::math::policies::digits<T, Policy>()) * fabs(result) < 1e34));
   if((dz > -0.5) && (dz < 2) && (ldexp(1.0, autoboost::math::policies::digits<T, Policy>()) * fabs(result) < 1e34))
   {
      result = tgammap1m1_imp(dz, pol, autoboost::math::lanczos::lanczos24m113());
      AUTOBOOST_MATH_INSTRUMENT_CODE(result);
   }
   return result;
}
