...
#ifndef AUTOBOOST_MATH_COS_PI_HPP
#define AUTOBOOST_MATH_COS_PI_HPP
...
inline typename tools::promote_args<T>::type cos_pi(T x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(autoboost::math::detail::cos_pi_imp<value_type>(x, forwarding_policy()), "cos_pi");
}

template <class T>
inline typename tools::promote_args<T>::type cos_pi(T x)
{
   return autoboost::math::cos_pi(x, policies::policy<>());
}
