...
#ifndef AUTOBOOST_MATH_LOG1P_INCLUDED
#define AUTOBOOST_MATH_LOG1P_INCLUDED
...
{ // The function returns the natural logarithm of 1 + x.
   ...
   static const char* function = "autoboost::math::log1p<%1%>(%1%)";

   if((x < -1) || (autoboost::math::isnan)(x))
...
{ // The function returns the natural logarithm of 1 + x.
   ...
   static const T P[] = {
      AUTOBOOST_MATH_BIG_CONSTANT(T, 64, -0.807533446680736736712e-19),
      AUTOBOOST_MATH_BIG_CONSTANT(T, 64, -0.490881544804798926426e-18),
      AUTOBOOST_MATH_BIG_CONSTANT(T, 64, 0.333333333333333373941),
      AUTOBOOST_MATH_BIG_CONSTANT(T, 64, 1.17141290782087994162),
      AUTOBOOST_MATH_BIG_CONSTANT(T, 64, 1.62790522814926264694),
      AUTOBOOST_MATH_BIG_CONSTANT(T, 64, 1.13156411870766876113),
      AUTOBOOST_MATH_BIG_CONSTANT(T, 64, 0.408087379932853785336),
      AUTOBOOST_MATH_BIG_CONSTANT(T, 64, 0.0706537026422828914622),
      AUTOBOOST_MATH_BIG_CONSTANT(T, 64, 0.00441709903782239229447)
   };
   static const T Q[] = {
      AUTOBOOST_MATH_BIG_CONSTANT(T, 64, 1.0),
      AUTOBOOST_MATH_BIG_CONSTANT(T, 64, 4.26423872346263928361),
      AUTOBOOST_MATH_BIG_CONSTANT(T, 64, 7.48189472704477708962),
      AUTOBOOST_MATH_BIG_CONSTANT(T, 64, 6.94757016732904280913),
      AUTOBOOST_MATH_BIG_CONSTANT(T, 64, 3.6493508622280767304),
      AUTOBOOST_MATH_BIG_CONSTANT(T, 64, 1.06884863623790638317),
      AUTOBOOST_MATH_BIG_CONSTANT(T, 64, 0.158292216998514145947),
      AUTOBOOST_MATH_BIG_CONSTANT(T, 64, 0.00885295524069924328658),
      AUTOBOOST_MATH_BIG_CONSTANT(T, 64, -0.560026216133415663808e-6)
   };
...
struct log1p_initializer
{
   struct init
   {
      ...
      {
         autoboost::math::log1p(static_cast<T>(0.25), Policy());
      }
...
inline typename tools::promote_args<T>::type log1p(T x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::log1p_imp(static_cast<value_type>(x), forwarding_policy(), tag_type()), "autoboost::math::log1p<%1%>(%1%)");
}
...
#if defined(AUTOBOOST_HAS_LOG1P) && !(defined(__osf__) && defined(__DECCXX_VER))
...
#elif defined(_MSC_VER) && (AUTOBOOST_MSVC >= 1400)
//
...
inline float log1p(float x, const Policy& pol)
{
   return static_cast<float>(autoboost::math::log1p(static_cast<double>(x), pol));
}
#ifndef _WIN32_WCE
//
...
{
   return autoboost::math::log1p(x, policies::policy<>());
}
//
// Compute log(1+x)-x:
//
   ...
   log1pmx(T x, const Policy& pol)
{
   typedef typename tools::promote_args<T>::type result_type;
   AUTOBOOST_MATH_STD_USING
   static const char* function = "autoboost::math::log1pmx<%1%>(%1%)";
   ...
   if(a < tools::epsilon<result_type>())
      return -x * x / 2;
   autoboost::math::detail::log1p_series<T> s(x);
   s();
   autoboost::uintmax_t max_iter = policies::get_max_series_iterations<Policy>();
#if AUTOBOOST_WORKAROUND(__BORLANDC__, AUTOBOOST_TESTED_AT(0x582))
   T zero = 0;
   T result = autoboost::math::tools::sum_series(s, policies::get_epsilon<T, Policy>(), max_iter, zero);
#else
   T result = autoboost::math::tools::sum_series(s, policies::get_epsilon<T, Policy>(), max_iter);
#endif
   policies::check_series_iterations<T>(function, max_iter, pol);
   return result;
}
...
#endif // AUTOBOOST_MATH_LOG1P_INCLUDED
