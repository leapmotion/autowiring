...
#ifndef AUTOBOOST_MATH_ELLINT_3_HPP
#define AUTOBOOST_MATH_ELLINT_3_HPP
...
#include <autoboost/math/special_functions/ellint_2.hpp>
#include <autoboost/math/special_functions/log1p.hpp>
#include <autoboost/math/special_functions/atanh.hpp>
...
T ellint_pi_imp(T v, T phi, T k, T vc, const Policy& pol)
{
   ...
   static const char* function = "autoboost::math::ellint_3<%1%>(%1%,%1%,%1%)";
   ...
   if((phi > constants::half_pi<T>()) || (phi < 0))
   {
      ...
      {
         T rphi = autoboost::math::tools::fmod_workaround(T(fabs(phi)), T(constants::half_pi<T>()));
         T m = autoboost::math::round((fabs(phi) - rphi) / constants::half_pi<T>());
         int sign = 1;
         if((m != 0) && (k >= 1))
         {
   ...
   if(k == 0)
   {
      ...
      else
      {
         ...
         T vcr = sqrt(-vc);
         T arg = vcr * tan(phi);
         return (autoboost::math::log1p(arg, pol) - autoboost::math::log1p(-arg, pol)) / (2 * vcr);
      }
   }
   if(v < 0)
   {
   ...
   // normalised above.
   //
   AUTOBOOST_ASSERT(fabs(phi) < constants::half_pi<T>());
   AUTOBOOST_ASSERT(phi >= 0);
   T x, y, z, p, t;
   T cosp = cos(phi);
   x = cosp * cosp;
   t = sphi * sphi;
...
T ellint_pi_imp(T v, T k, T vc, const Policy& pol)
{
    ...
    static const char* function = "autoboost::math::ellint_pi<%1%>(%1%,%1%)";
    ...
    if(v == 0)
    {
       return (k == 0) ? autoboost::math::constants::pi<T>() / 2 : ellint_k_imp(k, pol);
    }
    ...
    if(v < 0)
    {
       ...
       T Nm1 = (1 - k2) / (1 - v);
       T result = 0;
       result = autoboost::math::detail::ellint_pi_imp(N, k, Nm1, pol);
...
{
   return autoboost::math::ellint_3(k, v, phi, policies::policy<>());
}
...
inline typename tools::promote_args<T1, T2>::type ellint_3(T1 k, T2 v, const Policy& pol, const mpl::true_&)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(
      detail::ellint_pi_imp(
         ...
         pol), "autoboost::math::ellint_3<%1%>(%1%,%1%)");
}
...
inline typename tools::promote_args<T1, T2, T3>::type ellint_3(T1 k, T2 v, T3 phi, const Policy& pol)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(
      detail::ellint_pi_imp(
         ...
         static_cast<value_type>(k),
         static_cast<value_type>(1-v),
         pol), "autoboost::math::ellint_3<%1%>(%1%,%1%,%1%)");
}
...
#endif // AUTOBOOST_MATH_ELLINT_3_HPP
