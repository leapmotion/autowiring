...
#ifndef AUTOBOOST_MATH_ELLINT_RJ_HPP
#define AUTOBOOST_MATH_ELLINT_RJ_HPP
...
#include <autoboost/math/special_functions/ellint_rc.hpp>
...
{
   using namespace autoboost::math;
   // Calculate RC(1, 1 + x)
  ...
  static const char* function = "autoboost::math::ellint_rc<%1%>(%1%,%1%)";
   ...
   else
   {
      ...
      {
         T arg = sqrt(-y);
         result = (autoboost::math::log1p(arg) - autoboost::math::log1p(-arg)) / (2 * sqrt(-y));
      }
      else
      {
...
T ellint_rj_imp(T x, T y, T z, T p, const Policy& pol)
{
   ...
   static const char* function = "autoboost::math::ellint_rj<%1%>(%1%,%1%,%1%)";
   ...
   if(p < 0)
   {
      ...
      AUTOBOOST_ASSERT(x <= y);
      AUTOBOOST_ASSERT(y <= z);
   ...
   ellint_rj(T1 x, T2 y, T3 z, T4 p, const Policy& pol)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(
      detail::ellint_rj_imp(
         ...
         pol), "autoboost::math::ellint_rj<%1%>(%1%,%1%,%1%,%1%)");
}
...
#endif // AUTOBOOST_MATH_ELLINT_RJ_HPP
