...
#ifndef AUTOBOOST_MATH_SPECIAL_NEXT_HPP
#define AUTOBOOST_MATH_SPECIAL_NEXT_HPP
...
#include "xmmintrin.h"
#define AUTOBOOST_MATH_CHECK_SSE2
#endif
#endif
...
#ifdef AUTOBOOST_MATH_CHECK_SSE2
...
inline double float_next(const double& val, const Policy& pol)
{
   ...
   if(!(autoboost::math::isfinite)(val) && (val > 0))
...
inline typename tools::promote_args<T, U>::type nextafter(const T& val, const U& direction, const Policy& pol)
{
   ...
   return val < direction ? autoboost::math::float_next<result_type>(val, pol) : val == direction ? val : autoboost::math::float_prior<result_type>(val, pol);
}
...
T float_distance_imp(const T& a, const T& b, const Policy& pol)
{
   ...
   if(!(autoboost::math::isfinite)(a))
      return policies::raise_domain_error<T>(
         function,
         "Argument a must be finite, but got %1%", a, pol);
   if(!(autoboost::math::isfinite)(b))
      return policies::raise_domain_error<T>(
         function,
   ...
   if(autoboost::math::sign(a) != autoboost::math::sign(b))
   ...
   AUTOBOOST_ASSERT(a >= 0);
   AUTOBOOST_ASSERT(b >= a);
   ...
   //
   frexp(((autoboost::math::fpclassify)(a) == (int)FP_SUBNORMAL) ? tools::min_value<T>() : a, &expon);
   ...
   //
   T mb, x, y, z;
   if(((autoboost::math::fpclassify)(a) == (int)FP_SUBNORMAL) || (b - a < tools::min_value<T>()))
   {
      //
   ...
   //
   AUTOBOOST_ASSERT(result == floor(result));
   return result;
} // float_distance_imp
...
{
   return autoboost::math::float_distance(a, b, policies::policy<>());
}
...
{
   return autoboost::math::float_advance(val, distance, policies::policy<>());
}

}} // autoboost math namespaces

#endif // AUTOBOOST_MATH_SPECIAL_NEXT_HPP
