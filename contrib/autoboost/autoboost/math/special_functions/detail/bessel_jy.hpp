...
#ifndef AUTOBOOST_MATH_BESSEL_JY_HPP
#define AUTOBOOST_MATH_BESSEL_JY_HPP
...
#include <autoboost/math/special_functions/hypot.hpp>
#include <autoboost/math/special_functions/sin_pi.hpp>
#include <autoboost/math/special_functions/cos_pi.hpp>
#include <autoboost/math/special_functions/detail/bessel_jy_asym.hpp>
#include <autoboost/math/special_functions/detail/bessel_jy_series.hpp>
...
namespace autoboost { namespace math {
   ...
   namespace detail {
      ...
      int temme_jy(T v, T x, T* Y, T* Y1, const Policy& pol)
      {
         ...
         AUTOBOOST_ASSERT(fabs(v) <= 0.5f);  // precondition for using this routine

         T gp = autoboost::math::tgamma1pm1(v, pol);
         T gm = autoboost::math::tgamma1pm1(-v, pol);
         T spv = autoboost::math::sin_pi(v, pol);
         T spv2 = autoboost::math::sin_pi(v/2, pol);
         T xp = pow(x/2, v);
         ...
         policies::check_series_iterations<T>("autoboost::math::bessel_jy<%1%>(%1%,%1%) in temme_jy", k, pol);
         *Y = -sum;
         *Y1 = -2 * sum1 / x;
      ...
      int CF1_jy(T v, T x, T* fv, int* sign, const Policy& pol)
      {
         ...
         policies::check_series_iterations<T>("autoboost::math::bessel_jy<%1%>(%1%,%1%) in CF1_jy", k / 100, pol);
         *fv = -f;
      ...
      int CF2_jy(T v, T x, T* p, T* q, const Policy& pol)
      {
         ...
         policies::check_series_iterations<T>("autoboost::math::bessel_jy<%1%>(%1%,%1%) in CF2_jy", k, pol);
         *p = fr;
         *q = fi;
      ...
      int bessel_jy(T v, T x, T* J, T* Y, int kind, const Policy& pol)
      {
         ...
         static const char* function = "autoboost::math::bessel_jy<%1%>(%1%,%1%)";
         ...
         {
            T z = (u + n % 2);
            cp = autoboost::math::cos_pi(z, pol);
            sp = autoboost::math::sin_pi(z, pol);
            if(u != 0)
         ...
         else if((x > 8) && hankel_PQ(v, x, &p, &q, pol))
         {
            ...
            //
            // chi = x - fmod(T(v / 2 + 0.25f), T(2)) * autoboost::math::constants::pi<T>();
            //
            ...
            T chi = autoboost::math::constants::root_two<T>() / (autoboost::math::constants::root_pi<T>() * sqrt(x)); //sqrt(2 / (autoboost::math::constants::pi<T>() * x));
            Yv = chi * (p * sc + q * cc);
            Jv = chi * (p * cc - q * sc);
         }
         ...
         else                                    // x in (2, \infty)
         {
            ...
            AUTOBOOST_MATH_INSTRUMENT_VARIABLE(init);
            prev = fv * s * init;
            current = s * init;
            ...
            AUTOBOOST_MATH_INSTRUMENT_VARIABLE(current);
            AUTOBOOST_MATH_INSTRUMENT_VARIABLE(W);
            AUTOBOOST_MATH_INSTRUMENT_VARIABLE(q);
            AUTOBOOST_MATH_INSTRUMENT_VARIABLE(gamma);
            AUTOBOOST_MATH_INSTRUMENT_VARIABLE(p);
            AUTOBOOST_MATH_INSTRUMENT_VARIABLE(t);
            Ju = sign(current) * sqrt(W / (q + gamma * (p - t)));
            AUTOBOOST_MATH_INSTRUMENT_VARIABLE(Ju);
...
#endif // AUTOBOOST_MATH_BESSEL_JY_HPP
