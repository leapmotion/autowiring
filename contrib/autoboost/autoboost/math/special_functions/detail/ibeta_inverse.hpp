...
#ifndef AUTOBOOST_MATH_SPECIAL_FUNCTIONS_IBETA_INVERSE_HPP
#define AUTOBOOST_MATH_SPECIAL_FUNCTIONS_IBETA_INVERSE_HPP

#ifdef _MSC_VER
#pragma once
#endif

#include <autoboost/math/special_functions/beta.hpp>
#include <autoboost/math/special_functions/erf.hpp>
#include <autoboost/math/tools/roots.hpp>
#include <autoboost/math/special_functions/detail/t_distribution_inv.hpp>
...
struct temme_root_finder
{
   ...
   autoboost::math::tuple<T, T> operator()(T x)
   {
      ...
      if(y == 0)
      {
         ...
         return autoboost::math::make_tuple(static_cast<T>(-big), static_cast<T>(-big));
      }
      if(x == 0)
      {
         T big = tools::max_value<T>() / 4;
         return autoboost::math::make_tuple(static_cast<T>(-big), big);
      }
      T f = log(x) + a * log(y) + t;
      T f1 = (1 / x) - (a / (y));
      return autoboost::math::make_tuple(f, f1);
   }
private:
   T t, a;
};
...
T temme_method_1_ibeta_inverse(T a, T b, T z, const Policy& pol)
{
   ...
   //
   T eta0 = autoboost::math::erfc_inv(2 * z, pol);
   eta0 /= -sqrt(a / 2);
   ...
   AUTOBOOST_ASSERT(x >= 0);
   AUTOBOOST_ASSERT(x <= 1);
   AUTOBOOST_ASSERT(eta * (x - 0.5) >= 0);
#ifdef AUTOBOOST_INSTRUMENT
...
T temme_method_2_ibeta_inverse(T /*a*/, T /*b*/, T z, T r, T theta, const Policy& pol)
{
   ...
   static const AUTOBOOST_MATH_INT_TABLE_TYPE(T, int) co1[] = { -1, -5, 5 };
   workspace[1] = -tools::evaluate_even_polynomial(co1, s, 3) / (36 * sc_2);
   static const AUTOBOOST_MATH_INT_TABLE_TYPE(T, int) co2[] = { 1, 21, -69, 46 };
   workspace[2] = tools::evaluate_even_polynomial(co2, s, 4) / (1620 * sc_3);
   static const AUTOBOOST_MATH_INT_TABLE_TYPE(T, int) co3[] = { 7, -2, 33, -62, 31 };
   workspace[3] = -tools::evaluate_even_polynomial(co3, s, 5) / (6480 * sc_4);
   static const AUTOBOOST_MATH_INT_TABLE_TYPE(T, int) co4[] = { 25, -52, -17, 88, -115, 46 };
   workspace[4] = tools::evaluate_even_polynomial(co4, s, 6) / (90720 * sc_5);
   terms[1] = tools::evaluate_polynomial(workspace, eta0, 5);
   //
   // Now evaluate e2 and put it in terms[2]:
   //
   static const AUTOBOOST_MATH_INT_TABLE_TYPE(T, int) co5[] = { 7, 12, -78, 52 };
   workspace[0] = -tools::evaluate_even_polynomial(co5, s, 4) / (405 * sc_3);
   static const AUTOBOOST_MATH_INT_TABLE_TYPE(T, int) co6[] = { -7, 2, 183, -370, 185 };
   workspace[1] = tools::evaluate_even_polynomial(co6, s, 5) / (2592 * sc_4);
   static const AUTOBOOST_MATH_INT_TABLE_TYPE(T, int) co7[] = { -533, 776, -1835, 10240, -13525, 5410 };
   workspace[2] = -tools::evaluate_even_polynomial(co7, s, 6) / (204120 * sc_5);
   static const AUTOBOOST_MATH_INT_TABLE_TYPE(T, int) co8[] = { -1579, 3747, -3372, -15821, 45588, -45213, 15071 };
   workspace[3] = -tools::evaluate_even_polynomial(co8, s, 7) / (2099520 * sc_6);
   terms[2] = tools::evaluate_polynomial(workspace, eta0, 4);
   //
   // And e3, and put it in terms[3]:
   //
   static const AUTOBOOST_MATH_INT_TABLE_TYPE(T, int) co9[] = {449, -1259, -769, 6686, -9260, 3704 };
   workspace[0] = tools::evaluate_even_polynomial(co9, s, 6) / (102060 * sc_5);
   static const AUTOBOOST_MATH_INT_TABLE_TYPE(T, int) co10[] = { 63149, -151557, 140052, -727469, 2239932, -2251437, 750479 };
   workspace[1] = -tools::evaluate_even_polynomial(co10, s, 7) / (20995200 * sc_6);
   static const AUTOBOOST_MATH_INT_TABLE_TYPE(T, int) co11[] = { 29233, -78755, 105222, 146879, -1602610, 3195183, -2554139, 729754 };
   ...
   if(fabs(eta) < 0.7)
   {
      ...
      static const AUTOBOOST_MATH_INT_TABLE_TYPE(T, int) co12[] = { 1, -13, 13 };
      workspace[3] = tools::evaluate_polynomial(co12, workspace[0], 3) / (36 * s * c);
      static const AUTOBOOST_MATH_INT_TABLE_TYPE(T, int) co13[] = { 1, 21, -69, 46 };
...
T temme_method_3_ibeta_inverse(T a, T b, T p, T q, const Policy& pol)
{
   ...
   T eta0;
   if(p < q)
      eta0 = autoboost::math::gamma_q_inv(b, p, pol);
   else
      eta0 = autoboost::math::gamma_p_inv(b, q, pol);
   eta0 /= a;
   //
...
struct ibeta_roots
{
   ...
   autoboost::math::tuple<T, T, T> operator()(T x)
   {
      AUTOBOOST_MATH_STD_USING // ADL of std names
      ...
      return autoboost::math::make_tuple(f, f1, f2);
   }
private:
   T a, b, target;
   bool invert;
...
{
   AUTOBOOST_MATH_STD_USING  // For ADL of math functions.
   ...
   else if(b == 1)
   {
      if(p < q)
      {
         ...
         {
            x = pow(p, 1 / a);
            y = -autoboost::math::expm1(log(p) / a, pol);
         }
         else
         {
            x = pow(p, 1 / a);
      ...
      {
         x = exp(autoboost::math::log1p(-q, pol) / a);
         y = -autoboost::math::expm1(autoboost::math::log1p(-q, pol) / a, pol);
      }
      if(invert)
         std::swap(x, y);
      if(py)
   ...
   else if(a + b > 5)
   {
      ...
      else
      {
         ...
         if((lambda >= 0.2) && (lambda <= 0.8) && (r >= 10))
         {
            ...
            {
               x = ppa * pow(a * autoboost::math::beta(a, b, pol), 1/a);
            }
            else
         ...
         else
         {
            ...
            T bet = 0;
            if(b < 2)
               bet = autoboost::math::beta(a, b, pol);
            if(bet != 0)
            {
               y = pow(b * q * bet, 1/b);
               x = 1 - y;
   ...
   else if((a < 1) && (b < 1))
   {
      ...
      //
      T fs = autoboost::math::ibeta(a, b, xs, pol) - p;
      ...
      T xg = pow(a * p * autoboost::math::beta(a, b, pol), 1/a);
      x = xg / (1 + xg);
      y = 1 / (1 + xg);
      //
   ...
   else if((a > 1) && (b > 1))
   {
      ...
      T xs = (a - 1) / (a + b - 2);
      T xs2 = (b - 1) / (a + b - 2);
      T ps = autoboost::math::ibeta(a, b, xs, pol) - p;
      ...
      //
      T lx = log(p * a * autoboost::math::beta(a, b, pol)) / a;
      x = exp(lx);
      y = x < 0.9 ? T(1 - x) : (T)(-autoboost::math::expm1(lx, pol));
   ...
   else /*if((a <= 1) != (b <= 1))*/
   {
      ...
      if(pow(p, 1/a) < 0.5)
      {
         x = pow(p * a * autoboost::math::beta(a, b, pol), 1 / a);
         if(x == 0)
            x = autoboost::math::tools::min_value<T>();
         y = 1 - x;
      }
      else /*if(pow(q, 1/b) < 0.1)*/
      {
         // model a distorted quarter circle:
         y = pow(1 - pow(p, b * autoboost::math::beta(a, b, pol)), 1/b);
         if(y == 0)
            y = autoboost::math::tools::min_value<T>();
         x = 1 - y;
      }
   }
   ...
   if(lower == 0)
   {
      if(invert && (py == 0))
      {
         ...
         //
         lower = autoboost::math::tools::epsilon<T>();
         if(x < lower)
            x = lower;
      }
      else
         lower = autoboost::math::tools::min_value<T>();
      if(x < lower)
         x = lower;
   }
   //
   ...
   //
   int digits = autoboost::math::policies::digits<T, Policy>() / 2;
   if((x < 1e-50) && ((a < 1) || (b < 1)))
   {
      //
   ...
   x = autoboost::math::tools::halley_iterate(
      autoboost::math::detail::ibeta_roots<T, Policy>(a, b, (p < q ? p : q), (p < q ? false : true)), x, lower, upper, digits, max_iter);
   policies::check_root_iterations<T>("autoboost::math::ibeta<%1%>(%1%, %1%, %1%)", max_iter, pol);
   //
   ...
   //
   //AUTOBOOST_ASSERT(x != upper);
   //AUTOBOOST_ASSERT((x != lower) || (x == autoboost::math::tools::min_value<T>()) || (x == autoboost::math::tools::epsilon<T>()));
   //
...
{
   static const char* function = "autoboost::math::ibeta_inv<%1%>(%1%,%1%,%1%)";
   AUTOBOOST_FPU_EXCEPTION_GUARD
...
{
   static const char* function = "autoboost::math::ibetac_inv<%1%>(%1%,%1%,%1%)";
   AUTOBOOST_FPU_EXCEPTION_GUARD
   typedef typename tools::promote_args<T1, T2, T3, T4>::type result_type;
   typedef typename policies::evaluation<result_type, Policy>::type value_type;
   typedef typename policies::normalise<
...
#endif // AUTOBOOST_MATH_SPECIAL_FUNCTIONS_IGAMMA_INVERSE_HPP
