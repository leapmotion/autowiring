...
#ifndef AUTOBOOST_MATH_SP_DETAIL_BETA_INV_AB
#define AUTOBOOST_MATH_SP_DETAIL_BETA_INV_AB
...
struct beta_inv_ab_t
{
   ...
   {
      return invert ?
         p - autoboost::math::ibetac(swap_ab ? b : a, swap_ab ? a : b, z, Policy())
         : autoboost::math::ibeta(swap_ab ? b : a, swap_ab ? a : b, z, Policy()) - p;
   }
private:
   T b, z, p;
   bool invert, swap_ab;
...
T inverse_negative_binomial_cornish_fisher(T n, T sf, T sfc, T p, T q, const Policy& pol)
{
   ...
   T x = autoboost::math::erfc_inv(p > q ? 2 * q : 2 * p, pol) * constants::root_two<T>();
   // Set the sign:
   if(p < 0.5)
      x = -x;
   T x2 = x * x;
...
{
   AUTOBOOST_MATH_STD_USING  // for ADL of std lib math functions
   //
   // Special cases first:
   //
   AUTOBOOST_MATH_INSTRUMENT_CODE("b = " << b << " z = " << z << " p = " << p << " q = " << " swap = " << swap_ab);
   if(p == 0)
   {
   ...
   AUTOBOOST_MATH_INSTRUMENT_CODE("guess = " << guess);
   //
   ...
   if(max_iter >= policies::get_max_root_iterations<Policy>())
      return policies::raise_evaluation_error<T>("autoboost::math::ibeta_invab_imp<%1%>(%1%,%1%,%1%)", "Unable to locate the root within a reasonable number of iterations, closest approximation so far was %1%", r.first, pol);
      ...
      ibeta_inva(RT1 b, RT2 x, RT3 p, const Policy& pol)
{
   ...
   static const char* function = "autoboost::math::ibeta_inva<%1%>(%1%,%1%,%1%)";
   if(p == 0)
   {
      ...
      ibetac_inva(RT1 b, RT2 x, RT3 q, const Policy& pol)
{
   ...
   static const char* function = "autoboost::math::ibetac_inva<%1%>(%1%,%1%,%1%)";
   if(q == 1)
   {
      return policies::raise_overflow_error<result_type>(function, 0, Policy());
   }
      ...
      ibeta_invb(RT1 a, RT2 x, RT3 p, const Policy& pol)
{
   ...
   static const char* function = "autoboost::math::ibeta_invb<%1%>(%1%,%1%,%1%)";
   if(p == 0)
   {
      return tools::min_value<result_type>();
   }
...
{
   return autoboost::math::ibeta_inva(b, x, p, policies::policy<>());
}
...
{
   return autoboost::math::ibetac_inva(b, x, q, policies::policy<>());
}
...
{
   return autoboost::math::ibeta_invb(a, x, p, policies::policy<>());
}
...
{
   return autoboost::math::ibetac_invb(a, x, q, policies::policy<>());
}

} // namespace math
} // namespace autoboost

#endif // AUTOBOOST_MATH_SP_DETAIL_BETA_INV_AB
