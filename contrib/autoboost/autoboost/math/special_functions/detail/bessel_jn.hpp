...
#ifndef AUTOBOOST_MATH_BESSEL_JN_HPP
#define AUTOBOOST_MATH_BESSEL_JN_HPP
...
T bessel_jn(int n, T x, const Policy& pol)
{
    ...
    AUTOBOOST_ASSERT(n > 1);
    T scale = 1;
    if (n < abs(x))                         // forward recurrence
    {
        prev = bessel_j0(x);
        current = bessel_j1(x);
        policies::check_series_iterations<T>("autoboost::math::bessel_j_n<%1%>(%1%,%1%)", n, pol);
        for (int k = 1; k < n; k++)
        {
            T fact = 2 * k / x;
            //
    ...
    else                                    // backward recurrence
    {
        ...
        autoboost::math::detail::CF1_jy(static_cast<T>(n), x, &fn, &s, pol);
        prev = fn;
        current = 1;
    ...
    if(tools::max_value<T>() * scale < fabs(value))
       return policies::raise_overflow_error<T>("autoboost::math::bessel_jn<%1%>(%1%,%1%)", 0, pol);
...
#endif // AUTOBOOST_MATH_BESSEL_JN_HPP
