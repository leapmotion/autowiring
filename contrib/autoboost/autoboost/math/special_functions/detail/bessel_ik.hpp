...
#ifndef AUTOBOOST_MATH_BESSEL_IK_HPP
#define AUTOBOOST_MATH_BESSEL_IK_HPP
...
inline T bessel_i_small_z_series(T v, T x, const Policy& pol)
{
   ...
   {
      prefix = pow(x / 2, v) / autoboost::math::tgamma(v + 1, pol);
   }
   else
   {
      prefix = v * log(x / 2) - autoboost::math::lgamma(v + 1, pol);
      prefix = exp(prefix);
   }
   if(prefix == 0)
      return prefix;
...
#endif
   policies::check_series_iterations<T>("autoboost::math::bessel_j_small_z_series<%1%>(%1%,%1%)", max_iter, pol);
...
int temme_ik(T v, T x, T* K, T* K1, const Policy& pol)
{
    ...
    AUTOBOOST_ASSERT(abs(x) <= 2);
    AUTOBOOST_ASSERT(abs(v) <= 0.5f);

    T gp = autoboost::math::tgamma1pm1(v, pol);
    T gm = autoboost::math::tgamma1pm1(-v, pol);

    a = log(x / 2);
    b = exp(v * a);
    sigma = -a * v;
    c = abs(v) < tools::epsilon<T>() ?
       T(1) : T(autoboost::math::sin_pi(v) / (v * pi<T>()));
    ...
    AUTOBOOST_MATH_INSTRUMENT_VARIABLE(p);
    AUTOBOOST_MATH_INSTRUMENT_VARIABLE(q);
    AUTOBOOST_MATH_INSTRUMENT_VARIABLE(f);
    AUTOBOOST_MATH_INSTRUMENT_VARIABLE(sigma);
    AUTOBOOST_MATH_INSTRUMENT_CODE(sinh(sigma));
    AUTOBOOST_MATH_INSTRUMENT_VARIABLE(gamma1);
    AUTOBOOST_MATH_INSTRUMENT_VARIABLE(gamma2);
    AUTOBOOST_MATH_INSTRUMENT_VARIABLE(c);
    AUTOBOOST_MATH_INSTRUMENT_VARIABLE(d);
    ...
    policies::check_series_iterations<T>("autoboost::math::bessel_ik<%1%>(%1%,%1%) in temme_ik", k, pol);
...
int CF1_ik(T v, T x, T* fv, const Policy& pol)
{
    ...
    AUTOBOOST_MATH_INSTRUMENT_VARIABLE(tolerance);
    tiny = sqrt(tools::min_value<T>());
    AUTOBOOST_MATH_INSTRUMENT_VARIABLE(tiny);
    ...
    for (k = 1; k < policies::get_max_series_iterations<Policy>(); k++)
    {
        ...
        delta = C * D;
        f *= delta;
        AUTOBOOST_MATH_INSTRUMENT_VARIABLE(delta-1);
    ...
    policies::check_series_iterations<T>("autoboost::math::bessel_ik<%1%>(%1%,%1%) in CF1_ik", k, pol);
...
int CF2_ik(T v, T x, T* Kv, T* Kv1, const Policy& pol)
{
    ...
    for (k = 2; k < policies::get_max_series_iterations<Policy>(); k++)     // starting from 2
    {
        ...
        AUTOBOOST_MATH_INSTRUMENT_VARIABLE(Q * delta);
        AUTOBOOST_MATH_INSTRUMENT_VARIABLE(abs(S) * tolerance);
        AUTOBOOST_MATH_INSTRUMENT_VARIABLE(S);
        if (abs(Q * delta) < abs(S) * tolerance)
        {
           break;
        }
    }
    policies::check_series_iterations<T>("autoboost::math::bessel_ik<%1%>(%1%,%1%) in CF2_ik", k, pol);
    ...
    *Kv1 = *Kv * (0.5f + v + x + (v * v - 0.25f) * f) / x;
    AUTOBOOST_MATH_INSTRUMENT_VARIABLE(*Kv);
    AUTOBOOST_MATH_INSTRUMENT_VARIABLE(*Kv1);
...
int bessel_ik(T v, T x, T* I, T* K, int kind, const Policy& pol)
{
    ...
    AUTOBOOST_MATH_INSTRUMENT_VARIABLE(kind);

    AUTOBOOST_MATH_STD_USING
    using namespace autoboost::math::tools;
    using namespace autoboost::math::constants;

    static const char* function = "autoboost::math::bessel_ik<%1%>(%1%,%1%)";
    ...
    n = iround(v, pol);
    u = v - n;                              // -1/2 <= u < 1/2
    AUTOBOOST_MATH_INSTRUMENT_VARIABLE(n);
    AUTOBOOST_MATH_INSTRUMENT_VARIABLE(u);
    ...
    if (x == 0)
    {
       ...
       {
           T z = (u + n % 2);
           Iv = autoboost::math::sin_pi(z, pol) == 0 ?
               Iv :
    ...
    AUTOBOOST_MATH_INSTRUMENT_VARIABLE(Ku);
    AUTOBOOST_MATH_INSTRUMENT_VARIABLE(Ku1);
    prev = Ku;
    current = Ku1;
    T scale = 1;
    T scale_sign = 1;
    for (k = 1; k <= n; k++)                   // forward recurrence for K
    {
        ...
        if((tools::max_value<T>() - fabs(prev)) / fact < fabs(current))
        {
           prev /= current;
           scale /= current;
           scale_sign *= autoboost::math::sign(current);
           current = 1;
        }
    ...
    {
        T z = (u + n % 2);
        T fact = (2 / pi<T>()) * (autoboost::math::sin_pi(z) * Kv);
        if(fact == 0)
           *I = Iv;
...
#endif // AUTOBOOST_MATH_BESSEL_IK_HPP
