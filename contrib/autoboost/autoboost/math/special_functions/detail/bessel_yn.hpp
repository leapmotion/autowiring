...
#ifndef AUTOBOOST_MATH_BESSEL_YN_HPP
#define AUTOBOOST_MATH_BESSEL_YN_HPP

#ifdef _MSC_VER
#pragma once
#endif

#include <autoboost/math/special_functions/detail/bessel_y0.hpp>
#include <autoboost/math/special_functions/detail/bessel_y1.hpp>
...
T bessel_yn(int n, T x, const Policy& pol)
{
    ...
    static const char* function = "autoboost::math::bessel_yn<%1%>(%1%,%1%)";
    ...
    if(x < policies::get_epsilon<T, Policy>())
    {
       ...
       if(tools::max_value<T>() * fabs(scale) < fabs(value))
          return autoboost::math::sign(scale) * autoboost::math::sign(value) * policies::raise_overflow_error<T>(function, 0, pol);
       value /= scale;
    }
    ...
    else
    {
       ...
       int k = 1;
       AUTOBOOST_ASSERT(k < n);
       policies::check_series_iterations<T>("autoboost::math::bessel_y_n<%1%>(%1%,%1%)", n, pol);
       T mult = 2 * k / x;
...
#endif // AUTOBOOST_MATH_BESSEL_YN_HPP
