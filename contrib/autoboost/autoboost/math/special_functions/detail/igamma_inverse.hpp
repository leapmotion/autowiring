...
#ifndef AUTOBOOST_MATH_SPECIAL_FUNCTIONS_IGAMMA_INVERSE_HPP
#define AUTOBOOST_MATH_SPECIAL_FUNCTIONS_IGAMMA_INVERSE_HPP
...
inline T didonato_FN(T p, T a, T x, unsigned N, T tolerance, const Policy& pol)
{
   ...
   //
   AUTOBOOST_MATH_STD_USING
   T u = log(p) + autoboost::math::lgamma(a + 1, pol);
...
T find_inverse_gamma(T a, T p, T q, const Policy& pol, bool* p_has_10_digits)
{
   ...
   *p_has_10_digits = false;
   ...
   {
      T g = autoboost::math::tgamma(a, pol);
      T b = q * g;
      AUTOBOOST_MATH_INSTRUMENT_VARIABLE(g);
   ...
   else
   {
      ...
      AUTOBOOST_MATH_INSTRUMENT_VARIABLE(ra);
      ...
      else if (p > 0.5)
      {
         ...
         else
         {
            ...
            T lg = autoboost::math::lgamma(a, pol);
            T lb = log(q) + lg;
            if(lb < -D * 2.3)
            {
      ...
      else
      {
         ...
         if(w < 0.15f * ap1)
         {
            ...
            T v = log(p) + autoboost::math::lgamma(ap1, pol);
            z = exp((v + w) / a);
            s = autoboost::math::log1p(z / ap1 * (1 + z / ap2), pol);
            z = exp((v + z - s) / a);
            s = autoboost::math::log1p(z / ap1 * (1 + z / ap2), pol);
            z = exp((v + z - s) / a);
            s = autoboost::math::log1p(z / ap1 * (1 + z / ap2 * (1 + z / (a + 3))), pol);
            z = exp((v + z - s) / a);
...
struct gamma_p_inverse_func
{
   ...
   autoboost::math::tuple<T, T, T> operator()(const T& x)const
   {
      ...
      AUTOBOOST_MATH_STD_USING  // For ADL of std functions.

      T f, f1;
      value_type ft;
      f = static_cast<T>(autoboost::math::detail::gamma_incomplete_imp(
      ...
      return autoboost::math::make_tuple(static_cast<T>(f - p), f1, f2);
   }
private:
   T a, p;
   bool invert;
...
T gamma_p_inv_imp(T a, T p, const Policy& pol)
{
   ...
   static const char* function = "autoboost::math::gamma_p_inv<%1%>(%1%, %1%)";
   ...
   AUTOBOOST_MATH_INSTRUMENT_VARIABLE(guess);
   //
...
T gamma_q_inv_imp(T a, T q, const Policy& pol)
{
   ...
   static const char* function = "autoboost::math::gamma_q_inv<%1%>(%1%, %1%)";
