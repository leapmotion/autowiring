...
//  LICENSE_1_0.txt or copy at http://www.boost.org/LICENSE_1_0.txt)
...
#ifndef _AUTOBOOST_POLYGAMMA_DETAIL_2013_07_30_HPP_
  #define _AUTOBOOST_POLYGAMMA_DETAIL_2013_07_30_HPP_
  ...
  #include <autoboost/math/special_functions/cos_pi.hpp>
  ...
  T polygamma_atinfinityplus(const int n, const T& x, const Policy& pol, const char* function) // for large values of x such as for x> 400
  {
     ...
     if(n + x == x)
     {
        ...
        if((nlx < tools::log_max_value<T>()) && (n < (int)max_factorial<T>::value))
           return ((n & 1) ? 1 : -1) * autoboost::math::factorial<T>(n - 1) * pow(x, -n);
        else
         return ((n & 1) ? 1 : -1) * exp(autoboost::math::lgamma(T(n), pol) - n * log(x));
     }
     ...
     //
     part_term = ((n > (int)autoboost::math::max_factorial<T>::value) && (T(n) * n > tools::log_max_value<T>()))
        ? T(0) : static_cast<T>(autoboost::math::factorial<T>(n - 1, pol) * pow(x, -n - 1));
     if(part_term == 0)
     {
        // Either n is very large, or the power term underflows,
        // set the initial values of part_term, term and sum via logs:
        part_term = static_cast<T>(autoboost::math::lgamma(n, pol) - (n + 1) * log(x));
        sum = exp(part_term + log(n + 2 * x) - autoboost::math::constants::ln_two<T>());
        part_term += log(T(n) * (n + 1)) - autoboost::math::constants::ln_two<T>() - log(x);
     ...
     for(unsigned k = 1;;)
     {
        term = part_term * autoboost::math::bernoulli_b2n<T>(k, pol);
        sum += term;
        //
  ...
  T polygamma_attransitionplus(const int n, const T& x, const Policy& pol, const char* function)
  {
    ...
    if(iter > (int)policies::get_max_series_iterations<Policy>())
       return policies::raise_evaluation_error<T>(function, ("Exceeded maximum series evaluations evaluating at n = " + autoboost::lexical_cast<std::string>(n) + " and x = %1%").c_str(), x, pol);
    ...
    if(log(z + iter) * minus_m_minus_one > -tools::log_max_value<T>())
    {
       ...
       sum0 *= autoboost::math::factorial<T>(n);
    }
    else
    {
       for(int k = 1; k <= iter; ++k)
       {
          T log_term = log(z) * minus_m_minus_one + autoboost::math::lgamma(T(n + 1), pol);
          sum0 += exp(log_term);
          z += 1;
       }
    }
  ...
  T polygamma_nearzero(int n, T x, const Policy& pol, const char* function)
  {
     ...
     //
     T scale = autoboost::math::factorial<T>(n, pol);
     //
     ...
     {
        // Get the k'th term:
        T term = factorial_part * autoboost::math::zeta(T(k + n + 1), pol);
        sum += term;
        // Termination condition:
        if(fabs(term) < fabs(sum * autoboost::math::policies::get_epsilon<T, Policy>()))
           break;
        //
     ...
     //
     if(autoboost::math::tools::max_value<T>() / scale < sum)
        return autoboost::math::policies::raise_overflow_error<T>(function, 0, pol);
     sum *= scale;
     return n & 1 ? sum : -sum;
  }
  ...
  T poly_cot_pi(int n, T x, T xc, const Policy& pol, const char* function)
  {
     ...
     //
     T s = fabs(x) < fabs(xc) ? autoboost::math::sin_pi(x, pol) : autoboost::math::sin_pi(xc, pol);
     T c = autoboost::math::cos_pi(x, pol);
     switch(n)
     {
     case 1:
     ...
     case 2:
     {
        return 2 * constants::pi<T, Policy>() * constants::pi<T, Policy>() * c / autoboost::math::pow<3>(s, pol);
     }
     case 3:
     {
        int P[] = { -2, -4 };
        return autoboost::math::pow<3>(constants::pi<T, Policy>(), pol) * tools::evaluate_even_polynomial(P, c) / autoboost::math::pow<4>(s, pol);
     }
     case 4:
     {
        int P[] = { 16, 8 };
        return autoboost::math::pow<4>(constants::pi<T, Policy>(), pol) * c * tools::evaluate_even_polynomial(P, c) / autoboost::math::pow<5>(s, pol);
     }
     case 5:
     {
        int P[] = { -16, -88, -16 };
        return autoboost::math::pow<5>(constants::pi<T, Policy>(), pol) * tools::evaluate_even_polynomial(P, c) / autoboost::math::pow<6>(s, pol);
     }
     case 6:
     {
        int P[] = { 272, 416, 32 };
        return autoboost::math::pow<6>(constants::pi<T, Policy>(), pol) * c * tools::evaluate_even_polynomial(P, c) / autoboost::math::pow<7>(s, pol);
     }
     case 7:
     {
        int P[] = { -272, -2880, -1824, -64 };
        return autoboost::math::pow<7>(constants::pi<T, Policy>(), pol) * tools::evaluate_even_polynomial(P, c) / autoboost::math::pow<8>(s, pol);
     }
     case 8:
     {
        int P[] = { 7936, 24576, 7680, 128 };
        return autoboost::math::pow<8>(constants::pi<T, Policy>(), pol) * c * tools::evaluate_even_polynomial(P, c) / autoboost::math::pow<9>(s, pol);
     }
     case 9:
     {
        int P[] = { -7936, -137216, -185856, -31616, -256 };
        return autoboost::math::pow<9>(constants::pi<T, Policy>(), pol) * tools::evaluate_even_polynomial(P, c) / autoboost::math::pow<10>(s, pol);
     }
     case 10:
     {
        int P[] = { 353792, 1841152, 1304832, 128512, 512 };
        return autoboost::math::pow<10>(constants::pi<T, Policy>(), pol) * c * tools::evaluate_even_polynomial(P, c) / autoboost::math::pow<11>(s, pol);
     }
     case 11:
     {
        int P[] = { -353792, -9061376, -21253376, -8728576, -518656, -1024};
        return autoboost::math::pow<11>(constants::pi<T, Policy>(), pol) * tools::evaluate_even_polynomial(P, c) / autoboost::math::pow<12>(s, pol);
     }
     case 12:
     {
        int P[] = { 22368256, 175627264, 222398464, 56520704, 2084864, 2048 };
        return autoboost::math::pow<12>(constants::pi<T, Policy>(), pol) * c * tools::evaluate_even_polynomial(P, c) / autoboost::math::pow<13>(s, pol);
     }
#ifndef AUTOBOOST_NO_LONG_LONG
     case 13:
     {
        long long P[] = { -22368256LL, -795300864LL, -2868264960LL, -2174832640LL, -357888000LL, -8361984LL, -4096 };
        return autoboost::math::pow<13>(constants::pi<T, Policy>(), pol) * tools::evaluate_even_polynomial(P, c) / autoboost::math::pow<14>(s, pol);
     }
     case 14:
     {
        long long P[] = { 1903757312LL, 21016670208LL, 41731645440LL, 20261765120LL, 2230947840LL, 33497088LL, 8192 };
        return autoboost::math::pow<14>(constants::pi<T, Policy>(), pol) * c * tools::evaluate_even_polynomial(P, c) / autoboost::math::pow<15>(s, pol);
     }
     case 15:
     {
        long long P[] = { -1903757312LL, -89702612992LL, -460858269696LL, -559148810240LL, -182172651520LL, -13754155008LL, -134094848LL, -16384 };
        return autoboost::math::pow<15>(constants::pi<T, Policy>(), pol) * tools::evaluate_even_polynomial(P, c) / autoboost::math::pow<16>(s, pol);
     }
     case 16:
     {
        ...
        return autoboost::math::pow<16>(constants::pi<T, Policy>(), pol) * c * tools::evaluate_even_polynomial(P, c) / autoboost::math::pow<17>(s, pol);
     }
     case 17:
     {
        ...
        return autoboost::math::pow<17>(constants::pi<T, Policy>(), pol) * tools::evaluate_even_polynomial(P, c) / autoboost::math::pow<18>(s, pol);
     }
     case 18:
     {
        ...
        return autoboost::math::pow<18>(constants::pi<T, Policy>(), pol) * c * tools::evaluate_even_polynomial(P, c) / autoboost::math::pow<19>(s, pol);
     }
     case 19:
     {
        ...
        return autoboost::math::pow<19>(constants::pi<T, Policy>(), pol) * tools::evaluate_even_polynomial(P, c) / autoboost::math::pow<20>(s, pol);
     }
     case 20:
     {
        ...
        return autoboost::math::pow<20>(constants::pi<T, Policy>(), pol) * c * tools::evaluate_even_polynomial(P, c) / autoboost::math::pow<21>(s, pol);
     }
#endif
     }
     ...
     static autoboost::detail::lightweight_mutex m;
     autoboost::detail::lightweight_mutex::scoped_lock l(m);
#endif
     ...
     if(index >= (int)table.size())
     {
        for(int i = (int)table.size() - 1; i < index; ++i)
        {
           ...
           for(int column = 0; column <= max_columns; ++column)
           {
              ...
              AUTOBOOST_ASSERT(column < (int)table[i].size());
              AUTOBOOST_ASSERT((cos_order + 1) / 2 < (int)table[i + 1].size());
     ...
     T sum = autoboost::math::tools::evaluate_even_polynomial(&table[index][0], c, table[index].size());
     if(index & 1)
     ...
     //
     T power_terms = n * log(autoboost::math::constants::pi<T>());
     if(s == 0)
        return sum * autoboost::math::policies::raise_overflow_error<T>(function, 0, pol);
     power_terms -= log(fabs(s)) * (n + 1);
     power_terms += autoboost::math::lgamma(T(n));
     power_terms += log(fabs(sum));

     if(power_terms > autoboost::math::tools::log_max_value<T>())
        return sum * autoboost::math::policies::raise_overflow_error<T>(function, 0, pol);

     return exp(power_terms) * ((s < 0) && ((n + 1) & 1) ? -1 : 1) * autoboost::math::sign(sum);
  }
  ...
  struct polygamma_initializer
  {
     struct init
     {
        init()
        {
           ...
           autoboost::math::polygamma(30, T(-2.5f), Policy());
        }
  ...
  {
    AUTOBOOST_MATH_STD_USING
    static const char* function = "autoboost::math::polygamma<%1%>(int, %1%)";
    ...
    {
       return (n & 1 ? 1 : -1) * autoboost::math::factorial<T>(n, pol) * autoboost::math::zeta(T(n + 1), pol);
    }
    else if(x == 0.5f)
    {
       T result = (n & 1 ? 1 : -1) * autoboost::math::factorial<T>(n, pol) * autoboost::math::zeta(T(n + 1), pol);
       if(fabs(result) >= ldexp(tools::max_value<T>(), -n - 1))
          return autoboost::math::sign(result) * policies::raise_overflow_error<T>(function, 0, pol);
...
} } } // namespace autoboost::math::detail

#endif // _AUTOBOOST_POLYGAMMA_DETAIL_2013_07_30_HPP_
