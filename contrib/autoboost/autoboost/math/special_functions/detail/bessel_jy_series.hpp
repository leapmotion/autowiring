...
#ifndef AUTOBOOST_MATH_BESSEL_JN_SERIES_HPP
#define AUTOBOOST_MATH_BESSEL_JN_SERIES_HPP
...
inline T bessel_y_small_z_series(T v, T x, T* pscale, const Policy& pol)
{
   ...
   *pscale = scale;
   ...
   T result = autoboost::math::tools::sum_series(s, autoboost::math::policies::get_epsilon<T, Policy>(), max_iter);
#endif
   policies::check_series_iterations<T>("autoboost::math::bessel_y_small_z_series<%1%>(%1%,%1%)", max_iter, pol);
   result *= prefix;

   if(!need_logs)
   {
      prefix = autoboost::math::tgamma(-v, pol) * autoboost::math::cos_pi(v) * p / constants::pi<T>();
   }
   else
   {
      int sgn;
      prefix = autoboost::math::lgamma(-v, &sgn, pol) + p;
...
T bessel_yn_small_z(int n, T z, T* scale, const Policy& pol)
{
   ...
   AUTOBOOST_ASSERT((z < policies::get_epsilon<T, Policy>()));
   ...
   {
      T p = pow(z / 2, n);
      T result = -((autoboost::math::factorial<T>(n - 1) / constants::pi<T>()));
...
#endif // AUTOBOOST_MATH_BESSEL_JN_SERIES_HPP
