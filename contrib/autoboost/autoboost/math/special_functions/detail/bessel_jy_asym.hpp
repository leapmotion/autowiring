...
// functions, as x -> INF.
//
#ifndef AUTOBOOST_MATH_SF_DETAIL_BESSEL_JY_ASYM_HPP
#define AUTOBOOST_MATH_SF_DETAIL_BESSEL_JY_ASYM_HPP
...
inline T asymptotic_bessel_y_large_x_2(T v, T x)
{
   ...
   AUTOBOOST_MATH_INSTRUMENT_VARIABLE(ampl);
   AUTOBOOST_MATH_INSTRUMENT_VARIABLE(phase);
   //
   ...
   AUTOBOOST_MATH_INSTRUMENT_CODE(sin(phase));
   AUTOBOOST_MATH_INSTRUMENT_CODE(cos(x));
   AUTOBOOST_MATH_INSTRUMENT_CODE(cos(phase));
   AUTOBOOST_MATH_INSTRUMENT_CODE(sin(x));
...
inline T asymptotic_bessel_j_large_x_2(T v, T x)
{
   ...
   AUTOBOOST_MATH_INSTRUMENT_VARIABLE(sin_phase);
   return sin_phase * ampl;
}
...
{
   T c = 1;
   T p = (v / autoboost::math::sin_pi(v, pol)) * pow(x / 2, -v) / autoboost::math::tgamma(1 - v, pol);
   T q = (v / autoboost::math::sin_pi(v, pol)) * pow(x / 2, v) / autoboost::math::tgamma(1 + v, pol);
   T f = (p - q) / v;
   T g_prefix = autoboost::math::sin_pi(v / 2, pol);
...
T asymptotic_bessel_i_large_x(T v, T x, const Policy& pol)
{
   ...
   return (autoboost::math::isfinite)(s) ?
      s : policies::raise_overflow_error<T>("autoboost::math::asymptotic_bessel_i_large_x<%1%>(%1%,%1%)", 0, pol);
}
