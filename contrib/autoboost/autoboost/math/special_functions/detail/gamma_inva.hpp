...
#ifndef AUTOBOOST_MATH_SP_DETAIL_GAMMA_INVA
#define AUTOBOOST_MATH_SP_DETAIL_GAMMA_INVA
...
struct gamma_inva_t
{
   ...
   T operator()(T a)
   {
      return invert ? p - autoboost::math::gamma_q(a, z, Policy()) : autoboost::math::gamma_p(a, z, Policy()) - p;
   }
private:
   T z, p;
   bool invert;
...
T gamma_inva_imp(const T& z, const T& p, const T& q, const Policy& pol)
{
   ...
   if(p == 0)
   {
      return policies::raise_overflow_error<T>("autoboost::math::gamma_p_inva<%1%>(%1%, %1%)", 0, Policy());
   }
   if(q == 0)
   {
   ...
   if(max_iter >= policies::get_max_root_iterations<Policy>())
      return policies::raise_evaluation_error<T>("autoboost::math::gamma_p_inva<%1%>(%1%, %1%)", "Unable to locate the root within a reasonable number of iterations, closest approximation so far was %1%", r.first, pol);
   ...
   gamma_p_inva(T1 x, T2 p, const Policy& pol)
{
   ...
   if(p == 0)
   {
      policies::raise_overflow_error<result_type>("autoboost::math::gamma_p_inva<%1%>(%1%, %1%)", 0, Policy());
   }
   if(p == 1)
   {
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::gamma_inva_imp(
         ...
         pol), "autoboost::math::gamma_p_inva<%1%>(%1%, %1%)");
}
   ...
   gamma_q_inva(T1 x, T2 q, const Policy& pol)
{
   ...
   if(q == 1)
   {
      policies::raise_overflow_error<result_type>("autoboost::math::gamma_q_inva<%1%>(%1%, %1%)", 0, Policy());
   }
   if(q == 0)
   {
      return tools::min_value<result_type>();
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::gamma_inva_imp(
         ...
         pol), "autoboost::math::gamma_q_inva<%1%>(%1%, %1%)");
}
...
{
   return autoboost::math::gamma_p_inva(x, p, policies::policy<>());
}
...
{
   return autoboost::math::gamma_q_inva(x, q, policies::policy<>());
}

} // namespace math
} // namespace autoboost

#endif // AUTOBOOST_MATH_SP_DETAIL_GAMMA_INVA
