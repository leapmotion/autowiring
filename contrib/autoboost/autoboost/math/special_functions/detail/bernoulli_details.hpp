...
#ifndef AUTOBOOST_MATH_BERNOULLI_DETAIL_HPP
#define AUTOBOOST_MATH_BERNOULLI_DETAIL_HPP

#include <autoboost/config.hpp>
#include <autoboost/detail/lightweight_mutex.hpp>
#include <autoboost/utility/enable_if.hpp>
#include <autoboost/math/tools/toms748_solve.hpp>
#include <vector>

#ifdef AUTOBOOST_HAS_THREADS

#ifndef AUTOBOOST_NO_CXX11_HDR_ATOMIC
#  include <atomic>
#  define AUTOBOOST_MATH_ATOMIC_NS std
...
//
#if AUTOBOOST_ATOMIC_INT_LOCK_FREE == 2
typedef autoboost::atomic<int> atomic_counter_type;
typedef int atomic_integer_type;
#elif AUTOBOOST_ATOMIC_SHORT_LOCK_FREE == 2
typedef autoboost::atomic<short> atomic_counter_type;
typedef short atomic_integer_type;
#elif AUTOBOOST_ATOMIC_LONG_LOCK_FREE == 2
typedef autoboost::atomic<long> atomic_counter_type;
typedef long atomic_integer_type;
#elif AUTOBOOST_ATOMIC_LLONG_LOCK_FREE == 2
typedef autoboost::atomic<long long> atomic_counter_type;
typedef long long atomic_integer_type;
#else
#  define AUTOBOOST_MATH_NO_ATOMIC_INT
#endif
...
#endif  // AUTOBOOST_NO_CXX11_HDR_ATOMIC
...
T b2n_asymptotic(int n)
{
   ...
   const T approximate_log_of_bernoulli_bn =
        ((autoboost::math::constants::half<T>() + nx) * log(nx))
        + ((autoboost::math::constants::half<T>() - nx) * log(autoboost::math::constants::pi<T>()))
        + (((T(3) / 2) - nx) * autoboost::math::constants::ln_two<T>())
        + ((nx * (T(2) - (nx2 * 7) * (1 + ((nx2 * 30) * ((nx2 * 12) - 1))))) / (((nx2 * nx2) * nx2) * 2520));
   return ((n / 2) & 1 ? 1 : -1) * (approximate_log_of_bernoulli_bn > tools::log_max_value<T>()
      ? policies::raise_overflow_error<T>("autoboost::math::bernoulli_b2n<%1%>(std::size_t)", 0, nx, Policy())
...
T t2n_asymptotic(int n)
{
   ...
   if(tools::max_value<T>() / p2 < t2n)
      return policies::raise_overflow_error<T>("autoboost::math::tangent_t2n<%1%>(std::size_t)", 0, T(n), Policy());
   t2n *= p2;
   p2 -= 1;
   if(tools::max_value<T>() / p2 < t2n)
      return policies::raise_overflow_error<T>("autoboost::math::tangent_t2n<%1%>(std::size_t)", 0, Policy());
   t2n *= p2;
   return t2n;
}
//
...
struct max_bernoulli_root_functor
{
   ...
   double operator()(double n)
   {
      ...
      const double approximate_log_of_bernoulli_bn
         =   ((autoboost::math::constants::half<double>() + n) * log(n))
           + ((autoboost::math::constants::half<double>() - n) * log(autoboost::math::constants::pi<double>()))
           + (((double(3) / 2) - n) * autoboost::math::constants::ln_two<double>())
...
{
   long long t = lltrunc(autoboost::math::tools::log_max_value<T>());
   max_bernoulli_root_functor fun(t);
   autoboost::math::tools::equal_floor tol;
   autoboost::uintmax_t max_iter = autoboost::math::policies::get_max_root_iterations<Policy>();
   return static_cast<std::size_t>(autoboost::math::tools::toms748_solve(fun, sqrt(double(t)), double(t), tol, max_iter).first) / 2;
}
...
struct bernoulli_initializer
{
   struct init
   {
      init()
      {
         ...
         //
         autoboost::math::bernoulli_b2n<T>(2, Policy());
#ifndef AUTOBOOST_NO_EXCEPTIONS
         try{
#endif
            autoboost::math::bernoulli_b2n<T>(max_bernoulli_b2n<T>::value + 1, Policy());
#ifndef AUTOBOOST_NO_EXCEPTIONS
         } catch(const std::overflow_error&){}
#endif
         autoboost::math::tangent_t2n<T>(2, Policy());
      }
...
// autoboost::container::static_vector here, but that allocates on the stack, which may well
...
struct fixed_vector : private std::allocator<T>
{
   ...
   T& operator[](unsigned n) { AUTOBOOST_ASSERT(n < m_used); return m_data[n]; }
   const T& operator[](unsigned n)const { AUTOBOOST_ASSERT(n < m_used); return m_data[n]; }
   ...
   void resize(unsigned n, const T& val)
   {
      if(n > m_capacity)
      {
         AUTOBOOST_THROW_EXCEPTION(std::runtime_error("Exhausted storage for Bernoulli numbers."));
      }
...
public:
...
#if defined(AUTOBOOST_HAS_THREADS) && !defined(AUTOBOOST_MATH_NO_ATOMIC_INT)
      , m_counter(0)
#endif
      , m_current_precision(autoboost::math::tools::digits<T>())
   {}
   ...
   void tangent(std::size_t m)
   {
      ...
      AUTOBOOST_MATH_INSTRUMENT_VARIABLE(min_overflow_index);
      ...
      if(prev_size == 0)
      {
         ...
         AUTOBOOST_MATH_INSTRUMENT_VARIABLE(tn[0]);
         AUTOBOOST_MATH_INSTRUMENT_VARIABLE(tn[1]);
      }

      for(std::size_t i = std::max<size_t>(2, prev_size); i < m; i++)
      {
         bool overflow_check = false;
         if(i >= min_overflow_index && (autoboost::math::tools::max_value<T>() / (i-1) < m_intermediates[1]) )
         {
            std::fill(tn.begin() + i, tn.end(), autoboost::math::tools::max_value<T>());
            break;
         }
         ...
         for(std::size_t j = 2; j <= i; j++)
         {
            overflow_check =
                  ...
                  (autoboost::math::tools::max_value<T>() / (i - j) < m_intermediates[j])
                  || (autoboost::math::tools::max_value<T>() / (i - j + 2) < m_intermediates[j-1])
                  || (autoboost::math::tools::max_value<T>() - m_intermediates[j] * (i - j) < m_intermediates[j-1] * (i - j + 2))
                  || ((autoboost::math::isinf)(m_intermediates[j]))
                );
         ...
         AUTOBOOST_MATH_INSTRUMENT_VARIABLE(i);
         AUTOBOOST_MATH_INSTRUMENT_VARIABLE(tn[static_cast<typename container_type::size_type>(i)]);
      }
   }
   ...
   OutputIterator copy_bernoulli_numbers(OutputIterator out, std::size_t start, std::size_t n, const Policy& pol)
   {
      ...
      //
      // 1) There are no threads (AUTOBOOST_HAS_THREADS is not defined).
      ...
      if(start + n > bn.capacity())
      {
         ...
         for(; n; ++start, --n)
         {
            *out = policies::raise_overflow_error<T>("autoboost::math::bernoulli_b2n<%1%>(std::size_t)", 0, T(start), pol);
            ++out;
         }
         return out;
      }
      ...
      //
      if(m_current_precision < autoboost::math::tools::digits<T>())
      {
         bn.clear();
         tn.clear();
      ...
      {
         *out = (i >= m_overflow_limit) ? policies::raise_overflow_error<T>("autoboost::math::bernoulli_b2n<%1%>(std::size_t)", 0, T(i), pol) : bn[i];
         ++out;
      }
   #elif defined(AUTOBOOST_MATH_NO_ATOMIC_INT)
      //
      // We need to grab a mutex every time we get here, for both readers and writers:
      //
      autoboost::detail::lightweight_mutex::scoped_lock l(m_mutex);
      if(m_current_precision < autoboost::math::tools::digits<T>())
      {
         bn.clear();
         tn.clear();
      ...
      //
      if((static_cast<std::size_t>(m_counter.load(AUTOBOOST_MATH_ATOMIC_NS::memory_order_consume)) < start + n)
         || (static_cast<int>(m_current_precision.load(AUTOBOOST_MATH_ATOMIC_NS::memory_order_consume)) < autoboost::math::tools::digits<T>()))
      {
         autoboost::detail::lightweight_mutex::scoped_lock l(m_mutex);
         ...
         if((static_cast<std::size_t>(m_counter.load(AUTOBOOST_MATH_ATOMIC_NS::memory_order_consume)) < start + n)
            || (static_cast<int>(m_current_precision.load(AUTOBOOST_MATH_ATOMIC_NS::memory_order_consume)) < autoboost::math::tools::digits<T>()))
         {
            if(static_cast<int>(m_current_precision.load(AUTOBOOST_MATH_ATOMIC_NS::memory_order_consume)) < autoboost::math::tools::digits<T>())
            {
               bn.clear();
               tn.clear();
               m_intermediates.clear();
               m_counter.store(0, AUTOBOOST_MATH_ATOMIC_NS::memory_order_release);
               m_current_precision = autoboost::math::tools::digits<T>();
            }
            if(start + n >= bn.size())
            {
            ...
            m_counter.store(static_cast<atomic_integer_type>(bn.size()), AUTOBOOST_MATH_ATOMIC_NS::memory_order_release);
         }
      }

      for(std::size_t i = (std::max)(static_cast<std::size_t>(max_bernoulli_b2n<T>::value + 1), start); i < start + n; ++i)
      {
         *out = (i >= m_overflow_limit) ? policies::raise_overflow_error<T>("autoboost::math::bernoulli_b2n<%1%>(std::size_t)", 0, T(i), pol) : bn[static_cast<typename container_type::size_type>(i)];
         ++out;
      }
   ...
   OutputIterator copy_tangent_numbers(OutputIterator out, std::size_t start, std::size_t n, const Policy& pol)
   {
      ...
      for(std::size_t i = start; i < start + n; ++i)
      {
         if(i >= m_overflow_limit)
            *out = policies::raise_overflow_error<T>("autoboost::math::bernoulli_b2n<%1%>(std::size_t)", 0, T(i), pol);
         else
         {
...
#elif defined(AUTOBOOST_MATH_NO_ATOMIC_INT)
   autoboost::detail::lightweight_mutex m_mutex;
   int m_current_precision;
#else
   autoboost::detail::lightweight_mutex m_mutex;
...
#endif // AUTOBOOST_MATH_BERNOULLI_DETAIL_HPP
