...
#ifndef AUTOBOOST_MATH_SPECIAL_BETA_HPP
#define AUTOBOOST_MATH_SPECIAL_BETA_HPP
...
#include <autoboost/math/special_functions/binomial.hpp>
...
T beta_imp(T a, T b, const Lanczos&, const Policy& pol)
{
   ...
   if(a <= 0)
      return policies::raise_domain_error<T>("autoboost::math::beta<%1%>(%1%,%1%)", "The arguments to the beta function must be greater than zero (got a=%1%).", a, pol);
   if(b <= 0)
      return policies::raise_domain_error<T>("autoboost::math::beta<%1%>(%1%,%1%)", "The arguments to the beta function must be greater than zero (got b=%1%).", b, pol);
   ...
   */
   ...
   if((fabs(b * ambh) < (cgh * 100)) && (a > 100))
   {
      ...
      result *= exp(ambh * autoboost::math::log1p(-b / cgh, pol));
   }
   else
   {
   ...
   result *= sqrt(autoboost::math::constants::e<T>() / bgh);
...
T beta_imp(T a, T b, const lanczos::undefined_lanczos& /* l */, const Policy& pol)
{
   ...
   // special cases:
   if((c == a) && (b < tools::epsilon<T>()))
      return autoboost::math::tgamma(b, pol);
   else if((c == b) && (a < tools::epsilon<T>()))
      return autoboost::math::tgamma(a, pol);
   if(b == 1)
      return 1/a;
   else if(a == 1)
      return 1/b;
   ...
   sa += detail::upper_gamma_fraction(a, la, ::autoboost::math::policies::get_epsilon<T, Policy>());
   T sb = detail::lower_gamma_series(b, lb, pol) / b;
   sb += detail::upper_gamma_fraction(b, lb, ::autoboost::math::policies::get_epsilon<T, Policy>());
   T sc = detail::lower_gamma_series(c, lc, pol) / c;
   sc += detail::upper_gamma_fraction(c, lc, ::autoboost::math::policies::get_epsilon<T, Policy>());
...
T ibeta_power_terms(T a,
                        ...
                        const Policy& pol,
                        T prefix = 1,
                        const char* function = "autoboost::math::ibeta<%1%>(%1%, %1%, %1%)")
{
   AUTOBOOST_MATH_STD_USING
   ...
   result *= sqrt(bgh / autoboost::math::constants::e<T>());
   result *= sqrt(agh / cgh);
   ...
   if(((std::min)(fabs(l1), fabs(l2)) < 0.2))
   {
      ...
      if((l1 * l2 > 0) || ((std::min)(a, b) < 1))
      {
         ...
         if(fabs(l1) < 0.1)
         {
            result *= exp(a * autoboost::math::log1p(l1, pol));
         ...
         if(fabs(l2) < 0.1)
         {
            result *= exp(b * autoboost::math::log1p(l2, pol));
            AUTOBOOST_MATH_INSTRUMENT_VARIABLE(result);
         }
         else
         {
      ...
      else if((std::max)(fabs(l1), fabs(l2)) < 0.5)
      {
         ...
         {
            T l3 = autoboost::math::expm1(ratio * autoboost::math::log1p(l2, pol), pol);
            l3 = l1 + l3 + l3 * l1;
            l3 = a * autoboost::math::log1p(l3, pol);
            result *= exp(l3);
            AUTOBOOST_MATH_INSTRUMENT_VARIABLE(result);
         }
         else
         {
            T l3 = autoboost::math::expm1(autoboost::math::log1p(l1, pol) / ratio, pol);
            l3 = l2 + l3 + l3 * l2;
            l3 = b * autoboost::math::log1p(l3, pol);
            result *= exp(l3);
            AUTOBOOST_MATH_INSTRUMENT_VARIABLE(result);
         }
      }
      else if(fabs(l1) < fabs(l2))
      {
         ...
         T l = a * autoboost::math::log1p(l1, pol)
            + b * log((y * cgh) / bgh);
      ...
      else
      {
         ...
         T l = b * autoboost::math::log1p(l2, pol)
            + a * log((x * cgh) / agh);
         if((l <= tools::log_min_value<T>()) || (l >= tools::log_max_value<T>()))
         {
            l += log(result);
   ...
   else
   {
      ...
      l1 = a * log(b1);
      l2 = b * log(b2);
      AUTOBOOST_MATH_INSTRUMENT_VARIABLE(b1);
      AUTOBOOST_MATH_INSTRUMENT_VARIABLE(b2);
      AUTOBOOST_MATH_INSTRUMENT_VARIABLE(l1);
      AUTOBOOST_MATH_INSTRUMENT_VARIABLE(l2);
...
T ibeta_power_terms(T a,
                        ...
                        const Policy& pol,
                        T prefix = 1,
                        const char* = "autoboost::math::ibeta<%1%>(%1%, %1%, %1%)")
{
   AUTOBOOST_MATH_STD_USING
   ...
   else
   {
      ...
      p1 = (x * b - y * la) / la;
      if(fabs(p1) < 0.5f)
         p1 = exp(a * autoboost::math::log1p(p1, pol));
      else
         p1 = pow(b1, a);
      p2 = (y * a - x * lb) / lb;
      if(fabs(p2) < 0.5f)
         p2 = exp(b * autoboost::math::log1p(p2, pol));
      else
         p2 = pow(b2, b);
      T p3 = exp(e1);
...
T ibeta_series(T a, T b, T x, T s0, const Lanczos&, bool normalised, T* p_derivative, T y, const Policy& pol)
{
   ...
   AUTOBOOST_ASSERT((p_derivative == 0) || normalised);
   ...
   if(normalised)
   {
      ...
      {
         if(a * b < bgh * 10)
            result *= exp((b - 0.5f) * autoboost::math::log1p(a / bgh, pol));
         else
            result *= pow(cgh / bgh, b - 0.5f);
         result *= pow(x * cgh / agh, a);
         result *= sqrt(agh / autoboost::math::constants::e<T>());

         if(p_derivative)
         {
            *p_derivative = result * pow(y, b);
            AUTOBOOST_ASSERT(*p_derivative >= 0);
         }
      }
      else
      {
   ...
   result = autoboost::math::tools::sum_series(s, autoboost::math::policies::get_epsilon<T, Policy>(), max_iter, s0);
   policies::check_series_iterations<T>("autoboost::math::ibeta<%1%>(%1%, %1%, %1%) in ibeta_series (with lanczos)", max_iter, pol);
   return result;
}
//
...
//
template <class T, class Policy>
T ibeta_series(T a, T b, T x, T s0, const autoboost::math::lanczos::undefined_lanczos&, bool normalised, T* p_derivative, T y, const Policy& pol)
{
   AUTOBOOST_MATH_STD_USING
   ...
   if(normalised)
   {
      ...
      else
      {
         result = pow(b1, a);
         if(a * b < lb * 10)
            result *= exp(b * autoboost::math::log1p(a / lb, pol));
         else
            result *= pow(b2, b);
         result /= exp(e1);
      }
   ...
   autoboost::uintmax_t max_iter = policies::get_max_series_iterations<Policy>();
   result = autoboost::math::tools::sum_series(s, autoboost::math::policies::get_epsilon<T, Policy>(), max_iter, s0);
   policies::check_series_iterations<T>("autoboost::math::ibeta<%1%>(%1%, %1%, %1%) in ibeta_series (without lanczos)", max_iter, pol);
   return result;
}
...
inline T ibeta_fraction2(T a, T b, T x, T y, const Policy& pol, bool normalised, T* p_derivative)
{
   ...
   T fract = autoboost::math::tools::continued_fraction_b(f, autoboost::math::policies::get_epsilon<T, Policy>());
   AUTOBOOST_MATH_INSTRUMENT_VARIABLE(fract);
   AUTOBOOST_MATH_INSTRUMENT_VARIABLE(result);
   return result / fract;
}
//
...
struct Pn_size
{
   ...
   AUTOBOOST_STATIC_CONSTANT(unsigned, value = 50);
   AUTOBOOST_STATIC_ASSERT(::autoboost::math::max_factorial<T>::value >= 100);
};
template <>
struct Pn_size<float>
{
   AUTOBOOST_STATIC_CONSTANT(unsigned, value = 15); // ~8-15 digit accuracy
   AUTOBOOST_STATIC_ASSERT(::autoboost::math::max_factorial<float>::value >= 30);
};
template <>
struct Pn_size<double>
{
   AUTOBOOST_STATIC_CONSTANT(unsigned, value = 30); // 16-20 digit accuracy
   AUTOBOOST_STATIC_ASSERT(::autoboost::math::max_factorial<double>::value >= 60);
};
template <>
struct Pn_size<long double>
{
   AUTOBOOST_STATIC_CONSTANT(unsigned, value = 50); // ~35-50 digit accuracy
   AUTOBOOST_STATIC_ASSERT(::autoboost::math::max_factorial<long double>::value >= 100);
};
...
T beta_small_b_large_a_series(T a, T b, T x, T y, T s0, T mult, const Policy& pol, bool normalised)
{
   ...
   if(normalised)
   {
      prefix = h / autoboost::math::tgamma_delta_ratio(a, b, pol);
      prefix /= pow(t, b);
   }
   else
   {
   ...
   //
   T p[ ::autoboost::math::detail::Pn_size<T>::value ] = { 1 };  // see 9.3.
   //
   // Now an initial value for J, see 9.6:
   //
   T j = autoboost::math::gamma_q(b, u, pol) / h;
   //
   ...
   for(unsigned n = 1; n < sizeof(p)/sizeof(p[0]); ++n)
   {
      ...
      */
      ...
      {
         mbn = m * b - n;
         p[n] += mbn * p[n-m] / autoboost::math::unchecked_factorial<T>(tmp1);
         tmp1 += 2;
      }
      p[n] /= n;
      p[n] += bm1 / autoboost::math::unchecked_factorial<T>(tnp1);
      //
...
T binomial_ccdf(T n, T k, T x, T y)
{
   ...
   else
   {
      ...
      if(start <= k + 1)
         start = itrunc(k + 2);
      result = pow(x, start) * pow(y, n - start) * autoboost::math::binomial_coefficient<T>(itrunc(n), itrunc(start));
      if(result == 0)
      {
         ...
         {
            result += pow(x, (int)i) * pow(y, n - i) * autoboost::math::binomial_coefficient<T>(itrunc(n), itrunc(i));
         }
      }
      else
      {
...
{
   static const char* function = "autoboost::math::ibeta<%1%>(%1%, %1%, %1%)";
   typedef typename lanczos::lanczos<T, Policy>::type lanczos_type;
   AUTOBOOST_MATH_STD_USING // for ADL of std math functions.

   AUTOBOOST_MATH_INSTRUMENT_VARIABLE(a);
   AUTOBOOST_MATH_INSTRUMENT_VARIABLE(b);
   AUTOBOOST_MATH_INSTRUMENT_VARIABLE(x);
   AUTOBOOST_MATH_INSTRUMENT_VARIABLE(inv);
   AUTOBOOST_MATH_INSTRUMENT_VARIABLE(normalised);
   ...
   if(x == 0)
   {
      ...
      return (invert ? (normalised ? T(1) : autoboost::math::beta(a, b, pol)) : T(0));
   }
   if(x == 1)
   {
      if(p_derivative)
      ...
      return (invert == 0 ? (normalised ? 1 : autoboost::math::beta(a, b, pol)) : 0);
   }
   if((a == 0.5f) && (b == 0.5f))
   {
   ...
   if(b == 1)
   {
      ...
      T p;
      if(y < 0.5)
         p = invert ? T(-autoboost::math::expm1(a * autoboost::math::log1p(-y, pol), pol)) : T(exp(a * autoboost::math::log1p(-y, pol)));
      else
         p = invert ? T(-autoboost::math::powm1(x, a, pol)) : T(pow(x, a));
      if(!normalised)
         p /= a;
      return p;
   }
   ...
   if((std::min)(a, b) <= 1)
   {
      if(x > 0.5)
      {
         ...
         std::swap(x, y);
         invert = !invert;
         AUTOBOOST_MATH_INSTRUMENT_VARIABLE(invert);
      }
      if((std::max)(a, b) <= 1)
      {
         // Both a,b < 1:
         if((a >= (std::min)(T(0.2), b)) || (pow(x, a) <= 0.9))
         {
            ...
            {
               fract = -(normalised ? 1 : autoboost::math::beta(a, b, pol));
               invert = false;
   ...
   else
   {
      ...
      if(b < 40)
      {
         if((floor(a) == a) && (floor(b) == b) && (a < (std::numeric_limits<int>::max)() - 100) && (y != 1))
         {
            ...
            if(!normalised)
               fract *= autoboost::math::beta(a, b, pol);
            AUTOBOOST_MATH_INSTRUMENT_VARIABLE(fract);
         }
         else if(b * x <= 0.7)
         {
   ...
   return invert ? (normalised ? 1 : autoboost::math::beta(a, b, pol)) - fract : fract;
...
T ibeta_derivative_imp(T a, T b, T x, const Policy& pol)
{
   ...
   {
      return (a > 1) ? 0 :
         (a == 1) ? 1 / autoboost::math::beta(a, b, pol) : policies::raise_overflow_error<T>(function, 0, pol);
   }
   else if(x == 1)
   {
      return (b > 1) ? 0 :
         (b == 1) ? 1 / autoboost::math::beta(a, b, pol) : policies::raise_overflow_error<T>(function, 0, pol);
   }
   //
   ...
   beta(RT1 a, RT2 b, const Policy&, const mpl::true_*)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::beta_imp(static_cast<value_type>(a), static_cast<value_type>(b), evaluation_type(), forwarding_policy()), "autoboost::math::beta<%1%>(%1%,%1%)");
}
template <class RT1, class RT2, class RT3>
inline typename tools::promote_args<RT1, RT2, RT3>::type
   beta(RT1 a, RT2 b, RT3 x, const mpl::false_*)
{
   return autoboost::math::beta(a, b, x, policies::policy<>());
}
} // namespace detail
   ...
   beta(RT1 a, RT2 b, A arg)
{
   ...
   return autoboost::math::detail::beta(a, b, arg, static_cast<tag*>(0));
}
...
inline typename tools::promote_args<RT1, RT2>::type
   beta(RT1 a, RT2 b)
{
   return autoboost::math::beta(a, b, policies::policy<>());
}
   ...
   beta(RT1 a, RT2 b, RT3 x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::ibeta_imp(static_cast<value_type>(a), static_cast<value_type>(b), static_cast<value_type>(x), forwarding_policy(), false, false), "autoboost::math::beta<%1%>(%1%,%1%,%1%)");
}
   ...
   betac(RT1 a, RT2 b, RT3 x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::ibeta_imp(static_cast<value_type>(a), static_cast<value_type>(b), static_cast<value_type>(x), forwarding_policy(), true, false), "autoboost::math::betac<%1%>(%1%,%1%,%1%)");
}
template <class RT1, class RT2, class RT3>
inline typename tools::promote_args<RT1, RT2, RT3>::type
   betac(RT1 a, RT2 b, RT3 x)
{
   return autoboost::math::betac(a, b, x, policies::policy<>());
}
   ...
   ibeta(RT1 a, RT2 b, RT3 x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::ibeta_imp(static_cast<value_type>(a), static_cast<value_type>(b), static_cast<value_type>(x), forwarding_policy(), false, true), "autoboost::math::ibeta<%1%>(%1%,%1%,%1%)");
}
template <class RT1, class RT2, class RT3>
inline typename tools::promote_args<RT1, RT2, RT3>::type
   ibeta(RT1 a, RT2 b, RT3 x)
{
   return autoboost::math::ibeta(a, b, x, policies::policy<>());
}
   ...
   ibetac(RT1 a, RT2 b, RT3 x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::ibeta_imp(static_cast<value_type>(a), static_cast<value_type>(b), static_cast<value_type>(x), forwarding_policy(), true, true), "autoboost::math::ibetac<%1%>(%1%,%1%,%1%)");
}
template <class RT1, class RT2, class RT3>
inline typename tools::promote_args<RT1, RT2, RT3>::type
   ibetac(RT1 a, RT2 b, RT3 x)
{
   return autoboost::math::ibetac(a, b, x, policies::policy<>());
}
   ...
   ibeta_derivative(RT1 a, RT2 b, RT3 x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::ibeta_derivative_imp(static_cast<value_type>(a), static_cast<value_type>(b), static_cast<value_type>(x), forwarding_policy()), "autoboost::math::ibeta_derivative<%1%>(%1%,%1%,%1%)");
}
template <class RT1, class RT2, class RT3>
inline typename tools::promote_args<RT1, RT2, RT3>::type
   ibeta_derivative(RT1 a, RT2 b, RT3 x)
{
   return autoboost::math::ibeta_derivative(a, b, x, policies::policy<>());
}

} // namespace math
} // namespace autoboost

#include <autoboost/math/special_functions/detail/ibeta_inverse.hpp>
#include <autoboost/math/special_functions/detail/ibeta_inv_ab.hpp>

#endif // AUTOBOOST_MATH_SPECIAL_BETA_HPP
