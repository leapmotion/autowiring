...
//
#ifndef AUTOBOOST_MATH_ELLINT_RF_HPP
#define AUTOBOOST_MATH_ELLINT_RF_HPP
...
namespace autoboost { namespace math { namespace detail{
   ...
   T ellint_rf_imp(T x, T y, T z, const Policy& pol)
   {
      ...
      T An = (x + y + z) / 3;
      T A0 = An;
      T Q = pow(3 * autoboost::math::tools::epsilon<T>(), T(-1) / 8) * (std::max)((std::max)(fabs(An - xn), fabs(An - yn)), fabs(An - zn));
      T fn = 1;


      // duplication
      unsigned k = 1;
      for(; k < autoboost::math::policies::get_max_series_iterations<Policy>(); ++k)
      {
         T root_x = sqrt(xn);
         T root_y = sqrt(yn);
         T root_z = sqrt(zn);
...
#endif // AUTOBOOST_MATH_ELLINT_RF_HPP
