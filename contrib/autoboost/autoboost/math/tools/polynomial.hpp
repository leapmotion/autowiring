...
#ifndef AUTOBOOST_MATH_TOOLS_POLYNOMIAL_HPP
#define AUTOBOOST_MATH_TOOLS_POLYNOMIAL_HPP
...
#include <autoboost/lambda/lambda.hpp>
...
T chebyshev_coefficient(unsigned n, unsigned m)
{
   ...
   AUTOBOOST_ASSERT(n - 2 * r == m);

   if(r & 1)
      result = -result;
   result /= n - r;
   result *= autoboost::math::binomial_coefficient<T>(n - r, r);
   result *= ldexp(1.0f, m);
   return result;
}
...
AUTOBOOST_DEDUCED_TYPENAME disable_if_c<std::numeric_limits<T>::is_integer, void >::type
...
*/
template <typename T, typename N>
AUTOBOOST_DEDUCED_TYPENAME enable_if_c<std::numeric_limits<T>::is_integer, void >::type
division_impl(polynomial<T> &q, polynomial<T> &u, const polynomial<T>& v, N n, N k)
{
...
{
    AUTOBOOST_ASSERT(v.size() <= u.size());
    AUTOBOOST_ASSERT(v);
    AUTOBOOST_ASSERT(u);
...
public:
   ...
   polynomial(const polynomial<U>& p)
   {
      ...
      {
         m_data.push_back(autoboost::math::tools::real_cast<T>(p[i]));
      }
   }

#if !defined(AUTOBOOST_NO_CXX11_HDR_INITIALIZER_LIST) && !AUTOBOOST_WORKAROUND(AUTOBOOST_GCC_VERSION, < 40500)
   ...
   {
      return m_data.size() > 0 ? autoboost::math::tools::evaluate_polynomial(&m_data[0], z, m_data.size()) : 0;
   }
   ...
   {
       AUTOBOOST_ASSERT(n <= m_data.size());
   ...
   AUTOBOOST_FORCEINLINE operator unmentionable_type() const
   {
   ...
   void normalize()
   {
       using namespace autoboost::lambda;
...
#endif // AUTOBOOST_MATH_TOOLS_POLYNOMIAL_HPP
