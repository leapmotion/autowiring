...
#ifndef AUTOBOOST_MATH_TOOLS_STATS_INCLUDED
#define AUTOBOOST_MATH_TOOLS_STATS_INCLUDED
...
public:
   ...
   T min AUTOBOOST_PREVENT_MACRO_SUBSTITUTION()const{ return m_min; }
   T max AUTOBOOST_PREVENT_MACRO_SUBSTITUTION()const{ return m_max; }
   T total()const{ return m_total; }
   T mean()const{ return m_total / static_cast<T>(m_count); }
   autoboost::uintmax_t count()const{ return m_count; }
   T variance()const
   {
