...
#ifndef AUTOBOOST_MATH_TOOLS_SOLVE_ROOT_HPP
#define AUTOBOOST_MATH_TOOLS_SOLVE_ROOT_HPP
...
#ifdef AUTOBOOST_MATH_LOG_ROOT_ITERATIONS
#  define AUTOBOOST_MATH_LOGGER_INCLUDE <autoboost/math/tools/iteration_logger.hpp>
#  include AUTOBOOST_MATH_LOGGER_INCLUDE
#  undef AUTOBOOST_MATH_LOGGER_INCLUDE
#else
#  define AUTOBOOST_MATH_LOG_COUNT(count)
#endif
...
void bracket(F f, T& a, T& b, T c, T& fa, T& fb, T& d, T& fd)
{
   ...
   //
   if(autoboost::math::sign(fa) * autoboost::math::sign(fc) < 0)
   {
      d = b;
      fd = fb;
      b = c;
                           ...
                           unsigned count)
{
   ...
   //
   T c;
   if(autoboost::math::sign(A) * autoboost::math::sign(fa) > 0)
   {
      c = a;
   }
   else
                    ...
                    const T& fd, const T& fe)
{
   ...
   //
   AUTOBOOST_MATH_INSTRUMENT_CODE(" a = " << a << " b = " << b
      << " d = " << d << " e = " << e << " fa = " << fa << " fb = " << fb
      << " fd = " << fd << " fe = " << fe);
   T q11 = (d - e) * fd / (fe - fd);
   T q21 = (b - d) * fb / (fd - fb);
   ...
   T d21 = (b - d) * fd / (fd - fb);
   T d31 = (a - b) * fb / (fb - fa);
   AUTOBOOST_MATH_INSTRUMENT_CODE(
...
std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, const T& fax, const T& fbx, Tol tol, autoboost::uintmax_t& max_iter, const Policy& pol)
{
   //
   ...
   static const char* function = "autoboost::math::tools::toms748_solve<%1%>";
   ...
   b = bx;
   if(a >= b)
      return autoboost::math::detail::pair_from_single(policies::raise_domain_error(
         function,
   ...
   if(autoboost::math::sign(fa) * autoboost::math::sign(fb) > 0)
      return autoboost::math::detail::pair_from_single(policies::raise_domain_error(
         function,
   ...
   while(count && (fa != 0) && !tol(a, b))
   {
      ...
      {
         c = detail::quadratic_interpolate(a, b, d, fa, fb, fd, 2);
         AUTOBOOST_MATH_INSTRUMENT_CODE("Can't take cubic step!!!!");
      }
      else
      {
      ...
      detail::bracket(f, a, b, c, fa, fb, d, fd);
      AUTOBOOST_MATH_INSTRUMENT_CODE(" a = " << a << " b = " << b);
      AUTOBOOST_MATH_INSTRUMENT_CODE(" tol = " << T((fabs(a) - fabs(b)) / fabs(a)));
      if((0 == --count) || (fa == 0) || tol(a, b))
         break;
      //
      ...
      --count;
      AUTOBOOST_MATH_INSTRUMENT_CODE("Not converging: Taking a bisection!!!!");
      AUTOBOOST_MATH_INSTRUMENT_CODE(" a = " << a << " b = " << b);
   } // while loop
   ...
   AUTOBOOST_MATH_LOG_COUNT(max_iter)
   return std::make_pair(a, b);
}

template <class F, class T, class Tol>
inline std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, const T& fax, const T& fbx, Tol tol, autoboost::uintmax_t& max_iter)
{
   return toms748_solve(f, ax, bx, fax, fbx, tol, max_iter, policies::policy<>());
}

template <class F, class T, class Tol, class Policy>
inline std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, Tol tol, autoboost::uintmax_t& max_iter, const Policy& pol)
{
   max_iter -= 2;
...
template <class F, class T, class Tol>
inline std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, Tol tol, autoboost::uintmax_t& max_iter)
{
   return toms748_solve(f, ax, bx, tol, max_iter, policies::policy<>());
}

template <class F, class T, class Tol, class Policy>
std::pair<T, T> bracket_and_solve_root(F f, const T& guess, T factor, bool rising, Tol tol, autoboost::uintmax_t& max_iter, const Policy& pol)
{
   AUTOBOOST_MATH_STD_USING
   static const char* function = "autoboost::math::tools::bracket_and_solve_root<%1%>";
   //
   ...
   //
   autoboost::uintmax_t count = max_iter - 1;
   ...
   if((fa < 0) == (guess < 0 ? !rising : rising))
   {
      ...
      // until we find it:
      //
      while((autoboost::math::sign)(fb) == (autoboost::math::sign)(fa))
      {
         if(count == 0)
            return autoboost::math::detail::pair_from_single(policies::raise_evaluation_error(function, "Unable to bracket root, last nearest value was %1%", b, pol));
         //
         ...
         fb = f(b);
         --count;
         AUTOBOOST_MATH_INSTRUMENT_CODE("a = " << a << " b = " << b << " fa = " << fa << " fb = " << fb << " count = " << count);
      }
   }
   else
   {
      ...
      while((autoboost::math::sign)(fb) == (autoboost::math::sign)(fa))
      {
         ...
         if(count == 0)
            return autoboost::math::detail::pair_from_single(policies::raise_evaluation_error(function, "Unable to bracket root, last nearest value was %1%", a, pol));
         //
         // Heuristic: normally it's best not to increase the step sizes as we'll just end up
         // with a really wide range to search for the root.  However, if the initial guess was *really*
         // bad then we need to speed up the search otherwise we'll take forever if we're orders of
   ...
   std::pair<T, T> r = toms748_solve(
      ...
      pol);
   max_iter += count;
   AUTOBOOST_MATH_INSTRUMENT_CODE("max_iter = " << max_iter << " count = " << count);
   AUTOBOOST_MATH_LOG_COUNT(max_iter)
   return r;
}

template <class F, class T, class Tol>
inline std::pair<T, T> bracket_and_solve_root(F f, const T& guess, const T& factor, bool rising, Tol tol, autoboost::uintmax_t& max_iter)
{
...
#endif // AUTOBOOST_MATH_TOOLS_SOLVE_ROOT_HPP
