...
#ifndef AUTOBOOST_INTERPROCESS_FILE_MAPPING_HPP
#define AUTOBOOST_INTERPROCESS_FILE_MAPPING_HPP
...
#if !defined(AUTOBOOST_INTERPROCESS_MAPPED_FILES)
...
class file_mapping
{
   ...
   AUTOBOOST_MOVABLE_BUT_NOT_COPYABLE(file_mapping)
   ...
   //!Does not throw
   file_mapping(AUTOBOOST_RV_REF(file_mapping) moved)
   ...
   //!Does not throw
   file_mapping &operator=(AUTOBOOST_RV_REF(file_mapping) moved)
   {
      file_mapping tmp(autoboost::move(moved));
      this->swap(tmp);
      return *this;
   }
...
#endif   //AUTOBOOST_INTERPROCESS_FILE_MAPPING_HPP
