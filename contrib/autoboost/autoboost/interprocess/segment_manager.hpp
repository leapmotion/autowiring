...
#ifndef AUTOBOOST_INTERPROCESS_SEGMENT_MANAGER_HPP
#define AUTOBOOST_INTERPROCESS_SEGMENT_MANAGER_HPP
...
#include <autoboost/interprocess/detail/nothrow.hpp>
#include <autoboost/interprocess/detail/segment_manager_helper.hpp>
#include <autoboost/interprocess/detail/named_proxy.hpp>
...
#include <autoboost/interprocess/allocators/allocator.hpp>
#include <autoboost/interprocess/smart_ptr/deleter.hpp>
...
#include <autoboost/intrusive/detail/minimal_pair_header.hpp>
   ...
   :  private MemoryAlgorithm
{
   ...
   {
      AUTOBOOST_ASSERT((sizeof(segment_manager_base<MemoryAlgorithm>) == sizeof(MemoryAlgorithm)));
   }
   ...
   //!Allocates nbytes bytes. Throws autoboost::interprocess::bad_alloc
   //!on failure
   ...
   template<class T>
   T *allocation_command  (autoboost::interprocess::allocation_type command, size_type limit_size,
                           size_type &prefer_in_recvd_out_size, T *&reuse)
   {
      T *ret = MemoryAlgorithm::allocation_command
         (command | autoboost::interprocess::nothrow_allocation, limit_size, prefer_in_recvd_out_size, reuse);
      if(!(command & autoboost::interprocess::nothrow_allocation) && !ret)
         throw bad_alloc();
      return ret;
   }

   void *raw_allocation_command  (autoboost::interprocess::allocation_type command,   size_type limit_objects,
                           size_type &prefer_in_recvd_out_size, void *&reuse, size_type sizeof_object = 1)
   {
      void *ret = MemoryAlgorithm::raw_allocation_command
         ( command | autoboost::interprocess::nothrow_allocation, limit_objects,
      ...
      (size_type num, bool dothrow, ipcdetail::in_place_interface &table)
   {
      ...
      block_header_t * hdr = ::new(ptr_struct, autoboost_container_new_t()) block_header_t(block_info);
   ...
   :  public segment_manager_base<MemoryAlgorithm>
{
      ...
      ,  m_header(static_cast<segment_manager_base_t*>(get_this_pointer()))
   {
      ...
      AUTOBOOST_ASSERT( this_addr == segm_addr);
   }
   ...
   {
      AUTOBOOST_ASSERT(!name.is_anonymous());
   ...
   //!Can throw autoboost::interprocess::bad_alloc if there is no enough memory.
   ...
   struct allocator
   {
      typedef autoboost::interprocess::allocator<T, segment_manager> type;
   };
   ...
   struct deleter
   {
      typedef autoboost::interprocess::deleter<T, segment_manager> type;
   };
   ...
   {
      autoboost::interprocess::allocation_type type = ctrl_data->alloc_type();
      if(type == anonymous_type){
         AUTOBOOST_ASSERT((type == anonymous_type && ctrl_data->m_num_char == 0) ||
      ...
      //Sanity checks
      AUTOBOOST_ASSERT(ctrl_data->sizeof_char() == sizeof(CharType));
      AUTOBOOST_ASSERT(ctrl_data->m_num_char == std::char_traits<CharType>::length(name));
      return name;
   }
   ...
   {
      //Get header
      AUTOBOOST_ASSERT((ctrl_data->value_bytes() %sizeofvalue) == 0);
   ...
   {
      //Get header
      AUTOBOOST_ASSERT((instance_type)ctrl_data->alloc_type() < max_allocation_type);
       ...
       size_type &length, ipcdetail::true_ is_intrusive, bool use_lock)
   {
      ...
      if(it != index.end()){
         ...
         //Sanity check
         AUTOBOOST_ASSERT((ctrl_data->m_value_bytes % table.size) == 0);
         AUTOBOOST_ASSERT(ctrl_data->sizeof_char() == sizeof(CharT));
                                   ...
                                   ipcdetail::in_place_interface &table, ipcdetail::true_ is_intrusive_index)
   {
      ...
      //Sanity check
      AUTOBOOST_ASSERT((ctrl_data->m_value_bytes % table.size) == 0);
      AUTOBOOST_ASSERT(sizeof(CharT) == ctrl_data->sizeof_char());
      ...
      ipcdetail::in_place_interface &table)
   {
      ...
      //Sanity check
      AUTOBOOST_ASSERT((ctrl_data->m_value_bytes % table.size) == 0);
      AUTOBOOST_ASSERT(static_cast<void*>(stored_name) == static_cast<void*>(ctrl_data->template name<CharT>()));
      AUTOBOOST_ASSERT(sizeof(CharT) == ctrl_data->sizeof_char());
      ...
      IndexType<ipcdetail::index_config<CharT, MemoryAlgorithm> > &index, ipcdetail::true_ is_intrusive)
   {
     ...
     std::size_t namelen  = std::char_traits<CharT>::length(name);
      ...
      intrusive_value_type * intrusive_hdr = ::new(buffer_ptr, autoboost_container_new_t()) intrusive_value_type();
      block_header_t * hdr = ::new(intrusive_hdr->get_block_header(), autoboost_container_new_t())block_header_t(block_info);
      void *ptr = 0; //avoid gcc warning
      ptr = hdr->value();
      ...
      IndexType<ipcdetail::index_config<CharT, MemoryAlgorithm> > &index, ipcdetail::false_ is_intrusive)
   {
      ...
      if(is_node_index_t::value){
         ...
         index_it *idr = ::new(buffer_ptr, autoboost_container_new_t()) index_it(it);
      ...
      hdr = ::new(hdr, autoboost_container_new_t())block_header_t(block_info);
      void *ptr = 0; //avoid gcc warning
      ptr = hdr->value();
   ...
   scoped_lock<rmutex> priv_get_lock(bool use_lock)
   {
      ...
      return scoped_lock<rmutex>(autoboost::move(local));
   }
...
#endif //#ifndef AUTOBOOST_INTERPROCESS_SEGMENT_MANAGER_HPP
