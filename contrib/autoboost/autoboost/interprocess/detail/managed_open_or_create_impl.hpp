...
#ifndef AUTOBOOST_INTERPROCESS_MANAGED_OPEN_OR_CREATE_IMPL
#define AUTOBOOST_INTERPROCESS_MANAGED_OPEN_OR_CREATE_IMPL
...
#include <autoboost/interprocess/mapped_region.hpp>
...
#endif   //AUTOBOOST_INTERPROCESS_XSI_SHARED_MEMORY_OBJECTS
...
{
   //Non-copyable
   AUTOBOOST_MOVABLE_BUT_NOT_COPYABLE(managed_open_or_create_impl)
   ...
   static const std::size_t
      ManagedOpenOrCreateUserOffset =
         ct_rounded_size
            < sizeof(autoboost::uint32_t)
            , MemAlignment ? (MemAlignment) :
               (::autoboost::container::container_detail::alignment_of
                  < ::autoboost::container::container_detail::max_align_t >::value)
            >::value;
   ...
   managed_open_or_create_impl(AUTOBOOST_RV_REF(managed_open_or_create_impl) moved)
   {  this->swap(moved);   }

   managed_open_or_create_impl &operator=(AUTOBOOST_RV_REF(managed_open_or_create_impl) moved)
   {
      managed_open_or_create_impl tmp(autoboost::move(moved));
      this->swap(tmp);
      return *this;
   }
       ...
       ConstructFunc construct_func)
   {
      ...
      if(created){
         try{
            ...
            autoboost::uint32_t *patomic_word = 0;  //avoid gcc warning
            patomic_word = static_cast<autoboost::uint32_t*>(region.get_address());
            autoboost::uint32_t previous = atomic_cas32(patomic_word, InitializingSegment, UninitializedSegment);
         ...
         autoboost::uint32_t *patomic_word = static_cast<autoboost::uint32_t*>(region.get_address());
         autoboost::uint32_t value = atomic_read32(patomic_word);
      ...
      if(StoreDevice){
         this->DevHolder::get_device() = autoboost::move(dev);
      }
   }
...
#endif   //#ifndef AUTOBOOST_INTERPROCESS_MANAGED_OPEN_OR_CREATE_IMPL
