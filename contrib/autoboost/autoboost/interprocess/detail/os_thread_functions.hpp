...
#  include <sched.h>
#  include <time.h>
#  ifdef AUTOBOOST_INTERPROCESS_BSD_DERIVATIVE
...
#ifndef AUTOBOOST_INTERPROCESS_MATCH_ABSOLUTE_TIME
...
inline OS_highres_count_t get_current_system_highres_count()
{
   #if defined(AUTOBOOST_INTERPROCESS_CLOCK_MONOTONIC)
      struct timespec count;
      ::clock_gettime(AUTOBOOST_INTERPROCESS_CLOCK_MONOTONIC, &count);
      return count;
   #elif defined(AUTOBOOST_INTERPROCESS_MATCH_ABSOLUTE_TIME)
...
inline unsigned int get_num_cores()
{
   ...
   #elif defined(AUTOBOOST_INTERPROCESS_BSD_DERIVATIVE) && defined(HW_NCPU)
...
#endif   //AUTOBOOST_INTERPROCESS_DETAIL_OS_THREAD_FUNCTIONS_HPP
