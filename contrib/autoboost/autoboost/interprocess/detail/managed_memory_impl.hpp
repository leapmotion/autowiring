...
#ifndef AUTOBOOST_INTERPROCESS_DETAIL_MANAGED_MEMORY_IMPL_HPP
#define AUTOBOOST_INTERPROCESS_DETAIL_MANAGED_MEMORY_IMPL_HPP
...
#include <autoboost/interprocess/segment_manager.hpp>
...
class basic_managed_memory_impl
{
   ...
   bool  create_impl   (void *addr, size_type size)
   {
      ...
      AUTOBOOST_TRY{
         ...
         mp_header       = ::new(addr, autoboost_container_new_t()) segment_manager(size);
      }
      AUTOBOOST_CATCH(...){
         return false;
      }
   ...
   //!memory is available throws a autoboost::interprocess::bad_alloc exception
   ...
   //!
   //!-> Throws autoboost::interprocess::bad_alloc if there is no available memory
   //!
...
#endif   //AUTOBOOST_INTERPROCESS_DETAIL_MANAGED_MEMORY_IMPL_HPP
