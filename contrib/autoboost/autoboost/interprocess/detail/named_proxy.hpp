...
#ifndef AUTOBOOST_INTERPROCESS_NAMED_PROXY_HPP
#define AUTOBOOST_INTERPROCESS_NAMED_PROXY_HPP
...
// interprocess/detail
#include <autoboost/interprocess/detail/in_place_interface.hpp>
#include <autoboost/interprocess/detail/mpl.hpp>
#include <autoboost/move/utility_core.hpp>
#ifndef AUTOBOOST_INTERPROCESS_PERFECT_FORWARDING
#include <autoboost/move/detail/fwd_macros.hpp>
#else
#include <autoboost/move/utility_core.hpp>
#include <autoboost/interprocess/detail/variadic_templates_tools.hpp>
#endif   //#ifdef AUTOBOOST_INTERPROCESS_PERFECT_FORWARDING
...
#ifdef AUTOBOOST_INTERPROCESS_PERFECT_FORWARDING
...
struct CtorArgN : public placement_destroy<T>
{
   ...
   {  ::new((void*)mem, autoboost_container_new_t())T(*autoboost::forward<Args>(get<IdxPack>(args_))...); }
...
#else //#ifdef AUTOBOOST_INTERPROCESS_PERFECT_FORWARDING
...
#define AUTOBOOST_INTERPROCESS_NAMED_PROXY_CTORARGN(N)\
\
...
{\
   ...
   \
   CtorArg##N ( AUTOBOOST_MOVE_UREF##N  )\
   ...
   {\
      T* memory      = static_cast<T*>(mem);\
      for(constructed = 0; constructed < num; ++constructed){\
         ::new((void*)memory++) T ( AUTOBOOST_MOVE_MFWD##N );\
      }\
   }\
   \
   private:\
...
//!
AUTOBOOST_MOVE_ITERATE_0TO9(AUTOBOOST_INTERPROCESS_NAMED_PROXY_CTORARGN)
#undef AUTOBOOST_INTERPROCESS_NAMED_PROXY_CTORARGN

#define AUTOBOOST_INTERPROCESS_NAMED_PROXY_CTORITN(N)\
\
template<class T AUTOBOOST_MOVE_I##N AUTOBOOST_MOVE_CLASS##N > \
...
{\
   ...
   \
   self_t& operator++()\
   {  AUTOBOOST_MOVE_MINC##N;  return *this;  }\
   \
   self_t  operator++(int) {  return ++*this; *this;  }\
   \
   CtorIt##N ( AUTOBOOST_MOVE_VAL##N  )\
      AUTOBOOST_MOVE_COLON##N AUTOBOOST_MOVE_VAL_INIT##N{}\
   \
   virtual void construct_n(void *mem, std::size_t num, std::size_t &constructed)\
   {\
      T* memory      = static_cast<T*>(mem);\
      for(constructed = 0; constructed < num; ++constructed){\
         ::new((void*)memory++) T( AUTOBOOST_MOVE_MITFWD##N );\
         ++(*this);\
      }\
   }\
   \
   private:\
   AUTOBOOST_MOVE_MEMB##N\
};\
//!
AUTOBOOST_MOVE_ITERATE_0TO9(AUTOBOOST_INTERPROCESS_NAMED_PROXY_CTORITN)
#undef AUTOBOOST_INTERPROCESS_NAMED_PROXY_CTORITN
...
class named_proxy
{
   ...
   #define AUTOBOOST_INTERPROCESS_NAMED_PROXY_CALL_OPERATOR(N)\
   \
   AUTOBOOST_MOVE_TMPL_LT##N AUTOBOOST_MOVE_CLASS##N AUTOBOOST_MOVE_GT##N \
   T *operator()( AUTOBOOST_MOVE_UREF##N ) const\
   {\
      typedef typename if_c<is_iterator \
         , CtorIt##N<T AUTOBOOST_MOVE_I##N AUTOBOOST_MOVE_TARG##N> \
         , CtorArg##N<T AUTOBOOST_MOVE_I##N AUTOBOOST_MOVE_TARG##N> \
         >::type ctor_obj_t;\
      ctor_obj_t ctor_obj = ctor_obj_t( AUTOBOOST_MOVE_FWD##N );\
      return mp_mngr->template generic_construct<T>(mp_name, m_num, m_find, m_dothrow, ctor_obj);\
   }\
   //
   AUTOBOOST_MOVE_ITERATE_0TO9(AUTOBOOST_INTERPROCESS_NAMED_PROXY_CALL_OPERATOR)
   #undef AUTOBOOST_INTERPROCESS_NAMED_PROXY_CALL_OPERATOR
...
#endif //#ifndef AUTOBOOST_INTERPROCESS_NAMED_PROXY_HPP
