...
#ifndef AUTOBOOST_INTERPROCESS_DETAIL_FILE_WRAPPER_HPP
#define AUTOBOOST_INTERPROCESS_DETAIL_FILE_WRAPPER_HPP
...
class file_wrapper
{
   ...
   AUTOBOOST_MOVABLE_BUT_NOT_COPYABLE(file_wrapper)
   ...
   //!Does not throw
   file_wrapper(AUTOBOOST_RV_REF(file_wrapper) moved)
   ...
   //!After the call, "moved" does not represent any file.
   //!Does not throw
   file_wrapper &operator=(AUTOBOOST_RV_REF(file_wrapper) moved)
   {
      file_wrapper tmp(autoboost::move(moved));
      this->swap(tmp);
      return *this;
   }
...
#endif   //AUTOBOOST_INTERPROCESS_DETAIL_FILE_WRAPPER_HPP
