...
#ifndef AUTOBOOST_INTERPROCESS_INTERSEGMENT_PTR_HPP
#define AUTOBOOST_INTERPROCESS_INTERSEGMENT_PTR_HPP
...
#include <autoboost/interprocess/containers/flat_map.hpp>
#include <autoboost/interprocess/containers/vector.hpp>   //vector
#include <autoboost/interprocess/containers/set.hpp>      //set
// interprocess/detail
#include <autoboost/interprocess/detail/multi_segment_services.hpp>
#include <autoboost/interprocess/detail/utilities.hpp>
#include <autoboost/interprocess/detail/math_functions.hpp>
...
#include <autoboost/static_assert.hpp>  //AUTOBOOST_STATIC_ASSERT
#include <autoboost/integer/static_log2.hpp>
#include <autoboost/assert.hpp>   //AUTOBOOST_ASSERT
// std
...
struct intersegment_base
{
   ...
   AUTOBOOST_STATIC_ASSERT((sizeof(void*)*CHAR_BIT == 32 || sizeof(void*)*CHAR_BIT == 64));
   ...
   AUTOBOOST_STATIC_ASSERT(((size_t_bits - pow_size_bits - frc_size_bits) >= ctrl_bits ));
   ...
   AUTOBOOST_STATIC_ASSERT(sizeof(members_t) == 2*sizeof(std::size_t));
   ...
   {
      AUTOBOOST_ASSERT(addr < static_cast<void*>(this));
   ...
   std::size_t relative_size() const
   {
      ...
      AUTOBOOST_ASSERT(pow >= frc_size_bits);
   ...
   static std::size_t calculate_size(std::size_t orig_size, std::size_t &pow, std::size_t &frc)
   {
      ...
      AUTOBOOST_ASSERT(((frc << (pow - frc_size_bits)) & (align-1))==0);
   ...
   {
      AUTOBOOST_ASSERT(mode < is_max_mode);
   ...
   :  public intersegment_base
{
   ...
   void set_from_pointer(const void *ptr)
   {
      ...
      if(!this_info.group){
         ...
         if(ptr_info.group != this_info.group){
            ...
            (void)s;
            AUTOBOOST_ASSERT(this_info.size == s);
   ...
   class segment_group_t
   {
      ...
      void pop_back()
      {
         AUTOBOOST_ASSERT(!m_segments.empty());
      ...
      {
         AUTOBOOST_ASSERT(segment_id < (std::size_t)m_segments.size());
   ...
   typedef autoboost::interprocess::flat_map
      <const void *
      ,segment_info_t
   ...
   struct mappings_t : Mutex
   {
      ...
      ~mappings_t()
      {
         ...
         AUTOBOOST_ASSERT(m_ptr_to_segment_info.empty());
      }
   };
   ...
   {
      //------------------------------------------------------------------
      autoboost::interprocess::scoped_lock<typename mappings_t::mutex_type> lock(s_map);
      //------------------------------------------------------------------
      base = 0;
...
//! swap specialization
template<class T> inline
void swap (autoboost::interprocess::intersegment_ptr<T> &pt,
           autoboost::interprocess::intersegment_ptr<T> &pt2)
{  pt.swap(pt2);  }

//!to_raw_pointer() enables autoboost::mem_fn to recognize intersegment_ptr.
//!Never throws.
template<class T> inline
T * to_raw_pointer(autoboost::interprocess::intersegment_ptr<T> const & p)
{  return p.get();   }

//!Simulation of static_cast between pointers.
//!Never throws.
template<class T, class U> inline
autoboost::interprocess::intersegment_ptr<T> static_pointer_cast(const autoboost::interprocess::intersegment_ptr<U> &r)
{  return autoboost::interprocess::intersegment_ptr<T>(r, autoboost::interprocess::ipcdetail::static_cast_tag());  }

//!Simulation of const_cast between pointers.
//!Never throws.
template<class T, class U> inline
autoboost::interprocess::intersegment_ptr<T> const_pointer_cast(const autoboost::interprocess::intersegment_ptr<U> &r)
{  return autoboost::interprocess::intersegment_ptr<T>(r, autoboost::interprocess::ipcdetail::const_cast_tag());  }

//!Simulation of dynamic_cast between pointers.
//!Never throws.
template<class T, class U> inline
autoboost::interprocess::intersegment_ptr<T> dynamic_pointer_cast(const autoboost::interprocess::intersegment_ptr<U> &r)
{  return autoboost::interprocess::intersegment_ptr<T>(r, autoboost::interprocess::ipcdetail::dynamic_cast_tag());  }

//!Simulation of reinterpret_cast between pointers.
//!Never throws.
template<class T, class U> inline
autoboost::interprocess::intersegment_ptr<T> reinterpret_pointer_cast(const autoboost::interprocess::intersegment_ptr<U> &r)
{  return autoboost::interprocess::intersegment_ptr<T>(r, autoboost::interprocess::ipcdetail::reinterpret_cast_tag());  }

//!Trait class to detect if an smart pointer has
//!multi-segment addressing capabilities.
template <class T>
struct is_multisegment_ptr
   <autoboost::interprocess::intersegment_ptr<T> >
{
...
#endif //#ifndef AUTOBOOST_INTERPROCESS_INTERSEGMENT_PTR_HPP
