...
#ifndef AUTOBOOST_INTERPROCESS_ROBUST_EMULATION_HPP
#define AUTOBOOST_INTERPROCESS_ROBUST_EMULATION_HPP
...
#include <autoboost/interprocess/detail/intermodule_singleton.hpp>
...
{
   public:
   static const autoboost::uint32_t correct_state = 0;
   static const autoboost::uint32_t fixing_state  = 1;
   static const autoboost::uint32_t broken_state  = 2;
   ...
   bool is_owner_dead(autoboost::uint32_t own);
   void owner_to_filename(autoboost::uint32_t own, std::string &s);
   //The real mutex
   Mutex mtx;
   //The pid of the owner
   volatile autoboost::uint32_t owner;
   //The state of the mutex (correct, fixing, broken)
   volatile autoboost::uint32_t state;
};
...
template<class Mutex>
inline void robust_spin_mutex<Mutex>::owner_to_filename(autoboost::uint32_t own, std::string &s)
{
...
{
   autoboost::uint32_t cur_owner = get_current_process_id();
   autoboost::uint32_t old_owner = atomic_read32(&this->owner), old_owner2;
...
template<class Mutex>
inline bool robust_spin_mutex<Mutex>::is_owner_dead(autoboost::uint32_t own)
{
   //If owner is an invalid id, then it's clear it's dead
   if(own == (autoboost::uint32_t)get_invalid_process_id()){
      return true;
   }
...
inline void robust_spin_mutex<Mutex>::consistent()
{
   ...
   if(atomic_read32(&this->state) != fixing_state &&
      atomic_read32(&this->owner) != (autoboost::uint32_t)get_current_process_id()){
      throw interprocess_exception(lock_error, "Broken id");
   }
