...
#ifndef AUTOBOOST_INTERPROCESS_DETAIL_CONDITION_ANY_ALGORITHM_HPP
#define AUTOBOOST_INTERPROCESS_DETAIL_CONDITION_ANY_ALGORITHM_HPP
...
class condition_any_algorithm
{
   ...
   template <class Lock>
   static bool do_timed_wait(ConditionAnyMembers &data, Lock& lock, const autoboost::posix_time::ptime &abs_time);

   public:
   template<class Lock>
   static bool wait  ( ConditionAnyMembers &data, Lock &mut
                     , bool timeout_enabled, const autoboost::posix_time::ptime &abs_time);
...
bool condition_any_algorithm<ConditionAnyMembers>::do_timed_wait
   (ConditionAnyMembers &data, Lock& lock, const autoboost::posix_time::ptime &abs_time)
{
   //lock internal before unlocking external to avoid race with a notifier
   scoped_lock<mutex_type> internal_lock(data.get_mutex());
   {
...
class condition_any_wrapper
{
   ...
   void wait(L& lock)
   {
      ...
      algo_type::wait(m_data, lock, false, autoboost::posix_time::ptime());
   }
...
#endif   //AUTOBOOST_INTERPROCESS_DETAIL_CONDITION_ANY_ALGORITHM_HPP
