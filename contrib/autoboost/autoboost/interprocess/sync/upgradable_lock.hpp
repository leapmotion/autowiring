...
#ifndef AUTOBOOST_INTERPROCESS_UPGRADABLE_LOCK_HPP
#define AUTOBOOST_INTERPROCESS_UPGRADABLE_LOCK_HPP
...
class upgradable_lock
{
   ...
   AUTOBOOST_MOVABLE_BUT_NOT_COPYABLE(upgradable_lock)
   ...
   upgradable_lock(mutex_type& m, const autoboost::posix_time::ptime& abs_time)
      : mp_mutex(&m), m_locked(false)
   ...
   //!   expression: "autoboost::move(lock);". This constructor does not alter the
   //!   state of the mutex, only potentially who owns it.
   upgradable_lock(AUTOBOOST_RV_REF(upgradable_lock<mutex_type>) upgr)
   ...
   //!   expression: "autoboost::move(lock);".
   template<class T>
   upgradable_lock(AUTOBOOST_RV_REF(scoped_lock<T>) scop
   ...
   template<class T>
   upgradable_lock( AUTOBOOST_RV_REF(sharable_lock<T>) shar, try_to_lock_type
                  , typename ipcdetail::enable_if< ipcdetail::is_same<T, UpgradableMutex> >::type * = 0)
      : mp_mutex(0), m_locked(false)
   {
   ...
   upgradable_lock &operator=(AUTOBOOST_RV_REF(upgradable_lock) upgr)
   {
      if(this->owns())
         this->unlock();
      m_locked = upgr.owns();
...
#endif // AUTOBOOST_INTERPROCESS_UPGRADABLE_LOCK_HPP
