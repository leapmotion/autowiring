...
#include <autoboost/interprocess/sync/lock_options.hpp>
...
class scoped_lock
{
   ...
   AUTOBOOST_MOVABLE_BUT_NOT_COPYABLE(scoped_lock)
   ...
   scoped_lock(mutex_type& m, const autoboost::posix_time::ptime& abs_time)
   ...
   //!   can be moved with the expression: "autoboost::move(lock);". This
   //!   constructor does not alter the state of the mutex, only potentially
   //!   who owns it.
   scoped_lock(AUTOBOOST_RV_REF(scoped_lock) scop)
   ...
   //!   the expression: "autoboost::move(lock);" This constructor may block if
   //!   other threads hold a sharable_lock on this mutex (sharable_lock's can
   //!   share ownership with an upgradable_lock).
   template<class T>
   explicit scoped_lock(AUTOBOOST_RV_REF(upgradable_lock<T>) upgr
   ...
   template<class T>
   scoped_lock(AUTOBOOST_RV_REF(upgradable_lock<T>) upgr, try_to_lock_type
         , typename ipcdetail::enable_if< ipcdetail::is_same<T, Mutex> >::type * = 0)
      : mp_mutex(0), m_locked(false)
   {
      upgradable_lock<mutex_type> &u_lock = upgr;
   ...
   template<class T>
   scoped_lock(AUTOBOOST_RV_REF(upgradable_lock<T>) upgr, autoboost::posix_time::ptime &abs_time
               , typename ipcdetail::enable_if< ipcdetail::is_same<T, Mutex> >::type * = 0)
      : mp_mutex(0), m_locked(false)
   {
      upgradable_lock<mutex_type> &u_lock = upgr;
   ...
   //!   do so in a non-blocking manner.
   template<class T>
   scoped_lock(AUTOBOOST_RV_REF(sharable_lock<T>) shar, try_to_lock_type
      , typename ipcdetail::enable_if< ipcdetail::is_same<T, Mutex> >::type * = 0)
      : mp_mutex(0), m_locked(false)
   {
   ...
   scoped_lock &operator=(AUTOBOOST_RV_REF(scoped_lock) scop)
   {
      if(this->owns())
         this->unlock();
      m_locked = scop.owns();
...
#endif // AUTOBOOST_INTERPROCESS_SCOPED_LOCK_HPP
