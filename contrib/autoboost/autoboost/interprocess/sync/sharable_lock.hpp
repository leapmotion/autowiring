...
#ifndef AUTOBOOST_INTERPROCESS_SHARABLE_LOCK_HPP
#define AUTOBOOST_INTERPROCESS_SHARABLE_LOCK_HPP
...
class sharable_lock
{
   ...
   AUTOBOOST_MOVABLE_BUT_NOT_COPYABLE(sharable_lock)
   ...
   sharable_lock(mutex_type& m, const autoboost::posix_time::ptime& abs_time)
      : mp_mutex(&m), m_locked(false)
   ...
   //!   "autoboost::move(lock);". This constructor does not alter the state of the mutex,
   //!   only potentially who owns it.
   sharable_lock(AUTOBOOST_RV_REF(sharable_lock<mutex_type>) upgr)
   ...
   //!   "autoboost::move(lock);".*/
   template<class T>
   sharable_lock(AUTOBOOST_RV_REF(upgradable_lock<T>) upgr
   ...
   //!   "autoboost::move(lock);".
   template<class T>
   sharable_lock(AUTOBOOST_RV_REF(scoped_lock<T>) scop
               , typename ipcdetail::enable_if< ipcdetail::is_same<T, SharableMutex> >::type * = 0)
      : mp_mutex(0), m_locked(false)
   {
   ...
   sharable_lock &operator=(AUTOBOOST_RV_REF(sharable_lock<mutex_type>) upgr)
   {
      if(this->owns())
         this->unlock();
      m_locked = upgr.owns();
...
#endif // AUTOBOOST_INTERPROCESS_SHARABLE_LOCK_HPP
