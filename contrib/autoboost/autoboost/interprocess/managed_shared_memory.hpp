...
#ifndef AUTOBOOST_INTERPROCESS_MANAGED_SHARED_MEMORY_HPP
#define AUTOBOOST_INTERPROCESS_MANAGED_SHARED_MEMORY_HPP
   ...
   , private ipcdetail::shmem_open_or_create<AllocationAlgorithm>::type
{
   ...
   AUTOBOOST_MOVABLE_BUT_NOT_COPYABLE(basic_managed_shared_memory)
   ...
   //!Does not throw
   basic_managed_shared_memory(AUTOBOOST_RV_REF(basic_managed_shared_memory) moved)
   {
   ...
   //!Moves the ownership of "moved"'s managed memory to *this.
   //!Does not throw
   basic_managed_shared_memory &operator=(AUTOBOOST_RV_REF(basic_managed_shared_memory) moved)
   {
      basic_managed_shared_memory tmp(autoboost::move(moved));
      this->swap(tmp);
      return *this;
   }
...
#endif   //AUTOBOOST_INTERPROCESS_MANAGED_SHARED_MEMORY_HPP
