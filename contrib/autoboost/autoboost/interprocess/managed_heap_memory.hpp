...
#ifndef AUTOBOOST_INTERPROCESS_MANAGED_HEAP_MEMORY_HPP
#define AUTOBOOST_INTERPROCESS_MANAGED_HEAP_MEMORY_HPP
   ...
   : public ipcdetail::basic_managed_memory_impl <CharType, AllocationAlgorithm, IndexType>
{
   ...
   AUTOBOOST_MOVABLE_BUT_NOT_COPYABLE(basic_managed_heap_memory)
   ...
   basic_managed_heap_memory(AUTOBOOST_RV_REF(basic_managed_heap_memory) moved)
   {  this->swap(moved);   }

   //!Moves the ownership of "moved"'s managed memory to *this. Does not throw
   basic_managed_heap_memory &operator=(AUTOBOOST_RV_REF(basic_managed_heap_memory) moved)
   {
      basic_managed_heap_memory tmp(autoboost::move(moved));
      this->swap(tmp);
      return *this;
   }
...
#endif   //AUTOBOOST_INTERPROCESS_MANAGED_HEAP_MEMORY_HPP
