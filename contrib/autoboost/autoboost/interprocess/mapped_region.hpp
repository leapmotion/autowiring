...
#define AUTOBOOST_INTERPROCESS_MADVISE_USES_CADDR_T
#include <sys/types.h>
#endif
...
#define AUTOBOOST_INTERPROCESS_MADV_DONTNEED_HAS_NONDESTRUCTIVE_SEMANTICS
#endif
...
class mapped_region
{
   #if !defined(AUTOBOOST_INTERPROCESS_DOXYGEN_INVOKED)
   //Non-copyable
   AUTOBOOST_MOVABLE_BUT_NOT_COPYABLE(mapped_region)
   #endif   //#ifndef AUTOBOOST_INTERPROCESS_DOXYGEN_INVOKED
   ...
   mapped_region(AUTOBOOST_RV_REF(mapped_region) other)
   #if defined (AUTOBOOST_INTERPROCESS_WINDOWS)
   :  m_base(0), m_size(0)
   ,  m_page_offset(0)
   ,  m_mode(read_only)
   ...
   mapped_region &operator=(AUTOBOOST_RV_REF(mapped_region) other)
   {
      mapped_region tmp(autoboost::move(other));
      this->swap(tmp);
      return *this;
   }
   ...
   #if defined(AUTOBOOST_INTERPROCESS_WINDOWS) && !defined(AUTOBOOST_INTERPROCESS_FORCE_GENERIC_EMULATION)
   template<int Dummy>
   ...
   (std::size_t bytes, bool from_back, void *&shrink_page_start, std::size_t &shrink_page_bytes)
{
   ...
   if(m_base == 0 || bytes > m_size){
      ...
      if(from_back){
         ...
         AUTOBOOST_ASSERT(shrink_page_bytes%page_size == 0);
      }
      return true;
   }
}
   ...
   (offset_t mapping_size, offset_t offset, offset_t page_offset, std::size_t &size)
{
   ...
   if(mapping_size < offset ||
      autoboost::uintmax_t(mapping_size - (offset - page_offset)) >
         autoboost::uintmax_t(std::size_t(-1))){
   ...
   : m_base(0), m_size(0), m_page_offset(0), m_mode(mode), m_is_xsi(false)
{
   ...
   #ifdef AUTOBOOST_INTERPROCESS_XSI_SHARED_MEMORY_OBJECTS
   if(map_hnd.is_xsi){
      //Get the size
      ::shmid_ds xsi_ds;
   ...
   #endif   //ifdef AUTOBOOST_INTERPROCESS_XSI_SHARED_MEMORY_OBJECTS
   ...
   #ifdef MAP_NOSYNC
      #define AUTOBOOST_INTERPROCESS_MAP_NOSYNC MAP_NOSYNC
   #else
      #define AUTOBOOST_INTERPROCESS_MAP_NOSYNC 0
   #endif   //MAP_NOSYNC

   //Create new mapping
   int prot    = 0;
   int flags   = map_options == default_map_options ? AUTOBOOST_INTERPROCESS_MAP_NOSYNC : map_options;

   #undef AUTOBOOST_INTERPROCESS_MAP_NOSYNC
...
inline bool mapped_region::advise(advice_types advice)
{
   ...
   switch(advice){
      ...
      case advice_dontneed:
         ...
         mode = mode_padv;
         #elif defined(MADV_DONTNEED) && defined(AUTOBOOST_INTERPROCESS_MADV_DONTNEED_HAS_NONDESTRUCTIVE_SEMANTICS)
   ...
   switch(mode){
         ...
         case mode_madv:
         return 0 == madvise(
            #if defined(AUTOBOOST_INTERPROCESS_MADVISE_USES_CADDR_T)
            (caddr_t)
            #endif
...
#endif   //AUTOBOOST_INTERPROCESS_MAPPED_REGION_HPP

#if !defined(AUTOBOOST_INTERPROCESS_DOXYGEN_INVOKED)

#ifndef AUTOBOOST_INTERPROCESS_MAPPED_REGION_EXT_HPP
#define AUTOBOOST_INTERPROCESS_MAPPED_REGION_EXT_HPP
...
#endif   //defined(AUTOBOOST_INTERPROCESS_WINDOWS) && !defined(AUTOBOOST_INTERPROCESS_FORCE_GENERIC_EMULATION)

#endif   //#ifdef AUTOBOOST_INTERPROCESS_MAPPED_REGION_EXT_HPP
