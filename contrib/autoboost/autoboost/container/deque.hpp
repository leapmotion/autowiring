...
struct deque_value_traits
{
   ...
   static const bool trivial_dctr = autoboost::has_trivial_destructor<value_type>::value;
...
class deque_iterator
{
   ...
   deque_iterator(val_alloc_ptr x, index_pointer y) AUTOBOOST_CONTAINER_NOEXCEPT
      : m_cur(x), m_first(*y), m_last(*y + s_buffer_size()), m_node(y)
   {}

   deque_iterator() AUTOBOOST_CONTAINER_NOEXCEPT
      : m_cur(), m_first(), m_last(), m_node()  //Value initialization to achieve "null iterators" (N3644)
   {}

   deque_iterator(deque_iterator<Pointer, false> const& x) AUTOBOOST_CONTAINER_NOEXCEPT
      : m_cur(x.get_cur()), m_first(x.get_first()), m_last(x.get_last()), m_node(x.get_node())
   {}

   deque_iterator(Pointer cur, Pointer first, Pointer last, index_pointer node) AUTOBOOST_CONTAINER_NOEXCEPT
      : m_cur(cur), m_first(first), m_last(last), m_node(node)
   {}

   deque_iterator<Pointer, false> unconst() const AUTOBOOST_CONTAINER_NOEXCEPT
   {
   ...
   difference_type operator-(const deque_iterator& x) const AUTOBOOST_CONTAINER_NOEXCEPT
   {
   ...
   deque_iterator& operator++() AUTOBOOST_CONTAINER_NOEXCEPT
   {
      ++this->m_cur;
   ...
   deque_iterator operator++(int) AUTOBOOST_CONTAINER_NOEXCEPT
   {
   ...
   deque_iterator& operator+=(difference_type n) AUTOBOOST_CONTAINER_NOEXCEPT
   {
   ...
   deque_iterator operator+(difference_type n) const AUTOBOOST_CONTAINER_NOEXCEPT
   ...
   reference operator[](difference_type n) const AUTOBOOST_CONTAINER_NOEXCEPT
      { return *(*this + n); }

   friend bool operator==(const deque_iterator& l, const deque_iterator& r) AUTOBOOST_CONTAINER_NOEXCEPT
   ...
   void priv_set_node(index_pointer new_node) AUTOBOOST_CONTAINER_NOEXCEPT
   {
   ...
   friend deque_iterator operator+(difference_type n, deque_iterator x) AUTOBOOST_CONTAINER_NOEXCEPT
      {  return x += n;  }
};
...
class deque_base
{
   ...
   static size_type s_buffer_size() AUTOBOOST_CONTAINER_NOEXCEPT
      { return deque_buf_size<val_alloc_val>::value; }

   val_alloc_ptr priv_allocate_node()
      {  return this->alloc().allocate(s_buffer_size());  }

   void priv_deallocate_node(val_alloc_ptr p) AUTOBOOST_CONTAINER_NOEXCEPT
      {  this->alloc().deallocate(p, s_buffer_size());  }

   ptr_alloc_ptr priv_allocate_map(size_type n)
      { return this->ptr_alloc().allocate(n); }

   void priv_deallocate_map(ptr_alloc_ptr p, size_type n) AUTOBOOST_CONTAINER_NOEXCEPT
   ...
   void swap_members(deque_base &x) AUTOBOOST_CONTAINER_NOEXCEPT
   {
...
//      }
   ...
   void priv_destroy_nodes(ptr_alloc_ptr nstart, ptr_alloc_ptr nfinish) AUTOBOOST_CONTAINER_NOEXCEPT
   {
      for (ptr_alloc_ptr n = nstart; n < nfinish; ++n)
         this->priv_deallocate_node(*n);
   }

   void priv_clear_map() AUTOBOOST_CONTAINER_NOEXCEPT
   {
      if (this->members_.m_map) {
         this->priv_destroy_nodes(this->members_.m_start.m_node, this->members_.m_finish.m_node + 1);
         this->priv_deallocate_map(this->members_.m_map, this->members_.m_map_size);
   ...
   ptr_alloc_t &ptr_alloc() AUTOBOOST_CONTAINER_NOEXCEPT
   {  return members_;  }

   const ptr_alloc_t &ptr_alloc() const AUTOBOOST_CONTAINER_NOEXCEPT
   {  return members_;  }

   allocator_type &alloc() AUTOBOOST_CONTAINER_NOEXCEPT
   {  return members_;  }

   const allocator_type &alloc() const AUTOBOOST_CONTAINER_NOEXCEPT
   {  return members_;  }
};
...
class deque : protected deque_base<Allocator>
{
   ...
   //!
   //! <b>Complexity</b>: Constant.
   explicit deque(const allocator_type& a) AUTOBOOST_CONTAINER_NOEXCEPT
      : Base(a)
   {}
   ...
   deque(AUTOBOOST_RV_REF(deque) mx, const allocator_type &a)
      :  Base(a)
   {
      if(mx.alloc() == a){
   ...
   ~deque() AUTOBOOST_CONTAINER_NOEXCEPT
   {
   ...
   void emplace_front(AUTOBOOST_PP_ENUM(n, AUTOBOOST_CONTAINER_PP_PARAM_LIST, _))                    \
   {                                                                                         \
      if(priv_push_front_simple_available()){                                                \
         ...
         typedef container_detail::AUTOBOOST_PP_CAT(insert_non_movable_emplace_proxy_arg, n)     \
               <Allocator, iterator AUTOBOOST_PP_ENUM_TRAILING_PARAMS(n, P)> type;               \
         priv_insert_front_aux_impl                                                          \
            (1, type(AUTOBOOST_PP_ENUM(n, AUTOBOOST_CONTAINER_PP_PARAM_FORWARD, _)));                \
      }                                                                                      \
   }                                                                                         \
                                                                                             \
   AUTOBOOST_PP_EXPR_IF(n, template<) AUTOBOOST_PP_ENUM_PARAMS(n, class P) AUTOBOOST_PP_EXPR_IF(n, >)    \
   ...
   {                                                                                         \
      if(p == this->cbegin()){                                                               \
         this->emplace_front(AUTOBOOST_PP_ENUM(n, AUTOBOOST_CONTAINER_PP_PARAM_FORWARD, _));         \
         return this->begin();                                                               \
      }                                                                                      \
      else if(p == cend()){                                                                  \
         this->emplace_back(AUTOBOOST_PP_ENUM(n, AUTOBOOST_CONTAINER_PP_PARAM_FORWARD, _));          \
         return (this->end()-1);                                                             \
      }                                                                                      \
      else{                                                                                  \
         ...
         return this->priv_insert_aux_impl                                                   \
            (p, 1, type(AUTOBOOST_PP_ENUM(n, AUTOBOOST_CONTAINER_PP_PARAM_FORWARD, _)));             \
      }                                                                                      \
   }                                                                                         \
   //!
   ...
   void pop_front() AUTOBOOST_CONTAINER_NOEXCEPT
   {
   ...
   iterator erase(const_iterator pos) AUTOBOOST_CONTAINER_NOEXCEPT
   {
      iterator next = pos.unconst();
      ++next;
      size_type index = pos - this->members_.m_start;
      if (index < (this->size()/2)) {
         autoboost::move_backward(this->begin(), pos.unconst(), next);
         pop_front();
      }
      else {
         autoboost::move(next, this->end(), pos.unconst());
         pop_back();
      }
   ...
   iterator erase(const_iterator first, const_iterator last) AUTOBOOST_CONTAINER_NOEXCEPT
   {
      if (first == this->members_.m_start && last == this->members_.m_finish) {
         ...
         if (elems_before < (this->size() - n) - elems_before) {
            autoboost::move_backward(begin(), first.unconst(), last.unconst());
            ...
            autoboost::move(last.unconst(), end(), first.unconst());
      ...
      if (elemsbefore < length / 2) {
         ...
         if(!elemsbefore){
            ...
            if (elemsbefore >= n) {
               ...
               this->members_.m_start = new_start;
               autoboost::move(start_n, pos, old_start);
   ...
   void priv_pop_back_aux() AUTOBOOST_CONTAINER_NOEXCEPT
   {
      this->priv_deallocate_node(this->members_.m_finish.m_first);
   ...
   void priv_pop_front_aux() AUTOBOOST_CONTAINER_NOEXCEPT
   {
      allocator_traits_type::destroy
         ( this->alloc()
         , container_detail::to_raw_pointer(this->members_.m_start.m_cur)
   ...
   void priv_reallocate_map(size_type nodes_to_add, bool add_at_front)
   {
      ...
      if (this->members_.m_map_size > 2 * new_num_nodes) {
         ...
         if (new_nstart < this->members_.m_start.m_node)
            autoboost::move(this->members_.m_start.m_node, this->members_.m_finish.m_node + 1, new_nstart);
         else
            autoboost::move_backward
