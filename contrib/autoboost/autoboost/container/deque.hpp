...
//
//////////////////////////////////////////////////////////////////////////////
#ifndef AUTOBOOST_CONTAINER_DEQUE_HPP
#define AUTOBOOST_CONTAINER_DEQUE_HPP
...
struct deque_value_traits
{
   ...
   static const bool trivial_dctr_after_move = ::autoboost::has_trivial_destructor_after_move<value_type>::value;
};
...
class deque_iterator
{
   ...
   deque_iterator(val_alloc_ptr x, index_pointer y) AUTOBOOST_NOEXCEPT_OR_NOTHROW
      : m_cur(x), m_first(*y), m_last(*y + s_buffer_size()), m_node(y)
   {}

   deque_iterator() AUTOBOOST_NOEXCEPT_OR_NOTHROW
      : m_cur(), m_first(), m_last(), m_node()  //Value initialization to achieve "null iterators" (N3644)
   {}

   deque_iterator(deque_iterator<Pointer, false> const& x) AUTOBOOST_NOEXCEPT_OR_NOTHROW
      : m_cur(x.get_cur()), m_first(x.get_first()), m_last(x.get_last()), m_node(x.get_node())
   {}

   deque_iterator(Pointer cur, Pointer first, Pointer last, index_pointer node) AUTOBOOST_NOEXCEPT_OR_NOTHROW
      : m_cur(cur), m_first(first), m_last(last), m_node(node)
   {}

   deque_iterator<Pointer, false> unconst() const AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
   ...
   difference_type operator-(const deque_iterator& x) const AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
   ...
   deque_iterator& operator++() AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
      ++this->m_cur;
   ...
   deque_iterator operator++(int) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
   ...
   deque_iterator& operator+=(difference_type n) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
   ...
   deque_iterator operator+(difference_type n) const AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   reference operator[](difference_type n) const AUTOBOOST_NOEXCEPT_OR_NOTHROW
      { return *(*this + n); }

   friend bool operator==(const deque_iterator& l, const deque_iterator& r) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   void priv_set_node(index_pointer new_node) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
   ...
   friend deque_iterator operator+(difference_type n, deque_iterator x) AUTOBOOST_NOEXCEPT_OR_NOTHROW
      {  return x += n;  }
};
...
class deque_base
{
   AUTOBOOST_COPYABLE_AND_MOVABLE(deque_base)
   public:
   ...
   static size_type s_buffer_size() AUTOBOOST_NOEXCEPT_OR_NOTHROW
      { return deque_buf_size<val_alloc_val>::value; }

   val_alloc_ptr priv_allocate_node()
      {  return this->alloc().allocate(s_buffer_size());  }

   void priv_deallocate_node(val_alloc_ptr p) AUTOBOOST_NOEXCEPT_OR_NOTHROW
      {  this->alloc().deallocate(p, s_buffer_size());  }

   ptr_alloc_ptr priv_allocate_map(size_type n)
      { return this->ptr_alloc().allocate(n); }

   void priv_deallocate_map(ptr_alloc_ptr p, size_type n) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   explicit deque_base(AUTOBOOST_RV_REF(deque_base) x)
      :  members_( autoboost::move(x.ptr_alloc())
                 , autoboost::move(x.alloc()) )
   {}
   ...
   void swap_members(deque_base &x) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
      ::autoboost::adl_move_swap(this->members_.m_start, x.members_.m_start);
      ::autoboost::adl_move_swap(this->members_.m_finish, x.members_.m_finish);
      ::autoboost::adl_move_swap(this->members_.m_map, x.members_.m_map);
      ::autoboost::adl_move_swap(this->members_.m_map_size, x.members_.m_map_size);
   }
...
//      }
   ...
   void priv_destroy_nodes(ptr_alloc_ptr nstart, ptr_alloc_ptr nfinish) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
      for (ptr_alloc_ptr n = nstart; n < nfinish; ++n)
         this->priv_deallocate_node(*n);
   }

   void priv_clear_map() AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
      if (this->members_.m_map) {
         this->priv_destroy_nodes(this->members_.m_start.m_node, this->members_.m_finish.m_node + 1);
         this->priv_deallocate_map(this->members_.m_map, this->members_.m_map_size);
      ...
      ,  public allocator_type
   {
      ...
      members_holder(AUTOBOOST_FWD_REF(PtrAllocConvertible) pa, AUTOBOOST_FWD_REF(ValAllocConvertible) va)
         : map_allocator_type(autoboost::forward<PtrAllocConvertible>(pa))
         , allocator_type    (autoboost::forward<ValAllocConvertible>(va))
         , m_map(0), m_map_size(0)
         , m_start(), m_finish(m_start)
      {}
   ...
   ptr_alloc_t &ptr_alloc() AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {  return members_;  }

   const ptr_alloc_t &ptr_alloc() const AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {  return members_;  }

   allocator_type &alloc() AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {  return members_;  }

   const allocator_type &alloc() const AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {  return members_;  }
};
...
class deque : protected deque_base<Allocator>
{
   ...
   typedef Allocator                                                                   allocator_type;
   typedef AUTOBOOST_CONTAINER_IMPDEF(allocator_type)                                      stored_allocator_type;
   typedef AUTOBOOST_CONTAINER_IMPDEF(typename Base::iterator)                             iterator;
   typedef AUTOBOOST_CONTAINER_IMPDEF(typename Base::const_iterator)                       const_iterator;
   ...
   AUTOBOOST_COPYABLE_AND_MOVABLE(deque)
   ...
   deque() AUTOBOOST_NOEXCEPT_IF(container_detail::is_nothrow_default_constructible<Allocator>::value)
      : Base()
   {}
   ...
   //!
   //! <b>Complexity</b>: Constant.
   explicit deque(const allocator_type& a) AUTOBOOST_NOEXCEPT_OR_NOTHROW
      : Base(a)
   {}
      ...
      :  Base(allocator_traits_type::select_on_container_copy_construction(x.alloc()))
   {
      if(x.size()){
         ...
         autoboost::container::uninitialized_copy_alloc
   ...
   //!
   //! <b>Complexity</b>: Constant.
   deque(AUTOBOOST_RV_REF(deque) x) AUTOBOOST_NOEXCEPT_OR_NOTHROW
      :  Base(AUTOBOOST_MOVE_BASE(Base, x))
   {  this->swap_members(x);   }
   ...
   deque(AUTOBOOST_RV_REF(deque) x, const allocator_type &a)
      :  Base(a)
   {
      if(x.alloc() == a){
         this->swap_members(x);
         ...
         if(x.size()){
            this->priv_initialize_map(x.size());
            autoboost::container::uninitialized_copy_alloc
               ( this->alloc(), autoboost::make_move_iterator(x.begin())
               , autoboost::make_move_iterator(x.end()), this->members_.m_start);
         }
      }
   }
   ...
   ~deque() AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
   ...
   deque& operator= (AUTOBOOST_COPY_ASSIGN_REF(deque) x)
   {
      if (&x != this){
   ...
   deque& operator= (AUTOBOOST_RV_REF(deque) x)
   ...
   {
      const size_type len = autoboost::container::iterator_distance(first, last);
      if (len > size()) {
         FwdIt mid = first;
         autoboost::container::iterator_advance(mid, this->size());
         autoboost::container::copy(first, mid, begin());
         this->insert(this->cend(), mid, last);
      }
      else{
         this->erase(autoboost::container::copy(first, last, this->begin()), cend());
      }
   }
   #endif
   ...
   template <class... Args>
   reference emplace_front(AUTOBOOST_FWD_REF(Args)... args)
   {
      if(this->priv_push_front_simple_available()){
         ...
         return *this->priv_insert_front_aux_impl(1, type(autoboost::forward<Args>(args)...));
      }
   }
   ...
   //! <b>Complexity</b>: Amortized constant time
   template <class... Args>
   reference emplace_back(AUTOBOOST_FWD_REF(Args)... args)
   {
      if(this->priv_push_back_simple_available()){
         ...
         typedef container_detail::insert_nonmovable_emplace_proxy<Allocator, iterator, Args...> type;
         return *this->priv_insert_back_aux_impl(1, type(autoboost::forward<Args>(args)...));
      }
   }
   ...
   template <class... Args>
   iterator emplace(const_iterator p, AUTOBOOST_FWD_REF(Args)... args)
   {
      AUTOBOOST_ASSERT(this->priv_in_range_or_end(p));
      if(p == this->cbegin()){
         this->emplace_front(autoboost::forward<Args>(args)...);
         return this->begin();
      }
      else if(p == this->cend()){
         this->emplace_back(autoboost::forward<Args>(args)...);
         return (this->end()-1);
      }
      else{
         typedef container_detail::insert_emplace_proxy<Allocator, iterator, Args...> type;
         return this->priv_insert_aux_impl(p, 1, type(autoboost::forward<Args>(args)...));
      }
   }

   #else //!defined(AUTOBOOST_NO_CXX11_VARIADIC_TEMPLATES)

   #define AUTOBOOST_CONTAINER_DEQUE_EMPLACE_CODE(N) \
   AUTOBOOST_MOVE_TMPL_LT##N AUTOBOOST_MOVE_CLASS##N AUTOBOOST_MOVE_GT##N\
   reference emplace_front(AUTOBOOST_MOVE_UREF##N)\
   {\
      if(priv_push_front_simple_available()){\
         reference r = *this->priv_push_front_simple_pos();\
         allocator_traits_type::construct\
            ( this->alloc(), this->priv_push_front_simple_pos() AUTOBOOST_MOVE_I##N AUTOBOOST_MOVE_FWD##N);\
         priv_push_front_simple_commit();\
         return r;\
      }\
      else{\
         typedef container_detail::insert_nonmovable_emplace_proxy##N\
               <Allocator, iterator AUTOBOOST_MOVE_I##N AUTOBOOST_MOVE_TARG##N> type;\
         return *priv_insert_front_aux_impl(1, type(AUTOBOOST_MOVE_FWD##N));\
      }\
   }\
   \
   AUTOBOOST_MOVE_TMPL_LT##N AUTOBOOST_MOVE_CLASS##N AUTOBOOST_MOVE_GT##N\
   reference emplace_back(AUTOBOOST_MOVE_UREF##N)\
   {\
      if(priv_push_back_simple_available()){\
         reference r = *this->priv_push_back_simple_pos();\
         allocator_traits_type::construct\
            ( this->alloc(), this->priv_push_back_simple_pos() AUTOBOOST_MOVE_I##N AUTOBOOST_MOVE_FWD##N);\
         ...
         typedef container_detail::insert_nonmovable_emplace_proxy##N\
               <Allocator, iterator AUTOBOOST_MOVE_I##N AUTOBOOST_MOVE_TARG##N> type;\
         return *priv_insert_back_aux_impl(1, type(AUTOBOOST_MOVE_FWD##N));\
      }\
   }\
   \
   AUTOBOOST_MOVE_TMPL_LT##N AUTOBOOST_MOVE_CLASS##N AUTOBOOST_MOVE_GT##N\
   iterator emplace(const_iterator p AUTOBOOST_MOVE_I##N AUTOBOOST_MOVE_UREF##N)\
   {\
      AUTOBOOST_ASSERT(this->priv_in_range_or_end(p));\
      if(p == this->cbegin()){\
         this->emplace_front(AUTOBOOST_MOVE_FWD##N);\
         return this->begin();\
      }\
      else if(p == cend()){\
         this->emplace_back(AUTOBOOST_MOVE_FWD##N);\
         return (--this->end());\
      }\
      else{\
         typedef container_detail::insert_emplace_proxy_arg##N\
               <Allocator, iterator AUTOBOOST_MOVE_I##N AUTOBOOST_MOVE_TARG##N> type;\
         return this->priv_insert_aux_impl(p, 1, type(AUTOBOOST_MOVE_FWD##N));\
      }\
   }
   //
   AUTOBOOST_MOVE_ITERATE_0TO9(AUTOBOOST_CONTAINER_DEQUE_EMPLACE_CODE)
   #undef AUTOBOOST_CONTAINER_DEQUE_EMPLACE_CODE
   ...
   void push_front(T &&x);
   #else
   AUTOBOOST_MOVE_CONVERSION_AWARE_CATCH(push_front, T, void, priv_push_front)
   #endif
      ...
      )
   {
      ...
      return priv_insert_aux_impl(p, autoboost::container::iterator_distance(first, last), proxy);
   }
   #endif
   ...
   void pop_front() AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
      AUTOBOOST_ASSERT(!this->empty());
   ...
   iterator erase(const_iterator pos) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
      AUTOBOOST_ASSERT(this->priv_in_range(pos));
      iterator next = pos.unconst();
      ++next;
      size_type index = pos - this->members_.m_start;
      if (index < (this->size()/2)) {
         autoboost::container::move_backward(this->begin(), pos.unconst(), next);
         pop_front();
      }
      else {
         autoboost::container::move(next, this->end(), pos.unconst());
         pop_back();
      }
   ...
   iterator erase(const_iterator first, const_iterator last) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
      ...
      if (first == this->members_.m_start && last == this->members_.m_finish) {
         ...
         if (elems_before < (this->size() - n) - elems_before) {
            autoboost::container::move_backward(begin(), first.unconst(), last.unconst());
            ...
            autoboost::container::move(last.unconst(), end(), first.unconst());
   ...
   {
      AUTOBOOST_ASSERT(this->cbegin() <= p);
      AUTOBOOST_ASSERT(p <= this->cend());
   ...
   template <class U>
   iterator priv_insert(const_iterator p, AUTOBOOST_FWD_REF(U) x)
   {
      AUTOBOOST_ASSERT(this->priv_in_range_or_end(p));
      if (p == cbegin()){
         this->push_front(::autoboost::forward<U>(x));
         return begin();
      }
      else if (p == cend()){
         this->push_back(::autoboost::forward<U>(x));
         return --end();
      }
      else {
         return priv_insert_aux_impl
            ( p, (size_type)1
            , container_detail::get_insert_value_proxy<iterator, Allocator>(::autoboost::forward<U>(x)));
      }
   }

   template <class U>
   void priv_push_front(AUTOBOOST_FWD_REF(U) x)
   {
      if(this->priv_push_front_simple_available()){
         allocator_traits_type::construct
            ( this->alloc(), this->priv_push_front_simple_pos(), ::autoboost::forward<U>(x));
         this->priv_push_front_simple_commit();
      }
      else{
   ...
   void priv_push_back(AUTOBOOST_FWD_REF(U) x)
   {
      if(this->priv_push_back_simple_available()){
         allocator_traits_type::construct
            ( this->alloc(), this->priv_push_back_simple_pos(), ::autoboost::forward<U>(x));
         this->priv_push_back_simple_commit();
      }
      else{
         priv_insert_aux_impl
      ...
      if (elemsbefore < length / 2) {
         ...
         if(!elemsbefore){
            ...
            if (elemsbefore >= n) {
               ...
               this->members_.m_start = new_start;
               autoboost::container::move(start_n, pos, old_start);
   ...
   void priv_fill_initialize(const value_type& value)
   {
      ...
      AUTOBOOST_TRY {
         for ( ; cur < this->members_.m_finish.m_node; ++cur){
            autoboost::container::uninitialized_fill_alloc
   ...
   {
      size_type n = 0;
      n = autoboost::container::iterator_distance(first, last);
      ...
      AUTOBOOST_TRY {
         for (; cur_node < this->members_.m_finish.m_node; ++cur_node) {
            FwdIt mid = first;
            autoboost::container::iterator_advance(mid, this->s_buffer_size());
            ::autoboost::container::uninitialized_copy_alloc(this->alloc(), first, mid, *cur_node);
            first = mid;
         }
         ::autoboost::container::uninitialized_copy_alloc(this->alloc(), first, last, this->members_.m_finish.m_first);
      }
      AUTOBOOST_CATCH(...){
   ...
   void priv_pop_back_aux() AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
      this->priv_deallocate_node(this->members_.m_finish.m_first);
   ...
   void priv_pop_front_aux() AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
      allocator_traits_type::destroy
         ( this->alloc()
         , container_detail::to_raw_pointer(this->members_.m_start.m_cur)
   ...
   void priv_reallocate_map(size_type nodes_to_add, bool add_at_front)
   {
      ...
      if (this->members_.m_map_size > 2 * new_num_nodes) {
         ...
         if (new_nstart < this->members_.m_start.m_node)
            autoboost::container::move(this->members_.m_start.m_node, this->members_.m_finish.m_node + 1, new_nstart);
         else
            autoboost::container::move_backward
...
template <class T, class Allocator>
struct has_trivial_destructor_after_move<autoboost::container::deque<T, Allocator> >
{
   typedef typename ::autoboost::container::allocator_traits<Allocator>::pointer pointer;
...
#endif //   #ifndef  AUTOBOOST_CONTAINER_DEQUE_HPP
