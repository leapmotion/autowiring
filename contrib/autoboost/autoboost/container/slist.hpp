...
#ifndef AUTOBOOST_CONTAINER_SLIST_HPP
#define AUTOBOOST_CONTAINER_SLIST_HPP
...
struct intrusive_slist_type
{
   ...
   typedef typename autoboost::intrusive::pointer_traits
      ...
      <Allocator, typename container_detail::intrusive_slist_type<Allocator>::type>
{
   ...
   typedef autoboost::container::equal_to_value<Allocator>      equal_to_value_type;

   AUTOBOOST_COPYABLE_AND_MOVABLE(slist)
   ...
   typedef AUTOBOOST_CONTAINER_IMPDEF(NodeAlloc)                                  stored_allocator_type;
   ...
   slist() AUTOBOOST_NOEXCEPT_IF(container_detail::is_nothrow_default_constructible<Allocator>::value)
      :  AllocHolder()
   {}
   ...
   //!
   //! <b>Complexity</b>: Constant.
   explicit slist(const allocator_type& a) AUTOBOOST_NOEXCEPT_OR_NOTHROW
      :  AllocHolder(a)
   {}
   ...
   //!
   //! <b>Complexity</b>: Constant.
   slist(AUTOBOOST_RV_REF(slist) x) AUTOBOOST_NOEXCEPT_OR_NOTHROW
      : AllocHolder(AUTOBOOST_MOVE_BASE(AllocHolder, x))
   {}
   ...
   slist(AUTOBOOST_RV_REF(slist) x, const allocator_type &a)
      : AllocHolder(a)
   {
      if(this->node_alloc() == x.node_alloc()){
         ...
         this->insert_after(this->cbefore_begin(), autoboost::make_move_iterator(x.begin()), autoboost::make_move_iterator(x.end()));
      }
   }
   ...
   ~slist() AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   slist& operator= (AUTOBOOST_COPY_ASSIGN_REF(slist) x)
   {
      if (&x != this){
   ...
   slist& operator=(AUTOBOOST_RV_REF(slist) x)
   ...
   //!
   //! <b>Complexity</b>: Constant.
   iterator before_begin() AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {  return iterator(end());  }
   ...
   //!
   //! <b>Complexity</b>: Constant.
   const_iterator before_begin() const AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   //!
   //! <b>Complexity</b>: Constant.
   const_iterator cbefore_begin() const AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   //!
   //! <b>Note</b>: Non-standard extension.
   iterator previous(iterator p) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   {  return *this->emplace_after(this->cbefore_begin(), autoboost::forward<Args>(args)...); }
   ...
   //! <b>Complexity</b>: Constant
   template <class... Args>
   iterator emplace_after(const_iterator prev, AUTOBOOST_FWD_REF(Args)... args)
   {
      NodePtr pnode(AllocHolder::create_node(autoboost::forward<Args>(args)...));
      return iterator(this->icont().insert_after(prev.get(), *pnode));
   }

   #else // !defined(AUTOBOOST_NO_CXX11_VARIADIC_TEMPLATES)

   #define AUTOBOOST_CONTAINER_SLIST_EMPLACE_CODE(N) \
   AUTOBOOST_MOVE_TMPL_LT##N AUTOBOOST_MOVE_CLASS##N AUTOBOOST_MOVE_GT##N \
   reference emplace_front(AUTOBOOST_MOVE_UREF##N)\
   {  return *this->emplace_after(this->cbefore_begin() AUTOBOOST_MOVE_I##N AUTOBOOST_MOVE_FWD##N);}\
   \
   AUTOBOOST_MOVE_TMPL_LT##N AUTOBOOST_MOVE_CLASS##N AUTOBOOST_MOVE_GT##N \
   iterator emplace_after(const_iterator p AUTOBOOST_MOVE_I##N AUTOBOOST_MOVE_UREF##N)\
   {\
      NodePtr pnode (AllocHolder::create_node(AUTOBOOST_MOVE_FWD##N));\
      return iterator(this->icont().insert_after(p.get(), *pnode));\
   }\
   //
   AUTOBOOST_MOVE_ITERATE_0TO9(AUTOBOOST_CONTAINER_SLIST_EMPLACE_CODE)
   #undef AUTOBOOST_CONTAINER_SLIST_EMPLACE_CODE
   ...
   #else
   AUTOBOOST_MOVE_CONVERSION_AWARE_CATCH_1ARG(insert_after, T, iterator, priv_insert_after, const_iterator, const_iterator)
   #endif
      ...
      )
   {
      ...
      this->allocate_many_and_construct(first, autoboost::container::iterator_distance(first, last), func);
   ...
   void splice_after(const_iterator prev_p, slist& x) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
      AUTOBOOST_ASSERT(this != &x);
      AUTOBOOST_ASSERT(this->node_alloc() == x.node_alloc());
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of
   //!    this list. Iterators of this list and all the references are not invalidated.
   void splice_after(const_iterator prev_p, AUTOBOOST_RV_REF(slist) x) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   void splice_after(const_iterator prev_p, slist& x, const_iterator prev) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
      AUTOBOOST_ASSERT(this->node_alloc() == x.node_alloc());
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of this
   //!   list. Iterators of this list and all the references are not invalidated.
   void splice_after(const_iterator prev_p, AUTOBOOST_RV_REF(slist) x, const_iterator prev) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   void splice_after(const_iterator prev_p,      slist& x,
      const_iterator before_first,  const_iterator before_last) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
      AUTOBOOST_ASSERT(this->node_alloc() == x.node_alloc());
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of this
   //!   list. Iterators of this list and all the references are not invalidated.
   void splice_after(const_iterator prev_p,      AUTOBOOST_RV_REF(slist) x,
      const_iterator before_first,  const_iterator before_last) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   void splice_after(const_iterator prev_p,      slist& x,
                     ...
                     size_type n) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
      AUTOBOOST_ASSERT(this->node_alloc() == x.node_alloc());
      this->icont().splice_after
   ...
   //! <b>Complexity</b>: This function is linear time: it performs at most
   //!   size() + x.size() - 1 comparisons.
   void merge(AUTOBOOST_RV_REF(slist) x)
   ...
   //! <b>Note</b>: Iterators and references to *this are not invalidated.
   template <class StrictWeakOrdering>
   void merge(AUTOBOOST_RV_REF(slist) x, StrictWeakOrdering comp)
   ...
   void reverse() AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {  this->icont().reverse();  }
   ...
   {  return this->emplace_after(this->previous(p), autoboost::forward<Args>(args)...);  }
   ...
   {\
      return this->emplace_after(this->previous(p) AUTOBOOST_MOVE_I##N AUTOBOOST_MOVE_FWD##N);\
   }\
   //
   AUTOBOOST_MOVE_ITERATE_0TO9(AUTOBOOST_CONTAINER_SLIST_EMPLACE_CODE)
   #undef AUTOBOOST_CONTAINER_SLIST_EMPLACE_CODE
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of
   //!    this list. Iterators of this list and all the references are not invalidated.
   void splice(const_iterator p, slist& x) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of
   //!    this list. Iterators of this list and all the references are not invalidated.
   void splice(const_iterator p, AUTOBOOST_RV_REF(slist) x) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of this
   //!   list. Iterators of this list and all the references are not invalidated.
   void splice(const_iterator p, slist& x, const_iterator i) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of this
   //!   list. Iterators of this list and all the references are not invalidated.
   void splice(const_iterator p, AUTOBOOST_RV_REF(slist) x, const_iterator i) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of this
   //!   list. Iterators of this list and all the references are not invalidated.
   void splice(const_iterator p, slist& x, const_iterator first, const_iterator last) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of this
   //!   list. Iterators of this list and all the references are not invalidated.
   void splice(const_iterator p, AUTOBOOST_RV_REF(slist) x, const_iterator first, const_iterator last) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   void priv_push_front (AUTOBOOST_RV_REF(T) x)
   {  this->insert_after(this->cbefore_begin(), ::autoboost::move(x));  }
   ...
   {  return this->insert_after(previous(p), ::autoboost::forward<U>(x)); }

   template<class U>
   iterator priv_insert_after(const_iterator prev_p, AUTOBOOST_FWD_REF(U) x)
   {  return iterator(this->icont().insert_after(prev_p.get(), *this->create_node(::autoboost::forward<U>(x)))); }
...
template <class T, class Allocator>
struct has_trivial_destructor_after_move<autoboost::container::slist<T, Allocator> >
{
   typedef typename ::autoboost::container::allocator_traits<Allocator>::pointer pointer;
...
AUTOBOOST_CONTAINER_DOC1ST(namespace std {, AUTOBOOST_MOVE_STD_NS_BEG)

//! A specialization of insert_iterator
//! that works with slist
template <class T, class Allocator>
class insert_iterator<autoboost::container::slist<T, Allocator> >
{
   private:
   typedef autoboost::container::slist<T, Allocator> Container;
   Container* container;
