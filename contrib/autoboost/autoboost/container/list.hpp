...
//
//////////////////////////////////////////////////////////////////////////////
#ifndef AUTOBOOST_CONTAINER_LIST_HPP
#define AUTOBOOST_CONTAINER_LIST_HPP
...
// container/detail
#include <autoboost/container/detail/algorithm.hpp>
...
#include <autoboost/intrusive/list.hpp>
// other
      ...
      <Allocator, typename container_detail::intrusive_list_type<Allocator>::type>
{
   ...
   AUTOBOOST_COPYABLE_AND_MOVABLE(list)
   ...
   list() AUTOBOOST_NOEXCEPT_IF(container_detail::is_nothrow_default_constructible<Allocator>::value)
      : AllocHolder()
   {}
   ...
   //!
   //! <b>Complexity</b>: Constant.
   explicit list(const allocator_type &a) AUTOBOOST_NOEXCEPT_OR_NOTHROW
      : AllocHolder(a)
   {}
   ...
   //!
   //! <b>Complexity</b>: Constant.
   list(AUTOBOOST_RV_REF(list) x) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   list(AUTOBOOST_RV_REF(list) x, const allocator_type &a)
      : AllocHolder(a)
   {
      if(this->node_alloc() == x.node_alloc()){
         ...
         this->insert(this->cbegin(), autoboost::make_move_iterator(x.begin()), autoboost::make_move_iterator(x.end()));
      }
   }
   ...
   ~list() AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   list& operator=(AUTOBOOST_COPY_ASSIGN_REF(list) x)
   {
      if (&x != this){
   ...
   list& operator=(AUTOBOOST_RV_REF(list) x)
   ...
   {  return *this->emplace(this->cend(), autoboost::forward<Args>(args)...); }
   ...
   {  return *this->emplace(this->cbegin(), autoboost::forward<Args>(args)...);  }
   ...
   {
      AUTOBOOST_ASSERT((priv_is_linked)(position));
   ...
   #define AUTOBOOST_CONTAINER_LIST_EMPLACE_CODE(N) \
   AUTOBOOST_MOVE_TMPL_LT##N AUTOBOOST_MOVE_CLASS##N AUTOBOOST_MOVE_GT##N \
   reference emplace_back(AUTOBOOST_MOVE_UREF##N)\
   {  return *this->emplace(this->cend() AUTOBOOST_MOVE_I##N AUTOBOOST_MOVE_FWD##N);  }\
   \
   AUTOBOOST_MOVE_TMPL_LT##N AUTOBOOST_MOVE_CLASS##N AUTOBOOST_MOVE_GT##N \
   reference emplace_front(AUTOBOOST_MOVE_UREF##N)\
   {  return *this->emplace(this->cbegin() AUTOBOOST_MOVE_I##N AUTOBOOST_MOVE_FWD##N);}\
   \
   AUTOBOOST_MOVE_TMPL_LT##N AUTOBOOST_MOVE_CLASS##N AUTOBOOST_MOVE_GT##N \
   iterator emplace(const_iterator position AUTOBOOST_MOVE_I##N AUTOBOOST_MOVE_UREF##N)\
   {\
      AUTOBOOST_ASSERT(position == this->cend() || (--(++position) == position) );\
      NodePtr pnode (AllocHolder::create_node(AUTOBOOST_MOVE_FWD##N));\
      return iterator(this->icont().insert(position.get(), *pnode));\
   }\
   //
   AUTOBOOST_MOVE_ITERATE_0TO9(AUTOBOOST_CONTAINER_LIST_EMPLACE_CODE)
   #undef AUTOBOOST_CONTAINER_LIST_EMPLACE_CODE
   ...
   {
      AUTOBOOST_ASSERT((priv_is_linked)(p));
   ...
   {
      AUTOBOOST_ASSERT(p != this->cend() && (priv_is_linked)(p));
   ...
   void splice(const_iterator p, list& x) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
      AUTOBOOST_ASSERT((priv_is_linked)(p));
      AUTOBOOST_ASSERT(this != &x);
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of
   //!    this list. Iterators of this list and all the references are not invalidated.
   void splice(const_iterator p, AUTOBOOST_RV_REF(list) x) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
   ...
   void splice(const_iterator p, list &x, const_iterator i) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
      AUTOBOOST_ASSERT((priv_is_linked)(p));
      AUTOBOOST_ASSERT(this->node_alloc() == x.node_alloc());
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of this
   //!   list. Iterators of this list and all the references are not invalidated.
   void splice(const_iterator p, AUTOBOOST_RV_REF(list) x, const_iterator i) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
      AUTOBOOST_ASSERT(this != &x);
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of this
   //!   list. Iterators of this list and all the references are not invalidated.
   void splice(const_iterator p, list &x, const_iterator first, const_iterator last) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
      AUTOBOOST_ASSERT((priv_is_linked)(p));
      AUTOBOOST_ASSERT(first == last || (first != x.cend() && x.priv_is_linked(first)));
      AUTOBOOST_ASSERT(first == last || x.priv_is_linked(last));
      AUTOBOOST_ASSERT(this->node_alloc() == x.node_alloc());
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of this
   //!   list. Iterators of this list and all the references are not invalidated.
   void splice(const_iterator p, AUTOBOOST_RV_REF(list) x, const_iterator first, const_iterator last) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
      AUTOBOOST_ASSERT(this != &x);
      //Additional checks done in splice()
   ...
   //!
   //! <b>Note</b>: Non-standard extension
   void splice(const_iterator p, list &x, const_iterator first, const_iterator last, size_type n) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
      AUTOBOOST_ASSERT(this->node_alloc() == x.node_alloc());
   ...
   //!
   //! <b>Note</b>: Non-standard extension
   void splice(const_iterator p, AUTOBOOST_RV_REF(list) x, const_iterator first, const_iterator last, size_type n) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   //! <b>Complexity</b>: This function is linear time: it performs at most
   //!   size() + x.size() - 1 comparisons.
   void merge(AUTOBOOST_RV_REF(list) x)
   ...
   //! <b>Note</b>: Iterators and references to *this are not invalidated.
   template <class StrictWeakOrdering>
   void merge(AUTOBOOST_RV_REF(list) x, StrictWeakOrdering comp)
   ...
   {
      AUTOBOOST_ASSERT((priv_is_linked)(p));
      NodePtr tmp = AllocHolder::create_node(autoboost::move(x));
      return iterator(this->icont().insert(p.get(), *tmp));
   }
   ...
   void priv_push_back (AUTOBOOST_RV_REF(T) x)
   {  this->insert(this->cend(), autoboost::move(x));    }

   void priv_push_front (const T &x)
   {  this->insert(this->cbegin(), x);  }

   void priv_push_front (AUTOBOOST_RV_REF(T) x)
   {  this->insert(this->cbegin(), autoboost::move(x));  }
...
template <class T, class Allocator>
struct has_trivial_destructor_after_move<autoboost::container::list<T, Allocator> >
{
   typedef typename ::autoboost::container::allocator_traits<Allocator>::pointer pointer;
...
#endif // AUTOBOOST_CONTAINER_LIST_HPP
