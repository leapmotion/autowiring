...
#ifndef AUTOBOOST_CONTAINER_ALLOCATOR_HPP
#define AUTOBOOST_CONTAINER_ALLOCATOR_HPP
...
template< class T
        , unsigned Version AUTOBOOST_CONTAINER_DOCONLY(=2)
        , unsigned int AllocationDisableMask AUTOBOOST_CONTAINER_DOCONLY(=0)>
class allocator
{
   ...
   static const unsigned int ForbiddenMask =
      AUTOBOOST_CONTAINER_ALLOCATE_NEW | AUTOBOOST_CONTAINER_EXPAND_BWD | AUTOBOOST_CONTAINER_EXPAND_FWD ;

   //The mask can't disable all the allocation types
   AUTOBOOST_STATIC_ASSERT((  (AllocationDisableMask & ForbiddenMask) != ForbiddenMask  ));

   //The mask is only valid for version 2 allocators
   AUTOBOOST_STATIC_ASSERT((  Version != 1 || (AllocationDisableMask == 0)  ));
   ...
   //!Default constructor
   //!Never throws
   allocator() AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {}

   //!Constructor from other allocator.
   //!Never throws
   allocator(const allocator &) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {}
   ...
   //!Never throws
   void deallocate(pointer ptr, size_type) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {  dlmalloc_free(ptr);  }
   ...
   friend bool operator==(const allocator &, const allocator &) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {  return true;   }
                         ...
                         pointer &reuse)
   {
      ...
      if(!ret && !(command & AUTOBOOST_CONTAINER_NOTHROW_ALLOCATION))
         autoboost::container::throw_bad_alloc();
      return ret;
   }
...
#endif   //AUTOBOOST_CONTAINER_ALLOCATOR_HPP
