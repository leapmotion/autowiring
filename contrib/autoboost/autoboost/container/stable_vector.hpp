...
#ifndef AUTOBOOST_CONTAINER_STABLE_VECTOR_HPP
#define AUTOBOOST_CONTAINER_STABLE_VECTOR_HPP
      ...
      >
{
   ...
   public:
   typename ::autoboost::intrusive::pointer_traits<Pointer>::element_type value;
};
...
struct index_traits
{
   ...
   typedef ::autoboost::container::vector
...
{
   typedef autoboost::intrusive::pointer_traits<Pointer>                                non_const_ptr_traits;
   public:
   ...
   typedef typename ::autoboost::container::container_detail::if_c
      < IsConst
      , typename non_const_ptr_traits::template
         rebind_pointer<const value_type>::type
      , Pointer
      >::type                                                                       pointer;
   typedef autoboost::intrusive::pointer_traits<pointer>                                ptr_traits;
   ...
   explicit stable_vector_iterator(node_base_ptr p) AUTOBOOST_NOEXCEPT_OR_NOTHROW
      : m_pn(p)
   {}

   stable_vector_iterator() AUTOBOOST_NOEXCEPT_OR_NOTHROW
      : m_pn() //Value initialization to achieve "null iterators" (N3644)
   {}

   stable_vector_iterator(stable_vector_iterator<Pointer, false> const& other) AUTOBOOST_NOEXCEPT_OR_NOTHROW
      :  m_pn(other.node_pointer())
   {}

   node_ptr node_pointer() const AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   //Increment / Decrement
   stable_vector_iterator& operator++() AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
   ...
   stable_vector_iterator operator++(int) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   stable_vector_iterator& operator+=(difference_type off) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
      if(off) this->m_pn = this->m_pn->up[off];
      return *this;
   }

   friend stable_vector_iterator operator+(const stable_vector_iterator &left, difference_type off) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
   ...
   friend stable_vector_iterator operator+(difference_type off, const stable_vector_iterator& right) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
   ...
   friend difference_type operator-(const stable_vector_iterator &left, const stable_vector_iterator &right) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {  return left.m_pn->up - right.m_pn->up;  }

   //Comparison operators
   friend bool operator==   (const stable_vector_iterator& l, const stable_vector_iterator& r) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   {  return l.m_pn->up >= r.m_pn->up;  }
      ...
      #define STABLE_VECTOR_CHECK_INVARIANT \
               invariant_checker AUTOBOOST_JOIN(check_invariant_,__LINE__)(*this); \
...
class stable_vector
{
   ...
   typedef ::autoboost::container::container_detail::integral_constant
      <unsigned, autoboost::container::container_detail::
   ...
   #ifndef AUTOBOOST_CONTAINER_DOXYGEN_INVOKED
   private:
   AUTOBOOST_COPYABLE_AND_MOVABLE(stable_vector)
   ...
   stable_vector() AUTOBOOST_NOEXCEPT_IF(container_detail::is_nothrow_default_constructible<Allocator>::value)
      : internal_data(), index()
   {
   ...
   //!
   //! <b>Complexity</b>: Constant.
   explicit stable_vector(const allocator_type& al) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   //!
   //! <b>Complexity</b>: Constant.
   stable_vector(AUTOBOOST_RV_REF(stable_vector) x) AUTOBOOST_NOEXCEPT_OR_NOTHROW
      : internal_data(autoboost::move(x.priv_node_alloc())), index(autoboost::move(x.index))
   {
   ...
   stable_vector(AUTOBOOST_RV_REF(stable_vector) x, const allocator_type &a)
      : internal_data(a), index(a)
   {
      if(this->priv_node_alloc() == x.priv_node_alloc()){
         ...
         stable_vector_detail::clear_on_destroy<stable_vector> cod(*this);
         this->insert(this->cend(), autoboost::make_move_iterator(x.begin()), autoboost::make_move_iterator(x.end()));
         STABLE_VECTOR_CHECK_INVARIANT;
         cod.release();
      }
   }
   ...
   stable_vector& operator=(AUTOBOOST_COPY_ASSIGN_REF(stable_vector) x)
   {
      STABLE_VECTOR_CHECK_INVARIANT;
      if (&x != this){
   ...
   stable_vector& operator=(AUTOBOOST_RV_REF(stable_vector) x)
   ...
   {
      const size_type index_size             = this->index.size();
      AUTOBOOST_ASSERT(!index_size || index_size >= ExtraPointers);
      ...
      AUTOBOOST_ASSERT(node_extra_capacity <= (this->index.capacity()- index_size));
   ...
   reference emplace_back(Args &&...args)
   {
      ...
      EmplaceFunctor &&ef = EmplaceFunctor(autoboost::forward<Args>(args)...);
   ...
   #define AUTOBOOST_CONTAINER_STABLE_VECTOR_EMPLACE_CODE(N) \
   ...
   {\
      typedef emplace_functor##N\
         AUTOBOOST_MOVE_LT##N AUTOBOOST_MOVE_TARG##N AUTOBOOST_MOVE_GT##N EmplaceFunctor;\
      typedef emplace_iterator<value_type, EmplaceFunctor, difference_type>  EmplaceIterator;\
      EmplaceFunctor ef AUTOBOOST_MOVE_LP##N AUTOBOOST_MOVE_FWD##N AUTOBOOST_MOVE_RP##N;\
      return *this->insert(this->cend() , EmplaceIterator(ef), EmplaceIterator());\
   }\
   \
   AUTOBOOST_MOVE_TMPL_LT##N AUTOBOOST_MOVE_CLASS##N AUTOBOOST_MOVE_GT##N \
   ...
   //
   AUTOBOOST_MOVE_ITERATE_0TO9(AUTOBOOST_CONTAINER_STABLE_VECTOR_EMPLACE_CODE)
   #undef AUTOBOOST_CONTAINER_STABLE_VECTOR_EMPLACE_CODE
   ...
   {
      AUTOBOOST_ASSERT(this->priv_in_range_or_end(p));
      const size_type num_new = static_cast<size_type>(autoboost::container::iterator_distance(first, last));
   ...
   {
      AUTOBOOST_ASSERT(this->priv_in_range(p));
      STABLE_VECTOR_CHECK_INVARIANT;
   ...
   void swap(stable_vector & x)
      AUTOBOOST_NOEXCEPT_IF( allocator_traits_type::propagate_on_container_swap::value
                                || allocator_traits_type::is_always_equal::value)
   {
      AUTOBOOST_ASSERT(allocator_traits_type::propagate_on_container_swap::value ||
                   allocator_traits_type::is_always_equal::value ||
   ...
   {
      //Check range
      AUTOBOOST_ASSERT(this->index.empty() || (this->index.data() <= p->up));
      AUTOBOOST_ASSERT(this->index.empty() || p->up <= (this->index.data() + this->index.size()));
   ...
   template <class U>
   void priv_push_back(AUTOBOOST_MOVE_CATCH_FWD(U) x)
   {
      if(AUTOBOOST_LIKELY(this->priv_capacity_bigger_than_size())){
         ...
         AUTOBOOST_ASSERT(!!p);
         {
            ...
            //This might throw
            this->priv_build_node_from_convertible(p, ::autoboost::forward<U>(x));
            rollback.release();
         }
         ...
         this->insert(this->cend(), ::autoboost::forward<U>(x));
      }
   }
   ...
   iterator priv_insert(const_iterator p, AUTOBOOST_RV_REF(T) x)
   {
      AUTOBOOST_ASSERT(this->priv_in_range_or_end(p));
      typedef repeat_iterator<T, difference_type>  repeat_it;
      typedef autoboost::move_iterator<repeat_it>      repeat_move_it;
   ...
   node_ptr priv_get_from_pool()
   {
      ...
      AUTOBOOST_ASSERT(!this->index.empty());
      node_base_ptr &pool_first_ref = *(this->index.end() - (ExtraPointers-1));
      node_base_ptr &pool_last_ref  = this->index.back();
      multiallocation_chain holder;
      holder.incorporate_after( holder.before_begin()
   ...
   {
      //This can throw
      autoboost::container::construct_in_place
         ( this->priv_node_alloc()
         , container_detail::addressof(p->value)
         , it);
      //This does not throw
      ::new(static_cast<node_base_type*>(container_detail::to_raw_pointer(p)), autoboost_container_new_t())
         node_base_type(index_traits_type::ptr_to_node_base_ptr(*up_index));
   }

   template<class ValueConvertible>
   void priv_build_node_from_convertible(const node_ptr &p, AUTOBOOST_FWD_REF(ValueConvertible) value_convertible)
   {
      //This can throw
      autoboost::container::allocator_traits<node_allocator_type>::construct
         ( this->priv_node_alloc()
         , container_detail::addressof(p->value)
         , ::autoboost::forward<ValueConvertible>(value_convertible));
      //This does not throw
      ::new(static_cast<node_base_type*>(container_detail::to_raw_pointer(p)), autoboost_container_new_t()) node_base_type;
   }

   void priv_swap_members(stable_vector &x)
   {
      autoboost::adl_move_swap(this->internal_data.pool_size, x.internal_data.pool_size);
   ...
   class invariant_checker
   {
      ...
      ~invariant_checker(){AUTOBOOST_ASSERT(p->priv_invariant());}
      void touch(){}
   };
   #endif
   ...
   {
      private:
      AUTOBOOST_MOVABLE_BUT_NOT_COPYABLE(ebo_holder)

      public:
      template<class AllocatorRLValue>
      explicit ebo_holder(AUTOBOOST_FWD_REF(AllocatorRLValue) a)
         : node_allocator_type(autoboost::forward<AllocatorRLValue>(a))
         , pool_size(0)
         , end_node()
      {}
...
template <class T, class Allocator>
struct has_trivial_destructor_after_move<autoboost::container::stable_vector<T, Allocator> >
{
   typedef typename ::autoboost::container::allocator_traits<Allocator>::pointer pointer;
...
#endif   //AUTOBOOST_CONTAINER_STABLE_VECTOR_HPP
