...
#ifndef AUTOBOOST_CONTAINER_DETAIL_ADAPTIVE_NODE_POOL_IMPL_HPP
#define AUTOBOOST_CONTAINER_DETAIL_ADAPTIVE_NODE_POOL_IMPL_HPP
...
#include <autoboost/container/detail/math_functions.hpp>
...
class private_adaptive_node_pool_impl
{
   ...
   void *allocate_node()
   {
      ...
      if(!m_block_container.empty()){
         ...
         AUTOBOOST_ASSERT(!free_nodes.empty());
   ...
   void deallocate_node(void *pElem)
   {
      ...
      AUTOBOOST_ASSERT(block_info.free_nodes.size() < m_real_num_node);
      ...
      AUTOBOOST_TRY{
         ...
         while(i != n){
            ...
            if (m_block_container.empty()){
               ...
               AUTOBOOST_ASSERT(m_block_container.empty() || (++m_block_container.cbegin() == m_block_container.cend()));
               AUTOBOOST_ASSERT(chain.size() == n);
               break;
            }
            free_nodes_t &free_nodes = m_block_container.begin()->free_nodes;
   ...
   {
      AUTOBOOST_ASSERT(m_max_free_blocks == other.m_max_free_blocks);
      AUTOBOOST_ASSERT(m_real_node_size == other.m_real_node_size);
      AUTOBOOST_ASSERT(m_real_block_alignment == other.m_real_block_alignment);
      AUTOBOOST_ASSERT(m_real_num_node == other.m_real_num_node);
   ...
   void priv_deallocate_free_blocks_impl(size_type max_free_blocks)
   {
      ...
      multiallocation_chain chain;
      {
         ...
         for( ; totally_free_blocks > max_free_blocks; --totally_free_blocks){
            AUTOBOOST_ASSERT(it->free_nodes.size() == m_real_num_node);
         ...
         AUTOBOOST_ASSERT((m_totally_free_blocks - max_free_blocks) == chain.size());
   ...
   class block_destroyer
   {
      ...
      {
         AUTOBOOST_ASSERT(to_deallocate->free_nodes.size() == mp_impl->m_real_num_node);
      ...
      {
         AUTOBOOST_ASSERT(to_deallocate->free_nodes.size() == mp_impl->m_real_num_node);
         AUTOBOOST_ASSERT(0 == to_deallocate->hdr_offset);
   ...
   //#define AUTOBOOST_CONTAINER_ADAPTIVE_NODE_POOL_CHECK_INVARIANTS
   void priv_invariants()
   #ifdef AUTOBOOST_CONTAINER_ADAPTIVE_NODE_POOL_CHECK_INVARIANTS
   #undef AUTOBOOST_CONTAINER_ADAPTIVE_NODE_POOL_CHECK_INVARIANTS
   {
      ...
      {  //Check that the total free nodes are correct
         ...
         AUTOBOOST_ASSERT(total_free_nodes >= m_totally_free_blocks*m_real_num_node);
      }
      {  //Check that the total totally free blocks are correct
         AUTOBOOST_ASSERT(m_block_container.size() >= m_totally_free_blocks);
      ...
      if(!AlignOnly){
         ...
         for(; it != itend; ++it){
            ...
            for(size_type i = 0, max = m_num_subblocks; i < max; ++i){
               ...
               AUTOBOOST_ASSERT(hdr_off_holder->hdr_offset == offset);
               AUTOBOOST_ASSERT(0 == ((size_type)hdr_off_holder & (m_real_block_alignment - 1)));
               AUTOBOOST_ASSERT(0 == (hdr_off_holder->hdr_offset & (m_real_block_alignment - 1)));
   ...
   void priv_clear()
   {
      ...
      AUTOBOOST_ASSERT(n_free_nodes == m_totally_free_blocks);
      #endif
   ...
   block_info_t *priv_block_from_node(void *node, AlignOnlyFalse) const
   {
      ...
      AUTOBOOST_ASSERT(0 == ((std::size_t)hdr_off_holder & (m_real_block_alignment - 1)));
      AUTOBOOST_ASSERT(0 == (hdr_off_holder->hdr_offset & (m_real_block_alignment - 1)));
      block_info_t *block = reinterpret_cast<block_info_t *>
         (reinterpret_cast<char*>(hdr_off_holder) + hdr_off_holder->hdr_offset);
      AUTOBOOST_ASSERT(block->hdr_offset == 0);
      return block;
   }
   ...
   hdr_offset_holder *priv_first_subblock_from_block(block_info_t *block, AlignOnlyFalse) const
   {
      ...
      AUTOBOOST_ASSERT(hdr_off_holder->hdr_offset == size_type(reinterpret_cast<char*>(block) - reinterpret_cast<char*>(hdr_off_holder)));
      AUTOBOOST_ASSERT(0 == ((std::size_t)hdr_off_holder & (m_real_block_alignment - 1)));
      AUTOBOOST_ASSERT(0 == (hdr_off_holder->hdr_offset & (m_real_block_alignment - 1)));
   ...
   {
      AUTOBOOST_ASSERT(chain.size() <= total_elements);
   ...
   {
      AUTOBOOST_ASSERT(m_block_container.empty());
      AUTOBOOST_ASSERT(min_elements > 0);
   ...
   void priv_append_from_new_blocks(size_type min_elements, multiallocation_chain &chain, AlignOnlyFalse)
   {
      ...
      for(size_type i = 0; i != n; ++i){
         ...
         AUTOBOOST_ASSERT(static_cast<void*>(&static_cast<hdr_offset_holder&>(c_info).hdr_offset) ==
...
#endif   //#ifndef AUTOBOOST_CONTAINER_DETAIL_ADAPTIVE_NODE_POOL_IMPL_HPP
