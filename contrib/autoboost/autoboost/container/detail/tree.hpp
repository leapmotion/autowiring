      ...
      >
{
   ...
   template<class MovableConvertible>
   iterator insert_unique(const_iterator hint, AUTOBOOST_FWD_REF(MovableConvertible) v)
   {
      AUTOBOOST_ASSERT((priv_is_linked)(hint));
      insert_commit_data data;
      std::pair<iterator,bool> ret =
   ...
   template<class MovableConvertible>
   iterator insert_equal(const_iterator hint, AUTOBOOST_FWD_REF(MovableConvertible) v)
   {
      AUTOBOOST_ASSERT((priv_is_linked)(hint));
      NodePtr tmp(AllocHolder::create_node(autoboost::forward<MovableConvertible>(v)));
      scoped_destroy_deallocator<NodeAlloc> destroy_deallocator(tmp, this->node_alloc());
