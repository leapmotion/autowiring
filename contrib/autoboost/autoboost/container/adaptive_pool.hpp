...
#ifndef AUTOBOOST_CONTAINER_ADAPTIVE_POOL_HPP
#define AUTOBOOST_CONTAINER_ADAPTIVE_POOL_HPP
...
#include <autoboost/container/detail/adaptive_node_pool.hpp>
...
template < class T
         , std::size_t NodesPerBlock   AUTOBOOST_CONTAINER_DOCONLY(= ADP_nodes_per_block)
         , std::size_t MaxFreeBlocks   AUTOBOOST_CONTAINER_DOCONLY(= ADP_max_free_blocks)
         , std::size_t OverheadPercent AUTOBOOST_CONTAINER_DOCONLY(= ADP_overhead_percent)
         AUTOBOOST_CONTAINER_DOCIGN(AUTOBOOST_MOVE_I unsigned Version)
         >
class adaptive_pool
{
   ...
   typedef adaptive_pool
      <T, NodesPerBlock, MaxFreeBlocks, OverheadPercent
         AUTOBOOST_CONTAINER_DOCIGN(AUTOBOOST_MOVE_I Version)
         >   self_t;
   ...
   AUTOBOOST_CONTAINER_DOCIGN(AUTOBOOST_STATIC_ASSERT((Version <=2)));
   ...
   public:
   //!Default constructor
   adaptive_pool() AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {}

   //!Copy constructor from other adaptive_pool.
   adaptive_pool(const adaptive_pool &) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {}
   ...
   adaptive_pool
      (const adaptive_pool<T2, NodesPerBlock, MaxFreeBlocks, OverheadPercent
            AUTOBOOST_CONTAINER_DOCIGN(AUTOBOOST_MOVE_I Version)> &) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {}
   ...
   void allocate_many(size_type elem_size, std::size_t n_elements, multiallocation_chain &chain)
   {
      ...
      chain.incorporate_after(chain.before_begin()
                             ,(T*)AUTOBOOST_CONTAINER_MEMCHAIN_FIRSTMEM(&ch)
                             ,(T*)AUTOBOOST_CONTAINER_MEMCHAIN_LASTMEM(&ch)
                             ,AUTOBOOST_CONTAINER_MEMCHAIN_SIZE(&ch) );*/
      if(AUTOBOOST_UNLIKELY(!dlmalloc_multialloc_nodes
   ...
   void allocate_many(const size_type *elem_sizes, size_type n_elements, multiallocation_chain &chain)
   {
      ...
      dlmalloc_memchain ch;
      AUTOBOOST_CONTAINER_MEMCHAIN_INIT(&ch);
      if(AUTOBOOST_UNLIKELY(!dlmalloc_multialloc_arrays(n_elements, elem_sizes, sizeof(T), DL_MULTIALLOC_DEFAULT_CONTIGUOUS, &ch))){
         autoboost::container::throw_bad_alloc();
      }
      chain.incorporate_after(chain.before_begin()
                             ,(T*)AUTOBOOST_CONTAINER_MEMCHAIN_FIRSTMEM(&ch)
                             ,(T*)AUTOBOOST_CONTAINER_MEMCHAIN_LASTMEM(&ch)
                             ,AUTOBOOST_CONTAINER_MEMCHAIN_SIZE(&ch) );*/
      if(AUTOBOOST_UNLIKELY(!dlmalloc_multialloc_arrays
   ...
   {/*
      ...
      AUTOBOOST_CONTAINER_MEMCHAIN_INIT_FROM(&ch, beg, last, size);
   ...
   friend void swap(adaptive_pool &, adaptive_pool &) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {}

   //!An allocator always compares to true, as memory allocated with one
   //!instance can be deallocated by another instance
   friend bool operator==(const adaptive_pool &, const adaptive_pool &) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {  return true;   }
      ...
      ,size_type &prefer_in_recvd_out_size, pointer &reuse_ptr)
   {
      ...
      if(AUTOBOOST_UNLIKELY(limit_size > this->max_size() || preferred_size > this->max_size())){
         return pointer();
      }
...
#endif   //#ifndef AUTOBOOST_CONTAINER_ADAPTIVE_POOL_HPP
