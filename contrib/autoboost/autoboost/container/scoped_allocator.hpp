...
#ifndef AUTOBOOST_CONTAINER_ALLOCATOR_SCOPED_ALLOCATOR_HPP
#define AUTOBOOST_CONTAINER_ALLOCATOR_SCOPED_ALLOCATOR_HPP
...
#include <autoboost/container/scoped_allocator_fwd.hpp>
...
//! (when AUTOBOOST_NO_SFINAE_EXPR is NOT defined), this trait is ignored and C++11 rules will be used
...
struct uses_allocator
   : autoboost::integral_constant<bool, container_detail::uses_allocator_imp<T, Alloc>::value>
{};
...
struct is_scoped_allocator
   : autoboost::integral_constant<bool, container_detail::is_scoped_allocator_imp<Alloc>::value>
{};
...
// Check if we can detect is_convertible using advanced SFINAE expressions
   ...
   struct is_constructible_impl
   {
      ...
      template<class X>
      static yes_type test(dummy<sizeof(X(autoboost::move_detail::declval<Args>()...))>*);
   ...
   struct is_constructible
      : autoboost::integral_constant<bool, is_constructible_impl<T, Args...>::value>
   {};
   ...
   #define AUTOBOOST_CONTAINER_MAX_IS_CONSTRUCTIBLE_PARAMETERS \
      AUTOBOOST_PP_ADD(AUTOBOOST_CONTAINER_MAX_CONSTRUCTOR_PARAMETERS, 3)
   //!

   //Generate N+1 template parameters so that we can specialize N
   template<class T
            AUTOBOOST_PP_ENUM_TRAILING( AUTOBOOST_PP_ADD(AUTOBOOST_CONTAINER_MAX_IS_CONSTRUCTIBLE_PARAMETERS, 1)
                                 , AUTOBOOST_CONTAINER_PP_TEMPLATE_PARAM_WITH_DEFAULT
                                 , void)
         >
   struct is_constructible_impl;

   //Generate N specializations, from 0 to
   //AUTOBOOST_CONTAINER_MAX_IS_CONSTRUCTIBLE_PARAMETERS parameters
   #define AUTOBOOST_PP_LOCAL_MACRO(n)                                                                 \
   template<class T AUTOBOOST_PP_ENUM_TRAILING_PARAMS(n, class P)>                                     \
   struct is_constructible_impl                                                                    \
      <T AUTOBOOST_PP_ENUM_TRAILING_PARAMS(n, P)                                                       \
         AUTOBOOST_PP_ENUM_TRAILING                                                                    \
            ( AUTOBOOST_PP_SUB(AUTOBOOST_CONTAINER_MAX_IS_CONSTRUCTIBLE_PARAMETERS, n)                     \
            , AUTOBOOST_CONTAINER_PP_IDENTITY, void)                                                   \
      , void>                                                                                      \
   {                                                                                               \
      typedef char yes_type;                                                                       \
      struct no_type                                                                               \
      ...
      struct dummy;                                                                                \
                                                                                                   \
      template<class X>                                                                            \
      static yes_type test(dummy<sizeof(X(AUTOBOOST_PP_ENUM(n, AUTOBOOST_CONTAINER_PP_DECLVAL, ~)))>*);    \
                                                                                                   \
      template<class X>                                                                            \
      static no_type test(...);                                                                    \
                                                                                                   \
   ...
   #define AUTOBOOST_PP_LOCAL_LIMITS (0, AUTOBOOST_CONTAINER_MAX_IS_CONSTRUCTIBLE_PARAMETERS)
   #include AUTOBOOST_PP_LOCAL_ITERATE()

   //Finally just inherit from the implementation to define he trait
   template< class T
           AUTOBOOST_PP_ENUM_TRAILING( AUTOBOOST_CONTAINER_MAX_IS_CONSTRUCTIBLE_PARAMETERS
                                 , AUTOBOOST_CONTAINER_PP_TEMPLATE_PARAM_WITH_DEFAULT
                                 , void)
           >
   struct is_constructible
      : autoboost::integral_constant
         < bool
         , is_constructible_impl
            < T
            AUTOBOOST_PP_ENUM_TRAILING_PARAMS(AUTOBOOST_CONTAINER_MAX_IS_CONSTRUCTIBLE_PARAMETERS, P)
            , void>::value
         >
   {};
   ...
   template <class T
            ,class InnerAlloc
            AUTOBOOST_PP_ENUM_TRAILING( AUTOBOOST_PP_SUB(AUTOBOOST_CONTAINER_MAX_IS_CONSTRUCTIBLE_PARAMETERS, 2)
                                 , AUTOBOOST_CONTAINER_PP_TEMPLATE_PARAM_WITH_DEFAULT
                                 , void)
            >
   struct is_constructible_with_allocator_prefix
      : is_constructible
         ...
         AUTOBOOST_PP_ENUM_TRAILING_PARAMS(AUTOBOOST_PP_SUB(AUTOBOOST_CONTAINER_MAX_IS_CONSTRUCTIBLE_PARAMETERS, 2), P)
         >
   {};
/*
   template <class T
            ,class InnerAlloc
            AUTOBOOST_PP_ENUM_TRAILING( AUTOBOOST_PP_SUB(AUTOBOOST_CONTAINER_MAX_IS_CONSTRUCTIBLE_PARAMETERS, 1)
                                 , AUTOBOOST_CONTAINER_PP_TEMPLATE_PARAM_WITH_DEFAULT
                                 , void)
            >
   struct is_constructible_with_allocator_suffix
      : is_constructible
         < T
         AUTOBOOST_PP_ENUM_TRAILING_PARAMS(AUTOBOOST_PP_SUB(AUTOBOOST_CONTAINER_MAX_IS_CONSTRUCTIBLE_PARAMETERS, 1), P)
         , InnerAlloc
         >
   {};*/
...
#else    // #if !defined(AUTOBOOST_NO_SFINAE_EXPR)
...
inline void dispatch_allocator_prefix_suffix
   ( autoboost::true_type  use_alloc_prefix, OutermostAlloc& outermost_alloc
   , InnerAlloc& inner_alloc, T* p, AUTOBOOST_FWD_REF(Args) ...args)
{
   (void)use_alloc_prefix;
   allocator_traits<OutermostAlloc>::construct
      ( outermost_alloc, p, allocator_arg, inner_alloc, ::autoboost::forward<Args>(args)...);
}
...
template < typename OutermostAlloc
         ...
         >
inline void dispatch_allocator_prefix_suffix
   ( autoboost::false_type use_alloc_prefix, OutermostAlloc& outermost_alloc
   , InnerAlloc &inner_alloc, T* p, AUTOBOOST_FWD_REF(Args)...args)
{
   (void)use_alloc_prefix;
   allocator_traits<OutermostAlloc>::construct
      (outermost_alloc, p, ::autoboost::forward<Args>(args)..., inner_alloc);
}
...
inline void dispatch_uses_allocator
   ( autoboost::true_type uses_allocator, OutermostAlloc& outermost_alloc
   , InnerAlloc& inner_alloc, T* p, AUTOBOOST_FWD_REF(Args)...args)
{
   (void)uses_allocator;
   //AUTOBOOST_STATIC_ASSERT((is_constructible_with_allocator_prefix<T, InnerAlloc, Args...>::value ||
   ...
   dispatch_allocator_prefix_suffix
      ...
      , outermost_alloc, inner_alloc, p, ::autoboost::forward<Args>(args)...);
}
...
template < typename OutermostAlloc
         ...
         >
inline void dispatch_uses_allocator
   ( autoboost::false_type uses_allocator, OutermostAlloc & outermost_alloc
   , InnerAlloc & inner_alloc
   ,T* p, AUTOBOOST_FWD_REF(Args)...args)
{
   (void)uses_allocator; (void)inner_alloc;
   allocator_traits<OutermostAlloc>::construct
      (outermost_alloc, p, ::autoboost::forward<Args>(args)...);
}
...
template < typename OutermostAlloc                                                           \
         , typename InnerAlloc                                                               \
         , typename T                                                                        \
         AUTOBOOST_PP_ENUM_TRAILING_PARAMS(n, class P)                                           \
         >                                                                                   \
inline void dispatch_allocator_prefix_suffix(                                                \
                                       autoboost::true_type  use_alloc_prefix,                   \
...
template < typename OutermostAlloc                                                           \
         ...
         >                                                                                   \
inline void dispatch_allocator_prefix_suffix(                                                \
                           autoboost::false_type use_alloc_prefix,                               \
                           OutermostAlloc& outermost_alloc,                                  \
                           InnerAlloc&    inner_alloc,                                       \
                           T* p AUTOBOOST_PP_ENUM_TRAILING(n, AUTOBOOST_CONTAINER_PP_PARAM_LIST, _)) \
{                                                                                            \
   (void)use_alloc_prefix;                                                                   \
   allocator_traits<OutermostAlloc>::construct                                               \
      (outermost_alloc, p                                                                    \
...
template < typename OutermostAlloc                                                           \
         ...
         AUTOBOOST_PP_ENUM_TRAILING_PARAMS(n, class P)                                           \
         >                                                                                   \
inline void dispatch_uses_allocator(autoboost::true_type uses_allocator,                         \
                        OutermostAlloc& outermost_alloc,                                     \
                        InnerAlloc&    inner_alloc,                                          \
                        T* p AUTOBOOST_PP_ENUM_TRAILING(n, AUTOBOOST_CONTAINER_PP_PARAM_LIST, _))    \
{                                                                                            \
   ...
   dispatch_allocator_prefix_suffix                                                          \
      (is_constructible_with_allocator_prefix                                                \
         < T, InnerAlloc AUTOBOOST_PP_ENUM_TRAILING_PARAMS(n, P)>()                              \
...
template < typename OutermostAlloc                                                           \
         ...
         AUTOBOOST_PP_ENUM_TRAILING_PARAMS(n, class P)                                           \
         >                                                                                   \
inline void dispatch_uses_allocator(autoboost::false_type uses_allocator                         \
                        ,OutermostAlloc &    outermost_alloc                                 \
                        ,InnerAlloc &    inner_alloc                                         \
                        ,T* p AUTOBOOST_PP_ENUM_TRAILING(n, AUTOBOOST_CONTAINER_PP_PARAM_LIST, _))   \
{                                                                                            \
   (void)uses_allocator; (void)inner_alloc;                                                  \
   allocator_traits<OutermostAlloc>::construct                                               \
      (outermost_alloc, p AUTOBOOST_PP_ENUM_TRAILING(n, AUTOBOOST_CONTAINER_PP_PARAM_FORWARD, _));   \
}                                                                                            \
//!
   ...
   : public OuterAlloc
{
   ...
   AUTOBOOST_COPYABLE_AND_MOVABLE(scoped_allocator_adaptor_base)
   ...
   typedef autoboost::integral_constant<
      bool,
   ...
   template <class OuterA2>
   scoped_allocator_adaptor_base(AUTOBOOST_FWD_REF(OuterA2) outerAlloc, const InnerAllocs &...args)
      : outer_allocator_type(::autoboost::forward<OuterA2>(outerAlloc))
      , m_inner(args...)
      {}
   ...
   scoped_allocator_adaptor_base(AUTOBOOST_RV_REF(scoped_allocator_adaptor_base) other)
      : outer_allocator_type(::autoboost::move(other.outer_allocator()))
      , m_inner(::autoboost::move(other.inner_allocator()))
      {}
   ...
   template <class OuterA2>
   scoped_allocator_adaptor_base
      (AUTOBOOST_RV_REF_BEG scoped_allocator_adaptor_base
         <OuterA2, InnerAllocs...> AUTOBOOST_RV_REF_END other)
      : outer_allocator_type(other.outer_allocator())
      , m_inner(other.inner_allocator())
      {}
   ...
   scoped_allocator_adaptor_base
      ( internal_type_t
      , AUTOBOOST_FWD_REF(OuterA2) outerAlloc
   ...
   scoped_allocator_adaptor_base &operator=
      (AUTOBOOST_COPY_ASSIGN_REF(scoped_allocator_adaptor_base) other)
   {
   ...
   scoped_allocator_adaptor_base &operator=(AUTOBOOST_RV_REF(scoped_allocator_adaptor_base) other)
   {
      outer_allocator_type::operator=(autoboost::move(other.outer_allocator()));
      m_inner = ::autoboost::move(other.inner_allocator());
      return *this;
   }
   ...
   {
      autoboost::container::swap_dispatch(this->outer_allocator(), r.outer_allocator());
      autoboost::container::swap_dispatch(this->m_inner, r.inner_allocator());
   }

   friend void swap(scoped_allocator_adaptor_base &l, scoped_allocator_adaptor_base &r)
   {  l.swap(r);  }

   inner_allocator_type&       inner_allocator() AUTOBOOST_CONTAINER_NOEXCEPT
      { return m_inner; }

   inner_allocator_type const& inner_allocator() const AUTOBOOST_CONTAINER_NOEXCEPT
      { return m_inner; }

   outer_allocator_type      & outer_allocator() AUTOBOOST_CONTAINER_NOEXCEPT
      { return static_cast<outer_allocator_type&>(*this); }

   const outer_allocator_type &outer_allocator() const AUTOBOOST_CONTAINER_NOEXCEPT
...
template <
   typename OuterAlloc
   , bool Dummy
   AUTOBOOST_PP_ENUM_TRAILING_PARAMS(AUTOBOOST_CONTAINER_MAX_CONSTRUCTOR_PARAMETERS, class Q)
   >
class scoped_allocator_adaptor_base;
...
AUTOBOOST_PP_ENUM_TRAILING_PARAMS(n, class Q)                                                       \
>                                                                                               \
class scoped_allocator_adaptor_base<OuterAlloc, true                                            \
   AUTOBOOST_PP_ENUM_TRAILING_PARAMS(n, Q)                                                          \
   AUTOBOOST_PP_ENUM_TRAILING( AUTOBOOST_PP_SUB(AUTOBOOST_CONTAINER_MAX_CONSTRUCTOR_PARAMETERS, n)          \
                         , AUTOBOOST_CONTAINER_PP_IDENTITY, nat)                                    \
   >                                                                                            \
   : public OuterAlloc                                                                          \
{                                                                                               \
   typedef allocator_traits<OuterAlloc> outer_traits_type;                                      \
   ...
   struct rebind_base                                                                           \
   {                                                                                            \
      typedef scoped_allocator_adaptor_base<OuterA2, true AUTOBOOST_PP_ENUM_TRAILING_PARAMS(n, Q)   \
         AUTOBOOST_PP_ENUM_TRAILING                                                                 \
            ( AUTOBOOST_PP_SUB(AUTOBOOST_CONTAINER_MAX_CONSTRUCTOR_PARAMETERS, n)                       \
            , AUTOBOOST_CONTAINER_PP_IDENTITY, nat)                                                 \
         > other;                                                                               \
   };                                                                                           \
                                                                                                \
   typedef OuterAlloc outer_allocator_type;                                                     \
   typedef scoped_allocator_adaptor<AUTOBOOST_PP_ENUM_PARAMS(n, Q)                                  \
      AUTOBOOST_PP_ENUM_TRAILING                                                                    \
         ( AUTOBOOST_PP_SUB(AUTOBOOST_CONTAINER_MAX_CONSTRUCTOR_PARAMETERS, n)                          \
         , AUTOBOOST_CONTAINER_PP_IDENTITY, nat)                                                    \
      > inner_allocator_type;                                                                   \
   typedef scoped_allocator_adaptor<OuterAlloc, AUTOBOOST_PP_ENUM_PARAMS(n, Q)                      \
      AUTOBOOST_PP_ENUM_TRAILING                                                                    \
         ( AUTOBOOST_PP_SUB(AUTOBOOST_CONTAINER_MAX_CONSTRUCTOR_PARAMETERS, n)                          \
         , AUTOBOOST_CONTAINER_PP_IDENTITY, nat)                                                    \
   ...
   scoped_allocator_adaptor_base()                                                              \
      {}                                                                                        \
                                                                                                \
   template <class OuterA2>                                                                     \
   scoped_allocator_adaptor_base(AUTOBOOST_FWD_REF(OuterA2) outerAlloc                              \
      AUTOBOOST_PP_ENUM_TRAILING(n, AUTOBOOST_CONTAINER_PP_CONST_REF_PARAM_LIST_Q, _))                  \
      : outer_allocator_type(::autoboost::forward<OuterA2>(outerAlloc))                             \
      , m_inner(AUTOBOOST_PP_ENUM_PARAMS(n, q))                                                     \
      {}                                                                                        \
                                                                                                \
   scoped_allocator_adaptor_base(const scoped_allocator_adaptor_base& other)                    \
      : outer_allocator_type(other.outer_allocator())                                           \
   ...
   template <class OuterA2>                                                                     \
   scoped_allocator_adaptor_base                                                                \
      (AUTOBOOST_RV_REF_BEG scoped_allocator_adaptor_base<OuterA2, true                             \
         AUTOBOOST_PP_ENUM_TRAILING_PARAMS(n, Q)                                                    \
         AUTOBOOST_PP_ENUM_TRAILING                                                                 \
            ( AUTOBOOST_PP_SUB(AUTOBOOST_CONTAINER_MAX_CONSTRUCTOR_PARAMETERS, n)                       \
            , AUTOBOOST_CONTAINER_PP_IDENTITY, nat)                                                 \
         > AUTOBOOST_RV_REF_END other)                                                              \
      : outer_allocator_type(other.outer_allocator())                                           \
      , m_inner(other.inner_allocator())                                                        \
      {}                                                                                        \
                                                                                                \
...
class scoped_allocator_adaptor_base
   ...
   #if defined(AUTOBOOST_NO_CXX11_VARIADIC_TEMPLATES)
      , true
      AUTOBOOST_PP_ENUM_TRAILING(AUTOBOOST_CONTAINER_MAX_CONSTRUCTOR_PARAMETERS, AUTOBOOST_CONTAINER_PP_IDENTITY, nat)
   #endif
   >
   : public OuterAlloc
{
   ...
   struct rebind_base
   {
      typedef scoped_allocator_adaptor_base
         ...
         #if defined(AUTOBOOST_NO_CXX11_VARIADIC_TEMPLATES)
         , true
         AUTOBOOST_PP_ENUM_TRAILING(AUTOBOOST_CONTAINER_MAX_CONSTRUCTOR_PARAMETERS, AUTOBOOST_CONTAINER_PP_IDENTITY, container_detail::nat)
         #endif
         > other;
   };
   ...
   template <class OuterA2>
   scoped_allocator_adaptor_base(internal_type_t, AUTOBOOST_FWD_REF(OuterA2) outerAlloc, const inner_allocator_type &)
      : outer_allocator_type(::autoboost::forward<OuterA2>(outerAlloc))
      {}

   public:
   scoped_allocator_adaptor_base &operator=(AUTOBOOST_COPY_ASSIGN_REF(scoped_allocator_adaptor_base) other)
   {
      outer_allocator_type::operator=(other.outer_allocator());
      return *this;
   }
...
class scoped_allocator_adaptor
   ...
   : public container_detail::scoped_allocator_adaptor_base
         ...
         , InnerAllocs...
         #else
         , true AUTOBOOST_PP_ENUM_TRAILING_PARAMS(AUTOBOOST_CONTAINER_MAX_CONSTRUCTOR_PARAMETERS, Q)
         #endif
         >
{
   ...
   struct rebind
   {
      typedef scoped_allocator_adaptor
         ...
         , InnerAllocs...
         #else
         AUTOBOOST_PP_ENUM_TRAILING_PARAMS(AUTOBOOST_CONTAINER_MAX_CONSTRUCTOR_PARAMETERS, Q)
         #endif
         > other;
   };
   ...
   //! the corresponding allocator from other.
   scoped_allocator_adaptor(AUTOBOOST_RV_REF(scoped_allocator_adaptor) other)
      : base_type(::autoboost::move(other.base()))
      {}
   ...
   //!
   //! <b>Effects</b>: initializes the OuterAlloc base class with autoboost::forward<OuterA2>(outerAlloc) and inner
   //! with innerAllocs...(hence recursively initializing each allocator within the adaptor with the
   //! corresponding allocator from the argument list).
   template <class OuterA2>
   scoped_allocator_adaptor(AUTOBOOST_FWD_REF(OuterA2) outerAlloc, const InnerAllocs & ...innerAllocs)
      : base_type(::autoboost::forward<OuterA2>(outerAlloc), innerAllocs...)
      {}
   #else // #if !defined(AUTOBOOST_NO_CXX11_VARIADIC_TEMPLATES) || defined(AUTOBOOST_CONTAINER_DOXYGEN_INVOKED)

   #define AUTOBOOST_PP_LOCAL_MACRO(n)                                                              \
   template <class OuterA2>                                                                     \
   scoped_allocator_adaptor(AUTOBOOST_FWD_REF(OuterA2) outerAlloc                                   \
                     AUTOBOOST_PP_ENUM_TRAILING(n, AUTOBOOST_CONTAINER_PP_CONST_REF_PARAM_LIST_Q, _))   \
      : base_type(::autoboost::forward<OuterA2>(outerAlloc)                                         \
                  AUTOBOOST_PP_ENUM_TRAILING_PARAMS(n, q)                                           \
                  )                                                                             \
      {}                                                                                        \
   //!
   ...
   //! rvalue from other.
   template <class OuterA2>
   scoped_allocator_adaptor(AUTOBOOST_RV_REF_BEG scoped_allocator_adaptor<OuterA2
      #if !defined(AUTOBOOST_NO_CXX11_VARIADIC_TEMPLATES) || defined(AUTOBOOST_CONTAINER_DOXYGEN_INVOKED)
      , InnerAllocs...
      #else
      AUTOBOOST_PP_ENUM_TRAILING_PARAMS(AUTOBOOST_CONTAINER_MAX_CONSTRUCTOR_PARAMETERS, Q)
   ...
   scoped_allocator_adaptor &operator=(AUTOBOOST_COPY_ASSIGN_REF(scoped_allocator_adaptor) other)
   {  return static_cast<scoped_allocator_adaptor&>(base_type::operator=(static_cast<const base_type &>(other))); }

   scoped_allocator_adaptor &operator=(AUTOBOOST_RV_REF(scoped_allocator_adaptor) other)
   {  return static_cast<scoped_allocator_adaptor&>(base_type::operator=(autoboost::move(static_cast<base_type&>(other)))); }
   ...
   template <class T>
   void destroy(T* p) AUTOBOOST_CONTAINER_NOEXCEPT
   {
   ...
   #endif
   construct(T* p, AUTOBOOST_FWD_REF(Args)...args)
   {
   ...
   template < typename T                                                                        \
            ...
            >                                                                                   \
   typename container_detail::enable_if_c<!container_detail::is_pair<T>::value, void>::type     \
      construct(T* p AUTOBOOST_PP_ENUM_TRAILING(n, AUTOBOOST_CONTAINER_PP_PARAM_LIST, _))               \
   {                                                                                            \
      container_detail::dispatch_uses_allocator                                                 \
         ( uses_allocator<T, inner_allocator_type>()                                            \
         , get_outermost_allocator(this->outer_allocator())                                     \
         , this->inner_allocator()                                                              \
         , p AUTOBOOST_PP_ENUM_TRAILING(n, AUTOBOOST_CONTAINER_PP_PARAM_FORWARD, _));                   \
   }                                                                                            \
   //!
   #define AUTOBOOST_PP_LOCAL_LIMITS (0, AUTOBOOST_CONTAINER_MAX_CONSTRUCTOR_PARAMETERS)
   #include AUTOBOOST_PP_LOCAL_ITERATE()
   ...
   void construct(std::pair<T1, T2>* p, AUTOBOOST_FWD_REF(U) x, AUTOBOOST_FWD_REF(V) y)
   {  this->construct_pair(p, ::autoboost::forward<U>(x), ::autoboost::forward<V>(y));   }

   template <class T1, class T2, class U, class V>
   void construct(container_detail::pair<T1, T2>* p, AUTOBOOST_FWD_REF(U) x, AUTOBOOST_FWD_REF(V) y)
   {  this->construct_pair(p, ::autoboost::forward<U>(x), ::autoboost::forward<V>(y));   }
   ...
   template <class T1, class T2, class U, class V>
   void construct( std::pair<T1, T2>* p
                 , AUTOBOOST_RV_REF_BEG std::pair<U, V> AUTOBOOST_RV_REF_END x)
   {  this->construct_pair(p, x);   }

   template <class T1, class T2, class U, class V>
   void construct( container_detail::pair<T1, T2>* p
                 , AUTOBOOST_RV_REF_BEG container_detail::pair<U, V> AUTOBOOST_RV_REF_END x)
   {  this->construct_pair(p, x);   }
   ...
   void construct_pair(Pair* p, AUTOBOOST_FWD_REF(U) x, AUTOBOOST_FWD_REF(V) y)
   {
      this->construct(container_detail::addressof(p->first), ::autoboost::forward<U>(x));
      AUTOBOOST_TRY{
         this->construct(container_detail::addressof(p->second), ::autoboost::forward<V>(y));
      }
      AUTOBOOST_CATCH(...){
         this->destroy(container_detail::addressof(p->first));
         AUTOBOOST_RETHROW
   ...
   template <class Pair, class Pair2>
   void construct_pair(Pair* p, AUTOBOOST_RV_REF(Pair2) pr)
   {
      this->construct(container_detail::addressof(p->first), ::autoboost::move(pr.first));
      AUTOBOOST_TRY{
         this->construct(container_detail::addressof(p->second), ::autoboost::move(pr.second));
      }
      AUTOBOOST_CATCH(...){
         this->destroy(container_detail::addressof(p->first));
         AUTOBOOST_RETHROW
   ...
   //Internal function
   template <class OuterA2>
   scoped_allocator_adaptor(internal_type_t, AUTOBOOST_FWD_REF(OuterA2) outer, const inner_allocator_type& inner)
      : base_type(internal_type_t(), ::autoboost::forward<OuterA2>(outer), inner)
   {}
   ...
   >& b)
{
   ...
   const bool has_zero_inner =
      autoboost::container::container_detail::is_same
...
#endif //  AUTOBOOST_CONTAINER_ALLOCATOR_SCOPED_ALLOCATOR_HPP
