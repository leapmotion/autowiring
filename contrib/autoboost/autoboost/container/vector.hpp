...
class vec_iterator
{
   ...
   public:
   const Pointer &get_ptr() const AUTOBOOST_CONTAINER_NOEXCEPT
   {  return   m_ptr;  }

   Pointer &get_ptr() AUTOBOOST_CONTAINER_NOEXCEPT
   {  return   m_ptr;  }

   explicit vec_iterator(Pointer ptr) AUTOBOOST_CONTAINER_NOEXCEPT
      : m_ptr(ptr)
   {}
   ...
   //Constructors
   vec_iterator() AUTOBOOST_CONTAINER_NOEXCEPT
      : m_ptr()   //Value initialization to achieve "null iterators" (N3644)
   {}

   vec_iterator(vec_iterator<Pointer, false> const& other) AUTOBOOST_CONTAINER_NOEXCEPT
      :  m_ptr(other.get_ptr())
   {}
   ...
   reference operator[](difference_type off) const AUTOBOOST_CONTAINER_NOEXCEPT
   {  return m_ptr[off];   }

   //Increment / Decrement
   vec_iterator& operator++() AUTOBOOST_CONTAINER_NOEXCEPT
   { ++m_ptr;  return *this; }

   vec_iterator operator++(int) AUTOBOOST_CONTAINER_NOEXCEPT
   ...
   //Arithmetic
   vec_iterator& operator+=(difference_type off) AUTOBOOST_CONTAINER_NOEXCEPT
   {  m_ptr += off; return *this;   }

   vec_iterator& operator-=(difference_type off) AUTOBOOST_CONTAINER_NOEXCEPT
   {  m_ptr -= off; return *this;   }

   friend vec_iterator operator+(const vec_iterator &x, difference_type off) AUTOBOOST_CONTAINER_NOEXCEPT
   {  return vec_iterator(x.m_ptr+off);  }

   friend vec_iterator operator+(difference_type off, vec_iterator right) AUTOBOOST_CONTAINER_NOEXCEPT
   {  right.m_ptr += off;  return right; }

   friend vec_iterator operator-(vec_iterator left, difference_type off) AUTOBOOST_CONTAINER_NOEXCEPT
   {  left.m_ptr -= off;  return left; }

   friend difference_type operator-(const vec_iterator &left, const vec_iterator& right) AUTOBOOST_CONTAINER_NOEXCEPT
   {  return left.m_ptr - right.m_ptr;   }

   //Comparison operators
   friend bool operator==   (const vec_iterator& l, const vec_iterator& r) AUTOBOOST_CONTAINER_NOEXCEPT
...
template<class Pointer, bool IsConst>
const Pointer &vector_iterator_get_ptr(const container_detail::vec_iterator<Pointer, IsConst> &it) AUTOBOOST_CONTAINER_NOEXCEPT
{  return   it.get_ptr();  }

template<class Pointer, bool IsConst>
Pointer &get_ptr(container_detail::vec_iterator<Pointer, IsConst> &it) AUTOBOOST_CONTAINER_NOEXCEPT
{  return  it.get_ptr();  }
...
struct vector_get_ptr_pointer_to_non_const
{
   ...
   static return_type get_ptr(const const_pointer &ptr) AUTOBOOST_CONTAINER_NOEXCEPT
...
struct vector_get_ptr_pointer_to_non_const<Pointer, false>
{
   ...
   static return_type get_ptr(const Pointer &ptr) AUTOBOOST_CONTAINER_NOEXCEPT
   {  return ptr;  }
};
...
typename container_detail::vector_get_ptr_pointer_to_non_const<MaybeConstPointer>::return_type
   vector_iterator_get_ptr(const MaybeConstPointer &ptr) AUTOBOOST_CONTAINER_NOEXCEPT
{
...
{
   static const bool trivial_dctr = autoboost::has_trivial_destructor<T>::value;
   static const bool trivial_dctr_after_move = ::autoboost::has_trivial_destructor_after_move<T>::value;
   ...
   : public Allocator
{
   ...
   vector_alloc_holder()
      AUTOBOOST_CONTAINER_NOEXCEPT_IF(::autoboost::has_nothrow_default_constructor<Allocator>::value)
      : Allocator(), m_start(), m_size(), m_capacity()
   {}

   //Constructor, does not throw
   template<class AllocConvertible>
   explicit vector_alloc_holder(AUTOBOOST_FWD_REF(AllocConvertible) a) AUTOBOOST_CONTAINER_NOEXCEPT
   ...
   vector_alloc_holder(AUTOBOOST_RV_REF(vector_alloc_holder) holder) AUTOBOOST_CONTAINER_NOEXCEPT
      : Allocator(autoboost::move(static_cast<Allocator&>(holder)))
   ...
   ~vector_alloc_holder() AUTOBOOST_CONTAINER_NOEXCEPT
   {
      if(this->m_capacity){
   ...
   std::pair<pointer, bool>
      allocation_command(autoboost::container::allocation_type command,
   ...
   void swap(vector_alloc_holder &x) AUTOBOOST_CONTAINER_NOEXCEPT
   {
      autoboost::container::swap_dispatch(this->m_start, x.m_start);
      autoboost::container::swap_dispatch(this->m_size, x.m_size);
      autoboost::container::swap_dispatch(this->m_capacity, x.m_capacity);
   }

   void move_from_empty(vector_alloc_holder &x) AUTOBOOST_CONTAINER_NOEXCEPT
   {
   ...
   Allocator &alloc() AUTOBOOST_CONTAINER_NOEXCEPT
   {  return *this;  }

   const Allocator &alloc() const AUTOBOOST_CONTAINER_NOEXCEPT
   {  return *this;  }

   const pointer   &start() const     AUTOBOOST_CONTAINER_NOEXCEPT {  return m_start;  }
   const size_type &capacity() const  AUTOBOOST_CONTAINER_NOEXCEPT {  return m_capacity;  }
   void start(const pointer &p)       AUTOBOOST_CONTAINER_NOEXCEPT {  m_start = p;  }
   void capacity(const size_type &c)  AUTOBOOST_CONTAINER_NOEXCEPT {  m_capacity = c;  }
};
   ...
   : public Allocator
{
   ...
   void first_allocation_same_allocator_type(size_type) AUTOBOOST_CONTAINER_NOEXCEPT
   {}
   ...
   pointer start() const       AUTOBOOST_CONTAINER_NOEXCEPT {  return Allocator::internal_storage();  }
   size_type  capacity() const AUTOBOOST_CONTAINER_NOEXCEPT {  return Allocator::internal_capacity;  }
   size_type   m_size;
...
template <class T, class Allocator AUTOBOOST_CONTAINER_DOCONLY(= std::allocator<T>) >
class vector
{
   #ifndef AUTOBOOST_CONTAINER_DOXYGEN_INVOKED

   typedef typename container_detail::version<Allocator>::type alloc_version;
   autoboost::container::container_detail::vector_alloc_holder
      <Allocator, alloc_version>                            m_holder;
   typedef allocator_traits<Allocator>                      allocator_traits_type;
   template <class U, class UAllocator>
   friend class vector;

   typedef typename ::autoboost::container::allocator_traits
   ...
   #endif
   typedef AUTOBOOST_CONTAINER_IMPDEF(container_detail::reverse_iterator<iterator>)                          reverse_iterator;
   typedef AUTOBOOST_CONTAINER_IMPDEF(container_detail::reverse_iterator<const_iterator>)                    const_reverse_iterator;
   ...
   //!
   //! <b>Complexity</b>: Constant.
   explicit vector(const Allocator& a) AUTOBOOST_CONTAINER_NOEXCEPT
      : m_holder(a)
   {}
   ...
   //!
   //! <b>Complexity</b>: Constant.
   vector(AUTOBOOST_RV_REF(vector) x) AUTOBOOST_CONTAINER_NOEXCEPT
   ...
   ~vector() AUTOBOOST_CONTAINER_NOEXCEPT
   {
   ...
   void assign(InIt first, InIt last
      AUTOBOOST_CONTAINER_DOCIGN(AUTOBOOST_CONTAINER_I typename container_detail::enable_if_c
         < !container_detail::is_convertible<InIt AUTOBOOST_CONTAINER_I size_type>::value &&
            ( container_detail::is_input_iterator<InIt>::value ||
              container_detail::is_same<alloc_version AUTOBOOST_CONTAINER_I allocator_v0>::value )
         >::type * = 0) )
   {
   ...
   void assign(FwdIt first, FwdIt last
      AUTOBOOST_CONTAINER_DOCIGN(AUTOBOOST_CONTAINER_I typename container_detail::enable_if_c
         < !container_detail::is_convertible<FwdIt AUTOBOOST_CONTAINER_I size_type>::value &&
   ...
   //!
   //! <b>Complexity</b>: Constant.
   allocator_type get_allocator() const AUTOBOOST_CONTAINER_NOEXCEPT
   ...
   //!
   //! <b>Complexity</b>: Constant.
   size_type capacity() const AUTOBOOST_CONTAINER_NOEXCEPT
   ...
   //!
   //! <b>Complexity</b>: Constant.
   reference         front() AUTOBOOST_CONTAINER_NOEXCEPT
   ...
   //!
   //! <b>Complexity</b>: Constant.
   const_reference   front() const AUTOBOOST_CONTAINER_NOEXCEPT
   { return *this->m_holder.start(); }
   ...
   //!
   //! <b>Complexity</b>: Constant.
   reference         back() AUTOBOOST_CONTAINER_NOEXCEPT
   ...
   //!
   //! <b>Complexity</b>: Constant.
   const_reference   back()  const AUTOBOOST_CONTAINER_NOEXCEPT
   { return this->m_holder.start()[this->m_holder.m_size - 1]; }
   ...
   //!
   //! <b>Complexity</b>: Constant.
   reference operator[](size_type n) AUTOBOOST_CONTAINER_NOEXCEPT
   ...
   //!
   //! <b>Complexity</b>: Constant.
   const_reference operator[](size_type n) const AUTOBOOST_CONTAINER_NOEXCEPT
   { return this->m_holder.start()[n]; }
   ...
   //!
   //! <b>Complexity</b>: Constant.
   T* data() AUTOBOOST_CONTAINER_NOEXCEPT
   ...
   //!
   //! <b>Complexity</b>: Constant.
   const T * data()  const AUTOBOOST_CONTAINER_NOEXCEPT
   { return container_detail::to_raw_pointer(this->m_holder.start()); }
   ...
   {
      if (AUTOBOOST_LIKELY(this->m_holder.m_size < this->m_holder.capacity())){
         T* const back_pos = container_detail::to_raw_pointer(this->m_holder.start()) + this->m_holder.m_size;
         //There is more memory, just construct a new object at the end
         allocator_traits_type::construct(this->m_holder.alloc(), back_pos, ::autoboost::forward<Args>(args)...);
         ++this->m_holder.m_size;
      }
      else{
         typedef container_detail::insert_emplace_proxy<Allocator, T*, Args...> type;
         this->priv_forward_range_insert_no_capacity
            (vector_iterator_get_ptr(this->cend()), 1, type(::autoboost::forward<Args>(args)...), alloc_version());
      }
   }
   ...
   iterator emplace(const_iterator position, Args && ...args)
   {
      ...
      return this->priv_forward_range_insert( vector_iterator_get_ptr(position), 1
                                            , type(::autoboost::forward<Args>(args)...), alloc_version());
   }
   ...
   void emplace_back(AUTOBOOST_PP_ENUM(n, AUTOBOOST_CONTAINER_PP_PARAM_LIST, _))                              \
   {                                                                                                  \
      T* const back_pos = container_detail::to_raw_pointer                                            \
         (this->m_holder.start()) + this->m_holder.m_size;                                            \
      if (AUTOBOOST_LIKELY(this->m_holder.m_size < this->m_holder.capacity())){                                         \
         allocator_traits_type::construct (this->m_holder.alloc()                                     \
            , back_pos AUTOBOOST_PP_ENUM_TRAILING(n, AUTOBOOST_CONTAINER_PP_PARAM_FORWARD, _) );              \
         ++this->m_holder.m_size;                                                                     \
      }                                                                                               \
      else{                                                                                           \
         typedef container_detail::AUTOBOOST_PP_CAT(insert_emplace_proxy_arg, n)                          \
            <Allocator, T* AUTOBOOST_PP_ENUM_TRAILING_PARAMS(n, P)> type;                                 \
         this->priv_forward_range_insert_no_capacity                                                  \
            ( vector_iterator_get_ptr(this->cend()), 1                                                \
            , type(AUTOBOOST_PP_ENUM(n, AUTOBOOST_CONTAINER_PP_PARAM_FORWARD, _)), alloc_version());          \
      }                                                                                               \
   }                                                                                                  \
                                                                                                      \
   AUTOBOOST_PP_EXPR_IF(n, template<) AUTOBOOST_PP_ENUM_PARAMS(n, class P) AUTOBOOST_PP_EXPR_IF(n, >)             \
   ...
   void pop_back() AUTOBOOST_CONTAINER_NOEXCEPT
   {
   ...
   iterator erase(const_iterator position)
   {
      ...
      T *const new_end_ptr = ::autoboost::move(pos_ptr + 1, beg_ptr + this->m_holder.m_size, pos_ptr);
   ...
   iterator erase(const_iterator first, const_iterator last)
   {
      if (first != last){
         ...
         T* const ptr = container_detail::to_raw_pointer(autoboost::move(last_ptr, old_end_ptr, first_ptr));
   ...
   //!
   //! <b>Complexity</b>: Constant.
   void swap(vector& x) AUTOBOOST_CONTAINER_NOEXCEPT_IF((!container_detail::is_version<Allocator, 0>::value))
   {
   ...
   void insert_ordered_at(const size_type element_count, BiDirPosConstIt last_position_it, BiDirValueIt last_value_it)
   {
      ...
      while(insertions_left){
         ...
         AUTOBOOST_ASSERT(pos != size_type(-1) && pos <= old_size_pos);
   ...
   bool stable_emplace_back(Args &&...args)
   {
      ...
      if (AUTOBOOST_LIKELY(room_enough)){
         T* const back_pos = container_detail::to_raw_pointer(this->m_holder.start()) + this->m_holder.m_size;
         //There is more memory, just construct a new object at the end
         allocator_traits_type::construct(this->m_holder.alloc(), back_pos, ::autoboost::forward<Args>(args)...);
         ++this->m_holder.m_size;
   ...
   #define AUTOBOOST_PP_LOCAL_MACRO(n)                                                           \
   AUTOBOOST_PP_EXPR_IF(n, template<) AUTOBOOST_PP_ENUM_PARAMS(n, class P) AUTOBOOST_PP_EXPR_IF(n, >)    \
   bool stable_emplace_back(AUTOBOOST_PP_ENUM(n, AUTOBOOST_CONTAINER_PP_PARAM_LIST, _))              \
   {                                                                                         \
      const bool room_enough = this->m_holder.m_size < this->m_holder.capacity();            \
      if (AUTOBOOST_LIKELY(room_enough)){                                                                      \
         T* const back_pos = container_detail::to_raw_pointer                                \
           ...
           container_detail::is_same<OtherAllocator, allocator_type>::value>::type * = 0)
   {
      ...
      //Else do a one by one move
         ...
         this->assign( autoboost::make_move_iterator(x.begin())
                     , autoboost::make_move_iterator(x.end()));
      }
   }
   ...
   void priv_destroy_last() AUTOBOOST_CONTAINER_NOEXCEPT
   {
   ...
   void priv_destroy_last(const bool moved) AUTOBOOST_CONTAINER_NOEXCEPT
   {
      (void)moved;
   ...
   void priv_destroy_last_n(const size_type n) AUTOBOOST_CONTAINER_NOEXCEPT
   {
   ...
   void priv_destroy_last_n(const size_type n, const bool moved) AUTOBOOST_CONTAINER_NOEXCEPT
   {
      AUTOBOOST_ASSERT(n <= this->m_holder.m_size);
      (void)moved;
      if(!(value_traits::trivial_dctr || (value_traits::trivial_dctr_after_move && moved))){
   ...
   void priv_destroy_all() AUTOBOOST_CONTAINER_NOEXCEPT
   {
      autoboost::container::destroy_alloc_n
         (this->get_stored_allocator(), container_detail::to_raw_pointer(this->m_holder.start()), this->m_holder.m_size);
      this->m_holder.m_size = 0;
   ...
   iterator priv_insert(const const_iterator &p, AUTOBOOST_FWD_REF(U) x)
   {
      return this->priv_forward_range_insert
         ( vector_iterator_get_ptr(p), 1, container_detail::get_insert_value_proxy<T*, Allocator>
            (::autoboost::forward<U>(x)), alloc_version());
   }
   ...
   void priv_shrink_to_fit(allocator_v0) AUTOBOOST_CONTAINER_NOEXCEPT
   {}
   ...
   void priv_shrink_to_fit(allocator_v2) AUTOBOOST_CONTAINER_NOEXCEPT
   {
      const size_type cp = this->m_holder.capacity();
      if(cp){
         const size_type sz = this->size();
                              ...
                              , bool do_skip, BiDirSkipConstIt last_skip_it, BiDirValueIt last_value_it)
   {
      ...
      while(insertions_left){
         ...
         AUTOBOOST_ASSERT(pos <= old_size_pos);
         //If needed shift the range after the insertion point and the previous insertion point.
         //Function will take care if the shift crosses the size() boundary, using copy/move
         //or uninitialized copy/move if necessary.
         size_type new_hole_size = (pos != next_pos)
      ...
      (size_type first_pos, size_type last_pos, size_type limit_pos, size_type shift_count)
   {
      ...
      if((last_pos + shift_count) <= limit_pos){
         //All move assigned
         autoboost::move_backward(first_ptr, last_ptr, last_ptr + shift_count);
      }
      //Case B:
      ...
      //Case C:
         ...
         autoboost::move_backward(first_ptr, boundary_ptr, limit_ptr);
      }
      return hole_size;
   }
   ...
   void priv_forward_range_insert_expand_forward(T* const pos, const size_type n, InsertionProxy insert_range_proxy)
   {
      ...
      if (!elems_after){
         ...
         autoboost::move_backward(pos, old_finish - n, old_finish);
      ...
      if(s_before >= before_plus_new){
         ...
         //s_before is so big that divides old_end
            ...
            if(!value_traits::trivial_dctr){
               ...
               ::autoboost::move(pos + raw_gap, old_finish, old_start);
         ...
         if (s_before <= elemsbefore) {
            ...
            T *const next = ::autoboost::move(old_start + s_before, pos, old_start);
            ...
            if(!do_after && (n != s_before)){
               ...
               ::autoboost::move(pos, old_finish, next + new_1st_range);
            }
         }
         else {
            ...
            if(do_after){
               ...
               //Displace old_end
               T* const move_end = ::autoboost::move(pos, old_finish, move_start);
         ...
         if(do_after){
            ...
            if (elemsafter >= n_after){
               ...
               autoboost::move_backward(pos, finish_n, old_finish);
...
struct has_trivial_destructor_after_move<autoboost::container::vector<T, Allocator> >
   : public ::autoboost::has_trivial_destructor_after_move<Allocator>
{};
*/
}

//#define AUTOBOOST_CONTAINER_PUT_SWAP_OVERLOAD_IN_NAMESPACE_STD

#ifdef AUTOBOOST_CONTAINER_PUT_SWAP_OVERLOAD_IN_NAMESPACE_STD

namespace std {

template <class T, class Allocator>
inline void swap(autoboost::container::vector<T, Allocator>& x, autoboost::container::vector<T, Allocator>& y)
{  x.swap(y);  }
