...
class vec_iterator
{
   ...
   public:
   const Pointer &get_ptr() const AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {  return   m_ptr;  }

   Pointer &get_ptr() AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {  return   m_ptr;  }

   explicit vec_iterator(Pointer ptr) AUTOBOOST_NOEXCEPT_OR_NOTHROW
      : m_ptr(ptr)
   {}
   ...
   //Constructors
   vec_iterator() AUTOBOOST_NOEXCEPT_OR_NOTHROW
      : m_ptr()   //Value initialization to achieve "null iterators" (N3644)
   {}

   vec_iterator(vec_iterator<Pointer, false> const& other) AUTOBOOST_NOEXCEPT_OR_NOTHROW
      :  m_ptr(other.get_ptr())
   {}
   ...
   reference operator[](difference_type off) const AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {  return m_ptr[off];   }

   //Increment / Decrement
   vec_iterator& operator++() AUTOBOOST_NOEXCEPT_OR_NOTHROW
   { ++m_ptr;  return *this; }

   vec_iterator operator++(int) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   //Arithmetic
   vec_iterator& operator+=(difference_type off) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {  m_ptr += off; return *this;   }

   vec_iterator& operator-=(difference_type off) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {  m_ptr -= off; return *this;   }

   friend vec_iterator operator+(const vec_iterator &x, difference_type off) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {  return vec_iterator(x.m_ptr+off);  }

   friend vec_iterator operator+(difference_type off, vec_iterator right) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {  right.m_ptr += off;  return right; }

   friend vec_iterator operator-(vec_iterator left, difference_type off) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {  left.m_ptr -= off;  return left; }

   friend difference_type operator-(const vec_iterator &left, const vec_iterator& right) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {  return left.m_ptr - right.m_ptr;   }

   //Comparison operators
   friend bool operator==   (const vec_iterator& l, const vec_iterator& r) AUTOBOOST_NOEXCEPT_OR_NOTHROW
...
template<class Pointer, bool IsConst>
const Pointer &vector_iterator_get_ptr(const container_detail::vec_iterator<Pointer, IsConst> &it) AUTOBOOST_NOEXCEPT_OR_NOTHROW
{  return   it.get_ptr();  }

template<class Pointer, bool IsConst>
Pointer &get_ptr(container_detail::vec_iterator<Pointer, IsConst> &it) AUTOBOOST_NOEXCEPT_OR_NOTHROW
{  return  it.get_ptr();  }
...
struct vector_get_ptr_pointer_to_non_const
{
   ...
   static return_type get_ptr(const const_pointer &ptr) AUTOBOOST_NOEXCEPT_OR_NOTHROW
...
struct vector_get_ptr_pointer_to_non_const<Pointer, false>
{
   ...
   static return_type get_ptr(const Pointer &ptr) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {  return ptr;  }
};
   ...
   : public Allocator
{
   ...
   ~vector_alloc_holder() AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {
      if(this->m_capacity){
         this->alloc().deallocate(this->m_start, this->m_capacity);
      }
   ...
   pointer allocation_command(autoboost::container::allocation_type command,
   ...
   Allocator &alloc() AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {  return *this;  }

   const Allocator &alloc() const AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {  return *this;  }
   ...
   pointer priv_allocation_command(version_1, autoboost::container::allocation_type command,
                         size_type ,
   ...
   : public Allocator
{
   ...
   void swap_resources(vector_alloc_holder &) AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   pointer start() const       AUTOBOOST_NOEXCEPT_OR_NOTHROW {  return Allocator::internal_storage();  }
   size_type  capacity() const AUTOBOOST_NOEXCEPT_OR_NOTHROW {  return Allocator::internal_capacity;  }
   size_type   m_size;
...
class vector
{
   ...
   template<class AllocFwd>
   vector(initial_capacity_t, pointer initial_memory, size_type capacity, AUTOBOOST_FWD_REF(AllocFwd) a)
   ...
   vector& operator=(AUTOBOOST_COPY_ASSIGN_REF(vector) x)
   {
      if (&x != this){
   ...
   vector& operator=(AUTOBOOST_RV_REF(vector) x)
   ...
   template<class ...Args>
   void emplace_back(AUTOBOOST_FWD_REF(Args)...args)
   {
   ...
   //! <b>Note</b>: Non-standard extension.
   template<class ...Args>
   bool stable_emplace_back(AUTOBOOST_FWD_REF(Args)...args)
   {
   ...
   void emplace_back(AUTOBOOST_MOVE_UREF##N)\
   {\
      if (AUTOBOOST_LIKELY(this->room_enough())){\
   ...
   \
   AUTOBOOST_MOVE_TMPL_LT##N AUTOBOOST_MOVE_CLASS##N AUTOBOOST_MOVE_GT##N \
   bool stable_emplace_back(AUTOBOOST_MOVE_UREF##N)\
   {\
      const bool is_room_enough = this->room_enough() || (alloc_version::value == 2 && this->m_holder.try_expand_fwd(1u));\
      if (AUTOBOOST_LIKELY(is_room_enough)){\
         allocator_traits_type::construct (this->m_holder.alloc()\
      ...
      if(s_before >= before_plus_new){
         ...
         //s_before is so big that divides old_end
            ...
            if(!value_traits::trivial_dctr){
               ...
               ::autoboost::container::move(pos + raw_gap, old_finish, old_start);
         ...
         if (s_before <= elemsbefore) {
            ...
            if(do_after){
               ...
               //Displace old_end
               T* const move_end = ::autoboost::container::move(pos, old_finish, move_start);
