...
#ifndef AUTOBOOST_CONTAINER_STRING_HPP
#define AUTOBOOST_CONTAINER_STRING_HPP
...
#include <autoboost/container/detail/minimal_char_traits_header.hpp>
...
class basic_string_base
{
 ...
 public:
   ...
   typedef ::autoboost::intrusive::pointer_traits<pointer> pointer_traits;
   ...
   explicit basic_string_base(AUTOBOOST_RV_REF(allocator_type) a)
      :  members_(autoboost::move(a))
   {  this->init();  }
      ...
      :  public Allocator
   {
      ...
      explicit members_holder(AUTOBOOST_FWD_REF(AllocatorConvertible) a)
         :  Allocator(autoboost::forward<AllocatorConvertible>(a))
      {}
   ...
   bool is_short() const
   {
      ...
      short_header hdr;
      AUTOBOOST_STATIC_ASSERT((sizeof(short_header) == 1));
   ...
   typedef container_detail::integral_constant<unsigned,
      autoboost::container::container_detail::version<Allocator>::value> alloc_version;
   ...
   void swap_data(basic_string_base& other)
   {
      if(this->is_short()){
         ...
         if(other.is_short()){
            ...
            autoboost::adl_move_swap(this->members_.m_repr.long_repr(), other.members_.m_repr.long_repr());
         }
      }
   }
};
   ...
   :  private container_detail::basic_string_base<Allocator>
{
   ...
   private:
   typedef allocator_traits<Allocator> allocator_traits_type;
   AUTOBOOST_COPYABLE_AND_MOVABLE(basic_string)
   ...
   basic_string() AUTOBOOST_NOEXCEPT_IF(container_detail::is_nothrow_default_constructible<Allocator>::value)
      : base_t()
   { this->priv_terminate_string(); }
   ...
   //!
   //! <b>Throws</b>: Nothing
   explicit basic_string(const allocator_type& a) AUTOBOOST_NOEXCEPT_OR_NOTHROW
      : base_t(a)
   { this->priv_terminate_string(); }
   ...
   basic_string(AUTOBOOST_RV_REF(basic_string) s) AUTOBOOST_NOEXCEPT_OR_NOTHROW
      : base_t(autoboost::move(s.alloc()))
   {
   ...
   basic_string(AUTOBOOST_RV_REF(basic_string) s, const allocator_type &a)
      : base_t(a)
   {
      this->priv_terminate_string();
      if(a == this->alloc()){
   ...
   //!
   //! <b>Complexity</b>: Constant.
   ~basic_string() AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {}
   ...
   basic_string& operator=(AUTOBOOST_COPY_ASSIGN_REF(basic_string) x)
   {
      if (&x != this){
   ...
   basic_string& operator=(AUTOBOOST_RV_REF(basic_string) x)
   ...
   //!
   //! <b>Complexity</b>: Constant.
   size_type length() const AUTOBOOST_NOEXCEPT_OR_NOTHROW
   { return this->size(); }
   ...
   //!
   //! <b>Returns</b>: *this
   basic_string& assign(AUTOBOOST_RV_REF(basic_string) ms) AUTOBOOST_NOEXCEPT_OR_NOTHROW
      ...
      )
   {
      const size_type n_pos = p - this->cbegin();
      if (first != last) {
         const size_type n = autoboost::container::iterator_distance(first, last);
         const size_type old_size = this->priv_size();
         ...
         if(enough_capacity){
            ...
            if (elems_after >= n) {
               ...
               autoboost::container::iterator_advance(mid, elems_after + 1);
   ...
   {
      difference_type n = autoboost::container::iterator_distance(j1, j2);
      ...
      if (len >= n) {
         ...
         ForwardIter m = j1;
         autoboost::container::iterator_advance(m, len);
   ...
   //!
   //! <b>Complexity</b>: constant time.
   const CharT* c_str() const AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   //!
   //! <b>Complexity</b>: constant time.
   const CharT* data()  const AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {  return container_detail::to_raw_pointer(this->priv_addr()); }

   //! <b>Returns</b>: A pointer p such that p + i == &operator[](i) for each i in [0,size()].
   //!
   //! <b>Complexity</b>: constant time.
   CharT* data()  AUTOBOOST_NOEXCEPT_OR_NOTHROW
   {  return container_detail::to_raw_pointer(this->priv_addr()); }

   #ifndef AUTOBOOST_CONTAINER_TEMPLATED_CONVERSION_OPERATOR_BROKEN
   //! <b>Returns</b>: a string_view to the characters in the string.
   //!
   //! <b>Complexity</b>: constant time.
   template<template <class, class> class BasicStringView>
   operator BasicStringView<CharT, Traits>() const AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   BasicStringView to_view() const AUTOBOOST_NOEXCEPT_OR_NOTHROW
   ...
   {
      typedef typename autoboost::container::iterator_traits<InputIter>::iterator_category Category;
...
template <class CharT, class Traits, class Allocator> inline
   basic_string<CharT, Traits, Allocator> operator+
      ( AUTOBOOST_RV_REF_BEG basic_string<CharT, Traits, Allocator> AUTOBOOST_RV_REF_END x
      , AUTOBOOST_RV_REF_BEG basic_string<CharT, Traits, Allocator> AUTOBOOST_RV_REF_END y)
{
   x += y;
...
struct has_trivial_destructor_after_move<autoboost::container::basic_string<C, T, Allocator> >
{
   typedef typename ::autoboost::container::allocator_traits<Allocator>::pointer pointer;
...
#endif // AUTOBOOST_CONTAINER_STRING_HPP
