...
#ifndef AUTOBOOST_MOVE_ADL_MOVE_SWAP_HPP
#define AUTOBOOST_MOVE_ADL_MOVE_SWAP_HPP
...
//Try to avoid including <algorithm>, as it's quite big
   ...
   #if !defined(AUTOBOOST_GCC) || ((__GNUC__ < 4) || ((__GNUC__ == 4) && (__GNUC_MINOR__ < 3)))
...
#include <autoboost/move/utility_core.hpp> //for autoboost::move

#if !defined(AUTOBOOST_MOVE_DOXYGEN_INVOKED)

#if defined(AUTOBOOST_NO_CXX11_RVALUE_REFERENCES)
namespace autoboost_move_member_swap {
...
namespace autoboost_move_adl_swap{
...
template<class T>
AUTOBOOST_MOVE_FORCEINLINE void swap_proxy(T& x, T& y, typename autoboost::move_detail::enable_if_c<!autoboost::move_detail::has_move_emulation_enabled_impl<T>::value>::type* = 0)
{
...
template<class T>
AUTOBOOST_MOVE_FORCEINLINE void swap_proxy(T& x, T& y
               , typename autoboost::move_detail::enable_if< and_op_not_impl<autoboost::move_detail::has_move_emulation_enabled_impl<T>
                                                                        , autoboost_move_member_swap::has_member_swap<T> >
                                                       >::type* = 0)
{  T t(::autoboost::move(x)); x = ::autoboost::move(y); y = ::autoboost::move(t);  }

template<class T>
AUTOBOOST_MOVE_FORCEINLINE void swap_proxy(T& x, T& y
               , typename autoboost::move_detail::enable_if< and_op_impl< autoboost::move_detail::has_move_emulation_enabled_impl<T>
                                                                    , autoboost_move_member_swap::has_member_swap<T> >
                                                       >::type* = 0)
{  x.swap(y);  }
...
#endif   //#if defined(AUTOBOOST_NO_CXX11_RVALUE_REFERENCES)
...
void swap_proxy(T (& x)[N], T (& y)[N])
{
   for (std::size_t i = 0; i < N; ++i){
      ::autoboost_move_adl_swap::swap_proxy(x[i], y[i]);
   }
}
...
//!      <code>T t(::autoboost::move(x)); x = ::autoboost::move(y); y = ::autoboost::move(t);</code>.
template<class T>
AUTOBOOST_MOVE_FORCEINLINE void adl_move_swap(T& x, T& y)
{
   ::autoboost_move_adl_swap::swap_proxy(x, y);
}

//! Exchanges elements between range [first1, last1) and another range starting at first2
//! using autoboost::adl_move_swap.
//!
//! Parameters:
...
{
    while (first1 != last1) {
      ::autoboost::adl_move_swap(*first1, *first2);
      ++first1;
      ++first2;
    }
   return first2;
...
{
   while (first1 != last1) {
      ::autoboost::adl_move_swap(*(--last1), *(--last2));
   }
   return last2;
}
...
#endif   //#ifndef AUTOBOOST_MOVE_ADL_MOVE_SWAP_HPP
