...
#ifndef AUTOBOOST_MOVE_DETAIL_META_UTILS_HPP
#define AUTOBOOST_MOVE_DETAIL_META_UTILS_HPP
...
#include <autoboost/move/detail/meta_utils_core.hpp>
...
{
   T & v_;
   AUTOBOOST_MOVE_FORCEINLINE addr_impl_ref( T & v ): v_( v ) {}
   AUTOBOOST_MOVE_FORCEINLINE operator T& () const { return v_; }
...
{
   AUTOBOOST_MOVE_FORCEINLINE static T * f( T & v, long )
   {
      return reinterpret_cast<T*>(
         &const_cast<char&>(reinterpret_cast<const volatile char &>(v)));
   }

   AUTOBOOST_MOVE_FORCEINLINE static T * f( T * v, int )
   {  return v;  }
};

template<class T>
AUTOBOOST_MOVE_FORCEINLINE T * addressof( T & v )
{
   return ::autoboost::move_detail::addressof_impl<T>::f
      ( ::autoboost::move_detail::addr_impl_ref<T>( v ), 0 );
}
...
#if defined(AUTOBOOST_GCC) && (AUTOBOOST_GCC <= 40000)
#define AUTOBOOST_MOVE_HELPERS_RETURN_SFINAE_BROKEN
#endif
...
struct has_move_emulation_enabled_impl
   : is_convertible< T, ::autoboost::rv<T>& >
{};
...
template<class T>
struct has_move_emulation_enabled_impl< ::autoboost::rv<T> >
{  static const bool value = false;  };
...
#else // #ifndef AUTOBOOST_NO_CXX11_RVALUE_REFERENCES

template< class T >
struct is_rvalue_reference< autoboost::rv<T>& >
{  static const bool value = true;  };

template< class T >
struct is_rvalue_reference< const autoboost::rv<T>& >
{  static const bool value = true;  };
...
namespace detail_add_rvalue_reference
{
   ...
   struct add_rvalue_reference_impl< T, true, rv > { typedef ::autoboost::rv<T>& type; };
} // namespace detail_add_rvalue_reference
...
#endif //#ifndef AUTOBOOST_MOVE_DETAIL_META_UTILS_HPP
