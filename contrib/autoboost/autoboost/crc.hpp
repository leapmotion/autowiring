...
#ifndef AUTOBOOST_CRC_HPP
#define AUTOBOOST_CRC_HPP
...
#if !(defined(AUTOBOOST_NO_DEPENDENT_TYPES_IN_TEMPLATE_VALUE_PARAMETERS) || (defined(AUTOBOOST_MSVC) && (AUTOBOOST_MSVC <= 1300)))
#define AUTOBOOST_CRC_PARM_TYPE  typename ::autoboost::uint_t<Bits>::fast
#else
#define AUTOBOOST_CRC_PARM_TYPE  unsigned long
#endif
...
#if defined(__GNUC__) || !defined(AUTOBOOST_NO_EXPLICIT_FUNCTION_TEMPLATE_ARGUMENTS)
#define AUTOBOOST_CRC_DUMMY_PARM_TYPE
#define AUTOBOOST_CRC_DUMMY_INIT
#define AUTOBOOST_ACRC_DUMMY_PARM_TYPE
#define AUTOBOOST_ACRC_DUMMY_INIT
#else
namespace autoboost { namespace detail {
    template < std::size_t Bits, AUTOBOOST_CRC_PARM_TYPE TruncPoly,
     AUTOBOOST_CRC_PARM_TYPE InitRem, AUTOBOOST_CRC_PARM_TYPE FinalXor,
     bool ReflectIn, bool ReflectRem >
    struct dummy_crc_argument  { };
} }
#define AUTOBOOST_CRC_DUMMY_PARM_TYPE   , detail::dummy_crc_argument<Bits, \
 TruncPoly, InitRem, FinalXor, ReflectIn, ReflectRem> *p_
#define AUTOBOOST_CRC_DUMMY_INIT        AUTOBOOST_CRC_DUMMY_PARM_TYPE = 0
#define AUTOBOOST_ACRC_DUMMY_PARM_TYPE  , detail::dummy_crc_argument<Bits, \
 TruncPoly, 0, 0, false, false> *p_
#define AUTOBOOST_ACRC_DUMMY_INIT       AUTOBOOST_ACRC_DUMMY_PARM_TYPE = 0
#endif
...
template < std::size_t Bits, AUTOBOOST_CRC_PARM_TYPE TruncPoly = 0u,
           AUTOBOOST_CRC_PARM_TYPE InitRem = 0u,
           AUTOBOOST_CRC_PARM_TYPE FinalXor = 0u, bool ReflectIn = false,
...
template < std::size_t Bits, AUTOBOOST_CRC_PARM_TYPE TruncPoly >
    typename uint_t<Bits>::fast  augmented_crc( void const *buffer,
     ...
     AUTOBOOST_ACRC_DUMMY_PARM_TYPE );
...
namespace detail
{
    ...
    template < std::size_t Bits, AUTOBOOST_CRC_PARM_TYPE TruncPoly, bool Reflect >
        struct crc_table_t;

    template < std::size_t Bits, bool DoReflect >
        class crc_helper;

    #ifndef AUTOBOOST_NO_TEMPLATE_PARTIAL_SPECIALIZATION
    template < std::size_t Bits >
...
public:
    ...
    AUTOBOOST_STATIC_CONSTANT( std::size_t, bit_count = Bits );
    AUTOBOOST_STATIC_CONSTANT( value_type, truncated_polynominal = TruncPoly );
    AUTOBOOST_STATIC_CONSTANT( value_type, initial_remainder = InitRem );
    AUTOBOOST_STATIC_CONSTANT( value_type, final_xor_value = FinalXor );
    AUTOBOOST_STATIC_CONSTANT( bool, reflect_input = ReflectIn );
    AUTOBOOST_STATIC_CONSTANT( bool, reflect_remainder = ReflectRem );
...
private:
    ...
    AUTOBOOST_STATIC_CONSTANT( bool, reflect_output = (ReflectRem != ReflectIn) );

    #ifndef __BORLANDC__
    #define AUTOBOOST_CRC_REF_OUT_VAL  reflect_output
    #else
    typedef crc_optimal  self_type;
    #define AUTOBOOST_CRC_REF_OUT_VAL  (self_type::reflect_output)
    #endif
    ...
    typedef detail::crc_helper<Bits, AUTOBOOST_CRC_REF_OUT_VAL>  reflect_out_type;

    #undef AUTOBOOST_CRC_REF_OUT_VAL

    // Member data
    value_type  rem_;

};  // autoboost::crc_optimal
...
namespace detail
{
    ...
    template < std::size_t Bits >
    struct high_uint_t
        : autoboost::uint_t< Bits >
    {
        typedef autoboost::uint_t<Bits>        base_type;
...
#else
        AUTOBOOST_STATIC_CONSTANT( least, high_bit = (least( 1u ) << ( Bits
         - 1u )) );
        AUTOBOOST_STATIC_CONSTANT( fast, high_bit_fast = (fast( 1u ) << ( Bits
         - 1u )) );
#endif

    };  // autoboost::detail::high_uint_t
    ...
    struct reflector
    {
        typedef typename autoboost::uint_t<Bits>::fast  value_type;

        static  value_type  reflect( value_type x );

    };  // autoboost::detail::reflector
        ...
        : high_uint_t< Bits >
    {
        ...
        #else
        AUTOBOOST_STATIC_CONSTANT( least, high_bit = base_type::high_bit );
        AUTOBOOST_STATIC_CONSTANT( fast, high_bit_fast = base_type::high_bit_fast );
        #endif
        ...
        AUTOBOOST_STATIC_CONSTANT( fast, sig_bits_fast = static_cast<fast>(sig_bits) );
#else
        AUTOBOOST_STATIC_CONSTANT( fast, sig_bits_fast = fast(sig_bits) );
#endif
    };  // autoboost::detail::mask_uint_t
    ...
    struct crc_table_t
    {
        AUTOBOOST_STATIC_CONSTANT( std::size_t, byte_combos = (1ul << CHAR_BIT) );
...
#define AUTOBOOST_CRC_OPTIMAL_NAME  crc_optimal<Bits, TruncPoly, InitRem, \
...
template < std::size_t Bits, AUTOBOOST_CRC_PARM_TYPE TruncPoly,
           ...
           bool ReflectIn, bool ReflectRem >
inline
AUTOBOOST_CRC_OPTIMAL_NAME::crc_optimal
(
    typename AUTOBOOST_CRC_OPTIMAL_NAME::value_type  init_rem  // = InitRem
)
...
template < std::size_t Bits, AUTOBOOST_CRC_PARM_TYPE TruncPoly,
           ...
           bool ReflectIn, bool ReflectRem >
inline
typename AUTOBOOST_CRC_OPTIMAL_NAME::value_type
AUTOBOOST_CRC_OPTIMAL_NAME::get_truncated_polynominal
(
) const
{
    return TruncPoly;
...
inline
typename AUTOBOOST_CRC_OPTIMAL_NAME::value_type
AUTOBOOST_CRC_OPTIMAL_NAME::get_initial_remainder
(
) const
{
    return InitRem;
...
inline
typename AUTOBOOST_CRC_OPTIMAL_NAME::value_type
AUTOBOOST_CRC_OPTIMAL_NAME::get_final_xor_value
(
) const
{
    return FinalXor;
...
inline
bool
AUTOBOOST_CRC_OPTIMAL_NAME::get_reflect_input
(
) const
{
    return ReflectIn;
...
inline
bool
AUTOBOOST_CRC_OPTIMAL_NAME::get_reflect_remainder
(
) const
{
    return ReflectRem;
...
inline
typename AUTOBOOST_CRC_OPTIMAL_NAME::value_type
AUTOBOOST_CRC_OPTIMAL_NAME::get_interim_remainder
(
) const
{
...
inline
void
AUTOBOOST_CRC_OPTIMAL_NAME::reset
(
    typename AUTOBOOST_CRC_OPTIMAL_NAME::value_type  new_rem  // = InitRem
)
{
...
inline
void
AUTOBOOST_CRC_OPTIMAL_NAME::process_byte
(
    unsigned char  byte
)
{
...
template < std::size_t Bits, AUTOBOOST_CRC_PARM_TYPE TruncPoly,
           ...
           bool ReflectIn, bool ReflectRem >
void
AUTOBOOST_CRC_OPTIMAL_NAME::process_block
(
    void const *  bytes_begin,
    void const *  bytes_end
)
...
inline
void
AUTOBOOST_CRC_OPTIMAL_NAME::process_bytes
(
    void const *   buffer,
    std::size_t  byte_count
)
...
inline
typename AUTOBOOST_CRC_OPTIMAL_NAME::value_type
AUTOBOOST_CRC_OPTIMAL_NAME::checksum
(
) const
{
...
inline
void
AUTOBOOST_CRC_OPTIMAL_NAME::operator ()
(
    unsigned char  byte
)
{
...
(
    void const *  buffer,
    std::size_t   byte_count
    AUTOBOOST_CRC_DUMMY_INIT
)
{
    AUTOBOOST_CRC_OPTIMAL_NAME  computer;
...
)
{
   ...
   // AUTOBOOST_ACRC_DUMMY_INIT added a third argument (for a workaround), the "0"
...
#undef AUTOBOOST_CRC_OPTIMAL_NAME
#undef AUTOBOOST_ACRC_DUMMY_INIT
#undef AUTOBOOST_ACRC_DUMMY_PARM_TYPE
#undef AUTOBOOST_CRC_DUMMY_INIT
#undef AUTOBOOST_CRC_DUMMY_PARM_TYPE
#undef AUTOBOOST_CRC_PARM_TYPE


#endif  // AUTOBOOST_CRC_HPP
