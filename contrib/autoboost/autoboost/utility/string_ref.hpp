...
#ifndef AUTOBOOST_STRING_REF_HPP
#define AUTOBOOST_STRING_REF_HPP

#include <autoboost/config.hpp>
#include <autoboost/detail/workaround.hpp>
#include <autoboost/utility/string_ref_fwd.hpp>
...
#define AUTOBOOST_STRING_REF_NO_CXX11_DEFAULTED_NOEXCEPT_FUNCTIONS
#endif
...
namespace autoboost {
    ...
    public:
        ...
        // construct/copy
        AUTOBOOST_CONSTEXPR basic_string_ref () AUTOBOOST_NOEXCEPT
            : ptr_(NULL), len_(0) {}

        // by defaulting these functions, basic_string_ref becomes
        //  trivially copy/move constructible.
        AUTOBOOST_CONSTEXPR basic_string_ref (const basic_string_ref &rhs) AUTOBOOST_NOEXCEPT
#ifndef AUTOBOOST_STRING_REF_NO_CXX11_DEFAULTED_NOEXCEPT_FUNCTIONS
            = default;
#else
            : ptr_(rhs.ptr_), len_(rhs.len_) {}
#endif

        basic_string_ref& operator=(const basic_string_ref &rhs) AUTOBOOST_NOEXCEPT
#ifndef AUTOBOOST_STRING_REF_NO_CXX11_DEFAULTED_NOEXCEPT_FUNCTIONS
            = default;
#else
            {
        ...
        basic_string_ref(const charT* str) AUTOBOOST_NOEXCEPT
...
// #endif
        ...
        AUTOBOOST_CONSTEXPR basic_string_ref(const charT* str, size_type len) AUTOBOOST_NOEXCEPT
            : ptr_(str), len_(len) {}
        ...
        // iterators
        AUTOBOOST_CONSTEXPR const_iterator   begin() const { return ptr_; }
        AUTOBOOST_CONSTEXPR const_iterator  cbegin() const { return ptr_; }
        AUTOBOOST_CONSTEXPR const_iterator     end() const { return ptr_ + len_; }
        AUTOBOOST_CONSTEXPR const_iterator    cend() const { return ptr_ + len_; }
        ...
        // capacity
        AUTOBOOST_CONSTEXPR size_type size()     const { return len_; }
        AUTOBOOST_CONSTEXPR size_type length()   const { return len_; }
        AUTOBOOST_CONSTEXPR size_type max_size() const { return len_; }
        AUTOBOOST_CONSTEXPR bool empty()         const { return len_ == 0; }

        // element access
        AUTOBOOST_CONSTEXPR const charT& operator[](size_type pos) const { return ptr_[pos]; }

        const charT& at(size_t pos) const {
            if ( pos >= len_ )
                AUTOBOOST_THROW_EXCEPTION( std::out_of_range ( "autoboost::string_ref::at" ) );
            return ptr_[pos];
            }

        AUTOBOOST_CONSTEXPR const charT& front() const { return ptr_[0]; }
        AUTOBOOST_CONSTEXPR const charT& back()  const { return ptr_[len_-1]; }
        AUTOBOOST_CONSTEXPR const charT* data()  const { return ptr_; }
        ...
        basic_string_ref substr(size_type pos, size_type n=npos) const {
            if ( pos > size())
                AUTOBOOST_THROW_EXCEPTION( std::out_of_range ( "string_ref::substr" ) );
...
namespace std {
    // Hashing
    template<> struct hash<autoboost::string_ref>;
    template<> struct hash<autoboost::u16string_ref>;
    template<> struct hash<autoboost::u32string_ref>;
    template<> struct hash<autoboost::wstring_ref>;
}
#endif
