...
#ifndef AUTOBOOST_INTRUSIVE_SLIST_HPP
#define AUTOBOOST_INTRUSIVE_SLIST_HPP
...
#include <autoboost/intrusive/slist_hook.hpp>
#include <autoboost/intrusive/circular_slist_algorithms.hpp>
...
#include <autoboost/intrusive/detail/slist_iterator.hpp>
#include <autoboost/intrusive/detail/array_initializer.hpp>
#include <autoboost/intrusive/detail/exception_disposer.hpp>
#include <autoboost/intrusive/detail/equal_to_value.hpp>
...
class slist_impl
{
   ...
   //noncopyable
   AUTOBOOST_MOVABLE_BUT_NOT_COPYABLE(slist_impl)
   ...
   AUTOBOOST_STATIC_ASSERT(!(linear && ((int)value_traits::link_mode == (int)auto_unlink)));
   //A list with cached last node is incompatible with auto-unlink hooks!
   AUTOBOOST_STATIC_ASSERT(!(cache_last && ((int)value_traits::link_mode == (int)auto_unlink)));
   ...
   static node_ptr get_last_node(detail::bool_<false>)
   {
      ...
      AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(cache_last);
      return node_ptr();
   }
   ...
   //! <b>Effects</b>: to-do
   //!
   slist_impl(AUTOBOOST_RV_REF(slist_impl) x)
      : data_(::autoboost::move(x.priv_value_traits()))
   {
      this->priv_size_traits().set_size(size_type(0));
      node_algorithms::init_header(this->get_root_node());
      //nothrow, no need to rollback to release elements on exception
   ...
   //! <b>Effects</b>: to-do
   //!
   slist_impl& operator=(AUTOBOOST_RV_REF(slist_impl) x)
   ...
   void push_front(reference value)
   {
      ...
      if(safemode_or_autounlink)
         AUTOBOOST_INTRUSIVE_SAFE_HOOK_DEFAULT_ASSERT(node_algorithms::inited(to_insert));
      if(cache_last){
         if(this->empty()){
   ...
   {
      AUTOBOOST_STATIC_ASSERT((cache_last));
      node_ptr n = priv_value_traits().to_node_ptr(value);
      if(safemode_or_autounlink)
         AUTOBOOST_INTRUSIVE_SAFE_HOOK_DEFAULT_ASSERT(node_algorithms::inited(n));
   ...
   {
      AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(node_algorithms::distance((++const_iterator(before_f)).pointed_node(), l.pointed_node()) == n);
      if(safemode_or_autounlink){
   ...
   {
      AUTOBOOST_STATIC_ASSERT(((!cache_last)&&(!constant_time_size)&&(!stateful_value_traits)));
      const_iterator it(prev);
      ++it;
      node_ptr to_erase(it.pointed_node());
      ++it;
   ...
   {
      AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(node_algorithms::distance(before_f.pointed_node(), before_l.pointed_node()) == n);
      this->priv_splice_after
         (prev_pos.pointed_node(), x, before_f.pointed_node(), before_l.pointed_node());
      if(constant_time_size){
   ...
   void sort(Predicate p)
   {
      if (node_traits::get_next(node_traits::get_next(this->get_root_node()))
         ...
         while(!this->empty()){
            ...
            AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(counter[i].empty());
   ...
   {
      AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(linear || !node_algorithms::inited(this->priv_value_traits().to_node_ptr(value)));
   ...
   {
      reference r =*pointer_traits<pointer>::const_cast_from(pointer_traits<const_pointer>::pointer_to(value));
      AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT (linear || !node_algorithms::inited(this->priv_value_traits().to_node_ptr(r)));
   ...
   {
      if(n){
         AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(n > 0);
   ...
   void check() const
   {
      ...
      AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(node_traits::get_next(header_ptr));
      if (node_traits::get_next(header_ptr) == header_ptr)
      {
         if (constant_time_size)
            AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(this->priv_size_traits().get_size() == 0);
         return;
      }
      size_t node_count = 0;
      ...
      while (true)
      {
         ...
         if (!linear)
         {
            AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(next_p);
         }
         else
         {
            AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(next_p != header_ptr);
         }
         if ((!linear && next_p == header_ptr) || (linear && !next_p))
         {
            if (cache_last)
               AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(get_last_node() == p);
            break;
         }
         p = next_p;
         ++node_count;
      ...
      if (constant_time_size)
         AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(this->priv_size_traits().get_size() == node_count);
   }
   ...
   static slist_impl &priv_container_from_end_iterator(const const_iterator &end_iterator)
   {
      ...
      AUTOBOOST_STATIC_ASSERT(!linear);
      ...
      >::type
{
   ...
   AUTOBOOST_STATIC_ASSERT((detail::is_same<typename Base::value_traits::value_type, T>::value));
   AUTOBOOST_MOVABLE_BUT_NOT_COPYABLE(slist)
   ...
   {  return static_cast<slist &>(this->Base::operator=(::autoboost::move(static_cast<Base&>(x))));  }
...
#endif //AUTOBOOST_INTRUSIVE_SLIST_HPP
