...
//
/////////////////////////////////////////////////////////////////////////////
#ifndef AUTOBOOST_INTRUSIVE_BSTREE_HPP
#define AUTOBOOST_INTRUSIVE_BSTREE_HPP
...
#include <autoboost/intrusive/intrusive_fwd.hpp>
#include <autoboost/intrusive/bs_set_hook.hpp>
#include <autoboost/intrusive/detail/tree_node.hpp>
#include <autoboost/intrusive/detail/tree_iterator.hpp>
...
#include <autoboost/intrusive/detail/is_stateful_value_traits.hpp>
#include <autoboost/intrusive/detail/empty_node_checker.hpp>
#include <autoboost/intrusive/detail/default_header_holder.hpp>
#include <autoboost/intrusive/detail/reverse_iterator.hpp>
#include <autoboost/intrusive/detail/exception_disposer.hpp>
#include <autoboost/intrusive/detail/node_cloner_disposer.hpp>
#include <autoboost/intrusive/detail/key_nodeptr_comp.hpp>
#include <autoboost/intrusive/detail/simple_disposers.hpp>
#include <autoboost/intrusive/detail/size_holder.hpp>
...
#include <autoboost/intrusive/parent_from_member.hpp>
...
struct bstbase3
{
   ...
   typedef autoboost::intrusive::detail::reverse_iterator<iterator>                                 reverse_iterator;
   typedef autoboost::intrusive::detail::reverse_iterator<const_iterator>                           const_reverse_iterator;
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(typename value_traits::pointer)                               pointer;
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(typename value_traits::const_pointer)                         const_pointer;
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(typename pointer_traits<pointer>::element_type)               value_type;
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(value_type)                                                   key_type;
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(typename pointer_traits<pointer>::reference)                  reference;
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(typename pointer_traits<const_pointer>::reference)            const_reference;
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(typename pointer_traits<const_pointer>::difference_type)      difference_type;
   ...
   {
      AUTOBOOST_STATIC_ASSERT(has_container_from_iterator);
   ...
   typedef typename autoboost::intrusive::value_traits_pointers
   ...
   {
      AUTOBOOST_STATIC_ASSERT((!stateful_value_traits));
   ...
   , public bstbase3<ValueTraits, AlgoType, HeaderHolder>
{
   ...
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(value_compare)                     key_compare;
   ...
   :  public bstbase<ValueTraits, VoidKeyComp, ConstantTimeSize, SizeType, AlgoType, HeaderHolder>
{
   ...
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(ValueTraits)                                                  value_traits;
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(typename value_traits::pointer)                               pointer;
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(typename value_traits::const_pointer)                         const_pointer;
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(typename pointer_traits<pointer>::element_type)               value_type;
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(value_type)                                                   key_type;
   ...
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(typename pointer_traits<const_pointer>::reference)            const_reference;
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(typename pointer_traits<const_pointer>::difference_type)      difference_type;
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(SizeType)                                                     size_type;
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(typename data_type::value_compare)                            value_compare;
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(value_compare)                                                key_compare;
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(iterator_type)                                                iterator;
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(const_iterator_type)                                          const_iterator;
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(autoboost::intrusive::detail::reverse_iterator<iterator>)         reverse_iterator;
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(autoboost::intrusive::detail::reverse_iterator<const_iterator>)   const_reverse_iterator;
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(typename value_traits::node_traits)                           node_traits;
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(typename node_traits::node)                                   node;
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(typename node_traits::node_ptr)                               node_ptr;
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(typename node_traits::const_node_ptr)                         const_node_ptr;
   /// @cond
   typedef typename get_algo<AlgoType, node_traits>::type                                       algo_type;
   /// @endcond
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(algo_type)                                                    node_algorithms;
   ...
   //noncopyable
   AUTOBOOST_MOVABLE_BUT_NOT_COPYABLE(bstree_impl)

   static const bool safemode_or_autounlink = is_safe_autounlink<value_traits::link_mode>::value;

   //Constant-time size is incompatible with auto-unlink hooks!
   AUTOBOOST_STATIC_ASSERT(!(constant_time_size && ((int)value_traits::link_mode == (int)auto_unlink)));
   ...
   //! <b>Effects</b>: to-do
   //!
   bstree_impl(AUTOBOOST_RV_REF(bstree_impl) x)
      : data_type(::autoboost::move(x.comp()), ::autoboost::move(x.get_value_traits()))
   {
      this->swap(x);
   }
   ...
   //! <b>Effects</b>: to-do
   //!
   bstree_impl& operator=(AUTOBOOST_RV_REF(bstree_impl) x)
   ...
   iterator insert_equal(reference value)
   {
      ...
      if(safemode_or_autounlink)
         AUTOBOOST_INTRUSIVE_SAFE_HOOK_DEFAULT_ASSERT(node_algorithms::unique(to_insert));
   ...
   iterator erase(const_iterator i)
   {
      ...
      if(safemode_or_autounlink)
         AUTOBOOST_INTRUSIVE_SAFE_HOOK_DEFAULT_ASSERT(!node_algorithms::unique(to_erase));
   ...
   #else //defined(AUTOBOOST_INTRUSIVE_DOXYGEN_INVOKED)
   ...
   {
      AUTOBOOST_STATIC_ASSERT((!constant_time_size));
   ...
   void check(ExtraChecker extra_checker) const
   {
      ...
      if (constant_time_size)
         AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(this->sz_traits().get_size() == checker_return.node_count);
   }
      ...
      >::type
{
   typedef typename make_bstree
      ...
      #endif
      >::type   Base;
   AUTOBOOST_MOVABLE_BUT_NOT_COPYABLE(bstree)
   ...
   bstree(AUTOBOOST_RV_REF(bstree) x)
      :  Base(::autoboost::move(static_cast<Base&>(x)))
   {}

   bstree& operator=(AUTOBOOST_RV_REF(bstree) x)
   {  return static_cast<bstree &>(this->Base::operator=(::autoboost::move(static_cast<Base&>(x))));  }
...
#endif //AUTOBOOST_INTRUSIVE_BSTREE_HPP
