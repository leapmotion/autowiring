...
#ifndef AUTOBOOST_INTRUSIVE_LIST_HPP
#define AUTOBOOST_INTRUSIVE_LIST_HPP
...
#include <autoboost/intrusive/list_hook.hpp>
#include <autoboost/intrusive/circular_list_algorithms.hpp>
...
#include <autoboost/intrusive/detail/list_iterator.hpp>
...
class list_impl
{
   ...
   //noncopyable
   AUTOBOOST_MOVABLE_BUT_NOT_COPYABLE(list_impl)
   ...
   //! <b>Effects</b>: to-do
   //!
   list_impl(AUTOBOOST_RV_REF(list_impl) x)
   ...
   //! <b>Effects</b>: to-do
   //!
   list_impl& operator=(AUTOBOOST_RV_REF(list_impl) x)
   ...
   {
      AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(node_algorithms::distance(b.pointed_node(), e.pointed_node()) == n);
      if(safemode_or_autounlink || constant_time_size){
         return this->erase_and_dispose(b, e, detail::null_disposer());
      }
      else{
   ...
   void splice(const_iterator p, list_impl&x, const_iterator f, const_iterator e, size_type n)
   {
      if(n){
         if(constant_time_size){
            AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(n == node_algorithms::distance(f.pointed_node(), e.pointed_node()));
   ...
   static iterator s_iterator_to(reference value)
   {
      ...
      AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(!node_algorithms::inited(value_traits::to_node_ptr(value)));
   ...
   static const_iterator s_iterator_to(const_reference value)
   {
      ...
      AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(!node_algorithms::inited(value_traits::to_node_ptr(r)));
   ...
   {
      AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(!node_algorithms::inited(this->priv_value_traits().to_node_ptr(value)));
   ...
   {
      reference r = *pointer_traits<pointer>::const_cast_from(pointer_traits<const_pointer>::pointer_to(value));
      AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(!node_algorithms::inited(this->priv_value_traits().to_node_ptr(r)));
   ...
   void check() const
   {
      ...
      AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(node_traits::get_previous(header_ptr));
      // header's next and prev either both point to header (empty list) or neither does
      AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT((node_traits::get_next(header_ptr) == header_ptr)
      ...
      while (true)
      {
         ...
         AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(node_traits::get_previous(next_p) == p);
         p = next_p;
   ...
   >::type
{
   ...
   AUTOBOOST_MOVABLE_BUT_NOT_COPYABLE(list)
   ...
   {  return static_cast<list &>(this->Base::operator=(::autoboost::move(static_cast<Base&>(x))));  }
...
#endif //AUTOBOOST_INTRUSIVE_LIST_HPP
