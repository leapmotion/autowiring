...
#ifndef AUTOBOOST_INTRUSIVE_SGTREE_HPP
#define AUTOBOOST_INTRUSIVE_SGTREE_HPP
...
#include <autoboost/intrusive/detail/math.hpp>
#include <autoboost/intrusive/detail/get_value_traits.hpp>
#include <autoboost/intrusive/sgtree_algorithms.hpp>
...
struct alpha_holder
{
   typedef autoboost::intrusive::detail::h_alpha_t           h_alpha_t;
   typedef autoboost::intrusive::detail::alpha_by_max_size_t multiply_by_alpha_t;
...
struct alpha_holder<false, SizeType>
{
   ...
   typedef autoboost::intrusive::detail::h_alpha_sqrt2_t           h_alpha_t;
   typedef autoboost::intrusive::detail::alpha_0_75_by_max_size_t  multiply_by_alpha_t;
   ...
   {  //alpha CAN't be changed.
      AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(0);
   }
   ...
   /// @endcond
{
   ...
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(sgtree_algorithms<node_traits>)    node_algorithms;
   ...
   AUTOBOOST_MOVABLE_BUT_NOT_COPYABLE(sgtree_impl)
   AUTOBOOST_STATIC_ASSERT(((int)value_traits::link_mode != (int)auto_unlink));
   ...
   typedef AUTOBOOST_INTRUSIVE_IMPDEF(typename node_algorithms::insert_commit_data) insert_commit_data;
   ...
   sgtree_impl(AUTOBOOST_RV_REF(sgtree_impl) x)
      :  tree_type(::autoboost::move(static_cast<tree_type&>(x))), alpha_traits(x.get_alpha_traits())
   {  std::swap(this->get_alpha_traits(), x.get_alpha_traits());   }

   //! @copydoc ::autoboost::intrusive::bstree::operator=(bstree &&)
   sgtree_impl& operator=(AUTOBOOST_RV_REF(sgtree_impl) x)
   {
      this->get_alpha_traits() = x.get_alpha_traits();
      return static_cast<sgtree_impl&>(tree_type::operator=(::autoboost::move(static_cast<tree_type&>(x))));
   }
   ...
   {
      AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT((new_alpha > 0.5f && new_alpha < 1.0f));
      ...
      AUTOBOOST_STATIC_ASSERT((floating_point));
      ...
      >::type
{
   typedef typename make_sgtree
      ...
      #endif
      >::type   Base;
   AUTOBOOST_MOVABLE_BUT_NOT_COPYABLE(sgtree)
   ...
   sgtree(AUTOBOOST_RV_REF(sgtree) x)
      :  Base(::autoboost::move(static_cast<Base&>(x)))
   {}

   sgtree& operator=(AUTOBOOST_RV_REF(sgtree) x)
   {  return static_cast<sgtree &>(this->Base::operator=(::autoboost::move(static_cast<Base&>(x))));  }
...
#endif //AUTOBOOST_INTRUSIVE_SGTREE_HPP
