...
#ifndef AUTOBOOST_INTRUSIVE_RBTREE_ALGORITHMS_HPP
#define AUTOBOOST_INTRUSIVE_RBTREE_ALGORITHMS_HPP
                     ...
                     return_type& check_return)
   {
      if (node_traits::get_color(p) == node_traits::red())
      {
         if (node_traits::get_left(p))
            AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(node_traits::get_color(node_traits::get_left(p)) == node_traits::black());
         if (node_traits::get_right(p))
            AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(node_traits::get_color(node_traits::get_right(p)) == node_traits::black());
      }
...
#endif   //#ifndef AUTOBOOST_INTRUSIVE_DOXYGEN_INVOKED
   ...
   static void rebalance_after_erasure(const node_ptr & header, node_ptr x, node_ptr x_parent)
   {
      while(1){
         ...
         if(x == x_parent_left){ //x is left child
            ...
            AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(w);
...
#endif //AUTOBOOST_INTRUSIVE_RBTREE_ALGORITHMS_HPP
