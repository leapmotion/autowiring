...
#ifndef AUTOBOOST_INTRUSIVE_BSTREE_ALGORITHMS_HPP
#define AUTOBOOST_INTRUSIVE_BSTREE_ALGORITHMS_HPP
   ...
   : public ExtraChecker
{
                     ...
                     return_type& check_return)
   {
      if (check_return_left.max_key_node_ptr)
         AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(!comp_(p, check_return_left.max_key_node_ptr));
      if (check_return_right.min_key_node_ptr)
         AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(!comp_(check_return_right.min_key_node_ptr, p));
...
class bstree_algorithms
{
   ...
   static void swap_nodes(const node_ptr & node1, const node_ptr & header1, const node_ptr & node2, const node_ptr & header2)
   {
      ...
      //AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT((header1 != node1 && header2 != node2));
      if(header1 != header2){
      ...
      , bool right_closed)
   {
      ...
      while(x){
         ...
         if(comp(x, lower_key)){
            ...
            AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(comp(x, upper_key));
         ...
         //range is on the left part
            ...
            AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(left_closed || right_closed || comp(lower_key, x) || comp(x, upper_key));
   ...
   static node_ptr rebalance_subtree(const node_ptr & old_root)
   {
      ...
      AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(super_root);
   ...
   static void check(const const_node_ptr& header, Checker checker, typename Checker::return_type& checker_return)
   {
       ...
       if (!root_node_ptr)
       {
           ...
           AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(NodeTraits::get_left(header) == header);
           AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(NodeTraits::get_right(header) == header);
       }
       else
       {
           // check parent pointer of root node
           AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(NodeTraits::get_parent(root_node_ptr) == header);
           ...
           AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(NodeTraits::get_left(header) == p);
           p = root_node_ptr;
           while (NodeTraits::get_right(p)) { p = NodeTraits::get_right(p); }
           AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(NodeTraits::get_right(header) == p);
       }
   }
   ...
   static void erase(const node_ptr & header, const node_ptr & z, data_for_rebalance &info)
   {
      ...
      if(y != z){ //has two children and y is the minimum of z
         ...
         if(y != z_right){
            ...
            AUTOBOOST_ASSERT(NodeTraits::get_left(x_parent) == y);
            if(x)
      ...
      AUTOBOOST_ASSERT(!x || NodeTraits::get_parent(x) == x_parent);
      ...
      (const node_ptr & header, const node_ptr & new_node, const insert_commit_data &commit_data)
   {
      ...
      AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(commit_data.node != node_ptr());
   ...
   {
      AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT((!inited(node)));
      node_ptr x = NodeTraits::get_parent(node);
      if(x){
         while(!is_header(x)){
            x = NodeTraits::get_parent(x);
   ...
   static void check_subtree(const const_node_ptr& node, Checker checker, typename Checker::return_type& check_return)
   {
      ...
      if (left)
      {
         AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(NodeTraits::get_parent(left) == node);
         check_subtree(left, checker, check_return_left);
      }
      if (right)
      {
         AUTOBOOST_INTRUSIVE_INVARIANT_ASSERT(NodeTraits::get_parent(right) == node);
...
#endif //AUTOBOOST_INTRUSIVE_BSTREE_ALGORITHMS_HPP
