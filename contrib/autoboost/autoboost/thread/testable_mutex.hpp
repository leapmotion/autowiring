...
#ifndef AUTOBOOST_THREAD_TESTABLE_LOCKABLE_HPP
#define AUTOBOOST_THREAD_TESTABLE_LOCKABLE_HPP
...
namespace autoboost
{
   ...
   *
   * Many mutex services (including autoboost::mutex) don't provide a way to ask,
  ...
  public:
    ...
    /// Non copyable
    AUTOBOOST_THREAD_NO_COPYABLE(testable_mutex)

    testable_mutex() : id_(thread::id()) {}

    void lock()
    {
      AUTOBOOST_ASSERT(! is_locked_by_this_thread());
      mtx_.lock();
