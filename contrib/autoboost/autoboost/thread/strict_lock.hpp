...
#ifndef AUTOBOOST_THREAD_STRICT_LOCK_HPP
#define AUTOBOOST_THREAD_STRICT_LOCK_HPP
...
namespace autoboost
{
  ...
  class strict_lock
  {
    ...
    AUTOBOOST_CONCEPT_ASSERT(( BasicLockable<Lockable> ));
  public:
    typedef Lockable mutex_type;

    // construct/copy/destroy:

    AUTOBOOST_THREAD_NO_COPYABLE( strict_lock)
    ...
    //AUTOBOOST_ADRESS_OF_DELETE(strict_lock) /*< disable aliasing >*/
    //AUTOBOOST_HEAP_ALLOCATION_DELETE(strict_lock) /*< disable heap allocation >*/
  ...
  {
    AUTOBOOST_CONCEPT_ASSERT(( BasicLock<Lock> )); /*< The Lock must be a movable lock >*/
  public:
    typedef typename Lock::mutex_type mutex_type; /*< Name the lockable type locked by Lock >*/

    AUTOBOOST_THREAD_NO_COPYABLE( nested_strict_lock)
     ...
     * __Throws:
     *
     * - lock_error when AUTOBOOST_THREAD_THROW_IF_PRECONDITION_NOT_SATISFIED is defined and lk.mutex() == null_ptr
     *
    ...
    {
      /*< Define AUTOBOOST_THREAD_DONT_CHECK_PRECONDITIONS if you don't want to check lk ownership >*/
      AUTOBOOST_THREAD_ASSERT_PRECONDITION(  lk.mutex() != 0,
          lock_error()
      );
    ...
    {
      /*< Define AUTOBOOST_THREAD_DONT_CHECK_PRECONDITIONS if you don't want to check lk ownership >*/
      AUTOBOOST_THREAD_ASSERT_PRECONDITION(  lk_.mutex() != 0,
          lock_error()
      );
     ...
     */
    ~nested_strict_lock()AUTOBOOST_NOEXCEPT
    {
     ...
     */
    bool owns_lock(mutex_type const* l) const AUTOBOOST_NOEXCEPT
    {
      return l == mutex();
    }

    //AUTOBOOST_ADRESS_OF_DELETE(nested_strict_lock)
    //AUTOBOOST_HEAP_ALLOCATEION_DELETE(nested_strict_lock)
...
#if ! defined AUTOBOOST_THREAD_NO_MAKE_STRICT_LOCK
  template <typename Lockable>
