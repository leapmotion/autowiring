...
#ifndef AUTOBOOST_THREAD_LOCK_CONCEPTS_HPP
#define AUTOBOOST_THREAD_LOCK_CONCEPTS_HPP
...
namespace autoboost
{
  ...
  struct BasicLock
  {
    ...
    AUTOBOOST_CONCEPT_ASSERT(( BasicLockable<mutex_type> ));

    AUTOBOOST_CONCEPT_USAGE(BasicLock)
    {
      const Lk l1(mtx);
      Lk l2(mtx, defer_lock);
      Lk l3(mtx, adopt_lock);
      Lk l4(( Lk()));
      Lk l5(( autoboost::move(l2)));
  ...
  struct Lock
  {
    AUTOBOOST_CONCEPT_ASSERT(( BasicLock<Lk> ));
    typedef typename Lk::mutex_type mutex_type;
    AUTOBOOST_CONCEPT_ASSERT(( Lockable<mutex_type> ));

    AUTOBOOST_CONCEPT_USAGE(Lock)
    {
  ...
  struct TimedLock
  {
    AUTOBOOST_CONCEPT_ASSERT(( Lock<Lk> ));
    typedef typename Lk::mutex_type mutex_type;
    AUTOBOOST_CONCEPT_ASSERT(( TimedLockable<mutex_type> ));

    AUTOBOOST_CONCEPT_USAGE(TimedLock)
    {
      const Lk l1(mtx, t);
      Lk l2(mtx, d);
  ...
  private:
    ...
    mutex_type& mtx;
    autoboost::chrono::system_clock::time_point t;
    autoboost::chrono::system_clock::duration d;
  };

  template <typename Lk>
  struct UniqueLock
  {
    AUTOBOOST_CONCEPT_ASSERT(( TimedLock<Lk> ));
    typedef typename Lk::mutex_type mutex_type;

    AUTOBOOST_CONCEPT_USAGE(UniqueLock)
    {
  ...
  struct SharedLock
  {
    ...
    AUTOBOOST_CONCEPT_USAGE(SharedLock)
    {
    }
    SharedLock() :
      mtx(*static_cast<mutex_type*>(0))
  ...
  struct UpgradeLock
  {
    AUTOBOOST_CONCEPT_ASSERT(( SharedLock<Lk> ));
    typedef typename Lk::mutex_type mutex_type;

    AUTOBOOST_CONCEPT_USAGE(UpgradeLock)
    {
    }
    UpgradeLock() :
      mtx(*static_cast<mutex_type*>(0))
  ...
  struct StrictLock
  {
    typedef typename Lk::mutex_type mutex_type;
    AUTOBOOST_CONCEPT_ASSERT(( BasicLockable<mutex_type> ));
    AUTOBOOST_STATIC_ASSERT(( is_strict_lock<Lk>::value ));

    AUTOBOOST_CONCEPT_USAGE( StrictLock)
    {
