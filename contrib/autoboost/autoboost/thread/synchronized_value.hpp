...
#ifndef AUTOBOOST_THREAD_SYNCHRONIZED_VALUE_HPP
#define AUTOBOOST_THREAD_SYNCHRONIZED_VALUE_HPP
...
#include <autoboost/thread/lock_factories.hpp>
...
//#include <autoboost/thread/detail/is_nothrow_default_constructible.hpp>
//#if ! defined AUTOBOOST_NO_CXX11_HDR_TYPE_TRAITS
//#include <type_traits>
//#endif

#if ! defined(AUTOBOOST_THREAD_NO_SYNCHRONIZE)
#include <tuple> // todo change to <autoboost/tuple.hpp> once Boost.Tuple or Boost.Fusion provides Move semantics on C++98 compilers.
#include <functional>
#endif
...
namespace autoboost
{
  ...
  protected:
    ...
    autoboost::unique_lock<mutex_type> lk_;
    T const& value_;

  public:
    AUTOBOOST_THREAD_MOVABLE_ONLY( const_strict_lock_ptr )
    ...
    const_strict_lock_ptr(T const& val, Lockable & mtx, adopt_lock_t tag) AUTOBOOST_NOEXCEPT :
     ...
     */
    const_strict_lock_ptr(AUTOBOOST_THREAD_RV_REF(const_strict_lock_ptr) other) AUTOBOOST_NOEXCEPT
    : lk_(autoboost::move(AUTOBOOST_THREAD_RV(other).lk_)),value_(AUTOBOOST_THREAD_RV(other).value_)
    {
    }
  ...
  public:
    AUTOBOOST_THREAD_MOVABLE_ONLY( strict_lock_ptr )
     ...
     * @effects takes ownership of the mutex owned by @c other, stores a reference to the mutex and the value type of @c other.
     */
    strict_lock_ptr(AUTOBOOST_THREAD_RV_REF(strict_lock_ptr) other)
    : base_type(autoboost::move(static_cast<base_type&>(other)))
    {
    }
  ...
  public:
    AUTOBOOST_THREAD_MOVABLE_ONLY(const_unique_lock_ptr)
     ...
     */
    const_unique_lock_ptr(T const& val, Lockable & mtx, adopt_lock_t) AUTOBOOST_NOEXCEPT
     ...
     */
    const_unique_lock_ptr(T const& val, Lockable & mtx, defer_lock_t) AUTOBOOST_NOEXCEPT
     ...
     */
    const_unique_lock_ptr(T const& val, Lockable & mtx, try_to_lock_t) AUTOBOOST_NOEXCEPT
     ...
     * @effects takes ownership of the mutex owned by @c other, stores a reference to the mutex and the value type of @c other.
     */
    const_unique_lock_ptr(AUTOBOOST_THREAD_RV_REF(const_unique_lock_ptr) other) AUTOBOOST_NOEXCEPT
    : base_type(autoboost::move(static_cast<base_type&>(other))), value_(AUTOBOOST_THREAD_RV(other).value_)
    {
    }
    ...
    const T* operator->() const
    {
      AUTOBOOST_ASSERT (this->owns_lock());
      return &value_;
    }
  ...
  public:
    ...
    AUTOBOOST_THREAD_MOVABLE_ONLY(unique_lock_ptr)
     ...
     * @effects stores a reference to it and to the value type @c value taking ownership.
     */
    unique_lock_ptr(T & value, Lockable & mtx, adopt_lock_t) AUTOBOOST_NOEXCEPT
     ...
     * @effects stores a reference to it and to the value type @c value c.
     */
    unique_lock_ptr(T & value, Lockable & mtx, defer_lock_t) AUTOBOOST_NOEXCEPT
     ...
     * @effects try to lock the mutex @c mtx, stores a reference to it and to the value type @c value.
     */
    unique_lock_ptr(T & value, Lockable & mtx, try_to_lock_t) AUTOBOOST_NOEXCEPT
     ...
     * @effects takes ownership of the mutex owned by @c other, stores a reference to the mutex and the value type of @c other.
     */
    unique_lock_ptr(AUTOBOOST_THREAD_RV_REF(unique_lock_ptr) other) AUTOBOOST_NOEXCEPT
    : base_type(autoboost::move(static_cast<base_type&>(other)))
    {
    }
  ...
  public:
     ...
     */
    synchronized_value()
    //AUTOBOOST_NOEXCEPT_IF(is_nothrow_default_constructible<T>::value)
    : value_()
    {
    }
    ...
    //AUTOBOOST_NOEXCEPT_IF(is_nothrow_copy_constructible<T>::value)
    : value_(other)
    {
    }
     ...
     */
    synchronized_value(AUTOBOOST_THREAD_RV_REF(T) other)
    //AUTOBOOST_NOEXCEPT_IF(is_nothrow_move_constructible<T>::value)
    : value_(autoboost::move(other))
    {
    }
     ...
     *
     */
    synchronized_value(AUTOBOOST_THREAD_RV_REF(synchronized_value) other)
    {
      strict_lock<mutex_type> lk(AUTOBOOST_THREAD_RV(other).mtx_);
      value_= autoboost::move(AUTOBOOST_THREAD_RV(other).value_);
    }
    ...
    void swap(synchronized_value & rhs)
    {
      ...
      unique_lock<mutex_type> lk2(rhs.mtx_, defer_lock);
      lock(lk1,lk2);
      autoboost::swap(value_, rhs.value_);
    }
    /**
    ...
    {
      return AUTOBOOST_THREAD_MAKE_RV_REF((strict_lock_ptr<T,Lockable>(value_, mtx_)));
    }
    /**
    ...
    {
      return AUTOBOOST_THREAD_MAKE_RV_REF((const_strict_lock_ptr<T,Lockable>(value_, mtx_)));
    }
    ...
    template <typename F>
    inline
    typename autoboost::result_of<F(value_type&)>::type
    operator()(AUTOBOOST_THREAD_RV_REF(F) fct)
    {
      strict_lock<mutex_type> lk(mtx_);
      return fct(value_);
    }
    template <typename F>
    inline
    typename autoboost::result_of<F(value_type const&)>::type
    operator()(AUTOBOOST_THREAD_RV_REF(F) fct) const
    {
      strict_lock<mutex_type> lk(mtx_);
      return fct(value_);
    }
    ...
    {
      return AUTOBOOST_THREAD_MAKE_RV_REF((unique_lock_ptr<T,Lockable>(value_, mtx_)));
    }
    const_unique_lock_ptr<T,Lockable> unique_synchronize() const
    {
      return AUTOBOOST_THREAD_MAKE_RV_REF((const_unique_lock_ptr<T,Lockable>(value_, mtx_)));
    }
    unique_lock_ptr<T,Lockable> unique_synchronize(defer_lock_t tag)
    {
      return AUTOBOOST_THREAD_MAKE_RV_REF((unique_lock_ptr<T,Lockable>(value_, mtx_, tag)));
    }
    const_unique_lock_ptr<T,Lockable> unique_synchronize(defer_lock_t tag) const
    {
      return AUTOBOOST_THREAD_MAKE_RV_REF((const_unique_lock_ptr<T,Lockable>(value_, mtx_, tag)));
    }
    unique_lock_ptr<T,Lockable> defer_synchronize() AUTOBOOST_NOEXCEPT
    {
      return AUTOBOOST_THREAD_MAKE_RV_REF((unique_lock_ptr<T,Lockable>(value_, mtx_, defer_lock)));
    }
    const_unique_lock_ptr<T,Lockable> defer_synchronize() const AUTOBOOST_NOEXCEPT
    {
      return AUTOBOOST_THREAD_MAKE_RV_REF((const_unique_lock_ptr<T,Lockable>(value_, mtx_, defer_lock)));
    }
    unique_lock_ptr<T,Lockable> try_to_synchronize() AUTOBOOST_NOEXCEPT
    {
      return AUTOBOOST_THREAD_MAKE_RV_REF((unique_lock_ptr<T,Lockable>(value_, mtx_, try_to_lock)));
    }
    const_unique_lock_ptr<T,Lockable> try_to_synchronize() const AUTOBOOST_NOEXCEPT
    {
      return AUTOBOOST_THREAD_MAKE_RV_REF((const_unique_lock_ptr<T,Lockable>(value_, mtx_, try_to_lock)));
    }
    unique_lock_ptr<T,Lockable> adopt_synchronize() AUTOBOOST_NOEXCEPT
    {
      return AUTOBOOST_THREAD_MAKE_RV_REF((unique_lock_ptr<T,Lockable>(value_, mtx_, adopt_lock)));
    }
    const_unique_lock_ptr<T,Lockable> adopt_synchronize() const AUTOBOOST_NOEXCEPT
    {
      return AUTOBOOST_THREAD_MAKE_RV_REF((const_unique_lock_ptr<T,Lockable>(value_, mtx_, adopt_lock)));
    }
    ...
    public:
      AUTOBOOST_THREAD_MOVABLE_ONLY(deref_value)

      deref_value(AUTOBOOST_THREAD_RV_REF(deref_value) other):
      lk_(autoboost::move(AUTOBOOST_THREAD_RV(other).lk_)),value_(AUTOBOOST_THREAD_RV(other).value_)
      {}
      operator T&()
      {
    ...
    public:
      AUTOBOOST_THREAD_MOVABLE_ONLY(const_deref_value)

      const_deref_value(AUTOBOOST_THREAD_RV_REF(const_deref_value) other):
      lk_(autoboost::move(AUTOBOOST_THREAD_RV(other).lk_)), value_(AUTOBOOST_THREAD_RV(other).value_)
      {}
  ...
  public:
    ...
    {
      return AUTOBOOST_THREAD_MAKE_RV_REF(deref_value(*this));
    }

    const_deref_value operator*() const
    {
      return AUTOBOOST_THREAD_MAKE_RV_REF(const_deref_value(*this));
    }
...
//  struct hash<synchronized_value<T,L> >;
  ...
  synchronize(SV1& sv1, SV2& sv2)
  {
    autoboost::lock(sv1.mtx_, sv2.mtx_);
    typedef std::tuple<
        typename synchronized_value_strict_lock_ptr<SV1>::type,
        typename synchronized_value_strict_lock_ptr<SV2>::type
        > t_type;
