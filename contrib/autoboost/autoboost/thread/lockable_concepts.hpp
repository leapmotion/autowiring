...
#ifndef AUTOBOOST_THREAD_LOCKABLE_CONCEPTS_HPP
#define AUTOBOOST_THREAD_LOCKABLE_CONCEPTS_HPP
...
namespace autoboost
{
  ...
  struct BasicLockable
  {
    ...
    AUTOBOOST_CONCEPT_USAGE(BasicLockable)
    {
      l.lock();
      l.unlock();
    }
  ...
  struct Lockable
  {
    AUTOBOOST_CONCEPT_ASSERT(( BasicLockable<Mutex> ));

    AUTOBOOST_CONCEPT_USAGE(Lockable)
    {
  ...
  {
    AUTOBOOST_CONCEPT_ASSERT(( Lockable<Mutex> ));

    AUTOBOOST_CONCEPT_USAGE(TimedLockable)
    {
  ...
  {
    AUTOBOOST_CONCEPT_ASSERT(( TimedLockable<Mutex> ));

    AUTOBOOST_CONCEPT_USAGE(SharedLockable)
    {
      l.lock_shared();
      l.unlock_shared();
  ...
  {
    AUTOBOOST_CONCEPT_ASSERT(( SharedLockable<Mutex> ));

    AUTOBOOST_CONCEPT_USAGE(UpgradeLockable)
    {
      l.lock_upgrade();
      l.unlock_upgrade();
