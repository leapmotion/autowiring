...
#ifndef AUTOBOOST_THREAD_FUTURES_WAIT_FOR_ANY_HPP
#define AUTOBOOST_THREAD_FUTURES_WAIT_FOR_ANY_HPP
    ...
    class waiter_for_any_in_seq
    {
      ...
      struct all_futures_lock
      {
        ...
        all_futures_lock(std::vector<registered_waiter>& waiters) :
          count(waiters.size()), locks(new autoboost::unique_lock<autoboost::mutex>[count])
        {
          for (count_type_portable i = 0; i < count; ++i)
          {
            locks[i] = AUTOBOOST_THREAD_MAKE_RV_REF(autoboost::unique_lock<autoboost::mutex>(waiters[i].future_->mutex()));
          }
        }
