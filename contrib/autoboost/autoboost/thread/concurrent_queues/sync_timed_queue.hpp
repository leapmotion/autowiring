...
#ifndef AUTOBOOST_THREAD_SYNC_TIMED_QUEUE_HPP
#define AUTOBOOST_THREAD_SYNC_TIMED_QUEUE_HPP

#include <autoboost/thread/detail/config.hpp>

#include <autoboost/thread/concurrent_queues/sync_priority_queue.hpp>
#include <autoboost/chrono/duration.hpp>
#include <autoboost/chrono/time_point.hpp>
#include <autoboost/chrono/system_clocks.hpp>
#include <autoboost/chrono/chrono_io.hpp>
...
namespace detail
{
  ...
  struct scheduled_type
  {
    ...
    AUTOBOOST_THREAD_COPYABLE_AND_MOVABLE(scheduled_type)

    scheduled_type(T const& pdata, time_point tp) : data(pdata), time(tp) {}
    scheduled_type(AUTOBOOST_THREAD_RV_REF(T) pdata, time_point tp) : data(autoboost::move(pdata)), time(tp) {}

    scheduled_type(scheduled_type const& other) : data(other.data), time(other.time) {}
    scheduled_type& operator=(AUTOBOOST_THREAD_COPY_ASSIGN_REF(scheduled_type) other) {
      data = other.data;
      time = other.time;
      return *this;
    }

    scheduled_type(AUTOBOOST_THREAD_RV_REF(scheduled_type) other) : data(autoboost::move(other.data)), time(other.time) {}
    scheduled_type& operator=(AUTOBOOST_THREAD_RV_REF(scheduled_type) other) {
      data = autoboost::move(other.data);
      time = other.time;
      return *this;
    }
  ...
  public:
    ...
    template <class Duration>
    void push(AUTOBOOST_THREAD_RV_REF(T) elem, chrono::time_point<clock,Duration> const& tp);
    template <class Rep, class Period>
    void push(AUTOBOOST_THREAD_RV_REF(T) elem, chrono::duration<Rep,Period> const& dura);
    ...
    template <class Duration>
    queue_op_status try_push(AUTOBOOST_THREAD_RV_REF(T) elem, chrono::time_point<clock,Duration> const& tp);
    template <class Rep, class Period>
    queue_op_status try_push(AUTOBOOST_THREAD_RV_REF(T) elem, chrono::duration<Rep,Period> const& dura);
  ...
  private:
    ...
    sync_timed_queue(AUTOBOOST_THREAD_RV_REF(sync_timed_queue));
    sync_timed_queue& operator=(AUTOBOOST_THREAD_RV_REF(sync_timed_queue));
  }; //end class
  ...
  template <class T, class Clock>
  template <class Duration>
  void sync_timed_queue<T, Clock>::push(AUTOBOOST_THREAD_RV_REF(T) elem, chrono::time_point<clock,Duration> const& tp)
  {
    super::push(stype(autoboost::move(elem),tp));
  }

  template <class T, class Clock>
  template <class Rep, class Period>
  void sync_timed_queue<T, Clock>::push(AUTOBOOST_THREAD_RV_REF(T) elem, chrono::duration<Rep,Period> const& dura)
  {
    push(autoboost::move(elem), clock::now() + dura);
  }
  ...
  template <class T, class Clock>
  template <class Duration>
  queue_op_status sync_timed_queue<T, Clock>::try_push(AUTOBOOST_THREAD_RV_REF(T) elem, chrono::time_point<clock,Duration> const& tp)
  {
    return super::try_push(stype(autoboost::move(elem), tp));
  }

  template <class T, class Clock>
  template <class Rep, class Period>
  queue_op_status sync_timed_queue<T, Clock>::try_push(AUTOBOOST_THREAD_RV_REF(T) elem, chrono::duration<Rep,Period> const& dura)
  {
    return try_push(autoboost::move(elem), clock::now() + dura);
  }
  ...
  T sync_timed_queue<T, Clock>::pull(unique_lock<mutex>&)
  {
    ...
    return autoboost::move(super::data_.pull().data);
#else
  ...
  {
#if ! defined  AUTOBOOST_NO_CXX11_RVALUE_REFERENCES
    elem = autoboost::move(super::data_.pull().data);
#else
