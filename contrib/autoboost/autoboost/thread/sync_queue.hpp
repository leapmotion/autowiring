#ifndef AUTOBOOST_THREAD_SYNC_QUEUE_HPP
#define AUTOBOOST_THREAD_SYNC_QUEUE_HPP
...
#include <autoboost/thread/csbl/deque.hpp>
...
namespace autoboost
{
  ...
  private:
    ...
    inline size_type size(lock_guard<mutex>& ) const AUTOBOOST_NOEXCEPT
    {
      return data_.size();
    }
    ...
    {
      elem = autoboost::move(data_.front());
      data_.pop_front();
    }
    inline value_type pull(unique_lock<mutex>& )
    {
      value_type e = autoboost::move(data_.front());
      data_.pop_front();
      return autoboost::move(e);
    }
    inline autoboost::shared_ptr<value_type> ptr_pull(unique_lock<mutex>& )
    {
      shared_ptr<value_type> res = make_shared<value_type>(autoboost::move(data_.front()));
      data_.pop_front();
      return res;
    }
#endif
    ...
    {
      data_.push_back(autoboost::move(elem));
      notify_not_empty_if_needed(lk);
    }
#endif
    ...
    inline void push_back(AUTOBOOST_THREAD_RV_REF(value_type) elem, unique_lock<mutex>& lk)
    {
      data_.push_back(autoboost::move(elem));
      notify_not_empty_if_needed(lk);
    }
...
//  }
  ...
  template <typename ValueType>
  autoboost::shared_ptr<ValueType> sync_queue<ValueType>::try_pull()
  {
      unique_lock<mutex> lk(mtx_);
      return try_pull(lk);
  }
  ...
  template <typename ValueType>
  autoboost::shared_ptr<ValueType> sync_queue<ValueType>::ptr_pull()
  {
      unique_lock<mutex> lk(mtx_);
      wait_until_not_empty(lk);
      return ptr_pull(lk);
...
#ifndef AUTOBOOST_THREAD_QUEUE_DEPRECATE_OLD
  template <typename ValueType>
  bool sync_queue<ValueType>::try_push(AUTOBOOST_THREAD_RV_REF(ValueType) elem, unique_lock<mutex>& lk)
  {
    throw_if_closed(lk);
  ...
  template <typename ValueType>
  bool sync_queue<ValueType>::try_push(AUTOBOOST_THREAD_RV_REF(ValueType) elem)
  {
      unique_lock<mutex> lk(mtx_);
  ...
  template <typename ValueType>
  queue_op_status sync_queue<ValueType>::try_push_back(AUTOBOOST_THREAD_RV_REF(ValueType) elem, unique_lock<mutex>& lk)
  {
    if (closed(lk)) return queue_op_status::closed;
    push_back(autoboost::move(elem), lk);
    return queue_op_status::success;
  }

  template <typename ValueType>
  queue_op_status sync_queue<ValueType>::try_push_back(AUTOBOOST_THREAD_RV_REF(ValueType) elem)
  {
    unique_lock<mutex> lk(mtx_);
  ...
  template <typename ValueType>
  queue_op_status sync_queue<ValueType>::wait_push_back(AUTOBOOST_THREAD_RV_REF(ValueType) elem, unique_lock<mutex>& lk)
  {
    if (closed(lk)) return queue_op_status::closed;
    push_back(autoboost::move(elem), lk);
    return queue_op_status::success;
  ...
  template <typename ValueType>
  queue_op_status sync_queue<ValueType>::wait_push_back(AUTOBOOST_THREAD_RV_REF(ValueType) elem)
  {
    unique_lock<mutex> lk(mtx_);
    return wait_push_back(autoboost::move(elem), lk);
  }

#ifndef AUTOBOOST_THREAD_QUEUE_DEPRECATE_OLD
  template <typename ValueType>
  bool sync_queue<ValueType>::try_push(no_block_tag, AUTOBOOST_THREAD_RV_REF(ValueType) elem)
  {
      unique_lock<mutex> lk(mtx_, try_to_lock);
      if (!lk.owns_lock())
      {
  ...
  template <typename ValueType>
  queue_op_status sync_queue<ValueType>::nonblocking_push_back(AUTOBOOST_THREAD_RV_REF(ValueType) elem)
  {
    unique_lock<mutex> lk(mtx_, try_to_lock);
    if (!lk.owns_lock())
    {
...
#ifndef AUTOBOOST_THREAD_QUEUE_DEPRECATE_OLD
  template <typename ValueType>
  void sync_queue<ValueType>::push(AUTOBOOST_THREAD_RV_REF(ValueType) elem)
  {
      unique_lock<mutex> lk(mtx_);
      throw_if_closed(lk);
      push(autoboost::move(elem), lk);
  ...
  template <typename ValueType>
  void sync_queue<ValueType>::push_back(AUTOBOOST_THREAD_RV_REF(ValueType) elem)
  {
      unique_lock<mutex> lk(mtx_);
      throw_if_closed(lk);
      push_back(autoboost::move(elem), lk);
  ...
  template <typename ValueType>
  sync_queue<ValueType>& operator<<(sync_queue<ValueType>& sbq, AUTOBOOST_THREAD_RV_REF(ValueType) elem)
  {
