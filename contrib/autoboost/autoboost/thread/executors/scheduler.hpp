...
#ifndef AUTOBOOST_THREAD_EXECUTORS_SCHEDULER_HPP
#define AUTOBOOST_THREAD_EXECUTORS_SCHEDULER_HPP

#include <autoboost/thread/detail/config.hpp>
#include <autoboost/thread/executors/detail/scheduled_executor_base.hpp>
...
namespace autoboost
{
  namespace executors
  {
    ...
    resubmit(Executor& ex, AUTOBOOST_THREAD_FWD_REF(Function) funct) {
      return resubmitter<Executor, typename decay<Function>::type >(ex, autoboost::move(funct));
    }
    ...
    public:
      ...
      template <class Work>
      void submit(AUTOBOOST_THREAD_FWD_REF(Work) w)
      {
        if (closed())
        {
          AUTOBOOST_THROW_EXCEPTION( sync_queue_is_closed() );
        }
        sch.submit_at(resubmit(ex,autoboost::forward<Work>(w)), tp);
      }
    ...
    public:
      ...
      void submit(AUTOBOOST_THREAD_FWD_REF(Work) w)
      {
        ...
        sch.submit_at(autoboost::forward<Work>(w), tp);
      }
