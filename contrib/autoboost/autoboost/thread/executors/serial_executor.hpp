...
#ifndef AUTOBOOST_THREAD_SERIAL_EXECUTOR_HPP
#define AUTOBOOST_THREAD_SERIAL_EXECUTOR_HPP
...
namespace executors
{
  ...
  private:
    ...
    struct try_executing_one_task {
      work& task;
      autoboost::promise<void> &p;
      try_executing_one_task(work& task, autoboost::promise<void> &p)
      : task(task), p(p) {}
      void operator()() {
        try {
          task();
  ...
  public:
     ...
     */
    generic_executor_ref& underlying_executor() AUTOBOOST_NOEXCEPT { return ex; }
  ...
  public:
    ...
    AUTOBOOST_THREAD_NO_COPYABLE(serial_executor)
