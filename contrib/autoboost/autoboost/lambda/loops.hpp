...
#if !defined(AUTOBOOST_LAMBDA_LOOPS_HPP)
#define AUTOBOOST_LAMBDA_LOOPS_HPP
...
public:
  ...
  RET call(CALL_FORMAL_ARGS) const {
    for(detail::select(autoboost::tuples::get<0>(args), CALL_ACTUAL_ARGS);
        detail::select(autoboost::tuples::get<1>(args), CALL_ACTUAL_ARGS);
        detail::select(autoboost::tuples::get<2>(args), CALL_ACTUAL_ARGS))

      detail::select(autoboost::tuples::get<3>(args), CALL_ACTUAL_ARGS);
  }
};
...
public:
  ...
  template<class RET, CALL_TEMPLATE_ARGS>
  RET call(CALL_FORMAL_ARGS) const {
    while(detail::select(autoboost::tuples::get<0>(args), CALL_ACTUAL_ARGS))
...
public:
  ...
  template<class RET, CALL_TEMPLATE_ARGS>
  RET call(CALL_FORMAL_ARGS) const {
          do {} while (detail::select(autoboost::tuples::get<0>(args), CALL_ACTUAL_ARGS) );
  }
};
...
#endif // AUTOBOOST_LAMBDA_LOOPS_HPP
