...
#ifndef AUTOBOOST_LAMBDA_LAMBDA_FUNCTORS_HPP
#define AUTOBOOST_LAMBDA_LAMBDA_FUNCTORS_HPP
...
#define AUTOBOOST_LAMBDA_DISABLE_IF_ARRAY1(A1, R1)\
  typename lazy_disable_if<is_array<A1>, typename R1 >::type
#define AUTOBOOST_LAMBDA_DISABLE_IF_ARRAY2(A1, A2, R1, R2) \
  typename lazy_disable_if<mpl::or_<is_array<A1>, is_array<A2> >, typename R1, R2 >::type
#define AUTOBOOST_LAMBDA_DISABLE_IF_ARRAY3(A1, A2, A3, R1, R2, R3) \
  typename lazy_disable_if<mpl::or_<is_array<A1>, is_array<A2>, is_array<A3> >, typename R1, R2, R3 >::type

#else

#define AUTOBOOST_LAMBDA_DISABLE_IF_ARRAY1(A1, R1) typename R1::type
#define AUTOBOOST_LAMBDA_DISABLE_IF_ARRAY2(A1, A2, R1, R2) typename R1, R2::type
#define AUTOBOOST_LAMBDA_DISABLE_IF_ARRAY3(A1, A2, A3, R1, R2, R3) typename R1, R2, R3::type
...
template<> struct placeholder<FIRST> {
  ...
  RET call(CALL_FORMAL_ARGS) const {
    AUTOBOOST_STATIC_ASSERT(autoboost::is_reference<RET>::value);
...
AUTOBOOST_STATIC_CONSTANT(int, arity_bits = get_arity<T>::value);
...
public:
  ...
  // Support for autoboost::result_of.
  ...
  template<class A>
  AUTOBOOST_LAMBDA_DISABLE_IF_ARRAY1(A, inherited::template sig<tuple<A const&> >)
  ...
  template<class A, class B>
  AUTOBOOST_LAMBDA_DISABLE_IF_ARRAY2(A, B, inherited::template sig<tuple<A const&, B&> >)
  ...
  template<class A, class B>
  AUTOBOOST_LAMBDA_DISABLE_IF_ARRAY2(A, B, inherited::template sig<tuple<A&, B const&> >)
  ...
  template<class A, class B>
  AUTOBOOST_LAMBDA_DISABLE_IF_ARRAY2(A, B, inherited::template sig<tuple<A const&, B const&> >)
  ...
  template<class A, class B, class C>
  AUTOBOOST_LAMBDA_DISABLE_IF_ARRAY3(A, B, C, inherited::template sig<tuple<A const&, B const&, C const&> >)
  ...
  const lambda_functor<lambda_functor_base<
                  ...
                  autoboost::tuple<lambda_functor,
...
{
    typedef typename autoboost::lambda::lambda_functor<T>::nullary_return_type type;
};

template<class T>
struct result_of<const autoboost::lambda::lambda_functor<T>()>
{
    typedef typename autoboost::lambda::lambda_functor<T>::nullary_return_type type;
};
...
template<class T>
struct tr1_result_of<autoboost::lambda::lambda_functor<T>()>
{
    typedef typename autoboost::lambda::lambda_functor<T>::nullary_return_type type;
};

template<class T>
struct tr1_result_of<const autoboost::lambda::lambda_functor<T>()>
{
    typedef typename autoboost::lambda::lambda_functor<T>::nullary_return_type type;
};
