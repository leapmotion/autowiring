...
#ifndef AUTOBOOST_LAMBDA_FUNCTION_ADAPTORS_HPP
#define AUTOBOOST_LAMBDA_FUNCTION_ADAPTORS_HPP
...
AUTOBOOST_MPL_HAS_XXX_TEMPLATE_DEF(sig)

template<class Tuple>
struct remove_references_from_elements {
  typedef typename autoboost::tuples::cons<
    typename autoboost::remove_reference<typename Tuple::head_type>::type,
    typename remove_references_from_elements<typename Tuple::tail_type>::type
  > type;
};

template<>
struct remove_references_from_elements<autoboost::tuples::null_type> {
  typedef autoboost::tuples::null_type type;
};
...
#if !defined(AUTOBOOST_NO_RESULT_OF)
  // Support functors that use the autoboost::result_of return type convention.
...
struct function_adaptor<T Object::*> {
  ...
  template<class Args> class sig {
    ...
    typedef typename detail::IF<autoboost::is_const<unref_type>::value,
      typename autoboost::add_const<T>::type,
      T
    >::RET properly_consted_return_type;

    typedef typename detail::IF<autoboost::is_volatile<unref_type>::value,
      typename autoboost::add_volatile<properly_consted_return_type>::type,
      properly_consted_return_type
    >::RET properly_cvd_return_type;


  public:
    typedef typename detail::IF<autoboost::is_reference<argument_type>::value,
      typename autoboost::add_reference<properly_cvd_return_type>::type,
      typename autoboost::remove_cv<T>::type
    >::RET type;
  };
