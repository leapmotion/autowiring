...
#ifndef AUTOBOOST_LAMBDA_ARITY_CODE_HPP
#define AUTOBOOST_LAMBDA_ARITY_CODE_HPP
...
template <class T> struct get_arity {
  ...
  AUTOBOOST_STATIC_CONSTANT(int, value = detail::get_arity_<typename autoboost::remove_cv<typename autoboost::remove_reference<T>::type>::type>::value);
...
struct get_arity_<lambda_functor<T> > {
  AUTOBOOST_STATIC_CONSTANT(int, value = get_arity<T>::value);
};

template<class Action, class Args>
struct get_arity_<lambda_functor_base<Action, Args> > {
  AUTOBOOST_STATIC_CONSTANT(int, value = get_tuple_arity<Args>::value);
};
...
template<class T>
struct get_tuple_arity {
  AUTOBOOST_STATIC_CONSTANT(int, value = get_arity<typename T::head_type>::value | get_tuple_arity<typename T::tail_type>::value);
};
...
struct includes_placeholder {
  AUTOBOOST_STATIC_CONSTANT(bool, value = (J & I) != 0);
};
