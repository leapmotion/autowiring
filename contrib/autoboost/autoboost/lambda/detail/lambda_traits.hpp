...
#ifndef AUTOBOOST_LAMBDA_LAMBDA_TRAITS_HPP
#define AUTOBOOST_LAMBDA_LAMBDA_TRAITS_HPP
...
struct IF_value_
{
  AUTOBOOST_STATIC_CONSTANT(int, value = T::value);
};
...
struct IF_value
{
  AUTOBOOST_STATIC_CONSTANT(int, value = (IF_value_<typename IF<C, T, E>::RET>::value));
};
...
template<class T> class remove_reference_if_valid
{
  ...
  typedef typename autoboost::remove_reference<T>::type plainT;
public:
  typedef typename IF<
    autoboost::is_function<plainT>::value,
    T,
    plainT
  >::RET type;
...
template<class T> struct remove_reference_and_cv {
   typedef typename autoboost::remove_cv<
     typename autoboost::remove_reference<T>::type
   >::type type;
};
