...
#ifndef AUTOBOOST_LAMBDA_RETURN_TYPE_TRAITS_HPP
#define AUTOBOOST_LAMBDA_RETURN_TYPE_TRAITS_HPP
...
using ::autoboost::type_traits::ice_and;
using ::autoboost::type_traits::ice_or;
using ::autoboost::type_traits::ice_not;
...
namespace detail {
  ...
  class protect_conversion {
      typedef typename autoboost::remove_reference<T>::type non_ref_T;
    public:

  // add const to rvalues, so that all rvalues are stored as const in
  // the args tuple
    typedef typename detail::IF_type<
//      autoboost::is_reference<T>::value && !autoboost::is_const<non_ref_T>::value,
      ice_and<autoboost::is_reference<T>::value,
              ice_not<autoboost::is_const<non_ref_T>::value>::value>::value,
...
// experimental feature
  ...
  typedef typename autoboost::remove_reference<A>::type non_ref_A;
  typedef typename autoboost::remove_reference<B>::type non_ref_B;
