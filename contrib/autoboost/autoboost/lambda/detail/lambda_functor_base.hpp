...
#ifndef AUTOBOOST_LAMBDA_LAMBDA_FUNCTOR_BASE_HPP
#define AUTOBOOST_LAMBDA_LAMBDA_FUNCTOR_BASE_HPP
...
public:
  ...
  // stay as they are.
  typedef typename autoboost::add_reference<
...
template<class Tuple> struct has_null_type {
  AUTOBOOST_STATIC_CONSTANT(bool, value = (is_null_type<typename Tuple::head_type>::value || has_null_type<typename Tuple::tail_type>::value));
};
...
//    typedef typename detail::IF<autoboost::is_function<t>::value, T, t>::RET type;
//  };
...
template<int N, class Tuple> struct element_or_null {
  typedef typename autoboost::tuples::element<N, Tuple>::type type;
};
...
#undef AUTOBOOST_LAMBDA_LAMBDA_FUNCTOR_BASE_FIRST_PART
