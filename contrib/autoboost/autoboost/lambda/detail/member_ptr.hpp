...
#if !defined(AUTOBOOST_LAMBDA_MEMBER_PTR_HPP)
#define AUTOBOOST_LAMBDA_MEMBER_PTR_HPP
...
// the autoboost type_traits member_pointer traits are not enough,
...
struct member_pointer {
  ...
  AUTOBOOST_STATIC_CONSTANT(bool, is_data_member = false);
  AUTOBOOST_STATIC_CONSTANT(bool, is_function_member = false);
};
...
struct member_pointer<T U::*> {
  ...
  AUTOBOOST_STATIC_CONSTANT(bool, is_data_member = true);
  AUTOBOOST_STATIC_CONSTANT(bool, is_function_member = false);
};

template<class T, class U>
struct member_pointer<const T U::*> {
  typedef typename autoboost::add_reference<const T>::type type;
  typedef U class_type;
...
struct member_pointer<volatile T U::*> {
  typedef typename autoboost::add_reference<volatile T>::type type;
  typedef U class_type;
...
struct member_pointer<const volatile T U::*> {
  typedef typename autoboost::add_reference<const volatile T>::type type;
  typedef U class_type;
...
struct member_pointer<T (U::*)()> {
  ...
  typedef U qualified_class_type;
  AUTOBOOST_STATIC_CONSTANT(bool, is_data_member = false);
  AUTOBOOST_STATIC_CONSTANT(bool, is_function_member = true);
};
...
public:
  ...
  private:
    ...
    typedef typename autoboost::remove_reference<type0>::type type1;
    ...
    typedef typename ::autoboost::remove_pointer<non_ref_A>::type non_pointer_A;
  ...
  public:
    ...
    typedef typename detail::IF<
      ::autoboost::is_const<non_pointer_A>::value,
      typename ::autoboost::add_const<type1>::type,
      type1
    >::RET type2;
    typedef typename detail::IF<
      ::autoboost::is_volatile<non_pointer_A>::value,
      typename ::autoboost::add_volatile<type2>::type,
      type2
    >::RET type3;
    // add reference back
    typedef typename ::autoboost::add_reference<type3>::type type;
  };
};
...
struct member_pointer_action_helper<false, true> {
  ...
  template<class RET, class A, class B>
  static RET apply(A& a, B& b) {
    typedef typename ::autoboost::remove_cv<B>::type plainB;
    typedef typename detail::member_pointer<plainB>::type ret_t;
    typedef typename ::autoboost::remove_cv<A>::type plainA;
...
public:
  ...
  static RET apply(A& a, B& b) {
    typedef typename
      ::autoboost::remove_cv<B>::type plainB;

    return detail::member_pointer_action_helper<
        autoboost::is_pointer<A>::value &&
...
struct return_type_N<other_action<member_pointer_action>, Args> {
  typedef typename autoboost::tuples::element<0, Args>::type A;
  typedef typename autoboost::tuples::element<1, Args>::type B;
  typedef typename
    return_type_2<other_action<member_pointer_action>,
                  typename autoboost::remove_reference<A>::type,
                  typename autoboost::remove_reference<B>::type
                 >::type type;
};
