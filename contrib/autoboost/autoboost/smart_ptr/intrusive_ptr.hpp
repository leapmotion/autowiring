#ifndef AUTOBOOST_SMART_PTR_INTRUSIVE_PTR_HPP_INCLUDED
#define AUTOBOOST_SMART_PTR_INTRUSIVE_PTR_HPP_INCLUDED
...
public:
    ...
    intrusive_ptr( intrusive_ptr<U> const & rhs, typename autoboost::detail::sp_enable_if_convertible<U,T>::type = autoboost::detail::sp_empty() )
...
#include <autoboost/smart_ptr/detail/operator_bool.hpp>
    ...
    void swap(intrusive_ptr & rhs) AUTOBOOST_NOEXCEPT
    {
        T * tmp = px;
        px = rhs.px;
        rhs.px = tmp;
...
template<class T> inline bool operator==( intrusive_ptr<T> const & p, autoboost::detail::sp_nullptr_t ) AUTOBOOST_NOEXCEPT
{
    return p.get() == 0;
}

template<class T> inline bool operator==( autoboost::detail::sp_nullptr_t, intrusive_ptr<T> const & p ) AUTOBOOST_NOEXCEPT
{
    return p.get() == 0;
}
...
template< class T > std::size_t hash_value( autoboost::intrusive_ptr<T> const & p )
{
...
#endif  // #ifndef AUTOBOOST_SMART_PTR_INTRUSIVE_PTR_HPP_INCLUDED
