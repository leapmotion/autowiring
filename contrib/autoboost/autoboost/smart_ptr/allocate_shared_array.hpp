...
struct sp_if_size_array<T[N]> {
    typedef autoboost::shared_ptr<T[N]> type;
};
...
sp_get_deleter(const
    autoboost::shared_ptr<T>& value) AUTOBOOST_NOEXCEPT_OR_NOTHROW
{
...
struct sp_array_storage {
    union type {
        char value[N];
        typename autoboost::type_with_alignment<A>::type other;
    };
};
...
template<class T>
inline
typename sp_enable<autoboost::has_trivial_destructor<T>::value>::type
sp_array_destroy(T*, std::size_t) AUTOBOOST_NOEXCEPT { }
...
template<class T>
inline
typename sp_enable<autoboost::has_trivial_constructor<T>::value ||
    autoboost::has_trivial_destructor<T>::value>::type
...
template<class T>
inline
typename sp_enable<autoboost::has_trivial_constructor<T>::value>::type
sp_array_default(T*, std::size_t) AUTOBOOST_NOEXCEPT { }
...
struct sp_less_align {
    enum {
        value = (autoboost::alignment_of<T>::value) < N
    };
};

template<class T, std::size_t N>
AUTOBOOST_CONSTEXPR inline
typename sp_enable<sp_less_align<T, N>::value, std::size_t>::type
sp_align(std::size_t size) AUTOBOOST_NOEXCEPT
{
...
sp_types(std::size_t size) AUTOBOOST_NOEXCEPT
{
...
public:
    template<class U>
    static void operator_fn(U) AUTOBOOST_NOEXCEPT { }

    sp_size_array_deleter() AUTOBOOST_NOEXCEPT
        : enabled_(false) { }

    template<class A>
    sp_size_array_deleter(const A&) AUTOBOOST_NOEXCEPT
        : enabled_(false) { }

    sp_size_array_deleter(const sp_size_array_deleter&) AUTOBOOST_NOEXCEPT
        : enabled_(false) { }
...
private:
    ...
    typename sp_array_storage<sizeof(T) * N,
        autoboost::alignment_of<T>::value>::type storage_;
};
...
public:
    ...
    template<class U>
    sp_size_array_destroyer(const U& allocator) AUTOBOOST_NOEXCEPT
    ...
    const A& allocator() const AUTOBOOST_NOEXCEPT {
        return allocator_;
    }
...
public:
    ...
    sp_array_deleter(std::size_t size) AUTOBOOST_NOEXCEPT
        : address_(0),
          size_(size) { }

    template<class A>
    sp_array_deleter(const A& allocator) AUTOBOOST_NOEXCEPT
        : address_(0),
          size_(allocator.size()) { }

    template<class A>
    sp_array_deleter(const A&, std::size_t size) AUTOBOOST_NOEXCEPT
        : address_(0),
          size_(size) { }

    sp_array_deleter(const sp_array_deleter& other) AUTOBOOST_NOEXCEPT
        : address_(0),
          size_(other.size_) { }
    ...
    std::size_t size() const AUTOBOOST_NOEXCEPT {
        return size_;
    }
...
public:
    ...
    template<class U>
    sp_array_destroyer(const U& allocator) AUTOBOOST_NOEXCEPT
        : allocator_(allocator.allocator()),
          size_(allocator.size()),
          address_(0) { }

    sp_array_destroyer(const sp_array_destroyer& other) AUTOBOOST_NOEXCEPT
        : allocator_(other.allocator_),
          size_(other.size_),
          address_(0) { }
...
private:
    enum {
        alignment = sp_max_size<autoboost::alignment_of<T>::value,
            autoboost::alignment_of<value_type>::value>::value
    };

    typedef typename autoboost::type_with_alignment<alignment>::type type;
...
public:
    ...
    sp_array_allocator(const A& allocator, std::size_t size,
        void** result) AUTOBOOST_NOEXCEPT
        : allocator_(allocator),
          size_(size),
          result_(result) { }
...
operator==(const sp_array_allocator<T, U>& first,
    const sp_array_allocator<T, V>& second) AUTOBOOST_NOEXCEPT
{
