...
#ifndef AUTOBOOST_TT_IS_CONSTRUCTIBLE_HPP_INCLUDED
#define AUTOBOOST_TT_IS_CONSTRUCTIBLE_HPP_INCLUDED

#include <autoboost/type_traits/integral_constant.hpp>
#include <autoboost/detail/workaround.hpp>

#if !defined(AUTOBOOST_NO_CXX11_VARIADIC_TEMPLATES) && !defined(AUTOBOOST_NO_CXX11_DECLTYPE) && !AUTOBOOST_WORKAROUND(AUTOBOOST_MSVC, < 1800) && !AUTOBOOST_WORKAROUND(AUTOBOOST_GCC_VERSION, < 40500)
...
namespace autoboost{
   ...
   namespace detail{
      ...
      {
         template<typename T, typename ...TheArgs, typename = decltype(T(autoboost::declval<TheArgs>()...))>
         static autoboost::type_traits::yes_type test(int);
         template<typename, typename...>
         static autoboost::type_traits::no_type test(...);

         template<typename T, typename Arg, typename = decltype(::new T(autoboost::declval<Arg>()))>
         static autoboost::type_traits::yes_type test1(int);
         template<typename, typename>
         static autoboost::type_traits::no_type test1(...);

         template <typename T>
         static autoboost::type_traits::yes_type ref_test(T);
         template <typename T>
         static autoboost::type_traits::no_type ref_test(...);
      };

   }

   template <class T, class ...Args> struct is_constructible : public integral_constant<bool, sizeof(detail::is_constructible_imp::test<T, Args...>(0)) == sizeof(autoboost::type_traits::yes_type)>{};
   template <class T, class Arg> struct is_constructible<T, Arg> : public integral_constant<bool, is_destructible<T>::value && sizeof(detail::is_constructible_imp::test1<T, Arg>(0)) == sizeof(autoboost::type_traits::yes_type)>{};
   template <class Ref, class Arg> struct is_constructible<Ref&, Arg> : public integral_constant<bool, sizeof(detail::is_constructible_imp::ref_test<Ref&>(autoboost::declval<Arg>())) == sizeof(autoboost::type_traits::yes_type)>{};
   template <class Ref, class Arg> struct is_constructible<Ref&&, Arg> : public integral_constant<bool, sizeof(detail::is_constructible_imp::ref_test<Ref&&>(autoboost::declval<Arg>())) == sizeof(autoboost::type_traits::yes_type)>{};
...
#endif // AUTOBOOST_TT_IS_CONSTRUCTIBLE_HPP_INCLUDED
