...
struct extent_imp
{
   AUTOBOOST_STATIC_CONSTANT(std::size_t, value = 0);
};
#if !defined(AUTOBOOST_NO_ARRAY_TYPE_SPECIALIZATIONS)
template <class T, std::size_t R, std::size_t N>
struct extent_imp<T[R], N>
{
   AUTOBOOST_STATIC_CONSTANT(std::size_t, value = (::autoboost::detail::extent_imp<T, N-1>::value));
};
...
{
   AUTOBOOST_STATIC_CONSTANT(std::size_t, value = R);
};
...
#if !AUTOBOOST_WORKAROUND(__BORLANDC__, < 0x600) && !defined(__IBMCPP__) &&  !AUTOBOOST_WORKAROUND(__DMC__, AUTOBOOST_TESTED_AT(0x840)) && !defined(__MWERKS__)
template <class T, std::size_t N>
