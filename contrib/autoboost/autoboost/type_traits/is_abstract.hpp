#ifndef AUTOBOOST_TT_IS_ABSTRACT_CLASS_HPP
#define AUTOBOOST_TT_IS_ABSTRACT_CLASS_HPP
...
//              and John Maddock.
//  - Dec 2004: Added new config macro AUTOBOOST_NO_IS_ABSTRACT which causes the template
...
{
   AUTOBOOST_STATIC_CONSTANT(bool, value = AUTOBOOST_IS_ABSTRACT(T));
};
...
struct is_abstract_imp2
{
   ...
   //
   AUTOBOOST_STATIC_ASSERT(sizeof(T) != 0);

   // GCC2 won't even parse this template if we embed the computation
   // of s1 in the computation of value.
#ifdef __GNUC__
   AUTOBOOST_STATIC_CONSTANT(std::size_t, s1 = sizeof(is_abstract_imp2<T>::template check_sig<T>(0)));
#else
#if AUTOBOOST_WORKAROUND(AUTOBOOST_MSVC_FULL_VER, >= 140050000)
#pragma warning(push)
#pragma warning(disable:6334)
#endif
   AUTOBOOST_STATIC_CONSTANT(std::size_t, s1 = sizeof(check_sig<T>(0)));
#if AUTOBOOST_WORKAROUND(AUTOBOOST_MSVC_FULL_VER, >= 140050000)
#pragma warning(pop)
#endif
#endif
...
struct is_abstract_imp
{
   typedef is_abstract_select< ::autoboost::is_class<T>::value> selector;
