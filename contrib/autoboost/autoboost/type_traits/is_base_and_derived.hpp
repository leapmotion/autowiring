...
namespace detail {
 ...
 && !AUTOBOOST_WORKAROUND(__SUNPRO_CC , <= 0x540) \
...
#pragma warning(disable:6334)
    ...
    //
    AUTOBOOST_STATIC_ASSERT(sizeof(B) != 0);
    AUTOBOOST_STATIC_ASSERT(sizeof(D) != 0);
...
{
    AUTOBOOST_STATIC_CONSTANT(bool, value =
        (::autoboost::is_convertible<D*,B*>::value));
};

#define AUTOBOOST_BROKEN_IS_BASE_AND_DERIVED
...
struct is_base_and_derived_impl
{
    ...
    typedef is_base_and_derived_select<
       ::autoboost::is_class<B>::value,
       ::autoboost::is_class<D>::value,
       ::autoboost::is_same<ncvB,ncvD>::value> selector;
    typedef typename selector::template rebind<ncvB,ncvD> binder;
    typedef typename binder::type bound_type;

    AUTOBOOST_STATIC_CONSTANT(bool, value = bound_type::value);
};
#else
template <typename B, typename D>
struct is_base_and_derived_impl
{
    ...
    AUTOBOOST_STATIC_CONSTANT(bool, value = (AUTOBOOST_IS_BASE_OF(B,D) && ! ::autoboost::is_same<ncvB,ncvD>::value));
};
#endif
} // namespace detail
