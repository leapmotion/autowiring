#ifndef AUTOBOOST_TYPE_TRAITS_DETAIL_COMPOSITE_POINTER_TYPE_HPP_INCLUDED
#define AUTOBOOST_TYPE_TRAITS_DETAIL_COMPOSITE_POINTER_TYPE_HPP_INCLUDED
...
#endif // !defined( AUTOBOOST_NO_CXX11_NULLPTR )
...
private:
    ...
    typedef typename autoboost::remove_cv<T>::type T2;
    typedef typename autoboost::remove_cv<U>::type U2;

public:

    AUTOBOOST_STATIC_CONSTANT( bool, value =
        (autoboost::is_same<T2, U2>::value)
        || autoboost::is_void<T2>::value
        || autoboost::is_void<U2>::value
        || (autoboost::is_base_of<T2, U2>::value)
        || (autoboost::is_base_of<U2, T2>::value) );
};
...
public:
    ...
    typedef typename autoboost::conditional<

        autoboost::is_same<T2, U2>::value || autoboost::is_void<T2>::value || autoboost::is_base_of<T2, U2>::value,
        typename autoboost::copy_cv<T, U>::type,
        typename autoboost::copy_cv<U, T>::type
...
public:
    ...
    typedef typename autoboost::copy_cv<typename autoboost::copy_cv<typename composite_pointer_type<T2, U2>::type const, T>::type, U>::type type;
};

//Old compilers like MSVC-7.1 have problems using autoboost::conditional in
...
#endif // #ifndef AUTOBOOST_TYPE_TRAITS_DETAIL_COMPOSITE_POINTER_TYPE_HPP_INCLUDED
