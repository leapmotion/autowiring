...
#define AUTOBOOST_REGEX_TRAITS_DEFAULTS_HPP_INCLUDED
...
AUTOBOOST_REGEX_DECL bool AUTOBOOST_REGEX_CALL is_combining_implementation(uint_least16_t s);
...
#elif !defined(__DECCXX) && !defined(__osf__) && !defined(__OSF__) && defined(WCHAR_MIN) && (WCHAR_MIN == 0) && !defined(AUTOBOOST_NO_INTRINSIC_WCHAR_T)
...
{
   return AUTOBOOST_REGEX_MAKE_BOOL(
      (c == static_cast<charT>('\n'))
      || (c == static_cast<charT>('\r'))
      || (c == static_cast<charT>('\f'))
      || (static_cast<autoboost::uint16_t>(c) == 0x2028u)
      || (static_cast<autoboost::uint16_t>(c) == 0x2029u)
      || (static_cast<autoboost::uint16_t>(c) == 0x85u));
}
template <>
inline bool is_separator<char>(char c)
{
   return AUTOBOOST_REGEX_MAKE_BOOL((c == '\n') || (c == '\r') || (c == '\f'));
}
...
struct character_pointer_range
{
   ...
   bool operator == (const character_pointer_range& r)const
   {
      ...
      // in the MS case.
      return ((p2 - p1) == (r.p2 - r.p1)) && AUTOBOOST_REGEX_DETAIL_NS::equal(p1, p2, r.p1);
   }
};
template <class charT>
...
#endif
template <class charT>
inline charT AUTOBOOST_REGEX_CALL global_lower(charT c)
{
   return c;
}
template <class charT>
inline charT AUTOBOOST_REGEX_CALL global_upper(charT c)
{
   return c;
}
...
//
template<> inline char AUTOBOOST_REGEX_CALL global_lower<char>(char c){ return do_global_lower(c); }
template<> inline char AUTOBOOST_REGEX_CALL global_upper<char>(char c){ return do_global_upper(c); }
#ifndef AUTOBOOST_NO_WREGEX
template<> inline wchar_t AUTOBOOST_REGEX_CALL global_lower<wchar_t>(wchar_t c){ return do_global_lower(c); }
template<> inline wchar_t AUTOBOOST_REGEX_CALL global_upper<wchar_t>(wchar_t c){ return do_global_upper(c); }
#endif
#ifdef AUTOBOOST_REGEX_HAS_OTHER_WCHAR_T
template<> inline unsigned short AUTOBOOST_REGEX_CALL global_lower<unsigned short>(unsigned short c){ return do_global_lower(c); }
template<> inline unsigned short AUTOBOOST_REGEX_CALL global_upper<unsigned short>(unsigned short c){ return do_global_upper(c); }
#endif
...
autoboost::intmax_t global_toi(const charT*& p1, const charT* p2, int radix, const traits& t)
{
   (void)t; // warning suppression
   autoboost::intmax_t next_value = t.value(*p1, radix);
   if((p1 == p2) || (next_value < 0) || (next_value >= radix))
      return -1;
   autoboost::intmax_t result = 0;
   while(p1 != p2)
   {
