...
#define AUTOBOOST_REGEX_V4_MATCH_RESULTS_HPP
...
public:
   ...
   {
      AUTOBOOST_STATIC_ASSERT(sizeof(charT) <= sizeof(char_type));
      if(i == j)
         return m_null;
   ...
   void AUTOBOOST_REGEX_CALL set_second(BidiIterator i)
   {
      AUTOBOOST_ASSERT(m_subs.size() > 2);
      m_subs[2].second = i;
   ...
   void AUTOBOOST_REGEX_CALL set_second(BidiIterator i, size_type pos, bool m = true, bool escape_k = false)
   {
      if(pos)
         m_last_closed_paren = static_cast<int>(pos);
      pos += 2;
      AUTOBOOST_ASSERT(m_subs.size() > pos);
      m_subs[pos].second = i;
   ...
   void AUTOBOOST_REGEX_CALL set_size(size_type n, BidiIterator i, BidiIterator j)
   {
      value_type v(j);
   ...
   void AUTOBOOST_REGEX_CALL set_base(BidiIterator pos)
   {
      m_base = pos;
   }
   ...
   void AUTOBOOST_REGEX_CALL set_first(BidiIterator i)
   {
      AUTOBOOST_ASSERT(m_subs.size() > 2);
      // set up prefix:
      m_subs[1].second = i;
   ...
   void AUTOBOOST_REGEX_CALL set_first(BidiIterator i, size_type pos, bool escape_k = false)
   {
      AUTOBOOST_ASSERT(pos+2 < m_subs.size());
      if(pos || escape_k)
      {
   ...
   void AUTOBOOST_REGEX_CALL maybe_assign(const match_results<BidiIterator, Allocator>& m);

   void AUTOBOOST_REGEX_CALL set_named_subs(autoboost::shared_ptr<named_sub_type> subs)
   {
      m_named_subs = subs;
   }
...
private:
   ...
   {
      std::logic_error e("Attempt to access an uninitialzed autoboost::match_results<> class.");
   ...
   autoboost::shared_ptr<named_sub_type> m_named_subs;     // Shared copy of named subs in the regex object
   int m_last_closed_paren;                            // Last ) to be seen - used for formatting
   bool m_is_singular;                                 // True if our stored iterators are singular
};

template <class BidiIterator, class Allocator>
void AUTOBOOST_REGEX_CALL match_results<BidiIterator, Allocator>::maybe_assign(const match_results<BidiIterator, Allocator>& m)
{
   if(m_is_singular)
   {
      *this = m;
