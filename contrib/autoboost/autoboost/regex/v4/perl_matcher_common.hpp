...
#ifndef AUTOBOOST_REGEX_V4_PERL_MATCHER_COMMON_HPP
#define AUTOBOOST_REGEX_V4_PERL_MATCHER_COMMON_HPP
...
void perl_matcher<BidiIterator, Allocator, traits>::estimate_max_state_count(std::random_access_iterator_tag*)
{
   ...
   {
      max_state_count = (std::min)((std::ptrdiff_t)AUTOBOOST_REGEX_MAX_STATE_COUNT, (std::numeric_limits<std::ptrdiff_t>::max)() - 2);
      return;
   }
   states *= dist;
   ...
   //
   if(states > AUTOBOOST_REGEX_MAX_STATE_COUNT)
      states = AUTOBOOST_REGEX_MAX_STATE_COUNT;
   //
...
inline void perl_matcher<BidiIterator, Allocator, traits>::estimate_max_state_count(void*)
{
   ...
   max_state_count = AUTOBOOST_REGEX_MAX_STATE_COUNT;
}
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_imp()
{
   ...
   used_block_count = AUTOBOOST_REGEX_MAX_BLOCKS;
...
#if defined(AUTOBOOST_REGEX_NON_RECURSIVE) && !defined(AUTOBOOST_NO_EXCEPTIONS)
   }
   catch(...)
   {
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_backref()
{
   ...
   if(index >= 10000)
   {
      ...
      AUTOBOOST_ASSERT(r.first != r.second);
      do
      {
         index = r.first->index;
         ++r.first;
