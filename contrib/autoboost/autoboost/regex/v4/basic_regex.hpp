...
#define AUTOBOOST_REGEX_V4_BASIC_REGEX_HPP
...
{
   std::size_t r = autoboost::hash_range(i, j);
...
public:
   ...
   basic_regex_implementation(const ::autoboost::shared_ptr<
      ::autoboost::regex_traits_wrapper<traits> >& t)
   ...
   const_iterator AUTOBOOST_REGEX_CALL expression()const
   {
      return this->m_expression;
   }
   std::pair<const_iterator, const_iterator> AUTOBOOST_REGEX_CALL subexpression(std::size_t n)const
   {
   ...
   int AUTOBOOST_REGEX_CALL status()const
   {
   ...
   const ::autoboost::regex_traits_wrapper<traits>& get_traits()const
   {
...
public:
   ...
   ~basic_regex(){}
   basic_regex& AUTOBOOST_REGEX_CALL operator=(const basic_regex& that)
   {
      return assign(that);
   }
   basic_regex& AUTOBOOST_REGEX_CALL operator=(const charT* ptr)
   {
      return assign(ptr);
   }
...
public:
   ...
   template <class ST, class SA>
   basic_regex& AUTOBOOST_REGEX_CALL operator=(const std::basic_string<charT, ST, SA>& p)
   {
      return assign(p.data(), p.data() + p.size(), regex_constants::normal);
   }

   template <class string_traits, class A>
   basic_regex& AUTOBOOST_REGEX_CALL assign(
   ...
   template <class InputIterator>
   basic_regex& AUTOBOOST_REGEX_CALL assign(InputIterator arg_first,
   ...
   {
      if(!m_pimpl.get())
         autoboost::throw_exception(std::logic_error("Can't access subexpressions in an invalid regex."));
   ...
   //
   // swap:
   void AUTOBOOST_REGEX_CALL swap(basic_regex& that)throw()
   {
   ...
   int AUTOBOOST_REGEX_CALL compare(const basic_regex& that) const
   {
...
public:
   ...
   ~reg_expression(){}
   reg_expression& AUTOBOOST_REGEX_CALL operator=(const reg_expression& that)
   {
   ...
   template <class ST, class SA>
   reg_expression& AUTOBOOST_REGEX_CALL operator=(const std::basic_string<charT, ST, SA>& p)
   {
      this->assign(p);
      return *this;
   }
