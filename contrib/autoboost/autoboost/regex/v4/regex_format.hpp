...
#define AUTOBOOST_REGEX_FORMAT_HPP
...
#endif
   inline int toi(ForwardIter& i, ForwardIter j, int base, const autoboost::mpl::false_&)
   {
      if(i != j)
      {
         std::vector<char_type> v(i, j);
...
#pragma warning(pop)
#endif
   inline int toi(ForwardIter& i, ForwardIter j, int base, const autoboost::mpl::true_&)
   {
...
void basic_regex_formatter<OutputIterator, Results, traits, ForwardIter>::format_all()
{
   ...
   while(m_position != m_end)
   {
      ...
      {
      case '&':
         if(m_flags & ::autoboost::regex_constants::format_sed)
         {
            ++m_position;
            put(m_results[0]);
            break;
      ...
      case '\\':
         ...
         break;
      case '(':
         if(m_flags & autoboost::regex_constants::format_all)
         {
            ++m_position;
            ...
            if(m_position == m_end)
               return;
            AUTOBOOST_ASSERT(*m_position == static_cast<char_type>(')'));
      ...
      case ')':
         ...
         break;
      case ':':
         if((m_flags & autoboost::regex_constants::format_all) && m_have_conditional)
         {
            return;
         }
         put(*m_position);
...
void basic_regex_formatter<OutputIterator, Results, traits, ForwardIter>::format_escape()
{
   ...
   default:
      ...
      if((m_flags & autoboost::regex_constants::format_sed) == 0)
      {
         bool breakout = false;
         switch(*m_position)
         {
...
template<typename Formatter, bool IsFunction = autoboost::is_function<Formatter>::value>
...
private:
   ...
   //
   AUTOBOOST_STATIC_ASSERT((::autoboost::is_pointer<F>::value || ::autoboost::is_function<F>::value || ::autoboost::is_class<F>::value));
   static formatter_wrapper<typename unwrap_reference<F>::type> f;
   static M m;
   static O out;
   static autoboost::regex_constants::match_flag_type flags;
public:
   AUTOBOOST_STATIC_CONSTANT(int, value = sizeof(check_is_formatter(f(m), f(m, out), f(m, out, flags))));
};
...
public:
   ...
   typedef typename autoboost::mpl::if_<
      autoboost::mpl::and_<autoboost::is_pointer<F>, autoboost::mpl::not_<autoboost::is_function<typename autoboost::remove_pointer<F>::type> > >,
      autoboost::mpl::int_<0>,
      typename autoboost::mpl::if_<
         has_const_iterator<F>,
         autoboost::mpl::int_<1>,
         autoboost::mpl::int_<format_traits_imp<F, M, O>::value>
      >::type
   >::type type;
   //
   ...
   //
   AUTOBOOST_STATIC_ASSERT( autoboost::is_class<F>::value && !has_const_iterator<F>::value ? (type::value > 1) : true);
};

#else // AUTOBOOST_NO_SFINAE
...
struct format_functor3
{
   ...
   OutputIter operator()(const Match& m, OutputIter i, autoboost::regex_constants::match_flag_type f)
   {
      return autoboost::unwrap_ref(func)(m, i, f);
   }
   template <class OutputIter, class Traits>
   OutputIter operator()(const Match& m, OutputIter i, autoboost::regex_constants::match_flag_type f, const Traits&)
   {
      return (*this)(m, i, f);
   }
private:
...
struct format_functor2
{
   ...
   OutputIter operator()(const Match& m, OutputIter i, autoboost::regex_constants::match_flag_type /*f*/)
   {
      return autoboost::unwrap_ref(func)(m, i);
   }
   template <class OutputIter, class Traits>
   OutputIter operator()(const Match& m, OutputIter i, autoboost::regex_constants::match_flag_type f, const Traits&)
   {
...
struct format_functor1
{
   ...
   OutputIter operator()(const Match& m, OutputIter i, autoboost::regex_constants::match_flag_type /*f*/)
   {
      return do_format_string(autoboost::unwrap_ref(func)(m), i);
   }
   template <class OutputIter, class Traits>
   OutputIter operator()(const Match& m, OutputIter i, autoboost::regex_constants::match_flag_type f, const Traits&)
   {
...
struct compute_functor_type
{
   ...
   typedef typename autoboost::remove_cv< typename autoboost::remove_pointer<Func>::type>::type maybe_char_type;

   typedef typename mpl::if_<
      ::autoboost::is_same<tag, mpl::int_<0> >, format_functor_c_string<maybe_char_type, Match, Traits>,
      typename mpl::if_<
         ::autoboost::is_same<tag, mpl::int_<1> >, format_functor_container<Func, Match, Traits>,
         typename mpl::if_<
...
#endif  // AUTOBOOST_REGEX_FORMAT_HPP
