...
#define AUTOBOOST_REGEX_V4_BASIC_REGEX_PARSER_HPP
...
inline autoboost::intmax_t umax(mpl::false_ const&)
{
   // Get out clause here, just in case numeric_limits is unspecialized:
   return std::numeric_limits<autoboost::intmax_t>::is_specialized ? (std::numeric_limits<autoboost::intmax_t>::max)() : INT_MAX;
}
inline autoboost::intmax_t umax(mpl::true_ const&)
{
   return (std::numeric_limits<std::size_t>::max)();
}

inline autoboost::intmax_t umax()
{
   return umax(mpl::bool_<std::numeric_limits<autoboost::intmax_t>::digits >= std::numeric_limits<std::size_t>::digits>());
}
...
void basic_regex_parser<charT, traits>::parse(const charT* p1, const charT* p2, unsigned l_flags)
{
   ...
   if(!result)
   {
      fail(regex_constants::error_paren, ::autoboost::AUTOBOOST_REGEX_DETAIL_NS::distance(m_base, m_position), "Found a closing ) with no corresponding openening parenthesis.");
      return;
   }
...
void basic_regex_parser<charT, traits>::fail(regex_constants::error_type error_code, std::ptrdiff_t position, std::string message, std::ptrdiff_t start_pos)
{
   ...
   {
      autoboost::regex_error e(message, error_code, position);
      e.raise();
   }
#else
...
bool basic_regex_parser<charT, traits>::parse_open_paren()
{
   ...
   {
      markid = ++m_mark_count;
#ifndef AUTOBOOST_NO_STD_DISTANCE
   ...
   if(m_position == m_end)
   {
      this->fail(regex_constants::error_paren, ::autoboost::AUTOBOOST_REGEX_DETAIL_NS::distance(m_base, m_end));
      return false;
   }
...
escape_type_class_jump:
   ...
   case regex_constants::escape_type_extended_backref:
      if(0 == (this->flags() & (regbase::main_option_type | regbase::no_perl_ex)))
      {
         ...
         autoboost::intmax_t i = this->m_traits.toi(pc, m_end, 10);
         if((i < 0) && syn_end)
         {
         ...
         if(negative)
            i = 1 + m_mark_count - i;
         if(((i > 0) && (i < std::numeric_limits<unsigned>::digits) && (i - 1 < static_cast<autoboost::intmax_t>(sizeof(unsigned) * CHAR_BIT)) && (this->m_backrefs & (1u << (i-1)))) || ((i > 10000) && (this->m_pdata->get_id(i) > 0) && (this->m_pdata->get_id(i)-1 < static_cast<autoboost::intmax_t>(sizeof(unsigned) * CHAR_BIT)) && (this->m_backrefs & (1u << (this->m_pdata->get_id(i)-1)))))
         {
            m_position = pc;
...
bool basic_regex_parser<charT, traits>::parse_match_any()
{
   ...
   static_cast<re_dot*>(
      ...
      ? AUTOBOOST_REGEX_DETAIL_NS::force_not_newline
         : this->flags() & regbase::mod_s ?
            AUTOBOOST_REGEX_DETAIL_NS::force_newline : AUTOBOOST_REGEX_DETAIL_NS::dont_care);
   return true;
}
...
bool basic_regex_parser<charT, traits>::parse_repeat(std::size_t low, std::size_t high)
{
   ...
   {
      fail(regex_constants::error_badrepeat, ::autoboost::AUTOBOOST_REGEX_DETAIL_NS::distance(m_base, m_position), "Nothing to repeat.");
      return false;
   }
...
bool basic_regex_parser<charT, traits>::parse_alt()
{
   ...
   //
   re_syntax_base* pj = this->append_state(AUTOBOOST_REGEX_DETAIL_NS::syntax_element_jump, sizeof(re_jump));
...
bool valid_value(charT, autoboost::intmax_t v, const mpl::true_&)
{
   return (v >> (sizeof(charT) * CHAR_BIT)) == 0;
}
template <class charT>
bool valid_value(charT, autoboost::intmax_t, const mpl::false_&)
{
   return true; // v will alsways fit in a charT
}
template <class charT>
bool valid_value(charT c, autoboost::intmax_t v)
{
   return valid_value(c, v, mpl::bool_<(sizeof(charT) < sizeof(autoboost::intmax_t))>());
}
...
#pragma warning(disable:4127)
   ...
   case regex_constants::escape_type_hex:
      ...
      if(this->m_traits.syntax_type(*m_position) == regex_constants::syntax_open_brace)
      {
         ...
         autoboost::intmax_t i = this->m_traits.toi(m_position, m_end, 16);
         if((m_position == m_end)
            || (i < 0)
            || ((std::numeric_limits<charT>::is_specialized) && (i > (autoboost::intmax_t)(std::numeric_limits<charT>::max)()))
      ...
      else
      {
         ...
         autoboost::intmax_t i = this->m_traits.toi(m_position, m_position + len, 16);
         if((i < 0)
   ...
   case regex_constants::syntax_digit:
      ...
      std::ptrdiff_t len = (std::min)(::autoboost::AUTOBOOST_REGEX_DETAIL_NS::distance(m_position, m_end), static_cast<std::ptrdiff_t>(4));
      const charT* bp = m_position;
      autoboost::intmax_t val = this->m_traits.toi(bp, bp + 1, 8);
      if(val != 0)
      {
         // Rewind to start of escape:
         --m_position;
      ...
      if((val < 0) || (val > (autoboost::intmax_t)(std::numeric_limits<charT>::max)()))
      {
         // Rewind to start of escape:
         --m_position;
         while(this->m_traits.syntax_type(*m_position) != regex_constants::syntax_escape) --m_position;
...
{
   AUTOBOOST_ASSERT(m_position != m_end);
   const charT* pc = m_position;
   autoboost::intmax_t i = this->m_traits.toi(pc, pc + 1, 10);
...
option_group_jump:
   ...
   AUTOBOOST_ASSERT(this->m_traits.syntax_type(*m_position) == regex_constants::syntax_close_mark);
   ++m_position;
   //
   // restore the flags:
   //
...
bool basic_regex_parser<charT, traits>::unwind_alts(std::ptrdiff_t last_paren_start)
{
   ...
   while(m_alt_jumps.size() && (m_alt_jumps.back() > last_paren_start))
   {
      ...
      this->m_pdata->m_data.align();
      re_jump* jmp = static_cast<re_jump*>(this->getaddress(jump_offset));
      AUTOBOOST_ASSERT(jmp->type == syntax_element_jump);
