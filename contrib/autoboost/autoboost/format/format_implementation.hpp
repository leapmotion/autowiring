...
#ifndef AUTOBOOST_FORMAT_IMPLEMENTATION_HPP
#define AUTOBOOST_FORMAT_IMPLEMENTATION_HPP
...
// ---  basic_format implementation -----------------------------------------//
    ...
    clear () {
        ...
        AUTOBOOST_ASSERT( bound_.size()==0 || num_args_ == static_cast<int>(bound_.size()) );
...
#pragma warning(disable:4267)
        ...
        for(i=0; i < items_.size(); ++i) {
            ...
            sz += item.res_.size();
            if( item.argN_ == format_item_t::argN_tabulation)
                sz = max AUTOBOOST_PREVENT_MACRO_SUBSTITUTION (sz,
...
namespace detail {
    ...
    bind_arg_body (basic_format<Ch, Tr, Alloc>& self, int argN, const T& val) {
        ...
        AUTOBOOST_ASSERT( self.cur_arg_ >= self.num_args_ || ! self.bound_[self.cur_arg_]);
        return self;
    }
...
#endif  // AUTOBOOST_FORMAT_IMPLEMENTATION_HPP
