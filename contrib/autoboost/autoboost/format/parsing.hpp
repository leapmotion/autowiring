...
#ifndef AUTOBOOST_FORMAT_PARSING_HPP
#define AUTOBOOST_FORMAT_PARSING_HPP
...
#if ! defined( AUTOBOOST_NO_LOCALE_ISDIGIT )
...
# else
                                 ...
                                 unsigned char exceptions)
    {
        ...
        using namespace autoboost::io;
        typename String::size_type i1=0;
        int num_items=0;
        while( (i1=buf.find(arg_mark,i1)) != String::npos ) {
            if( i1+1 >= buf.size() ) {
                if(exceptions & bad_format_string_bit)
                    autoboost::throw_exception(bad_format_string(i1, buf.size() )); // must not end in ".. %"
                else {
                  ++num_items;
                  break;
                }
...
#endif //  AUTOBOOST_FORMAT_PARSING_HPP
