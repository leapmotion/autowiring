...
#ifndef AUTOBOOST_TYPEOF_INT_ENCODING_HPP_INCLUDED
#define AUTOBOOST_TYPEOF_INT_ENCODING_HPP_INCLUDED
...
namespace autoboost { namespace type_of {
    ...
    struct pack
    {
        AUTOBOOST_STATIC_CONSTANT(std::size_t , value=((n + 1) * 2 + (Overflow ? 1 : 0)));
    };

    template<std::size_t m>
    struct unpack
    {
        AUTOBOOST_STATIC_CONSTANT(std::size_t, value = (m / 2) - 1);
        AUTOBOOST_STATIC_CONSTANT(std::size_t, overflow = (m % 2 == 1));
    };
    ...
    struct encode_size_t : push_back<
        V,
        autoboost::mpl::size_t<pack<n, false>::value>
    >
    {};
    ...
    struct encode_size_t<V, n, true> : push_back<typename push_back<
        V,
        autoboost::mpl::size_t<pack<n % 0x3ffffffe, true>::value> >::type,
        autoboost::mpl::size_t<n / 0x3ffffffe>
    >
    {};
    ...
    {
        AUTOBOOST_STATIC_CONSTANT(std::size_t,m = Iter::type::value);

        AUTOBOOST_STATIC_CONSTANT(std::size_t,value = (std::size_t)m * 0x3ffffffe + n);
    ...
    struct decode_integral
    {
        ...
        AUTOBOOST_STATIC_CONSTANT(std::size_t,n = unpack<m>::value);

        AUTOBOOST_STATIC_CONSTANT(std::size_t,overflow = unpack<m>::overflow);
...
#endif//AUTOBOOST_TYPEOF_INT_ENCODING_HPP_INCLUDED
