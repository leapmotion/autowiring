...
#     include <autoboost/system/detail/local_free_on_destruction.hpp>
#   endif
...
namespace
{
  ...
  public:
    ...
    const char *        name() const AUTOBOOST_SYSTEM_NOEXCEPT;
    std::string         message( int ev ) const;
    error_condition     default_error_condition( int ev ) const AUTOBOOST_SYSTEM_NOEXCEPT;
  };

  //  generic_error_category implementation  ---------------------------------//

  const char * generic_error_category::name() const AUTOBOOST_SYSTEM_NOEXCEPT
  {
    return "generic";
  }
  ...
  {
    using namespace autoboost::system::errc;
#if defined(__PGI)
      using autoboost::system::errc::invalid_argument;
#endif
  ...
  //      thread safe.
  # if defined(AUTOBOOST_WINDOWS_API) || defined(__hpux) || defined(__sun)\
     || (defined(__linux) && (!defined(__USE_XOPEN2K) || defined(AUTOBOOST_SYSTEM_USE_STRERROR)))\
...
#   endif
  ...
  const char * system_error_category::name() const AUTOBOOST_SYSTEM_NOEXCEPT
  {
    return "system";
  }

  error_condition system_error_category::default_error_condition( int ev ) const AUTOBOOST_SYSTEM_NOEXCEPT
  {
    using namespace autoboost::system::errc;
#if defined(__PGI)
      using autoboost::system::errc::invalid_argument;
...
# else
  ...
  std::string system_error_category::message( int ev ) const
  {
#if defined(UNDER_CE) || AUTOBOOST_PLAT_WINDOWS_RUNTIME || defined(AUTOBOOST_NO_ANSI_APIS)
...
# ifndef AUTOBOOST_SYSTEM_NO_DEPRECATED
    AUTOBOOST_SYSTEM_DECL error_code throws; // "throw on error" special error_code;
...
#   define AUTOBOOST_SYSTEM_LINKAGE inline
# else
#   define AUTOBOOST_SYSTEM_LINKAGE AUTOBOOST_SYSTEM_DECL
# endif

    AUTOBOOST_SYSTEM_LINKAGE const error_category & system_category() AUTOBOOST_SYSTEM_NOEXCEPT
    {
      static const system_error_category  system_category_const;
      return system_category_const;
    }

    AUTOBOOST_SYSTEM_LINKAGE const error_category & generic_category() AUTOBOOST_SYSTEM_NOEXCEPT
    {
