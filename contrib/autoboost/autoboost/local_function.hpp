...
#ifndef AUTOBOOST_LOCAL_FUNCTION_HPP_
#define AUTOBOOST_LOCAL_FUNCTION_HPP_

#ifndef DOXYGEN

#include <autoboost/local_function/aux_/macro/decl.hpp>
#include <autoboost/local_function/aux_/macro/name.hpp>
#include <autoboost/local_function/aux_/macro/typeof.hpp>
#include <autoboost/local_function/aux_/preprocessor/traits/decl.hpp>
#include <autoboost/local_function/detail/preprocessor/line_counter.hpp>
#include <autoboost/local_function/detail/preprocessor/void_list.hpp>
#include <autoboost/config.hpp>

// PUBLIC //

#ifdef AUTOBOOST_NO_CXX11_VARIADIC_MACROS
#   define AUTOBOOST_LOCAL_FUNCTION_ID(id, declarations) \
        AUTOBOOST_LOCAL_FUNCTION_AUX_DECL(id, 0 /* not within template */, \
                AUTOBOOST_LOCAL_FUNCTION_AUX_PP_DECL_TRAITS( \
                        AUTOBOOST_LOCAL_FUNCTION_DETAIL_PP_VOID_LIST( \
                                declarations)))
#   define AUTOBOOST_LOCAL_FUNCTION(declarations) \
        AUTOBOOST_LOCAL_FUNCTION_ID( \
                AUTOBOOST_LOCAL_FUNCTION_DETAIL_PP_LINE_COUNTER, declarations)
#   define AUTOBOOST_LOCAL_FUNCTION_ID_TPL(id, declarations) \
        AUTOBOOST_LOCAL_FUNCTION_AUX_DECL(id, 1 /* within template */, \
                AUTOBOOST_LOCAL_FUNCTION_AUX_PP_DECL_TRAITS( \
                        AUTOBOOST_LOCAL_FUNCTION_DETAIL_PP_VOID_LIST( \
                                declarations)))
#   define AUTOBOOST_LOCAL_FUNCTION_TPL(declarations) \
        AUTOBOOST_LOCAL_FUNCTION_ID_TPL( \
                AUTOBOOST_LOCAL_FUNCTION_DETAIL_PP_LINE_COUNTER, declarations)
#else // VARIADIC
#   define AUTOBOOST_LOCAL_FUNCTION_ID(id, ...) \
        AUTOBOOST_LOCAL_FUNCTION_AUX_DECL(id, 0 /* not within template */, \
                AUTOBOOST_LOCAL_FUNCTION_AUX_PP_DECL_TRAITS( \
                        AUTOBOOST_LOCAL_FUNCTION_DETAIL_PP_VOID_LIST(__VA_ARGS__)))
#   define AUTOBOOST_LOCAL_FUNCTION(...) \
        AUTOBOOST_LOCAL_FUNCTION_ID( \
                AUTOBOOST_LOCAL_FUNCTION_DETAIL_PP_LINE_COUNTER, __VA_ARGS__)
#   define AUTOBOOST_LOCAL_FUNCTION_ID_TPL(id, ...) \
        AUTOBOOST_LOCAL_FUNCTION_AUX_DECL(id, 1 /* within template */, \
                AUTOBOOST_LOCAL_FUNCTION_AUX_PP_DECL_TRAITS( \
                        AUTOBOOST_LOCAL_FUNCTION_DETAIL_PP_VOID_LIST(__VA_ARGS__)))
#   define AUTOBOOST_LOCAL_FUNCTION_TPL(...) \
        AUTOBOOST_LOCAL_FUNCTION_ID_TPL( \
                AUTOBOOST_LOCAL_FUNCTION_DETAIL_PP_LINE_COUNTER, __VA_ARGS__)
#endif // VARIADIC

#define AUTOBOOST_LOCAL_FUNCTION_NAME(qualified_name) \
    AUTOBOOST_LOCAL_FUNCTION_AUX_NAME(0 /* not within template */, qualified_name)
#define AUTOBOOST_LOCAL_FUNCTION_NAME_TPL(qualified_name) \
    AUTOBOOST_LOCAL_FUNCTION_AUX_NAME(1 /* within template */, qualified_name)

#define AUTOBOOST_LOCAL_FUNCTION_TYPEOF(bound_variable_name) \
    AUTOBOOST_LOCAL_FUNCTION_AUX_TYPEOF_TYPE(bound_variable_name)
...
then by the @RefMacro{AUTOBOOST_LOCAL_FUNCTION_NAME} macro (see the
@RefSect{tutorial, Tutorial} and @RefSect{advanced_topics, Advanced Topics}
sections):
@code
{ // Some declarative context.
    ...
    result_type AUTOBOOST_LOCAL_FUNCTION(declarations) {
        ... // Body code.
    } AUTOBOOST_LOCAL_FUNCTION_NAME(qualified_name)
    ...
}
@endcode
...
Within templates, the special macros @RefMacro{AUTOBOOST_LOCAL_FUNCTION_TPL}
and @RefMacro{AUTOBOOST_LOCAL_FUNCTION_NAME_TPL} must be used.
...
The bound variable types can be accessed using @RefMacro{AUTOBOOST_LOCAL_FUNCTION_TYPEOF}.

This macro cannot be portably expanded multiple times on the same line.
In these cases, use the @RefMacro{AUTOBOOST_LOCAL_FUNCTION_ID} macro instead.

The maximum number of local function parameters (excluding bound variables) is
specified by the configuration macro
@RefMacro{AUTOBOOST_LOCAL_FUNCTION_CONFIG_ARITY_MAX}.
The maximum number of bound variables is specified by the configuration macro
@RefMacro{AUTOBOOST_LOCAL_FUNCTION_CONFIG_BIND_MAX}.
The configuration macro
@RefMacro{AUTOBOOST_LOCAL_FUNCTION_CONFIG_LOCALS_AS_TPARAMS} can be used to force
...
like <c>autoboost::function</c> (see Boost.Function).

@See @RefSect{tutorial, Tutorial} section,
@RefSect{advanced_topics, Advanced Topics} section,
@RefMacro{AUTOBOOST_LOCAL_FUNCTION_NAME}, @RefMacro{AUTOBOOST_LOCAL_FUNCTION_TPL},
@RefMacro{AUTOBOOST_LOCAL_FUNCTION_NAME_TPL},
@RefMacro{AUTOBOOST_LOCAL_FUNCTION_TYPEOF}, @RefMacro{AUTOBOOST_LOCAL_FUNCTION_ID},
@RefMacro{AUTOBOOST_LOCAL_FUNCTION_CONFIG_ARITY_MAX},
@RefMacro{AUTOBOOST_LOCAL_FUNCTION_CONFIG_BIND_MAX},
@RefMacro{AUTOBOOST_LOCAL_FUNCTION_CONFIG_LOCALS_AS_TPARAMS}.
*/
#define AUTOBOOST_LOCAL_FUNCTION(declarations)
...
This macro must be used instead of @RefMacro{AUTOBOOST_LOCAL_FUNCTION} when
declaring a local function within a template.
A part from that, this macro has the exact same syntax a
@RefMacro{AUTOBOOST_LOCAL_FUNCTION} (see @RefMacro{AUTOBOOST_LOCAL_FUNCTION} for more
information):
@code
{ // Some declarative context within a template.
    ...
    result_type AUTOBOOST_LOCAL_FUNCTION_TPL(declarations) {
        ... // Body code.
    } AUTOBOOST_LOCAL_FUNCTION_NAME_TPL(qualified_name)
    ...
}
@endcode

Note that @RefMacro{AUTOBOOST_LOCAL_FUNCTION_NAME_TPL} must be used with this
macro instead of @RefMacro{AUTOBOOST_LOCAL_FUNCTION_NAME}.

This macro cannot be portably expanded multiple times on the same line.
In these cases, use the @RefMacro{AUTOBOOST_LOCAL_FUNCTION_ID_TPL} macro instead.
...
@See @RefSect{tutorial, Tutorial} section, @RefMacro{AUTOBOOST_LOCAL_FUNCTION},
@RefMacro{AUTOBOOST_LOCAL_FUNCTION_ID_TPL},
@RefMacro{AUTOBOOST_LOCAL_FUNCTION_NAME_TPL}.
*/
#define AUTOBOOST_LOCAL_FUNCTION_TPL(declarations)
...
This macro is equivalent to @RefMacro{AUTOBOOST_LOCAL_FUNCTION} but it can be
...
@RefMacro{AUTOBOOST_LOCAL_FUNCTION} macro.
}
@EndParams

The @RefMacro{AUTOBOOST_LOCAL_FUNCTION_NAME} macro should be used to end each one
of the multiple local function declarations as usual (and it will specify a
unique name for each local function).

Within templates, the special macros @RefMacro{AUTOBOOST_LOCAL_FUNCTION_ID_TPL}
must be used.
...
@RefMacro{AUTOBOOST_LOCAL_FUNCTION}, @RefMacro{AUTOBOOST_LOCAL_FUNCTION_NAME},
@RefMacro{AUTOBOOST_LOCAL_FUNCTION_ID_TPL}.
*/
#define AUTOBOOST_LOCAL_FUNCTION_ID(id, declarations)
...
This macro must be used instead of @RefMacro{AUTOBOOST_LOCAL_FUNCTION_TPL} when
...
@RefMacro{AUTOBOOST_LOCAL_FUNCTION_TPL} (see @RefMacro{AUTOBOOST_LOCAL_FUNCTION_TPL}
...
@Param{declarations,
Same as the <c>declarations</c> parameter of the
@RefMacro{AUTOBOOST_LOCAL_FUNCTION_TPL} macro.
}
@EndParams
...
Outside template, the macro @RefMacro{AUTOBOOST_LOCAL_FUNCTION_ID} should be used
...
@See @RefSect{advanced_topics, Advanced Topics} section,
@RefMacro{AUTOBOOST_LOCAL_FUNCTION_TPL}, @RefMacro{AUTOBOOST_LOCAL_FUNCTION_NAME},
@RefMacro{AUTOBOOST_LOCAL_FUNCTION_ID}.
*/
#define AUTOBOOST_LOCAL_FUNCTION_ID_TPL(id, declarations)
...
Within templates, the special macros @RefMacro{AUTOBOOST_LOCAL_FUNCTION_TPL} and
@RefMacro{AUTOBOOST_LOCAL_FUNCTION_NAME_TPL} must be used.
...
@code
    AUTOBOOST_LOCAL_FUNCTION_NAME(inline local_function_name)
@endcode
...
<c>AUTOBOOST_NO_CXX11_LOCAL_CLASS_TEMPLATE_PARAMETERS</c> is not defined but it also be
forced using @RefMacro{AUTOBOOST_LOCAL_FUNCTION_CONFIG_LOCALS_AS_TPARAMS}.

The local function name can also be qualified by prefixing it with the
"keyword" <c>recursive</c> (see the
@RefSect{advanced_topics, Advanced Topics} section):
@code
    AUTOBOOST_LOCAL_FUNCTION_NAME(recursive local_function_name)
@endcode
...
inline:
@code
    AUTOBOOST_LOCAL_FUNCTION(inline recursive local_function_name)
@endcode
...
declared within the same enclosing scope (but <c>autoboost::overloaded_function</c>
...
@See @RefSect{tutorial, Tutorial} section,
@RefSect{advanced_topics, Advanced Topics} section,
@RefMacro{AUTOBOOST_LOCAL_FUNCTION},
@RefMacro{AUTOBOOST_LOCAL_FUNCTION_NAME_TPL}.
*/
#define AUTOBOOST_LOCAL_FUNCTION_NAME(qualified_name)
...
This macro must be used instead of @RefMacro{AUTOBOOST_LOCAL_FUNCTION_NAME} when
declaring a local function within a template.
A part from that, this macro has the exact same syntax a
@RefMacro{AUTOBOOST_LOCAL_FUNCTION_NAME} (see @RefMacro{AUTOBOOST_LOCAL_FUNCTION_NAME}
for more information):
@code
{ // Some declarative context within a template.
    ...
...
Note that @RefMacro{AUTOBOOST_LOCAL_FUNCTION_TPL} must be used with this macro
instead of @RefMacro{AUTOBOOST_LOCAL_FUNCTION}.
...
@RefMacro{AUTOBOOST_LOCAL_FUNCTION_NAME}, @RefMacro{AUTOBOOST_LOCAL_FUNCTION_TPL}.
*/
#define AUTOBOOST_LOCAL_FUNCTION_NAME_TPL(name)
...
<c>AUTOBOOST_LOCAL_FUNCTION_TYPEOF(t)</c> is <c>T</c>.
@li Bound by constant value using <c>const bind t</c> then
<c>AUTOBOOST_LOCAL_FUNCTION_TYPEOF(t)</c> is <c>const T</c>.
