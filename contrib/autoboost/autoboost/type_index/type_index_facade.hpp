...
#ifndef AUTOBOOST_TYPE_INDEX_TYPE_INDEX_FACADE_HPP
#define AUTOBOOST_TYPE_INDEX_TYPE_INDEX_FACADE_HPP
...
// Forward declaration from #include <autoboost/functional/hash_fwd.hpp>
namespace autoboost {
...
private:
    /// @cond
    AUTOBOOST_CXX14_CONSTEXPR const Derived & derived() const AUTOBOOST_NOEXCEPT {
...
public:
    ...
    inline bool equal(const Derived& rhs) const AUTOBOOST_NOEXCEPT {
    ...
    inline bool before(const Derived& rhs) const AUTOBOOST_NOEXCEPT {
        const char* const left = derived().raw_name();
        const char* const right = rhs.raw_name();
    ...
    /// \note <autoboost/functional/hash.hpp> has to be included if this function is used.
    inline std::size_t hash_code() const AUTOBOOST_NOEXCEPT {
        const char* const name_raw = derived().raw_name();
        return autoboost::hash_range(name_raw, name_raw + std::strlen(name_raw));
    }
...
protected:
    ...
    /// autoboost::typeindex::type_id() will call this method, if Derived has same type as autoboost::typeindex::type_index.
    ///
    ...
    template <class T>
    static Derived type_id() AUTOBOOST_NOEXCEPT;

    /// This is a factory method that is used to create instances of Derived classes.
    /// autoboost::typeindex::type_id_with_cvr() will call this method, if Derived has same type as autoboost::typeindex::type_index.
    ///
    /// \b Override: This function \b may be redefined and made public in Derived class. Overrides \b must not throw.
    ...
    /// \return type_index for type T.
    template <class T>
    static Derived type_id_with_cvr() AUTOBOOST_NOEXCEPT;

    /// This is a factory method that is used to create instances of Derived classes.
    /// autoboost::typeindex::type_id_runtime(const T&) will call this method, if Derived has same type as autoboost::typeindex::type_index.
    ///
    ...
    template <class T>
    static Derived type_id_runtime(const T& variable) AUTOBOOST_NOEXCEPT;
...
/// @cond
template <class Derived, class TypeInfo>
AUTOBOOST_CXX14_CONSTEXPR inline bool operator == (const type_index_facade<Derived, TypeInfo>& lhs, const type_index_facade<Derived, TypeInfo>& rhs) AUTOBOOST_NOEXCEPT {
...
template <class Derived, class TypeInfo>
inline bool operator == (const TypeInfo& lhs, const type_index_facade<Derived, TypeInfo>& rhs) AUTOBOOST_NOEXCEPT {
...
template <class Derived, class TypeInfo>
inline bool operator == (const type_index_facade<Derived, TypeInfo>& lhs, const TypeInfo& rhs) AUTOBOOST_NOEXCEPT {
...
#ifdef AUTOBOOST_NO_TEMPLATED_IOSTREAMS
/// @cond
...
#endif // AUTOBOOST_NO_TEMPLATED_IOSTREAMS
#endif // AUTOBOOST_NO_IOSTREAM

/// This free function is used by Boost's unordered containers.
/// \note <autoboost/functional/hash.hpp> has to be included if this function is used.
template <class Derived, class TypeInfo>
inline std::size_t hash_value(const type_index_facade<Derived, TypeInfo>& lhs) AUTOBOOST_NOEXCEPT {
    return static_cast<Derived const&>(lhs).hash_code();
}

}} // namespace autoboost::typeindex

#endif // AUTOBOOST_TYPE_INDEX_TYPE_INDEX_FACADE_HPP
