...
#ifndef AUTOBOOST_TYPE_INDEX_CTTI_TYPE_INDEX_HPP
#define AUTOBOOST_TYPE_INDEX_CTTI_TYPE_INDEX_HPP

/// \file ctti_type_index.hpp
/// \brief Contains autoboost::typeindex::ctti_type_index class that is constexpr if C++14 constexpr is supported by compiler.
///
/// autoboost::typeindex::ctti_type_index class can be used as a drop-in replacement
/// for std::type_index.
///
/// It is used in situations when typeid() method is not available or
/// AUTOBOOST_TYPE_INDEX_FORCE_NO_RTTI_COMPATIBILITY macro is defined.

#include <autoboost/type_index/type_index_facade.hpp>
#include <autoboost/type_index/detail/compile_time_type_info.hpp>
...
template <class T>
inline const detail::ctti_data& ctti_construct() AUTOBOOST_NOEXCEPT {
    ...
    return *reinterpret_cast<const detail::ctti_data*>(autoboost::detail::ctti<T>::n());
}
...
class ctti_type_index: public type_index_facade<ctti_type_index, detail::ctti_data> {
    ...
    inline std::size_t get_raw_name_length() const AUTOBOOST_NOEXCEPT;

    AUTOBOOST_CXX14_CONSTEXPR inline explicit ctti_type_index(const char* data) AUTOBOOST_NOEXCEPT
        : data_(data)
    {}
...
public:
    ...
    AUTOBOOST_CXX14_CONSTEXPR inline ctti_type_index() AUTOBOOST_NOEXCEPT
        : data_(autoboost::detail::ctti<void>::n())
    {}

    inline ctti_type_index(const type_info_t& data) AUTOBOOST_NOEXCEPT
        : data_(reinterpret_cast<const char*>(&data))
    {}

    inline const type_info_t& type_info() const AUTOBOOST_NOEXCEPT;
    AUTOBOOST_CXX14_CONSTEXPR inline const char* raw_name() const AUTOBOOST_NOEXCEPT;
    AUTOBOOST_CXX14_CONSTEXPR inline const char* name() const AUTOBOOST_NOEXCEPT;
    inline std::string  pretty_name() const;
    inline std::size_t  hash_code() const AUTOBOOST_NOEXCEPT;

    AUTOBOOST_CXX14_CONSTEXPR inline bool equal(const ctti_type_index& rhs) const AUTOBOOST_NOEXCEPT;
    AUTOBOOST_CXX14_CONSTEXPR inline bool before(const ctti_type_index& rhs) const AUTOBOOST_NOEXCEPT;

    template <class T>
    AUTOBOOST_CXX14_CONSTEXPR inline static ctti_type_index type_id() AUTOBOOST_NOEXCEPT;

    template <class T>
    AUTOBOOST_CXX14_CONSTEXPR inline static ctti_type_index type_id_with_cvr() AUTOBOOST_NOEXCEPT;

    template <class T>
    inline static ctti_type_index type_id_runtime(const T& variable) AUTOBOOST_NOEXCEPT;
};


inline const ctti_type_index::type_info_t& ctti_type_index::type_info() const AUTOBOOST_NOEXCEPT {
    return *reinterpret_cast<const detail::ctti_data*>(data_);
}


AUTOBOOST_CXX14_CONSTEXPR inline bool ctti_type_index::equal(const ctti_type_index& rhs) const AUTOBOOST_NOEXCEPT {
    const char* const left = raw_name();
    const char* const right = rhs.raw_name();
    return /*left == right ||*/ !autoboost::typeindex::detail::constexpr_strcmp(left, right);
}

AUTOBOOST_CXX14_CONSTEXPR inline bool ctti_type_index::before(const ctti_type_index& rhs) const AUTOBOOST_NOEXCEPT {
    const char* const left = raw_name();
    const char* const right = rhs.raw_name();
    return /*left != right &&*/ autoboost::typeindex::detail::constexpr_strcmp(left, right) < 0;
}


template <class T>
AUTOBOOST_CXX14_CONSTEXPR inline ctti_type_index ctti_type_index::type_id() AUTOBOOST_NOEXCEPT {
    typedef AUTOBOOST_DEDUCED_TYPENAME autoboost::remove_reference<T>::type no_ref_t;
    typedef AUTOBOOST_DEDUCED_TYPENAME autoboost::remove_cv<no_ref_t>::type no_cvr_t;
    return ctti_type_index(autoboost::detail::ctti<no_cvr_t>::n());
}



template <class T>
AUTOBOOST_CXX14_CONSTEXPR inline ctti_type_index ctti_type_index::type_id_with_cvr() AUTOBOOST_NOEXCEPT {
    return ctti_type_index(autoboost::detail::ctti<T>::n());
}


template <class T>
inline ctti_type_index ctti_type_index::type_id_runtime(const T& variable) AUTOBOOST_NOEXCEPT {
    return variable.boost_type_index_type_id_runtime_();
}


AUTOBOOST_CXX14_CONSTEXPR inline const char* ctti_type_index::raw_name() const AUTOBOOST_NOEXCEPT {
    return data_;
}


AUTOBOOST_CXX14_CONSTEXPR inline const char* ctti_type_index::name() const AUTOBOOST_NOEXCEPT {
    return data_;
}

inline std::size_t ctti_type_index::get_raw_name_length() const AUTOBOOST_NOEXCEPT {
...
inline std::size_t ctti_type_index::hash_code() const AUTOBOOST_NOEXCEPT {
    return autoboost::hash_range(raw_name(), raw_name() + get_raw_name_length());
}


}} // namespace autoboost::typeindex

#endif // AUTOBOOST_TYPE_INDEX_CTTI_TYPE_INDEX_HPP
