...
#ifndef AUTOBOOST_TOKEN_FUNCTIONS_JRB120303_HPP_
#define AUTOBOOST_TOKEN_FUNCTIONS_JRB120303_HPP_
...
#if !defined(AUTOBOOST_NO_CWCTYPE)
#include <cwctype>
#endif
...
// MSVC does not like the following typename
  template <class Char,
    class Traits = AUTOBOOST_DEDUCED_TYPENAME std::basic_string<Char>::traits_type >
  ...
  private:
    ...
    void do_escape(iterator& next,iterator end,Token& tok) {
      if (++next == end)
        AUTOBOOST_THROW_EXCEPTION(escaped_list_error(std::string("cannot end with escape")));
      if (Traits::eq(*next,'n')) {
        tok+='\n';
        return;
      }
        ...
        AUTOBOOST_THROW_EXCEPTION(escaped_list_error(std::string("unknown escape sequence")));
    }
  ...
  public:
    ...
    bool operator()(InputIterator& next, InputIterator end, Token& tok)
    {
      typedef tokenizer_detail::assign_or_plus_equal<
        AUTOBOOST_DEDUCED_TYPENAME tokenizer_detail::get_iterator_category<
          InputIterator
        >::iterator_category
      > assigner;

      AUTOBOOST_ASSERT(!offsets_.empty());
  ...
  template <typename Char,
    typename Tr = AUTOBOOST_DEDUCED_TYPENAME std::basic_string<Char>::traits_type >
  ...
  // The out of the box GCC 2.95 on cygwin does not have a char_traits class.
  template <class Char,
    class Tr = AUTOBOOST_DEDUCED_TYPENAME std::basic_string<Char>::traits_type >
