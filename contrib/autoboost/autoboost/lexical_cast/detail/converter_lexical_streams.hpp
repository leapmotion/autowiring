...
#ifndef AUTOBOOST_LEXICAL_CAST_DETAIL_CONVERTER_LEXICAL_STREAMS_HPP
#define AUTOBOOST_LEXICAL_CAST_DETAIL_CONVERTER_LEXICAL_STREAMS_HPP
...
#include <autoboost/detail/basic_pointerbuf.hpp>
...
namespace autoboost {
    ...
    namespace detail // basic_unlockedbuf
    {
        ...
        public:
           ...
           typedef AUTOBOOST_DEDUCED_TYPENAME base_type::streamsize streamsize;
    ...
    namespace detail // optimized stream wrappers
    {
        template< class CharT // a result of widest_char transformation
                ...
                >
        class lexical_istream_limited_src: autoboost::noncopyable {
            typedef AUTOBOOST_DEDUCED_TYPENAME autoboost::mpl::if_c<
                RequiresStringbuffer,
                AUTOBOOST_DEDUCED_TYPENAME out_stream_helper_trait<CharT, Traits>::out_stream_t,
                do_not_construct_out_stream_t
            >::type deduced_out_stream_t;

            typedef AUTOBOOST_DEDUCED_TYPENAME autoboost::mpl::if_c<
                RequiresStringbuffer,
                AUTOBOOST_DEDUCED_TYPENAME out_stream_helper_trait<CharT, Traits>::stringbuffer_t,
        ...
        public:
            lexical_istream_limited_src() AUTOBOOST_NOEXCEPT
              : out_buffer()
            ...
            const CharT* cend() const AUTOBOOST_NOEXCEPT {
                return finish;
            }

        private:
/************************************ HELPER FUNCTIONS FOR OPERATORS << ( ... ) ********************************/
            bool shl_char(CharT ch) AUTOBOOST_NOEXCEPT {
            ...
            template <class T>
            bool shl_char(T ch) {
                AUTOBOOST_STATIC_ASSERT_MSG(( sizeof(T) <= sizeof(CharT)) ,
                    "autoboost::lexical_cast does not support narrowing of char types."
                    "Use autoboost::locale instead" );
#ifndef AUTOBOOST_LEXICAL_CAST_ASSUME_C_LOCALE
                std::locale loc;
                CharT const w = AUTOBOOST_USE_FACET(std::ctype<CharT>, loc).widen(ch);
#else
            ...
            bool shl_char_array(CharT const* str) AUTOBOOST_NOEXCEPT {
                start = str;
...
#if defined(AUTOBOOST_NO_STRINGSTREAM) || defined(AUTOBOOST_NO_STD_LOCALE)
                // If you have compilation error at this point, than your STL library
                // does not support such conversions. Try updating it.
                AUTOBOOST_STATIC_ASSERT((autoboost::is_same<char, CharT>::value));
#endif
            ...
            inline bool shl_signed(const T n) {
                CharT* tmp_finish = buffer + CharacterBufferSize;
                typedef AUTOBOOST_DEDUCED_TYPENAME autoboost::make_unsigned<T>::type utype;
            ...
            bool shl_real_type(float val, wchar_t* begin) {
                ...
                finish = start + swprintf(begin, CharacterBufferSize,
                                       L"%.*g",
                                       static_cast<int>(autoboost::detail::lcast_get_precision<float >()),
                                       val_as_double );
                return finish > start;
            }
            ...
            bool shl_real_type(double val, wchar_t* begin) {
                using namespace std;
                finish = start + swprintf(begin, CharacterBufferSize,
                                          L"%.*g", static_cast<int>(autoboost::detail::lcast_get_precision<double >()), val );
                return finish > start;
            }
            ...
            bool shl_real_type(long double val, wchar_t* begin) {
                using namespace std;
                finish = start + swprintf(begin, CharacterBufferSize,
                                          L"%.*Lg", static_cast<int>(autoboost::detail::lcast_get_precision<long double >()), val );
                return finish > start;
            }
#endif
            template <class T>
...
/************************************ OPERATORS << ( ... ) ********************************/
        public:
            ...
            bool operator<<(std::basic_string<CharT,Traits,Alloc> const& str) AUTOBOOST_NOEXCEPT {
                start = str.data();
            ...
            template<class Alloc>
            bool operator<<(autoboost::container::basic_string<CharT,Traits,Alloc> const& str) AUTOBOOST_NOEXCEPT {
                start = str.data();
                finish = start + str.length();
                return true;
            }

            bool operator<<(bool value) AUTOBOOST_NOEXCEPT {
            ...
            bool operator<<(const iterator_range<const unsigned char*>& rng) AUTOBOOST_NOEXCEPT {
                return (*this) << iterator_range<const char*>(
                    reinterpret_cast<const char*>(rng.begin()),
                    reinterpret_cast<const char*>(rng.end())
                );
            ...
            bool operator<<(autoboost::ulong_long_type n)   { return shl_unsigned(n); }
            bool operator<<(autoboost::long_long_type n)    { return shl_signed(n); }
            ...
            bool operator<<(const autoboost::uint128_type& n)   { return shl_unsigned(n); }
            bool operator<<(const autoboost::int128_type& n)    { return shl_signed(n); }
#endif
        ...
        public:
            lexical_ostream_limited_src(const CharT* begin, const CharT* end) AUTOBOOST_NOEXCEPT
              : start(begin)
              , finish(end)
            {}
...
/************************************ HELPER FUNCTIONS FOR OPERATORS >> ( ... ) ********************************/
        private:
            ...
            bool shr_signed(Type& output) {
                ...
                CharT const minus = lcast_char_constants<CharT>::minus;
                CharT const plus = lcast_char_constants<CharT>::plus;
                typedef AUTOBOOST_DEDUCED_TYPENAME make_unsigned<Type>::type utype;
                utype out_tmp = 0;
                bool const has_minus = Traits::eq(minus, *start);
            ...
            {
                AUTOBOOST_STATIC_ASSERT_MSG(
                    (!autoboost::is_pointer<InputStreamable>::value),
                    "autoboost::lexical_cast can not convert to pointers"
                );

#if defined(AUTOBOOST_NO_STRINGSTREAM) || defined(AUTOBOOST_NO_STD_LOCALE)
                AUTOBOOST_STATIC_ASSERT_MSG((autoboost::is_same<char, CharT>::value),
                    "autoboost::lexical_cast can not convert, because your STL library does not "
...
#else
                typedef AUTOBOOST_DEDUCED_TYPENAME out_stream_helper_trait<CharT, Traits>::buffer_t buffer_t;
                buffer_t buf;
...
#endif // AUTOBOOST_NO_STRINGSTREAM
            ...
            template<class T>
            inline bool shr_xchar(T& output) AUTOBOOST_NOEXCEPT {
                AUTOBOOST_STATIC_ASSERT_MSG(( sizeof(CharT) == sizeof(T) ),
                    "autoboost::lexical_cast does not support narrowing of character types."
                    "Use autoboost::locale instead" );
...
/************************************ OPERATORS >> ( ... ) ********************************/
        public:
...
#if defined(AUTOBOOST_HAS_LONG_LONG)
            bool operator>>(autoboost::ulong_long_type& output)     { return shr_unsigned(output); }
            bool operator>>(autoboost::long_long_type& output)      { return shr_signed(output); }
#elif defined(AUTOBOOST_HAS_MS_INT64)
...
#ifdef AUTOBOOST_HAS_INT128
            bool operator>>(autoboost::uint128_type& output)        { return shr_unsigned(output); }
            bool operator>>(autoboost::int128_type& output)         { return shr_signed(output); }
#endif
            ...
            bool operator>>(std::array<C, N>& output) AUTOBOOST_NOEXCEPT {
                ...
                return ((*this) >> reinterpret_cast<autoboost::array<C, N>& >(output));
            }
#endif

            bool operator>>(bool& output) AUTOBOOST_NOEXCEPT {
