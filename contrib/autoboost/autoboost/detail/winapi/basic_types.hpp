...
#ifndef AUTOBOOST_DETAIL_WINAPI_BASIC_TYPES_HPP
#define AUTOBOOST_DETAIL_WINAPI_BASIC_TYPES_HPP
...
#if defined(STRICT)
#define AUTOBOOST_DETAIL_WINAPI_DECLARE_HANDLE(x) struct x##__; typedef struct x##__ *x
#else
#define AUTOBOOST_DETAIL_WINAPI_DECLARE_HANDLE(x) typedef void* x
#endif
...
AUTOBOOST_DETAIL_WINAPI_DECLARE_HANDLE(HINSTANCE);
typedef HINSTANCE HMODULE;
}
#endif

#if defined(__GNUC__)
#define AUTOBOOST_DETAIL_WINAPI_MAY_ALIAS __attribute__ ((__may_alias__))
#else
#define AUTOBOOST_DETAIL_WINAPI_MAY_ALIAS
#endif

// MinGW64 gcc 4.8.2 fails to compile function declarations with autoboost::detail::winapi::VOID_ arguments even though
// the typedef expands to void. In Windows SDK, VOID is a macro which unfolds to void. We use our own macro in such cases.
#define AUTOBOOST_DETAIL_WINAPI_VOID void
...
typedef autoboost::int64_t LONGLONG_;
typedef autoboost::uint64_t ULONGLONG_;
...
typedef union AUTOBOOST_DETAIL_WINAPI_MAY_ALIAS _LARGE_INTEGER {
    struct {
        DWORD_ LowPart;
        LONG_ HighPart;
    } u;
...
typedef struct AUTOBOOST_DETAIL_WINAPI_MAY_ALIAS _SECURITY_ATTRIBUTES {
    DWORD_  nLength;
...
#endif // AUTOBOOST_DETAIL_WINAPI_BASIC_TYPES_HPP
