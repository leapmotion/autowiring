...
#ifndef AUTOBOOST_MPL_AUX_FULL_LAMBDA_HPP_INCLUDED
#define AUTOBOOST_MPL_AUX_FULL_LAMBDA_HPP_INCLUDED
...
#   define AUTOBOOST_MPL_PREPROCESSED_HEADER full_lambda.hpp
...
#   define AUX778076_ARITY_PARAM(param) \
    AUTOBOOST_MPL_AUX_LAMBDA_ARITY_PARAM(param) \
    /**/
...
#define AUTOBOOST_PP_ITERATION_PARAMS_1 \
    (3,(0, AUTOBOOST_MPL_LIMIT_METAFUNCTION_ARITY, <autoboost/mpl/aux_/full_lambda.hpp>))
...
struct lambda<
        ...
        , Tag
        AUX778076_ARITY_PARAM(int_<AUTOBOOST_PP_INC(AUTOBOOST_MPL_LIMIT_METAFUNCTION_ARITY)>)
        >
{
    typedef false_ is_le;
...
#elif !defined(AUTOBOOST_MPL_CFG_DMC_AMBIGUOUS_CTPS)
...
#if !defined(AUTOBOOST_MPL_CFG_EXTENDED_TEMPLATE_PARAMETERS_MATCHING)
AUTOBOOST_MPL_AUX_NA_SPEC(2, lambda)
#else
AUTOBOOST_MPL_AUX_NA_SPEC2(2, 3, lambda)
#endif

}}

#endif // AUTOBOOST_MPL_CFG_NO_PREPROCESSED_HEADERS
#endif // AUTOBOOST_MPL_AUX_FULL_LAMBDA_HPP_INCLUDED
...
#   define AUX778076_RESULT(unused, i_, T) \
    AUTOBOOST_PP_COMMA_IF(i_) \
    typename AUTOBOOST_PP_CAT(T, AUTOBOOST_PP_INC(i_))::result_ \
    /**/
...
template<
    ...
    >
struct AUTOBOOST_PP_CAT(le_result,i_)
{
    typedef F<
...
template<
    ...
    , AUX778076_LAMBDA_PARAMS(i_, typename L)
    >
struct AUTOBOOST_PP_CAT(le_result,i_)< true_,Tag,F,AUX778076_LAMBDA_PARAMS(i_, L) >
{
    typedef AUTOBOOST_PP_CAT(bind,i_)<
          AUTOBOOST_PP_CAT(quote,i_)<F,Tag>
        , AUTOBOOST_MPL_PP_REPEAT(i_, AUX778076_RESULT, L)
        > result_;
...
#   define AUX778076_LAMBDA_TYPEDEF(unused, i_, T) \
    typedef lambda< AUTOBOOST_PP_CAT(T, AUTOBOOST_PP_INC(i_)), Tag > \
...
#   define AUX778076_IS_LAMBDA_EXPR(unused, i_, unused2) \
    AUTOBOOST_PP_COMMA_IF(i_) \
    AUTOBOOST_PP_CAT(is_le,AUTOBOOST_PP_INC(i_))::value \
/**/
...
struct lambda<
        ...
        >
{
    AUTOBOOST_MPL_PP_REPEAT(i_, AUX778076_LAMBDA_TYPEDEF, T)
...
struct lambda<
        ...
        , Tag
        AUX778076_ARITY_PARAM(int_<AUTOBOOST_PP_INC(i_)>)
        >
{
    typedef false_ is_le;
    typedef AUTOBOOST_PP_CAT(bind,i_)<
