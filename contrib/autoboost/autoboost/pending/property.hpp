...
#ifndef AUTOBOOST_PROPERTY_HPP
#define AUTOBOOST_PROPERTY_HPP
...
namespace autoboost {
  ...
  namespace graph_introspect_detail {
    AUTOBOOST_MPL_HAS_XXX_TRAIT_DEF(kind)
  ...
  struct lookup_one_property_internal {AUTOBOOST_STATIC_CONSTANT(bool, found = false); typedef void type;};

  // Special-case properties (vertex_all, edge_all, graph_all)
#define BGL_ALL_PROP(tag) \
  template <typename T> \
  struct lookup_one_property_internal<T, tag> { \
    AUTOBOOST_STATIC_CONSTANT(bool, found = true); \
    typedef T type; \
...
#undef BGL_ALL_PROP
  ...
  template <typename T> \
  struct lookup_one_property_internal<T, AUTOBOOST_JOIN(kind, _bundle_t)> { \
    AUTOBOOST_STATIC_CONSTANT(bool, found = true); \
    typedef T type; \
    static T& lookup(T& x, AUTOBOOST_JOIN(kind, _bundle_t)) {return x;} \
    static const T& lookup(const T& x, AUTOBOOST_JOIN(kind, _bundle_t)) {return x;} \
  }; \
 \
  template <typename Tag, typename T, typename Base> \
  struct lookup_one_property_internal<property<Tag, T, Base>, AUTOBOOST_JOIN(kind, _bundle_t)>: lookup_one_property_internal<Base, AUTOBOOST_JOIN(kind, _bundle_t)> { \
    private: \
    typedef lookup_one_property_internal<Base, AUTOBOOST_JOIN(kind, _bundle_t)> base_type; \
    public: \
    template <typename BundleTag> \
    static typename lazy_enable_if_c<(base_type::found && (is_same<BundleTag, AUTOBOOST_JOIN(kind, _bundle_t)>::value)), \
                                     add_reference<typename base_type::type> >::type \
    lookup(property<Tag, T, Base>& p, BundleTag) {return base_type::lookup(p.m_base, AUTOBOOST_JOIN(kind, _bundle_t)());} \
    template <typename BundleTag> \
    static typename lazy_enable_if_c<(base_type::found && (is_same<BundleTag, AUTOBOOST_JOIN(kind, _bundle_t)>::value)), \
                                     add_reference<const typename base_type::type> >::type \
    lookup(const property<Tag, T, Base>& p, BundleTag) {return base_type::lookup(p.m_base, AUTOBOOST_JOIN(kind, _bundle_t)());} \
  }; \
...
#undef BGL_DO_ONE_BUNDLE_TYPE
  ...
  template <typename Tag, typename T, typename Base>
  struct lookup_one_property_internal<autoboost::property<Tag, T, Base>, Tag> {
    AUTOBOOST_STATIC_CONSTANT(bool, found = true);
  ...
  struct lookup_one_property_internal<autoboost::property<Tag, T, Base>, PropName>: lookup_one_property_internal<Base, PropName> {
    private:
    typedef lookup_one_property_internal<Base, PropName> base_type;
    public:
    template <typename PL>
    static typename lazy_enable_if<is_same<PL, autoboost::property<Tag, T, Base> >,
                                   add_reference<typename base_type::type> >::type
...
#ifndef AUTOBOOST_GRAPH_NO_BUNDLED_PROPERTIES
  ...
  struct lookup_one_property<const T, Tag> {
    AUTOBOOST_STATIC_CONSTANT(bool, found = (lookup_one_property_internal<T, Tag>::found));
  ...
  template <class P>
  struct has_property : autoboost::mpl::true_ {};
  template <>
  struct has_property<no_property> : autoboost::mpl::false_ {};

} // namespace autoboost

#include <autoboost/pending/detail/property.hpp>
...
namespace detail {
  ...
  struct remove_first_property {
    template <typename F>
    struct result {
      typedef typename autoboost::function_traits<F>::arg1_type a1;
      typedef typename autoboost::remove_reference<a1>::type non_ref;
      typedef typename non_ref::next_type nx;
      typedef typename autoboost::mpl::if_<autoboost::is_const<non_ref>, autoboost::add_const<nx>, nx>::type with_const;
      typedef typename autoboost::add_reference<with_const>::type type;
    };
