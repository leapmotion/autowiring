...
#ifndef AUTOBOOST_GRAPH_DETAIL_CONTAINER_TRAITS_H
#define AUTOBOOST_GRAPH_DETAIL_CONTAINER_TRAITS_H
...
#include <map>
#include <set>
#include <autoboost/unordered_set.hpp>
#include <autoboost/unordered_map.hpp>

#if !defined AUTOBOOST_NO_SLIST
#  ifdef AUTOBOOST_SLIST_HEADER
#    include AUTOBOOST_SLIST_HEADER
#  else
#    include <slist>
#  endif
#endif

#ifndef AUTOBOOST_NO_CXX11_HDR_UNORDERED_SET
#include <unordered_set>
#endif

#ifndef AUTOBOOST_NO_CXX11_HDR_UNORDERED_MAP
#include <unordered_map>
#endif

#ifdef AUTOBOOST_NO_CXX11_RVALUE_REFERENCES
#define AUTOBOOST_PENDING_FWD_TYPE(type) const type&
#define AUTOBOOST_PENDING_FWD_VALUE(type, var) (var)
#else
#define AUTOBOOST_PENDING_FWD_TYPE(type) type&&
#define AUTOBOOST_PENDING_FWD_VALUE(type, var) (std::forward<type>((var)))
#endif

// The content of this file is in 'graph_detail' because otherwise
// there will be name clashes with
// sandbox/autoboost/sequence_algo/container_traits.hpp
// The 'detail' subnamespace will still cause problems.
namespace autoboost { namespace graph_detail {
  ...
  // std::slist
#ifndef AUTOBOOST_NO_SLIST
  template <class T, class Alloc>
  struct container_traits<AUTOBOOST_STD_EXTENSION_NAMESPACE::slist<T,Alloc> > {
    typedef front_insertion_sequence_tag category;
    typedef stable_tag iterator_stability;
  };
  template <class T, class Alloc>
  front_insertion_sequence_tag container_category(
  const AUTOBOOST_STD_EXTENSION_NAMESPACE::slist<T,Alloc>&
  )
 ...
 // hash_set, hash_map
  ...
  struct container_traits< autoboost::unordered_set<Key,Eq,Hash,Alloc> > {
    typedef unordered_set_tag category;
    typedef unstable_tag iterator_stability;
  };
  template <class Key, class T, class Eq, class Hash, class Alloc>
  struct container_traits< autoboost::unordered_map<Key,T,Eq,Hash,Alloc> > {
    typedef unordered_map_tag category;
    typedef unstable_tag iterator_stability;
  };
  template <class Key, class Eq, class Hash, class Alloc>
  struct container_traits< autoboost::unordered_multiset<Key,Eq,Hash,Alloc> > {
    typedef unordered_multiset_tag category;
    typedef unstable_tag iterator_stability;
  };
  template <class Key, class T, class Eq, class Hash, class Alloc>
  struct container_traits< autoboost::unordered_multimap<Key,T,Eq,Hash,Alloc> > {
  ...
  template <class Key, class Eq, class Hash, class Alloc>
  unordered_set_tag
  container_category(const autoboost::unordered_set<Key,Eq,Hash,Alloc>&)
  { return unordered_set_tag(); }

  template <class Key, class T, class Eq, class Hash, class Alloc>
  unordered_map_tag
  container_category(const autoboost::unordered_map<Key,T,Eq,Hash,Alloc>&)
  { return unordered_map_tag(); }

  template <class Key, class Eq, class Hash, class Alloc>
  unstable_tag iterator_stability(const autoboost::unordered_set<Key,Eq,Hash,Alloc>&)
  { return unstable_tag(); }

  template <class Key, class T, class Eq, class Hash, class Alloc>
  unstable_tag iterator_stability(const autoboost::unordered_map<Key,T,Eq,Hash,Alloc>&)
  { return unstable_tag(); }
  template <class Key, class Eq, class Hash, class Alloc>
  unordered_multiset_tag
  container_category(const autoboost::unordered_multiset<Key,Eq,Hash,Alloc>&)
  { return unordered_multiset_tag(); }

  template <class Key, class T, class Eq, class Hash, class Alloc>
  unordered_multimap_tag
  container_category(const autoboost::unordered_multimap<Key,T,Eq,Hash,Alloc>&)
  { return unordered_multimap_tag(); }

  template <class Key, class Eq, class Hash, class Alloc>
  unstable_tag
  iterator_stability(const autoboost::unordered_multiset<Key,Eq,Hash,Alloc>&)
  { return unstable_tag(); }

  template <class Key, class T, class Eq, class Hash, class Alloc>
  unstable_tag
  iterator_stability(const autoboost::unordered_multimap<Key,T,Eq,Hash,Alloc>&)
  { return unstable_tag(); }
  ...
  push_dispatch(Container& c, AUTOBOOST_PENDING_FWD_TYPE(T) v, back_insertion_sequence_tag)
  {
    c.push_back(AUTOBOOST_PENDING_FWD_VALUE(T, v));
    return std::make_pair(autoboost::prior(c.end()), true);
  }

  template <class Container, class T>
  std::pair<typename Container::iterator, bool>
  push_dispatch(Container& c, AUTOBOOST_PENDING_FWD_TYPE(T) v, front_insertion_sequence_tag)
  {
    c.push_front(AUTOBOOST_PENDING_FWD_VALUE(T, v));
    return std::make_pair(c.begin(), true);
  }

  template <class AssociativeContainer, class T>
  std::pair<typename AssociativeContainer::iterator, bool>
  push_dispatch(AssociativeContainer& c, AUTOBOOST_PENDING_FWD_TYPE(T) v,
                unique_associative_container_tag)
  {
    return c.insert(AUTOBOOST_PENDING_FWD_VALUE(T, v));
  }
  ...
  {
    return std::make_pair(c.insert(AUTOBOOST_PENDING_FWD_VALUE(T, v)), true);
  }

  template <class Container, class T>
  std::pair<typename Container::iterator,bool>
  push(Container& c, AUTOBOOST_PENDING_FWD_TYPE(T) v)
  {
    return push_dispatch(c, AUTOBOOST_PENDING_FWD_VALUE(T, v), container_category(c));
  }
...
}} // namespace autoboost::graph_detail

#undef AUTOBOOST_PENDING_FWD_TYPE
#undef AUTOBOOST_PENDING_FWD_VALUE

#endif // AUTOBOOST_GRAPH_DETAIL_CONTAINER_TRAITS_H
