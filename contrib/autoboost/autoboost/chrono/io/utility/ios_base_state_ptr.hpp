...
#ifndef AUTOBOOST_CHRONO_UTILITY_IOS_BASE_STATE_PTR_HPP
#define AUTOBOOST_CHRONO_UTILITY_IOS_BASE_STATE_PTR_HPP
...
namespace autoboost
{
  namespace chrono
  {
    ...
    public:
       ...
       */
      T const* get() const AUTOBOOST_NOEXCEPT
      {
       ...
       * @return the retrieved pointer.
       */
      T const * operator->() const AUTOBOOST_NOEXCEPT
      {
        return get();
      }
       ...
       */
      void reset(T* new_ptr = 0)AUTOBOOST_NOEXCEPT
      {
        register_once(index(), ios_);
        void*& pw = ios_.pword(index());
      ...
      std::ios_base& getios()AUTOBOOST_NOEXCEPT
      {
        return ios_;
      }
      std::ios_base& getios() const AUTOBOOST_NOEXCEPT
      {
        return ios_;
      }
      /**
       ...
       */
      operator std::ios_base&() AUTOBOOST_NOEXCEPT
      {
        return ios_;
      }
      /**
       ...
       */
      operator std::ios_base&() const AUTOBOOST_NOEXCEPT
      {
        return ios_;
      }
    private:
    ...
    public:
      ...
      void reset(T* new_value) AUTOBOOST_NOEXCEPT
      {
        AUTOBOOST_ASSERT(new_value!=0);
    ...
    public:
       ...
       */
      long flags() const AUTOBOOST_NOEXCEPT
      {
        return value();
      }
       ...
       */
      long flags(long v)AUTOBOOST_NOEXCEPT
      {
        long tmp = flags();
        ref() = v;
        return tmp;
       ...
       */
      operator std::ios_base const&() const AUTOBOOST_NOEXCEPT
      {
        return ios_;
      }
    private:
      long value() const AUTOBOOST_NOEXCEPT
      {
        return ios_.iword(index());
      }
      long& ref()AUTOBOOST_NOEXCEPT
      {
        return ios_.iword(index());
      }
      static inline int index()
