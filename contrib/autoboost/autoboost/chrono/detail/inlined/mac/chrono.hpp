...
system_clock::now() AUTOBOOST_NOEXCEPT
{
    timeval tv;
    gettimeofday(&tv, 0);
...
time_t
system_clock::to_time_t(const time_point& t) AUTOBOOST_NOEXCEPT
{
...
system_clock::time_point
system_clock::from_time_t(time_t t) AUTOBOOST_NOEXCEPT
{
...
steady_clock::now(system::error_code & ec)
{
    ...
    if ( err != 0  )
    {
        ...
        {
            ec.assign( err, AUTOBOOST_CHRONO_SYSTEM_CATEGORY );
            return time_point();
        }
    }
    if (!AUTOBOOST_CHRONO_IS_THROWS(ec))
