...
#ifndef AUTOBOOST_ASIO_BUFFERED_READ_STREAM_HPP
#define AUTOBOOST_ASIO_BUFFERED_READ_STREAM_HPP
...
#include <autoboost/asio/detail/buffer_resize_guard.hpp>
...
public:
      ...
      AUTOBOOST_ASIO_MOVE_ARG(WriteHandler) handler)
  {
    ...
    next_layer_.async_write_some(buffers,
        AUTOBOOST_ASIO_MOVE_CAST(AUTOBOOST_ASIO_HANDLER_TYPE(WriteHandler,
  ...
  std::size_t fill(autoboost::system::error_code& ec);

  /// Start an asynchronous fill.
  template <typename ReadHandler>
  AUTOBOOST_ASIO_INITFN_RESULT_TYPE(ReadHandler,
      void (autoboost::system::error_code, std::size_t))
  async_fill(AUTOBOOST_ASIO_MOVE_ARG(ReadHandler) handler);
...
private:
  ...
  {
    std::size_t bytes_copied = autoboost::asio::buffer_copy(
  ...
  {
    return autoboost::asio::buffer_copy(buffers, storage_.data(), storage_.size());
  }
...
#include <autoboost/asio/impl/buffered_read_stream.hpp>

#endif // AUTOBOOST_ASIO_BUFFERED_READ_STREAM_HPP
