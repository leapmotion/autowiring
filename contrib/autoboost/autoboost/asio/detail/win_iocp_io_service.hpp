...
#ifndef AUTOBOOST_ASIO_DETAIL_WIN_IOCP_IO_SERVICE_HPP
#define AUTOBOOST_ASIO_DETAIL_WIN_IOCP_IO_SERVICE_HPP
...
class win_iocp_io_service
  : public autoboost::asio::detail::service_base<win_iocp_io_service>
{
public:

  // Constructor. Specifies a concurrency hint that is passed through to the
  // underlying I/O completion port.
  AUTOBOOST_ASIO_DECL win_iocp_io_service(autoboost::asio::io_service& io_service,
  ...
  AUTOBOOST_ASIO_DECL autoboost::system::error_code register_handle(
      HANDLE handle, autoboost::system::error_code& ec);

  // Run the event loop until stopped or no more work.
  AUTOBOOST_ASIO_DECL size_t run(autoboost::system::error_code& ec);

  // Run until stopped or one operation is performed.
  AUTOBOOST_ASIO_DECL size_t run_one(autoboost::system::error_code& ec);

  // Poll for operations without blocking.
  AUTOBOOST_ASIO_DECL size_t poll(autoboost::system::error_code& ec);

  // Poll for one operation without blocking.
  AUTOBOOST_ASIO_DECL size_t poll_one(autoboost::system::error_code& ec);
  ...
  AUTOBOOST_ASIO_DECL void post_deferred_completion(win_iocp_operation* op);

  // Request invocation of the given operation and return immediately. Assumes
  // that work_started() was previously called for the operations.
  AUTOBOOST_ASIO_DECL void post_deferred_completions(
  ...
  AUTOBOOST_ASIO_DECL void on_pending(win_iocp_operation* op);

  // Called after starting an overlapped I/O operation that completed
  // immediately. The caller must have already called work_started() prior to
  // starting the operation.
  AUTOBOOST_ASIO_DECL void on_completion(win_iocp_operation* op,
  ...
  // starting the operation.
  AUTOBOOST_ASIO_DECL void on_completion(win_iocp_operation* op,
      const autoboost::system::error_code& ec, DWORD bytes_transferred = 0);
...
private:
  ...
  // either 0 or 1).
  AUTOBOOST_ASIO_DECL size_t do_one(bool block, autoboost::system::error_code& ec);

  // Helper to calculate the GetQueuedCompletionStatus timeout.
  AUTOBOOST_ASIO_DECL static DWORD get_gqcs_timeout();
...
#include <autoboost/asio/detail/impl/win_iocp_io_service.hpp>
...
#endif // AUTOBOOST_ASIO_DETAIL_WIN_IOCP_IO_SERVICE_HPP
