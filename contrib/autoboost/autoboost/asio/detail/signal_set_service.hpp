...
#ifndef AUTOBOOST_ASIO_DETAIL_SIGNAL_SET_SERVICE_HPP
#define AUTOBOOST_ASIO_DETAIL_SIGNAL_SET_SERVICE_HPP
...
#include <autoboost/asio/detail/signal_handler.hpp>
#include <autoboost/asio/detail/signal_op.hpp>
...
public:
  ...
  // Constructor.
  AUTOBOOST_ASIO_DECL signal_set_service(autoboost::asio::io_service& io_service);

  // Destructor.
  AUTOBOOST_ASIO_DECL ~signal_set_service();
  ...
  AUTOBOOST_ASIO_DECL autoboost::system::error_code add(implementation_type& impl,
      int signal_number, autoboost::system::error_code& ec);

  // Remove a signal to a signal_set.
  AUTOBOOST_ASIO_DECL autoboost::system::error_code remove(implementation_type& impl,
      int signal_number, autoboost::system::error_code& ec);

  // Remove all signals from a signal_set.
  AUTOBOOST_ASIO_DECL autoboost::system::error_code clear(implementation_type& impl,
  ...
  void async_wait(implementation_type& impl, Handler& handler)
  {
    ...
    AUTOBOOST_ASIO_HANDLER_CREATION((p.p, "signal_set", &impl, "async_wait"));
  ...
  AUTOBOOST_ASIO_DECL static void deliver_signal(int signal_number);

private:
  // Helper function to add a service to the global signal state.
  AUTOBOOST_ASIO_DECL static void add_service(signal_set_service* service);

  // Helper function to remove a service from the global signal state.
  AUTOBOOST_ASIO_DECL static void remove_service(signal_set_service* service);

  // Helper function to create the pipe descriptors.
  AUTOBOOST_ASIO_DECL static void open_descriptors();

  // Helper function to close the pipe descriptors.
  AUTOBOOST_ASIO_DECL static void close_descriptors();

  // Helper function to start a wait operation.
  AUTOBOOST_ASIO_DECL void start_wait_op(implementation_type& impl, signal_op* op);
...
#endif // AUTOBOOST_ASIO_DETAIL_SIGNAL_SET_SERVICE_HPP
