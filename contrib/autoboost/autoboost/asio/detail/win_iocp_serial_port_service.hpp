...
#ifndef AUTOBOOST_ASIO_DETAIL_WIN_IOCP_SERIAL_PORT_SERVICE_HPP
#define AUTOBOOST_ASIO_DETAIL_WIN_IOCP_SERIAL_PORT_SERVICE_HPP
...
#if defined(AUTOBOOST_ASIO_HAS_IOCP) && defined(AUTOBOOST_ASIO_HAS_SERIAL_PORT)
...
public:
  ...
  // Constructor.
  AUTOBOOST_ASIO_DECL win_iocp_serial_port_service(
  ...
  AUTOBOOST_ASIO_DECL autoboost::system::error_code open(implementation_type& impl,
  ...
  autoboost::system::error_code send_break(implementation_type&,
      autoboost::system::error_code& ec)
  {
...
private:
  ...
  typedef autoboost::system::error_code (*store_function_type)(
      const void*, ::DCB&, autoboost::system::error_code&);

  // Helper function template to store a serial port option.
  template <typename SettableSerialPortOption>
  static autoboost::system::error_code store_option(const void* option,
      ::DCB& storage, autoboost::system::error_code& ec)
  {
  ...
  AUTOBOOST_ASIO_DECL autoboost::system::error_code do_set_option(
      ...
      const void* option, autoboost::system::error_code& ec);

  // Function pointer type for loading a serial port option.
  typedef autoboost::system::error_code (*load_function_type)(
      void*, const ::DCB&, autoboost::system::error_code&);

  // Helper function template to load a serial port option.
  template <typename GettableSerialPortOption>
  static autoboost::system::error_code load_option(void* option,
      const ::DCB& storage, autoboost::system::error_code& ec)
  {
    return static_cast<GettableSerialPortOption*>(option)->load(storage, ec);
  }

  // Helper function to get a serial port option.
  AUTOBOOST_ASIO_DECL autoboost::system::error_code do_get_option(
      const implementation_type& impl, load_function_type load,
      void* option, autoboost::system::error_code& ec) const;
...
#endif // defined(AUTOBOOST_ASIO_HAS_IOCP) && defined(AUTOBOOST_ASIO_HAS_SERIAL_PORT)

#endif // AUTOBOOST_ASIO_DETAIL_WIN_IOCP_SERIAL_PORT_SERVICE_HPP
