...
#ifndef AUTOBOOST_ASIO_DETAIL_IMPL_WIN_IOCP_HANDLE_SERVICE_IPP
#define AUTOBOOST_ASIO_DETAIL_IMPL_WIN_IOCP_HANDLE_SERVICE_IPP
...
{
public:
  explicit overlapped_wrapper(autoboost::system::error_code& ec)
  {
    Internal = 0;
    InternalHigh = 0;
    Offset = 0;
...
win_iocp_handle_service::win_iocp_handle_service(
  ...
  : iocp_service_(autoboost::asio::use_service<win_iocp_io_service>(io_service)),
    mutex_(),
    impl_list_(0)
{
}
...
autoboost::system::error_code win_iocp_handle_service::assign(
    win_iocp_handle_service::implementation_type& impl,
...
autoboost::system::error_code win_iocp_handle_service::close(
    win_iocp_handle_service::implementation_type& impl,
    autoboost::system::error_code& ec)
{
  if (is_open(impl))
  {
    AUTOBOOST_ASIO_HANDLER_OPERATION(("handle", &impl, "close"));
...
autoboost::system::error_code win_iocp_handle_service::cancel(
    win_iocp_handle_service::implementation_type& impl,
    autoboost::system::error_code& ec)
{
  if (!is_open(impl))
  ...
  AUTOBOOST_ASIO_HANDLER_OPERATION(("handle", &impl, "cancel"));
...
size_t win_iocp_handle_service::do_write(
    ...
    const autoboost::asio::const_buffer& buffer, autoboost::system::error_code& ec)
{
  if (!is_open(impl))
  {
    ec = autoboost::asio::error::bad_descriptor;
...
void win_iocp_handle_service::start_write_op(
    win_iocp_handle_service::implementation_type& impl, uint64_t offset,
    const autoboost::asio::const_buffer& buffer, operation* op)
{
  ...
  else if (autoboost::asio::buffer_size(buffer) == 0)
  {
    // A request to write 0 bytes on a handle is a no-op.
  ...
  else
  {
    ...
    BOOL ok = ::WriteFile(impl.handle_,
        autoboost::asio::buffer_cast<LPCVOID>(buffer),
        static_cast<DWORD>(autoboost::asio::buffer_size(buffer)),
...
size_t win_iocp_handle_service::do_read(
    win_iocp_handle_service::implementation_type& impl, uint64_t offset,
    const autoboost::asio::mutable_buffer& buffer, autoboost::system::error_code& ec)
{
  if (!is_open(impl))
  {
    ec = autoboost::asio::error::bad_descriptor;
...
void win_iocp_handle_service::start_read_op(
    win_iocp_handle_service::implementation_type& impl, uint64_t offset,
    const autoboost::asio::mutable_buffer& buffer, operation* op)
{
  update_cancellation_thread_id(impl);
  iocp_service_.work_started();
...
#endif // AUTOBOOST_ASIO_DETAIL_IMPL_WIN_IOCP_HANDLE_SERVICE_IPP
