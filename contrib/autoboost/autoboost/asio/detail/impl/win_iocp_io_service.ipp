...
#ifndef AUTOBOOST_ASIO_DETAIL_IMPL_WIN_IOCP_IO_SERVICE_IPP
#define AUTOBOOST_ASIO_DETAIL_IMPL_WIN_IOCP_IO_SERVICE_IPP
...
win_iocp_io_service::win_iocp_io_service(
    autoboost::asio::io_service& io_service, size_t concurrency_hint)
  : autoboost::asio::detail::service_base<win_iocp_io_service>(io_service),
    iocp_(),
    outstanding_work_(0),
    stopped_(0),
...
{
  AUTOBOOST_ASIO_HANDLER_TRACKING_INIT;
  ...
  if (!iocp_.handle)
  {
    ...
    autoboost::asio::detail::throw_error(ec, "iocp");
  }
}
...
autoboost::system::error_code win_iocp_io_service::register_handle(
    HANDLE handle, autoboost::system::error_code& ec)
{
...
size_t win_iocp_io_service::run(autoboost::system::error_code& ec)
{
...
size_t win_iocp_io_service::run_one(autoboost::system::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_service::poll(autoboost::system::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_service::poll_one(autoboost::system::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
  ...
  if (::InterlockedExchange(&stopped_, 1) == 0)
  {
    if (::InterlockedExchange(&stop_event_posted_, 1) == 0)
    {
      if (!::PostQueuedCompletionStatus(iocp_.handle, 0, 0, 0))
      {
        ...
        autoboost::system::error_code ec(last_error,
            autoboost::asio::error::get_system_category());
        autoboost::asio::detail::throw_error(ec, "pqcs");
      }
    }
  }
}
...
void win_iocp_io_service::on_completion(win_iocp_operation* op,
    const autoboost::system::error_code& ec, DWORD bytes_transferred)
{
  // Flag that the operation is ready for invocation.
  op->ready_ = 1;
...
size_t win_iocp_io_service::do_one(bool block, autoboost::system::error_code& ec)
{
  for (;;)
  {
    ...
    if (overlapped)
    {
      ...
      autoboost::system::error_code result_ec(last_error,
          autoboost::asio::error::get_system_category());
      ...
      {
        result_ec = autoboost::system::error_code(static_cast<int>(op->Offset),
            *reinterpret_cast<autoboost::system::error_category*>(op->Internal));
...
void win_iocp_io_service::do_add_timer_queue(timer_queue_base& queue)
{
  ...
  if (!waitable_timer_.handle)
  {
    ...
    if (waitable_timer_.handle == 0)
    {
      ...
      autoboost::system::error_code ec(last_error,
          autoboost::asio::error::get_system_category());
      autoboost::asio::detail::throw_error(ec, "timer");
    }
...
#endif // AUTOBOOST_ASIO_DETAIL_IMPL_WIN_IOCP_IO_SERVICE_IPP
