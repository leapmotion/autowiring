...
#ifndef AUTOBOOST_ASIO_DETAIL_IMPL_WINRT_SSOCKET_SERVICE_BASE_IPP
#define AUTOBOOST_ASIO_DETAIL_IMPL_WINRT_SSOCKET_SERVICE_BASE_IPP
...
autoboost::system::error_code winrt_ssocket_service_base::close(
    winrt_ssocket_service_base::base_implementation_type& impl,
    ...
    void* addr, std::size_t addr_len, autoboost::system::error_code& ec) const
{
  ...
  try
  {
    ...
    case AUTOBOOST_ASIO_OS_DEF(AF_INET):
      ...
      {
        socket_ops::inet_pton(AUTOBOOST_ASIO_OS_DEF(AF_INET), addr_string.c_str(),
    ...
    case AUTOBOOST_ASIO_OS_DEF(AF_INET6):
      ...
      {
        socket_ops::inet_pton(AUTOBOOST_ASIO_OS_DEF(AF_INET6), addr_string.c_str(),
...
autoboost::system::error_code winrt_ssocket_service_base::do_set_option(
    winrt_ssocket_service_base::base_implementation_type& impl,
    ...
    std::size_t optlen, autoboost::system::error_code& ec)
{
  ...
  try
  {
    if (level == AUTOBOOST_ASIO_OS_DEF(SOL_SOCKET)
        && optname == AUTOBOOST_ASIO_OS_DEF(SO_KEEPALIVE))
    {
    ...
    else if (level == AUTOBOOST_ASIO_OS_DEF(IPPROTO_TCP)
        && optname == AUTOBOOST_ASIO_OS_DEF(TCP_NODELAY))
    {
      if (optlen == sizeof(int))
      {
        int value = 0;
...
autoboost::system::error_code winrt_ssocket_service_base::do_connect(
    winrt_ssocket_service_base::base_implementation_type& impl,
    const void* addr, autoboost::system::error_code& ec)
{
  ...
  {
  case AUTOBOOST_ASIO_OS_DEF(AF_INET):
    socket_ops::inet_ntop(AUTOBOOST_ASIO_OS_DEF(AF_INET),
    ...
    break;
  case AUTOBOOST_ASIO_OS_DEF(AF_INET6):
    socket_ops::inet_ntop(AUTOBOOST_ASIO_OS_DEF(AF_INET6),
    ...
    const void* addr, winrt_async_op<void>* op, bool is_continuation)
{
  ...
  case AUTOBOOST_ASIO_OS_DEF(AF_INET6):
    ...
    break;
  default:
    op->ec_ = autoboost::asio::error::address_family_not_supported;
    break;
  }
    ...
    socket_base::message_flags flags, autoboost::system::error_code& ec)
{
  ...
  try
  {
    buffer_sequence_adapter<autoboost::asio::const_buffer,
      autoboost::asio::const_buffers_1> bufs(autoboost::asio::buffer(data));
      ...
      winrt_async_op<unsigned int>* op, bool is_continuation)
{
  if (flags)
  {
    op->ec_ = autoboost::asio::error::operation_not_supported;
    io_service_.post_immediate_completion(op, is_continuation);
    return;
  }
  ...
  {
    op->ec_ = autoboost::system::error_code(e->HResult,
        autoboost::system::system_category());
    io_service_.post_immediate_completion(op, is_continuation);
  }
}
    ...
    socket_base::message_flags flags, autoboost::system::error_code& ec)
{
  ...
  try
  {
    buffer_sequence_adapter<autoboost::asio::mutable_buffer,
        autoboost::asio::mutable_buffers_1> bufs(autoboost::asio::buffer(data));
...
#endif // AUTOBOOST_ASIO_DETAIL_IMPL_WINRT_SSOCKET_SERVICE_BASE_IPP
