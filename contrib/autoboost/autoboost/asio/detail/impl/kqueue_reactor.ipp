...
#ifndef AUTOBOOST_ASIO_DETAIL_IMPL_KQUEUE_REACTOR_IPP
#define AUTOBOOST_ASIO_DETAIL_IMPL_KQUEUE_REACTOR_IPP
...
# define AUTOBOOST_ASIO_KQUEUE_EV_SET(ev, ident, filt, flags, fflags, data, udata) \
...
kqueue_reactor::kqueue_reactor(autoboost::asio::io_service& io_service)
  : autoboost::asio::detail::service_base<kqueue_reactor>(io_service),
    ...
    shutdown_(false)
{
  ...
  AUTOBOOST_ASIO_KQUEUE_EV_SET(&events[0], interrupter_.read_descriptor(),
      EVFILT_READ, EV_ADD, 0, 0, &interrupter_);
  if (::kevent(kqueue_fd_, events, 1, 0, 0, 0) == -1)
  {
    autoboost::system::error_code error(errno,
        autoboost::asio::error::get_system_category());
    autoboost::asio::detail::throw_error(error);
  }
}
...
void kqueue_reactor::fork_service(autoboost::asio::io_service::fork_event fork_ev)
{
  if (fork_ev == autoboost::asio::io_service::fork_child)
  {
    ...
    if (::kevent(kqueue_fd_, events, 1, 0, 0, 0) == -1)
    {
      autoboost::system::error_code ec(errno,
          autoboost::asio::error::get_system_category());
      autoboost::asio::detail::throw_error(ec, "kqueue interrupter registration");
    }
        ...
        state != 0; state = state->next_)
    {
      ...
      {
        AUTOBOOST_ASIO_KQUEUE_EV_SET(&events[0], state->descriptor_,
            EVFILT_READ, EV_ADD | EV_CLEAR, 0, 0, state);
        AUTOBOOST_ASIO_KQUEUE_EV_SET(&events[1], state->descriptor_,
            EVFILT_WRITE, EV_ADD | EV_CLEAR, 0, 0, state);
        if (::kevent(kqueue_fd_, events, state->num_kevents_, 0, 0, 0) == -1)
        {
          autoboost::system::error_code ec(errno,
              autoboost::asio::error::get_system_category());
          autoboost::asio::detail::throw_error(ec, "kqueue re-registration");
        }
      }
    }
  }
    ...
    kqueue_reactor::per_descriptor_data& descriptor_data, reactor_op* op)
{
  ...
  struct kevent events[1];
  AUTOBOOST_ASIO_KQUEUE_EV_SET(&events[0], descriptor, EVFILT_READ,
  ...
  if (descriptor_data->op_queue_[op_type].empty())
  {
          ...
          || descriptor_data->op_queue_[except_op].empty()))
    {
      ...
      if (descriptor_data->num_kevents_ < num_kevents[op_type])
      {
        ...
        AUTOBOOST_ASIO_KQUEUE_EV_SET(&events[0], descriptor, EVFILT_READ,
            EV_ADD | EV_CLEAR, 0, 0, descriptor_data);
        AUTOBOOST_ASIO_KQUEUE_EV_SET(&events[1], descriptor, EVFILT_WRITE,
            EV_ADD | EV_CLEAR, 0, 0, descriptor_data);
        ...
        {
          op->ec_ = autoboost::system::error_code(errno,
              autoboost::asio::error::get_system_category());
          io_service_.post_immediate_completion(op, is_continuation);
          return;
        }
    ...
    kqueue_reactor::per_descriptor_data& descriptor_data)
{
  ...
  for (int i = 0; i < max_ops; ++i)
  {
    ...
    {
      op->ec_ = autoboost::asio::error::operation_aborted;
    ...
    kqueue_reactor::per_descriptor_data& descriptor_data, bool closing)
{
  ...
  if (!descriptor_data->shutdown_)
  {
    ...
    {
      struct kevent events[2];
      AUTOBOOST_ASIO_KQUEUE_EV_SET(&events[0], descriptor,
          EVFILT_READ, EV_DELETE, 0, 0, 0);
      AUTOBOOST_ASIO_KQUEUE_EV_SET(&events[1], descriptor,
  ...
  for (int i = 0; i < num_events; ++i)
  {
    ...
    else
    {
          ...
          && descriptor_data->op_queue_[write_op].empty())
      {
        ...
        AUTOBOOST_ASIO_KQUEUE_EV_SET(&delete_events[0],
...
int kqueue_reactor::do_kqueue_create()
{
  ...
  if (fd == -1)
  {
    autoboost::system::error_code ec(errno,
        autoboost::asio::error::get_system_category());
    autoboost::asio::detail::throw_error(ec, "kqueue");
  }
  return fd;
}
...
#undef AUTOBOOST_ASIO_KQUEUE_EV_SET

#include <autoboost/asio/detail/pop_options.hpp>

#endif // defined(AUTOBOOST_ASIO_HAS_KQUEUE)

#endif // AUTOBOOST_ASIO_DETAIL_IMPL_KQUEUE_REACTOR_IPP
