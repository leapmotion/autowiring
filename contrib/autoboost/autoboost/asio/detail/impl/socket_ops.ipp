...
#ifndef AUTOBOOST_ASIO_DETAIL_SOCKET_OPS_IPP
#define AUTOBOOST_ASIO_DETAIL_SOCKET_OPS_IPP
...
{
#if defined(AUTOBOOST_ASIO_WINDOWS) || defined(__CYGWIN__)
  ec = autoboost::system::error_code(WSAGetLastError(),
...
socket_type accept(socket_type s, socket_addr_type* addr,
    std::size_t* addrlen, autoboost::system::error_code& ec)
{
  if (s == invalid_socket)
  {
...
socket_type sync_accept(socket_type s, state_type state,
    socket_addr_type* addr, std::size_t* addrlen, autoboost::system::error_code& ec)
{
  // Accept a socket.
  for (;;)
  {
    ...
    // Operation failed.
    if (ec == autoboost::asio::error::would_block
        || ec == autoboost::asio::error::try_again)
    {
      if (state & user_set_non_blocking)
        return invalid_socket;
      // Fall through to retry operation.
    }
    else if (ec == autoboost::asio::error::connection_aborted)
    {
...
void complete_iocp_accept(socket_type s,
    ...
    socket_type new_socket, autoboost::system::error_code& ec)
{
  // Map non-portable errors to their portable counterparts.
  if (ec.value() == ERROR_NETNAME_DELETED)
    ec = autoboost::asio::error::connection_aborted;
...
bool non_blocking_accept(socket_type s,
    ...
    autoboost::system::error_code& ec, socket_type& new_socket)
{
  for (;;)
  {
    ...
    if (ec == autoboost::asio::error::interrupted)
      continue;
...
int close(socket_type s, state_type& state,
    bool destruction, autoboost::system::error_code& ec)
{
  int result = 0;
  if (s != invalid_socket)
  {
    ...
    if (destruction && (state & user_set_linger))
    {
      ...
      opt.l_onoff = 0;
      opt.l_linger = 0;
      autoboost::system::error_code ignored_ec;
    ...
    if (result != 0
        && (ec == autoboost::asio::error::would_block
          || ec == autoboost::asio::error::try_again))
    {
...
int shutdown(socket_type s, int what, autoboost::system::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = autoboost::asio::error::bad_descriptor;
    ...
    std::size_t addrlen, autoboost::system::error_code& ec)
{
  ...
  else if (ec == autoboost::asio::error::try_again)
    ec = autoboost::asio::error::no_buffer_space;
    ...
    std::size_t addrlen, autoboost::system::error_code& ec)
{
  ...
  if (ec != autoboost::asio::error::in_progress
      && ec != autoboost::asio::error::would_block)
  {
  ...
  ec = autoboost::system::error_code(connect_error,
      autoboost::asio::error::get_system_category());
}

#if defined(AUTOBOOST_ASIO_HAS_IOCP)

void complete_iocp_connect(socket_type s, autoboost::system::error_code& ec)
{
  // Map non-portable errors to their portable counterparts.
  switch (ec.value())
  {
  case ERROR_CONNECTION_REFUSED:
    ec = autoboost::asio::error::connection_refused;
    break;
  case ERROR_NETWORK_UNREACHABLE:
    ec = autoboost::asio::error::network_unreachable;
    break;
  case ERROR_HOST_UNREACHABLE:
    ec = autoboost::asio::error::host_unreachable;
    break;
  case ERROR_SEM_TIMEOUT:
    ec = autoboost::asio::error::timed_out;
    break;
  default:
    break;
  }
...
bool non_blocking_connect(socket_type s, autoboost::system::error_code& ec)
{
...
#else // defined(AUTOBOOST_ASIO_WINDOWS)
      // || defined(__CYGWIN__)
      // || defined(__SYMBIAN32__)
  pollfd fds;
  fds.fd = s;
...
int socketpair(int af, int type, int protocol,
    socket_type sv[2], autoboost::system::error_code& ec)
{
#if defined(AUTOBOOST_ASIO_WINDOWS) || defined(__CYGWIN__)
  (void)(af);
  (void)(type);
...
bool sockatmark(socket_type s, autoboost::system::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = autoboost::asio::error::bad_descriptor;
...
# if defined(ENOTTY)
  if (ec.value() == ENOTTY)
    ec = autoboost::asio::error::not_socket;
...
size_t available(socket_type s, autoboost::system::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = autoboost::asio::error::bad_descriptor;
...
int listen(socket_type s, int backlog, autoboost::system::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = autoboost::asio::error::bad_descriptor;
...
signed_size_type recv(socket_type s, buf* bufs, size_t count,
    int flags, autoboost::system::error_code& ec)
{
  clear_last_error();
#if defined(AUTOBOOST_ASIO_WINDOWS) || defined(__CYGWIN__)
  // Receive some data.
  ...
  if (ec.value() == ERROR_NETNAME_DELETED)
    ec = autoboost::asio::error::connection_reset;
...
size_t sync_recv(socket_type s, state_type state, buf* bufs,
    size_t count, int flags, bool all_empty, autoboost::system::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = autoboost::asio::error::bad_descriptor;
...
void complete_iocp_recv(state_type state,
    ...
    autoboost::system::error_code& ec, size_t bytes_transferred)
{
  // Map non-portable errors to their portable counterparts.
  if (ec.value() == ERROR_NETNAME_DELETED)
  {
...
signed_size_type recvmsg(socket_type s, buf* bufs, size_t count,
    int in_flags, int& out_flags, autoboost::system::error_code& ec)
{
  clear_last_error();
#if defined(AUTOBOOST_ASIO_WINDOWS) || defined(__CYGWIN__)
  out_flags = 0;
    ...
    autoboost::system::error_code& ec)
{
  ...
  if (af == AUTOBOOST_ASIO_OS_DEF(AF_INET6))
  {
...
int setsockopt(socket_type s, state_type& state, int level, int optname,
    const void* optval, std::size_t optlen, autoboost::system::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = autoboost::asio::error::bad_descriptor;
...
int getsockopt(socket_type s, state_type state, int level, int optname,
    void* optval, size_t* optlen, autoboost::system::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = autoboost::asio::error::bad_descriptor;
...
int getpeername(socket_type s, socket_addr_type* addr,
    std::size_t* addrlen, bool cached, autoboost::system::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = autoboost::asio::error::bad_descriptor;
...
#if defined(AUTOBOOST_ASIO_WINDOWS) && !defined(AUTOBOOST_ASIO_WINDOWS_APP) \
  || defined(__CYGWIN__)
  if (cached)
  {
    ...
    {
      ec = autoboost::asio::error::not_connected;
      return socket_error_retval;
    }
...
#else // defined(AUTOBOOST_ASIO_WINDOWS) && !defined(AUTOBOOST_ASIO_WINDOWS_APP)
      // || defined(__CYGWIN__)
  (void)cached;
#endif // defined(AUTOBOOST_ASIO_WINDOWS) && !defined(AUTOBOOST_ASIO_WINDOWS_APP)
       // || defined(__CYGWIN__)
...
int select(int nfds, fd_set* readfds, fd_set* writefds,
    fd_set* exceptfds, timeval* timeout, autoboost::system::error_code& ec)
{
  clear_last_error();
#if defined(AUTOBOOST_ASIO_WINDOWS) || defined(__CYGWIN__)
  ...
  // system clock (as wrapped by autoboost::posix_time::microsec_clock) typically
...
int poll_read(socket_type s, state_type state, autoboost::system::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = autoboost::asio::error::bad_descriptor;
  ...
  if (result == 0)
    ec = (state & user_set_non_blocking)
      ? autoboost::asio::error::would_block : autoboost::system::error_code();
  else if (result > 0)
    ec = autoboost::system::error_code();
  return result;
}

int poll_write(socket_type s, state_type state, autoboost::system::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = autoboost::asio::error::bad_descriptor;
...
int poll_connect(socket_type s, autoboost::system::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = autoboost::asio::error::bad_descriptor;
...
const char* inet_ntop(int af, const void* src, char* dest, size_t length,
    unsigned long scope_id, autoboost::system::error_code& ec)
{
  clear_last_error();
#if defined(AUTOBOOST_ASIO_WINDOWS_RUNTIME)
  using namespace std; // For sprintf.
  const unsigned char* bytes = static_cast<const unsigned char*>(src);
  if (af == AUTOBOOST_ASIO_OS_DEF(AF_INET))
  {
    sprintf_s(dest, length, "%u.%u.%u.%u",
        bytes[0], bytes[1], bytes[2], bytes[3]);
    return dest;
  }
  else if (af == AUTOBOOST_ASIO_OS_DEF(AF_INET6))
  {
    size_t n = 0, b = 0, z = 0;
    while (n < length && b < 16)
    {
  ...
  {
    ec = autoboost::asio::error::address_family_not_supported;
    return 0;
  }
#elif defined(AUTOBOOST_ASIO_WINDOWS) || defined(__CYGWIN__)
  using namespace std; // For memcpy.

  if (af != AUTOBOOST_ASIO_OS_DEF(AF_INET) && af != AUTOBOOST_ASIO_OS_DEF(AF_INET6))
  {
    ec = autoboost::asio::error::address_family_not_supported;
    return 0;
  }
  ...
  if (af == AUTOBOOST_ASIO_OS_DEF(AF_INET))
  {
    ...
    address.v4.sin_family = AUTOBOOST_ASIO_OS_DEF(AF_INET);
  ...
  else // AF_INET6
  {
    ...
    address.v6.sin6_family = AUTOBOOST_ASIO_OS_DEF(AF_INET6);
...
#if defined(AUTOBOOST_NO_ANSI_APIS) || (defined(_MSC_VER) && (_MSC_VER >= 1800))
  ...
  if (result == 0 && !ec)
    ec = autoboost::asio::error::invalid_argument;
  if (result != 0 && af == AUTOBOOST_ASIO_OS_DEF(AF_INET6) && scope_id != 0)
  {
    ...
    unsigned long* scope_id, autoboost::system::error_code& ec)
{
  ...
  const bool is_v6 = (af == AUTOBOOST_ASIO_OS_DEF(AF_INET6));
...
int gethostname(char* name, int namelen, autoboost::system::error_code& ec)
{
  clear_last_error();
#if defined(AUTOBOOST_ASIO_WINDOWS_RUNTIME)
  try
...
#else // defined(AUTOBOOST_ASIO_WINDOWS_RUNTIME)
...
inline autoboost::system::error_code translate_netdb_error(int error)
{
  switch (error)
  {
  case 0:
  ...
  case HOST_NOT_FOUND:
    return autoboost::asio::error::host_not_found;
  case TRY_AGAIN:
    return autoboost::asio::error::host_not_found_try_again;
  case NO_RECOVERY:
    return autoboost::asio::error::no_recovery;
  case NO_DATA:
    return autoboost::asio::error::no_data;
  default:
    AUTOBOOST_ASIO_ASSERT(false);
    return autoboost::asio::error::invalid_argument;
  }
}

inline hostent* gethostbyaddr(const char* addr, int length, int af,
    hostent* result, char* buffer, int buflength, autoboost::system::error_code& ec)
{
  clear_last_error();
#if defined(AUTOBOOST_ASIO_WINDOWS) || defined(__CYGWIN__)
  (void)(buffer);
...
inline hostent* gethostbyname(const char* name, int af, struct hostent* result,
    char* buffer, int buflength, int ai_flags, autoboost::system::error_code& ec)
{
  clear_last_error();
#if defined(AUTOBOOST_ASIO_WINDOWS) || defined(__CYGWIN__)
  (void)(buffer);
    ...
    const addrinfo_type* hints, gai_search (&search)[2])
{
  ...
  if (host == 0 || host[0] == '\0')
  {
    if (hints->ai_flags & AI_PASSIVE)
    {
      ...
      {
      case AUTOBOOST_ASIO_OS_DEF(AF_INET):
        search[search_count].host = "0.0.0.0";
        search[search_count].family = AUTOBOOST_ASIO_OS_DEF(AF_INET);
        ++search_count;
        break;
      case AUTOBOOST_ASIO_OS_DEF(AF_INET6):
        search[search_count].host = "0::0";
        search[search_count].family = AUTOBOOST_ASIO_OS_DEF(AF_INET6);
        ++search_count;
        break;
      case AUTOBOOST_ASIO_OS_DEF(AF_UNSPEC):
        search[search_count].host = "0::0";
        search[search_count].family = AUTOBOOST_ASIO_OS_DEF(AF_INET6);
        ++search_count;
        search[search_count].host = "0.0.0.0";
...
#else // defined(AUTOBOOST_ASIO_HAS_SECURE_RTL)
  *target = 0;
  if (max_size > 0)
  ...
  case AUTOBOOST_ASIO_OS_DEF(AF_INET):
    {
      ...
      if (sinptr == 0)
        return EAI_MEMORY;
      sinptr->sin_family = AUTOBOOST_ASIO_OS_DEF(AF_INET);
  ...
  case AUTOBOOST_ASIO_OS_DEF(AF_INET6):
    {
      ...
      if (sin6ptr == 0)
        return EAI_MEMORY;
      sin6ptr->sin6_family = AUTOBOOST_ASIO_OS_DEF(AF_INET6);
    ...
    const addrinfo_type* hintsp, addrinfo_type** result)
{
  ...
  hints.ai_family = AUTOBOOST_ASIO_OS_DEF(AF_UNSPEC);
  if (hintsp)
    hints = *hintsp;
  ...
  if (hints.ai_family != AUTOBOOST_ASIO_OS_DEF(AF_INET6))
  ...
  for (gai_search* sptr = search; sptr < search + search_count; ++sptr)
  {
    ...
    autoboost::system::error_code ec;
    if (socket_ops::inet_pton(AUTOBOOST_ASIO_OS_DEF(AF_INET),
          sptr->host, &inaddr, 0, ec) == 1)
    {
      if (hints.ai_family != AUTOBOOST_ASIO_OS_DEF(AF_UNSPEC)
          && hints.ai_family != AUTOBOOST_ASIO_OS_DEF(AF_INET))
      {
        freeaddrinfo_emulation(aihead);
        gai_free(canon);
        return EAI_FAMILY;
      }
      if (sptr->family == AUTOBOOST_ASIO_OS_DEF(AF_INET))
      {
        rc = gai_aistruct(&ainext, &hints, &inaddr, AUTOBOOST_ASIO_OS_DEF(AF_INET));
        if (rc != 0)
        {
          freeaddrinfo_emulation(aihead);
          gai_free(canon);
    ...
    if (socket_ops::inet_pton(AUTOBOOST_ASIO_OS_DEF(AF_INET6),
          sptr->host, &in6addr, 0, ec) == 1)
    {
      if (hints.ai_family != AUTOBOOST_ASIO_OS_DEF(AF_UNSPEC)
          && hints.ai_family != AUTOBOOST_ASIO_OS_DEF(AF_INET6))
      {
        freeaddrinfo_emulation(aihead);
        gai_free(canon);
        return EAI_FAMILY;
      }
      if (sptr->family == AUTOBOOST_ASIO_OS_DEF(AF_INET6))
      {
    ...
    if (hptr == 0)
    {
      ...
      freeaddrinfo_emulation(aihead);
      gai_free(canon);
      if (ec == autoboost::asio::error::host_not_found)
        return EAI_NONAME;
      if (ec == autoboost::asio::error::host_not_found_try_again)
        return EAI_AGAIN;
      if (ec == autoboost::asio::error::no_recovery)
        return EAI_FAIL;
      if (ec == autoboost::asio::error::no_data)
        return EAI_NONAME;
      return EAI_NONAME;
    }
...
inline autoboost::system::error_code getnameinfo_emulation(
    ...
    autoboost::system::error_code& ec)
{
  ...
  case AUTOBOOST_ASIO_OS_DEF(AF_INET):
    ...
    {
      return ec = autoboost::asio::error::invalid_argument;
    }
  ...
  case AUTOBOOST_ASIO_OS_DEF(AF_INET6):
    ...
    break;
  default:
    return ec = autoboost::asio::error::address_family_not_supported;
  }
  ...
  if (host && hostlen > 0)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        {
          return ec = autoboost::asio::error::host_not_found;
        }
  ...
  if (serv && servlen > 0)
  {
    if (flags & NI_NUMERICSERV)
    {
      if (servlen < 6)
      {
        return ec = autoboost::asio::error::no_buffer_space;
      }
#if defined(AUTOBOOST_ASIO_HAS_SECURE_RTL)
...
inline autoboost::system::error_code translate_addrinfo_error(int error)
{
  switch (error)
  {
  case 0:
  ...
  case EAI_FAIL:
    return autoboost::asio::error::no_recovery;
  case EAI_FAMILY:
    return autoboost::asio::error::address_family_not_supported;
  case EAI_MEMORY:
    return autoboost::asio::error::no_memory;
  case EAI_NONAME:
  ...
  case EAI_NODATA:
    ...
    return autoboost::asio::error::host_not_found;
  case EAI_SERVICE:
    return autoboost::asio::error::service_not_found;
  case EAI_SOCKTYPE:
    return autoboost::asio::error::socket_type_not_supported;
  default: // Possibly the non-portable EAI_SYSTEM.
#if defined(AUTOBOOST_ASIO_WINDOWS) || defined(__CYGWIN__)
    return autoboost::system::error_code(
        WSAGetLastError(), autoboost::asio::error::get_system_category());
#else
    return autoboost::system::error_code(
        errno, autoboost::asio::error::get_system_category());
#endif
  }
}

autoboost::system::error_code getaddrinfo(const char* host,
    const char* service, const addrinfo_type& hints,
    addrinfo_type** result, autoboost::system::error_code& ec)
{
...
# else
  ...
  return ec = translate_addrinfo_error(error);
# endif
#elif !defined(AUTOBOOST_ASIO_HAS_GETADDRINFO)
  int error = getaddrinfo_emulation(host, service, &hints, result);
  return ec = translate_addrinfo_error(error);
#else
  int error = ::getaddrinfo(host, service, &hints, result);
...
autoboost::system::error_code background_getaddrinfo(
...
autoboost::system::error_code getnameinfo(const socket_addr_type* addr,
    std::size_t addrlen, char* host, std::size_t hostlen,
    char* serv, std::size_t servlen, int flags, autoboost::system::error_code& ec)
{
#if defined(AUTOBOOST_ASIO_WINDOWS) || defined(__CYGWIN__)
# if defined(AUTOBOOST_ASIO_HAS_GETADDRINFO)
  // Building for Windows XP, Windows Server 2003, or later.
...
autoboost::system::error_code sync_getnameinfo(
    const socket_addr_type* addr, std::size_t addrlen,
    char* host, std::size_t hostlen, char* serv,
    std::size_t servlen, int sock_type, autoboost::system::error_code& ec)
{
...
autoboost::system::error_code background_getnameinfo(
    const weak_cancel_token_type& cancel_token,
    const socket_addr_type* addr, std::size_t addrlen,
    char* host, std::size_t hostlen, char* serv,
    std::size_t servlen, int sock_type, autoboost::system::error_code& ec)
...
#endif // AUTOBOOST_ASIO_DETAIL_SOCKET_OPS_IPP
