...
#ifndef AUTOBOOST_ASIO_DETAIL_IMPL_WIN_IOCP_SOCKET_SERVICE_BASE_IPP
#define AUTOBOOST_ASIO_DETAIL_IMPL_WIN_IOCP_SOCKET_SERVICE_BASE_IPP
    ...
    win_iocp_socket_service_base::base_implementation_type& other_impl)
{
  ...
  {
    // Insert implementation into linked list of all implementations.
    autoboost::asio::detail::mutex::scoped_lock lock(other_service.mutex_);
...
autoboost::system::error_code win_iocp_socket_service_base::close(
    win_iocp_socket_service_base::base_implementation_type& impl,
    autoboost::system::error_code& ec)
{
  if (is_open(impl))
  {
    AUTOBOOST_ASIO_HANDLER_OPERATION(("socket", &impl, "close"));
...
autoboost::system::error_code win_iocp_socket_service_base::cancel(
    win_iocp_socket_service_base::base_implementation_type& impl,
    autoboost::system::error_code& ec)
{
  if (!is_open(impl))
  ...
  AUTOBOOST_ASIO_HANDLER_OPERATION(("socket", &impl, "cancel"));
        ...
        ::GetModuleHandleA("KERNEL32"), "CancelIoEx"))
  {
    ...
    if (!cancel_io_ex(sock_as_handle, 0))
    {
      ...
      {
        ec = autoboost::system::error_code(last_error,
...
#else // defined(AUTOBOOST_ASIO_ENABLE_CANCELIO)
  else
  {
...
autoboost::system::error_code win_iocp_socket_service_base::do_open(
    win_iocp_socket_service_base::base_implementation_type& impl,
    int family, int type, int protocol, autoboost::system::error_code& ec)
{
  if (is_open(impl))
  {
...
autoboost::system::error_code win_iocp_socket_service_base::do_assign(
    win_iocp_socket_service_base::base_implementation_type& impl,
    int type, socket_type native_socket, autoboost::system::error_code& ec)
{
  if (is_open(impl))
  {
    ec = autoboost::asio::error::already_open;
    ...
    socket_base::message_flags flags, bool noop, operation* op)
{
  ...
  if (noop)
    ...
    iocp_service_.on_completion(op, autoboost::asio::error::bad_descriptor);
  else
  {
    ...
    int protocol, void* output_buffer, DWORD address_length, operation* op)
{
  ...
  if (!is_open(impl))
    ...
    iocp_service_.on_completion(op, autoboost::asio::error::already_open);
  else
  {
    autoboost::system::error_code ec;
    ...
    int op_type, reactor_op* op)
{
  ...
  if (is_open(impl))
  {
    ...
    op->ec_ = autoboost::asio::error::bad_descriptor;
    ...
    std::size_t addrlen, win_iocp_socket_connect_op_base* op)
{
  ...
  if (family == AUTOBOOST_ASIO_OS_DEF(AF_INET)
      || family == AUTOBOOST_ASIO_OS_DEF(AF_INET6))
  {
    if (connect_ex_fn connect_ex = get_connect_ex(impl, type))
    {
      ...
      socket_ops::bind(impl.socket_, &a.base,
          family == AUTOBOOST_ASIO_OS_DEF(AF_INET)
          ? sizeof(a.v4) : sizeof(a.v6), op->ec_);
      if (op->ec_ && op->ec_ != autoboost::asio::error::invalid_argument)
      {
        iocp_service_.post_immediate_completion(op, false);
        return;
      }
        ...
        impl.socket_, impl.state_, true, op->ec_))
  {
    ...
    {
      if (op->ec_ == autoboost::asio::error::in_progress
          || op->ec_ == autoboost::asio::error::would_block)
      {
        op->ec_ = autoboost::system::error_code();
...
{
#if defined(AUTOBOOST_ASIO_DISABLE_CONNECTEX)
  (void)impl;
  (void)type;
  return 0;
#else // defined(AUTOBOOST_ASIO_DISABLE_CONNECTEX)
  if (type != AUTOBOOST_ASIO_OS_DEF(SOCK_STREAM)
      && type != AUTOBOOST_ASIO_OS_DEF(SOCK_SEQPACKET))
    return 0;
...
#endif // defined(AUTOBOOST_ASIO_DISABLE_CONNECTEX)
}
...
#endif // AUTOBOOST_ASIO_DETAIL_IMPL_WIN_IOCP_SOCKET_SERVICE_BASE_IPP
