...
#ifndef AUTOBOOST_ASIO_BASIC_SOCKET_STREAMBUF_HPP
#define AUTOBOOST_ASIO_BASIC_SOCKET_STREAMBUF_HPP
...
# include <autoboost/asio/detail/variadic_templates.hpp>
...
# define AUTOBOOST_ASIO_PRIVATE_CONNECT_DEF(n) \
  template <AUTOBOOST_ASIO_VARIADIC_TPARAMS(n)> \
  basic_socket_streambuf<Protocol, StreamSocketService, \
    Time, TimeTraits, TimerService>* connect(AUTOBOOST_ASIO_VARIADIC_PARAMS(n)) \
  { \
    init_buffers(); \
    this->basic_socket<Protocol, StreamSocketService>::close(ec_); \
    typedef typename Protocol::resolver resolver_type; \
    typedef typename resolver_type::query resolver_query; \
    resolver_query query(AUTOBOOST_ASIO_VARIADIC_ARGS(n)); \
    resolve_and_connect(query); \
    return !ec_ ? this : 0; \
  } \
  /**/
...
template <typename Protocol,
  ...
  || defined(GENERATING_DOCUMENTATION)
    typename Time = autoboost::posix_time::ptime,
...
public:
      ...
      const endpoint_type& endpoint)
  {
    ...
    {
      ec_ = autoboost::asio::error::operation_aborted;
      return 0;
    }
  ...
  basic_socket_streambuf<Protocol, StreamSocketService>* connect(
      T1 t1, ..., TN tn);
#elif defined(AUTOBOOST_ASIO_HAS_VARIADIC_TEMPLATES)
  template <typename... T>
  basic_socket_streambuf<Protocol, StreamSocketService,
...
#else
  AUTOBOOST_ASIO_VARIADIC_GENERATE(AUTOBOOST_ASIO_PRIVATE_CONNECT_DEF)
#endif
   ...
   * buffer.
   */
  const autoboost::system::error_code& puberror() const
  {
    return error();
  }
   ...
   * autoboost::asio::error::operation_aborted.
   *
...
protected:
  int_type underflow()
  {
    if (gptr() == egptr())
    {
      ...
      this->get_service().async_receive(this->get_implementation(),
          autoboost::asio::buffer(autoboost::asio::buffer(get_buffer_) + putback_max),
          0, handler);
  ...
  int_type overflow(int_type c)
  {
    if (unbuffered_)
    {
      ...
      else
      {
        ...
        this->get_service().async_send(this->get_implementation(),
            autoboost::asio::buffer(&ch, sizeof(char_type)), 0, handler);
    ...
    else
    {
      ...
      autoboost::asio::const_buffer buffer =
        autoboost::asio::buffer(pbase(), pptr() - pbase());
      while (autoboost::asio::buffer_size(buffer) > 0)
      {
        if (timer_state_ == timer_has_expired)
        {
          ec_ = autoboost::asio::error::operation_aborted;
        ...
        io_handler handler = { this };
        this->get_service().async_send(this->get_implementation(),
            autoboost::asio::buffer(buffer), 0, handler);
   ...
   * buffer.
   */
  virtual const autoboost::system::error_code& error() const
  {
    return ec_;
  }
...
private:
  ...
  void resolve_and_connect(const ResolverQuery& query)
  {
    ...
    {
      iterator_type end;
      ec_ = autoboost::asio::error::host_not_found;
      while (ec_ && i != end)
      {
        this->basic_socket<Protocol, StreamSocketService>::close(ec_);
  ...
  struct timer_handler
  {
    ...
    void operator()(const autoboost::system::error_code&)
    {
  ...
  void start_timer()
  {
    if (timer_state_ != timer_is_pending)
    {
      ...
      handler(autoboost::system::error_code());
    }
  }

  enum { putback_max = 8 };
  enum { buffer_size = 512 };
  autoboost::asio::detail::array<char, buffer_size> get_buffer_;
  autoboost::asio::detail::array<char, buffer_size> put_buffer_;
  bool unbuffered_;
...
#if !defined(AUTOBOOST_ASIO_HAS_VARIADIC_TEMPLATES)
# undef AUTOBOOST_ASIO_PRIVATE_CONNECT_DEF
#endif // !defined(AUTOBOOST_ASIO_HAS_VARIADIC_TEMPLATES)

#endif // !defined(AUTOBOOST_ASIO_NO_IOSTREAM)

#endif // AUTOBOOST_ASIO_BASIC_SOCKET_STREAMBUF_HPP
