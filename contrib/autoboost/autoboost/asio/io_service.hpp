...
#ifndef AUTOBOOST_ASIO_IO_SERVICE_HPP
#define AUTOBOOST_ASIO_IO_SERVICE_HPP
...
#include <autoboost/asio/detail/wrapped_handler.hpp>
...
# include <autoboost/asio/detail/winsock_init.hpp>
  ...
  || defined(__osf__)
# include <autoboost/asio/detail/signal_init.hpp>
#endif
 ...
 *
 * @li autoboost::asio::ip::tcp::socket
 * @li autoboost::asio::ip::tcp::acceptor
 * @li autoboost::asio::ip::udp::socket
 * @li autoboost::asio::deadline_timer.
 *
 ...
 * creating an object of type autoboost::asio::io_service::work:
 *
 * @code autoboost::asio::io_service io_service;
 * autoboost::asio::io_service::work work(io_service);
 * ... @endcode
 *
 ...
 *
 * @code autoboost::asio::io_service io_service;
 * auto_ptr<autoboost::asio::io_service::work> work(
 *     new autoboost::asio::io_service::work(io_service));
 * ...
...
public:
  ...
  /// Constructor.
  AUTOBOOST_ASIO_DECL io_service();
   ...
   */
  AUTOBOOST_ASIO_DECL explicit io_service(std::size_t concurrency_hint);
   ...
   * without blocking.
   */
  AUTOBOOST_ASIO_DECL std::size_t run();
   ...
   * @throws autoboost::system::system_error Thrown on failure.
   */
  AUTOBOOST_ASIO_DECL std::size_t run_one();
   ...
   * @throws autoboost::system::system_error Thrown on failure.
   */
  AUTOBOOST_ASIO_DECL std::size_t poll();
   ...
   * @throws autoboost::system::system_error Thrown on failure.
   */
  AUTOBOOST_ASIO_DECL std::size_t poll_one();
  ...
  AUTOBOOST_ASIO_INITFN_RESULT_TYPE(CompletionHandler, void ())
  dispatch(AUTOBOOST_ASIO_MOVE_ARG(CompletionHandler) handler);
  ...
  template <typename CompletionHandler>
  AUTOBOOST_ASIO_INITFN_RESULT_TYPE(CompletionHandler, void ())
  post(AUTOBOOST_ASIO_MOVE_ARG(CompletionHandler) handler);
   ...
   * @code io_service.dispatch(autoboost::bind(f, a1, ... an)); @endcode
   */
   ...
   *
   * @throws autoboost::system::system_error Thrown on failure. If the notification
   ...
   * function:
   * @code my_io_service.notify_fork(autoboost::asio::io_service::fork_prepare);
   * if (fork() == 0)
   * {
   *   // This is the child process.
   *   my_io_service.notify_fork(autoboost::asio::io_service::fork_child);
   * }
   * else
   * {
   *   // This is the parent process.
   *   my_io_service.notify_fork(autoboost::asio::io_service::fork_parent);
   * } @endcode
   *
   ...
   */
  AUTOBOOST_ASIO_DECL void notify_fork(autoboost::asio::io_service::fork_event event);
   ...
   *
   * @throws autoboost::asio::service_already_exists Thrown if a service of the
   * given type is already present in the io_service.
   *
   * @throws autoboost::asio::invalid_service_owner Thrown if the service's owning
...
private:
  ...
  autoboost::asio::detail::service_registry* service_registry_;
...
public:
   ...
   */
  explicit work(autoboost::asio::io_service& io_service);
...
protected:
   ...
   */
  AUTOBOOST_ASIO_DECL service(autoboost::asio::io_service& owner);

  /// Destructor.
  AUTOBOOST_ASIO_DECL virtual ~service();
...
private:
   ...
   */
  AUTOBOOST_ASIO_DECL virtual void fork_service(
      autoboost::asio::io_service::fork_event event);

  friend class autoboost::asio::detail::service_registry;
  struct key
  {
    key() : type_info_(0), id_(0) {}
    const std::type_info* type_info_;
    const autoboost::asio::io_service::id* id_;
  } key_;
...
{
public:
  AUTOBOOST_ASIO_DECL service_already_exists();
};
...
{
public:
  AUTOBOOST_ASIO_DECL invalid_service_owner();
};
...
class service_id
  : public autoboost::asio::io_service::id
{
};
...
{
public:
  static autoboost::asio::detail::service_id<Type> id;

  // Constructor.
  service_base(autoboost::asio::io_service& io_service)
    : autoboost::asio::io_service::service(io_service)
  {
  }
};

template <typename Type>
autoboost::asio::detail::service_id<Type> service_base<Type>::id;
