...
#define AUTOBOOST_ASIO_BASIC_SOCKET_IOSTREAM_HPP
...
# define AUTOBOOST_ASIO_PRIVATE_CTR_DEF(n) \
  template <AUTOBOOST_ASIO_VARIADIC_TPARAMS(n)> \
  explicit basic_socket_iostream(AUTOBOOST_ASIO_VARIADIC_PARAMS(n)) \
    : std::basic_iostream<char>( \
        &this->detail::socket_iostream_base< \
          Protocol, StreamSocketService, Time, \
          TimeTraits, TimerService>::streambuf_) \
  { \
    ...
    if (rdbuf()->connect(AUTOBOOST_ASIO_VARIADIC_ARGS(n)) == 0) \
      this->setstate(std::ios_base::failbit); \
  } \
  /**/
...
# define AUTOBOOST_ASIO_PRIVATE_CONNECT_DEF(n) \
  template <AUTOBOOST_ASIO_VARIADIC_TPARAMS(n)> \
  void connect(AUTOBOOST_ASIO_VARIADIC_PARAMS(n)) \
  { \
    if (rdbuf()->connect(AUTOBOOST_ASIO_VARIADIC_ARGS(n)) == 0) \
      this->setstate(std::ios_base::failbit); \
  } \
...
public:
...
#else
  AUTOBOOST_ASIO_VARIADIC_GENERATE(AUTOBOOST_ASIO_PRIVATE_CTR_DEF)
#endif
   ...
   * } @endcode
   */
  const autoboost::system::error_code& error() const
  {
...
#if !defined(AUTOBOOST_ASIO_HAS_VARIADIC_TEMPLATES)
# undef AUTOBOOST_ASIO_PRIVATE_CTR_DEF
# undef AUTOBOOST_ASIO_PRIVATE_CONNECT_DEF
#endif // !defined(AUTOBOOST_ASIO_HAS_VARIADIC_TEMPLATES)

#endif // !defined(AUTOBOOST_ASIO_NO_IOSTREAM)

#endif // AUTOBOOST_ASIO_BASIC_SOCKET_IOSTREAM_HPP
