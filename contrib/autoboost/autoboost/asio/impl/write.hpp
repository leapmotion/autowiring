...
#ifndef AUTOBOOST_ASIO_IMPL_WRITE_HPP
#define AUTOBOOST_ASIO_IMPL_WRITE_HPP
...
inline std::size_t write(SyncWriteStream& s, const ConstBufferSequence& buffers)
{
  ...
  autoboost::asio::detail::throw_error(ec, "write");
...
namespace detail
{
      ...
      switch (start_ = start)
      {
        ...
        for (;;)
        {
          stream_.async_write_some(buffers_,
              AUTOBOOST_ASIO_MOVE_CAST(write_op)(*this));
          return; default:
  ...
  template <typename AsyncWriteStream,
      typename CompletionCondition, typename WriteHandler>
  class write_op<AsyncWriteStream, autoboost::asio::mutable_buffers_1,
  ...
  template <typename AsyncWriteStream,
      typename CompletionCondition, typename WriteHandler>
  class write_op<AsyncWriteStream, autoboost::asio::const_buffers_1,
      CompletionCondition, WriteHandler>
    : detail::base_from_completion_cond<CompletionCondition>
  {
  public:
    write_op(AsyncWriteStream& stream,
        const autoboost::asio::const_buffers_1& buffers,
        CompletionCondition completion_condition,
        WriteHandler& handler)
      : detail::base_from_completion_cond<
          CompletionCondition>(completion_condition),
  ...
  template <typename AsyncWriteStream, typename Elem,
      typename CompletionCondition, typename WriteHandler>
  class write_op<AsyncWriteStream, autoboost::array<Elem, 2>,
      CompletionCondition, WriteHandler>
    : detail::base_from_completion_cond<CompletionCondition>
  {
  public:
    write_op(AsyncWriteStream& stream, const autoboost::array<Elem, 2>& buffers,
        CompletionCondition completion_condition, WriteHandler& handler)
      : detail::base_from_completion_cond<
          CompletionCondition>(completion_condition),
        stream_(stream),
        ...
        std::size_t bytes_transferred, int start = 0)
    {
      ...
      switch (start_ = start)
      {
        ...
        for (;;)
        {
          ...
          stream_.async_write_some(bufs, AUTOBOOST_ASIO_MOVE_CAST(write_op)(*this));
          return; default:
          total_transferred_ += bytes_transferred;
          if ((!ec && bytes_transferred == 0)
              || (n = this->check_for_completion(ec, total_transferred_)) == 0
...
namespace detail
{
  ...
  {
  public:
    write_streambuf_handler(autoboost::asio::basic_streambuf<Allocator>& streambuf,
        WriteHandler& handler)
    ...
    AUTOBOOST_ASIO_MOVE_ARG(WriteHandler) handler)
{
  ...
  async_write(s, b.data(), completion_condition,
    detail::write_streambuf_handler<Allocator, AUTOBOOST_ASIO_HANDLER_TYPE(
      WriteHandler, void (autoboost::system::error_code, std::size_t))>(
        b, init.handler));
...
#endif // AUTOBOOST_ASIO_IMPL_WRITE_HPP
