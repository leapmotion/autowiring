...
#ifndef AUTOBOOST_ASIO_IMPL_BUFFERED_WRITE_STREAM_HPP
#define AUTOBOOST_ASIO_IMPL_BUFFERED_WRITE_STREAM_HPP
...
template <typename Stream>
std::size_t buffered_write_stream<Stream>::flush(autoboost::system::error_code& ec)
{
  std::size_t bytes_written = write(next_layer_,
      buffer(storage_.data(), storage_.size()),
      transfer_all(), ec);
    ...
    AUTOBOOST_ASIO_MOVE_ARG(WriteHandler) handler)
{
  ...
  async_write(next_layer_, buffer(storage_.data(), storage_.size()),
      detail::buffered_flush_handler<AUTOBOOST_ASIO_HANDLER_TYPE(
...
{
  if (autoboost::asio::buffer_size(buffers) == 0)
    return 0;
...
namespace detail
{
  ...
  public:
    ...
    void operator()(const autoboost::system::error_code& ec, std::size_t)
    {
      if (ec)
      {
      ...
      else
      {
        ...
        std::size_t bytes_avail = autoboost::asio::buffer_size(buffers_);
        std::size_t length = bytes_avail < space_avail
          ? bytes_avail : space_avail;
        storage_.resize(orig_size + length);
        const std::size_t bytes_copied = autoboost::asio::buffer_copy(
    ...
    AUTOBOOST_ASIO_MOVE_ARG(WriteHandler) handler)
{
  ...
  {
    next_layer_.async_write_some(autoboost::asio::const_buffers_1(0, 0),
        detail::buffered_write_some_handler<
          ConstBufferSequence, AUTOBOOST_ASIO_HANDLER_TYPE(
            WriteHandler, void (autoboost::system::error_code, std::size_t))>(
    ...
    const ConstBufferSequence& buffers)
{
  std::size_t orig_size = storage_.size();
  std::size_t space_avail = storage_.capacity() - orig_size;
  std::size_t bytes_avail = autoboost::asio::buffer_size(buffers);
  std::size_t length = bytes_avail < space_avail ? bytes_avail : space_avail;
  storage_.resize(orig_size + length);
  return autoboost::asio::buffer_copy(
...
#endif // AUTOBOOST_ASIO_IMPL_BUFFERED_WRITE_STREAM_HPP
