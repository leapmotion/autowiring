...
#ifndef AUTOBOOST_ASIO_IMPL_USE_FUTURE_HPP
#define AUTOBOOST_ASIO_IMPL_USE_FUTURE_HPP
...
#include <autoboost/asio/detail/memory.hpp>
  ...
  public:
    ...
    promise_handler(use_future_t<Alloc> uf)
      : promise_(std::allocate_shared<std::promise<T> >(
            AUTOBOOST_ASIO_REBIND_ALLOC(Alloc, char)(uf.get_allocator()),
            std::allocator_arg,
            AUTOBOOST_ASIO_REBIND_ALLOC(Alloc, char)(uf.get_allocator())))
    {
    }
    ...
    void operator()(const autoboost::system::error_code& ec, T t)
    {
      if (ec)
        promise_->set_exception(
            std::make_exception_ptr(
              autoboost::system::system_error(ec)));
      else
        promise_->set_value(t);
    }
...
#endif // AUTOBOOST_ASIO_IMPL_USE_FUTURE_HPP
