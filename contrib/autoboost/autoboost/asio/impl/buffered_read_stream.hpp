...
#ifndef AUTOBOOST_ASIO_IMPL_BUFFERED_READ_STREAM_HPP
#define AUTOBOOST_ASIO_IMPL_BUFFERED_READ_STREAM_HPP
...
template <typename Stream>
std::size_t buffered_read_stream<Stream>::fill(autoboost::system::error_code& ec)
{
  detail::buffer_resize_guard<detail::buffered_stream_storage>
    resize_guard(storage_);
  std::size_t previous_size = storage_.size();
    ...
    AUTOBOOST_ASIO_MOVE_ARG(ReadHandler) handler)
{
  ...
  next_layer_.async_read_some(
      buffer(
        storage_.data() + previous_size,
        storage_.size() - previous_size),
      detail::buffered_fill_handler<AUTOBOOST_ASIO_HANDLER_TYPE(
    ...
    AUTOBOOST_ASIO_MOVE_ARG(ReadHandler) handler)
{
  ...
  if (autoboost::asio::buffer_size(buffers) == 0 || !storage_.empty())
  {
    next_layer_.async_read_some(autoboost::asio::mutable_buffers_1(0, 0),
        detail::buffered_read_some_handler<
          MutableBufferSequence, AUTOBOOST_ASIO_HANDLER_TYPE(
            ReadHandler, void (autoboost::system::error_code, std::size_t))>(
...
#endif // AUTOBOOST_ASIO_IMPL_BUFFERED_READ_STREAM_HPP
