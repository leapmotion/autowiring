...
#ifndef AUTOBOOST_ASIO_IMPL_SPAWN_HPP
#define AUTOBOOST_ASIO_IMPL_SPAWN_HPP
...
namespace detail {
  ...
  public:
    ...
    {
      *ec_ = autoboost::system::error_code();
      *value_ = AUTOBOOST_ASIO_MOVE_CAST(T)(value);
      if (--*ready_ == 0)
        (*coro_)();
    }

    void operator()(autoboost::system::error_code ec, T value)
    {
      *ec_ = ec;
      *value_ = AUTOBOOST_ASIO_MOVE_CAST(T)(value);
      if (--*ready_ == 0)
  ...
  public:
    ...
    void operator()(autoboost::system::error_code ec)
    {
      *ec_ = ec;
      if (--*ready_ == 0)
        (*coro_)();
...
struct handler_type<basic_yield_context<Handler>,
    ReturnType(autoboost::system::error_code)>
{
  typedef detail::coro_handler<Handler, void> type;
};
...
struct handler_type<basic_yield_context<Handler>,
    ReturnType(autoboost::system::error_code, Arg2)>
{
...
public:
  ...
  type get()
  {
    ...
    if (--ready_ != 0)
      ca_();
    if (!out_ec_ && ec_) throw autoboost::system::system_error(ec_);
    return AUTOBOOST_ASIO_MOVE_CAST(type)(value_);
  }
...
private:
  ...
  autoboost::system::error_code* out_ec_;
  autoboost::system::error_code ec_;
  type value_;
};
...
namespace detail {
  ...
  {
    spawn_data(AUTOBOOST_ASIO_MOVE_ARG(Handler) handler,
        bool call_handler, AUTOBOOST_ASIO_MOVE_ARG(Function) function)
      : handler_(AUTOBOOST_ASIO_MOVE_CAST(Handler)(handler)),
        call_handler_(call_handler),
        function_(AUTOBOOST_ASIO_MOVE_CAST(Function)(function))
    {
    }
...
#if !defined(AUTOBOOST_COROUTINES_UNIDIRECT) && !defined(AUTOBOOST_COROUTINES_V2)
      ca(); // Yield until coroutine pointer has been initialised.
#endif // !defined(AUTOBOOST_COROUTINES_UNIDIRECT) && !defined(AUTOBOOST_COROUTINES_V2)
  ...
  struct spawn_helper
  {
    ...
    shared_ptr<spawn_data<Handler, Function> > data_;
    autoboost::coroutines::attributes attributes_;
  };
...
template <typename Handler, typename Function>
void spawn(AUTOBOOST_ASIO_MOVE_ARG(Handler) handler,
    AUTOBOOST_ASIO_MOVE_ARG(Function) function,
    const autoboost::coroutines::attributes& attributes)
{
  detail::spawn_helper<Handler, Function> helper;
  helper.data_.reset(
      new detail::spawn_data<Handler, Function>(
        AUTOBOOST_ASIO_MOVE_CAST(Handler)(handler), true,
        AUTOBOOST_ASIO_MOVE_CAST(Function)(function)));
  helper.attributes_ = attributes;
  autoboost_asio_handler_invoke_helpers::invoke(helper, helper.data_->handler_);
}
    ...
    const autoboost::coroutines::attributes& attributes)
{
  ...
  helper.data_.reset(
      new detail::spawn_data<Handler, Function>(
        AUTOBOOST_ASIO_MOVE_CAST(Handler)(handler), false,
        AUTOBOOST_ASIO_MOVE_CAST(Function)(function)));
  helper.attributes_ = attributes;
  autoboost_asio_handler_invoke_helpers::invoke(helper, helper.data_->handler_);
}
...
void spawn(autoboost::asio::io_service::strand strand,
    AUTOBOOST_ASIO_MOVE_ARG(Function) function,
    const autoboost::coroutines::attributes& attributes)
{
  autoboost::asio::spawn(strand.wrap(&detail::default_spawn_handler),
      AUTOBOOST_ASIO_MOVE_CAST(Function)(function), attributes);
}

template <typename Function>
void spawn(autoboost::asio::io_service& io_service,
    AUTOBOOST_ASIO_MOVE_ARG(Function) function,
    const autoboost::coroutines::attributes& attributes)
{
  autoboost::asio::spawn(autoboost::asio::io_service::strand(io_service),
      AUTOBOOST_ASIO_MOVE_CAST(Function)(function), attributes);
}
...
#endif // AUTOBOOST_ASIO_IMPL_SPAWN_HPP
