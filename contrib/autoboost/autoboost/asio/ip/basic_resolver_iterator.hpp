...
#ifndef AUTOBOOST_ASIO_IP_BASIC_RESOLVER_ITERATOR_HPP
#define AUTOBOOST_ASIO_IP_BASIC_RESOLVER_ITERATOR_HPP
...
/**
 * The autoboost::asio::ip::basic_resolver_iterator class template is used to define
...
public:
  ...
  static basic_resolver_iterator create(
      autoboost::asio::detail::addrinfo_type* address_info,
      const std::string& host_name, const std::string& service_name)
  {
    ...
    while (address_info)
    {
      if (address_info->ai_family == AUTOBOOST_ASIO_OS_DEF(AF_INET)
          || address_info->ai_family == AUTOBOOST_ASIO_OS_DEF(AF_INET6))
      {
  ...
  static basic_resolver_iterator create(
      ...
      const autoboost::asio::detail::addrinfo_type& hints,
      const std::string& host_name, const std::string& service_name)
  {
    basic_resolver_iterator iter;
    if (endpoints->Size)
    {
      ...
      for (unsigned int i = 0; i < endpoints->Size; ++i)
      {
        ...
        if (hints.ai_family == AUTOBOOST_ASIO_OS_DEF(AF_INET)
            ...
            basic_resolver_entry<InternetProtocol>(
              typename InternetProtocol::endpoint(
                ip::address::from_string(
                  autoboost::asio::detail::winrt_utils::string(
                    pair->RemoteHostName->CanonicalName)),
                autoboost::asio::detail::winrt_utils::integer(
...
private:
  ...
  autoboost::asio::detail::shared_ptr<values_type> values_;
  std::size_t index_;
};
...
#endif // AUTOBOOST_ASIO_IP_BASIC_RESOLVER_ITERATOR_HPP
