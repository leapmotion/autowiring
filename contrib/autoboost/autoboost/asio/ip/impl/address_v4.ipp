...
#ifndef AUTOBOOST_ASIO_IP_IMPL_ADDRESS_V4_IPP
#define AUTOBOOST_ASIO_IP_IMPL_ADDRESS_V4_IPP
...
address_v4::address_v4(unsigned long addr)
{
  ...
  addr_.s_addr = autoboost::asio::detail::socket_ops::host_to_network_long(
      static_cast<autoboost::asio::detail::u_long_type>(addr));
}
...
{
  return autoboost::asio::detail::socket_ops::network_to_host_long(addr_.s_addr);
}
...
std::string address_v4::to_string(autoboost::system::error_code& ec) const
{
  char addr_str[autoboost::asio::detail::max_addr_v4_str_len];
  const char* addr =
    autoboost::asio::detail::socket_ops::inet_ntop(
        AUTOBOOST_ASIO_OS_DEF(AF_INET), &addr_, addr_str,
        autoboost::asio::detail::max_addr_v4_str_len, 0, ec);
  if (addr == 0)
    return std::string();
  return addr;
}
...
{
  address_v4 tmp;
  if (autoboost::asio::detail::socket_ops::inet_pton(
        AUTOBOOST_ASIO_OS_DEF(AF_INET), str, &tmp.addr_, 0, ec) <= 0)
    return address_v4();
  return tmp;
}
...
#endif // AUTOBOOST_ASIO_IP_IMPL_ADDRESS_V4_IPP
