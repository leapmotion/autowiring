...
#ifndef AUTOBOOST_ASIO_STRAND_HPP
#define AUTOBOOST_ASIO_STRAND_HPP
...
#include <autoboost/asio/detail/strand_service.hpp>
...
public:
   ...
   */
  explicit strand(autoboost::asio::io_service& io_service)
    : service_(autoboost::asio::use_service<
        autoboost::asio::detail::strand_service>(io_service))
  {
  ...
  dispatch(AUTOBOOST_ASIO_MOVE_ARG(CompletionHandler) handler)
  {
    ...
    AUTOBOOST_ASIO_COMPLETION_HANDLER_CHECK(CompletionHandler, handler) type_check;

    detail::async_result_init<
      CompletionHandler, void ()> init(
        AUTOBOOST_ASIO_MOVE_CAST(CompletionHandler)(handler));
   ...
   * @code strand.dispatch(autoboost::bind(f, a1, ... an)); @endcode
   */
...
private:
  autoboost::asio::detail::strand_service& service_;
  autoboost::asio::detail::strand_service::implementation_type impl_;
};

/// (Deprecated: Use autoboost::asio::io_service::strand.) Typedef for backwards
/// compatibility.
typedef autoboost::asio::io_service::strand strand;
...
#endif // AUTOBOOST_ASIO_STRAND_HPP
