...
#ifndef AUTOBOOST_ASIO_BUFFER_HPP
#define AUTOBOOST_ASIO_BUFFER_HPP
...
#  if !defined(AUTOBOOST_ASIO_DISABLE_BUFFER_DEBUGGING)
#   define AUTOBOOST_ASIO_ENABLE_BUFFER_DEBUGGING
#  endif // !defined(AUTOBOOST_ASIO_DISABLE_BUFFER_DEBUGGING)
...
# include <autoboost/asio/detail/function.hpp>
#endif // AUTOBOOST_ASIO_ENABLE_BUFFER_DEBUGGING
...
#  define AUTOBOOST_ASIO_ENABLE_ARRAY_BUFFER_WORKAROUND
# endif // AUTOBOOST_WORKAROUND(__BORLANDC__, AUTOBOOST_TESTED_AT(0x582))
        // || AUTOBOOST_WORKAROUND(__SUNPRO_CC, AUTOBOOST_TESTED_AT(0x590))
#endif // defined(AUTOBOOST_ASIO_HAS_AUTOBOOST_WORKAROUND)

#if defined(AUTOBOOST_ASIO_ENABLE_ARRAY_BUFFER_WORKAROUND)
# include <autoboost/asio/detail/type_traits.hpp>
#endif // defined(AUTOBOOST_ASIO_ENABLE_ARRAY_BUFFER_WORKAROUND)
 ...
 *
 * @code autoboost::asio::mutable_buffer b1 = ...;
 * std::size_t s1 = autoboost::asio::buffer_size(b1);
 * unsigned char* p1 = autoboost::asio::buffer_cast<unsigned char*>(b1);
 * @endcode
 *
 * The autoboost::asio::buffer_cast function permits violations of type safety, so
...
public:
...
#if defined(AUTOBOOST_ASIO_ENABLE_BUFFER_DEBUGGING)
  mutable_buffer(void* data, std::size_t size,
      autoboost::asio::detail::function<void()> debug_check)
    : data_(data),
      size_(size),
  ...
  const autoboost::asio::detail::function<void()>& get_debug_check() const
  {
    return debug_check_;
  }
#endif // AUTOBOOST_ASIO_ENABLE_BUFFER_DEBUGGING
...
private:
  friend void* autoboost::asio::detail::buffer_cast_helper(
      const mutable_buffer& b);
  friend std::size_t autoboost::asio::detail::buffer_size_helper(
      const mutable_buffer& b);
...
#if defined(AUTOBOOST_ASIO_ENABLE_BUFFER_DEBUGGING)
  autoboost::asio::detail::function<void()> debug_check_;
#endif // AUTOBOOST_ASIO_ENABLE_BUFFER_DEBUGGING
};
 ...
 * and @ref buffer_cast functions:
 *
 * @code autoboost::asio::const_buffer b1 = ...;
 * std::size_t s1 = autoboost::asio::buffer_size(b1);
 * const unsigned char* p1 = autoboost::asio::buffer_cast<const unsigned char*>(b1);
 * @endcode
 *
 * The autoboost::asio::buffer_cast function permits violations of type safety, so
 * uses of it in application code should be carefully considered.
...
public:
  ...
  const_buffer(const mutable_buffer& b)
    : data_(autoboost::asio::detail::buffer_cast_helper(b)),
      size_(autoboost::asio::detail::buffer_size_helper(b))
#if defined(AUTOBOOST_ASIO_ENABLE_BUFFER_DEBUGGING)
...
private:
  friend const void* autoboost::asio::detail::buffer_cast_helper(
...
/** @defgroup buffer_size autoboost::asio::buffer_size
 *
 * @brief The autoboost::asio::buffer_size function determines the total number of
...
/** @defgroup buffer_cast autoboost::asio::buffer_cast
 *
 * @brief The autoboost::asio::buffer_cast function is used to obtain a pointer to
 ...
 * @code autoboost::asio::mutable_buffer b2 = ...;
 * unsigned char* p2 = autoboost::asio::buffer_cast<unsigned char*>(b2);
 * @endcode
 *
 * The autoboost::asio::buffer_cast function permits violations of type safety, so
 * uses of it in application code should be carefully considered.
...
public:
  ...
  ~buffer_debug_check()
  {
    ...
    iter_ = Iterator();
#endif // defined(AUTOBOOST_ASIO_MSVC) && (AUTOBOOST_ASIO_MSVC == 1400)
  }
...
/** @defgroup buffer autoboost::asio::buffer
 *
 * @brief The autoboost::asio::buffer function is used to create a buffer object to
 ...
 * specified size:
 *
 * @code sock.send(autoboost::asio::buffer(data, size)); @endcode
 *
 * In the above example, the return value of autoboost::asio::buffer meets the
 * requirements of the ConstBufferSequence concept so that it may be directly
 * passed to the socket's write function. A buffer created for modifiable
 * memory also meets the requirements of the MutableBufferSequence concept.
 *
 * An individual buffer may be created from a builtin array, std::vector,
 * std::array or autoboost::array of POD elements. This helps prevent buffer
 * overruns by automatically determining the size of the buffer:
 *
 * @code char d1[128];
 * size_t bytes_transferred = sock.receive(autoboost::asio::buffer(d1));
 *
 * std::vector<char> d2(128);
 * bytes_transferred = sock.receive(autoboost::asio::buffer(d2));
 *
 * std::array<char, 128> d3;
 * bytes_transferred = sock.receive(autoboost::asio::buffer(d3));
 *
 * autoboost::array<char, 128> d4;
 * bytes_transferred = sock.receive(autoboost::asio::buffer(d4)); @endcode
 *
 ...
 * unsigned char* p1 = autoboost::asio::buffer_cast<unsigned char*>(b1);
 *
 * autoboost::asio::const_buffer b2 = ...;
 * std::size_t s2 = autoboost::asio::buffer_size(b2);
 * const void* p2 = autoboost::asio::buffer_cast<const void*>(b2); @endcode
 *
 * The autoboost::asio::buffer_cast function permits violations of type safety, so
 * uses of it in application code should be carefully considered.
 *
 ...
 *
 * vector<unsigned char> data(autoboost::asio::buffer_size(buffers));
 * autoboost::asio::buffer_copy(autoboost::asio::buffer(data), buffers); @endcode
 *
 ...
 *
 * For the autoboost::asio::buffer overloads that accept an argument of type
 ...
 * follows:
 *
 * @code autoboost::array<char, 6> a = { 'a', 'b', 'c', 'd', 'e' }; @endcode
 *
 * A buffer object @c b1 created using:
 *
 * @code b1 = autoboost::asio::buffer(a); @endcode
 *
 * represents the entire array, <tt>{ 'a', 'b', 'c', 'd', 'e' }</tt>. An
 * optional second argument to the autoboost::asio::buffer function may be used to
 * limit the size, in bytes, of the buffer:
 *
 * @code b2 = autoboost::asio::buffer(a, 3); @endcode
 *
 ...
 *
 * @code b4 = autoboost::asio::buffer(b1 + 1, 3); @endcode
 *
 ...
 * char d1[128];
 * std::vector<char> d2(128);
 * autoboost::array<char, 128> d3;
 *
 * autoboost::array<mutable_buffer, 3> bufs1 = {
 *   autoboost::asio::buffer(d1),
 *   autoboost::asio::buffer(d2),
 *   autoboost::asio::buffer(d3) };
 * bytes_transferred = sock.receive(bufs1);
 *
 * std::vector<const_buffer> bufs2;
 * bufs2.push_back(autoboost::asio::buffer(d1));
 * bufs2.push_back(autoboost::asio::buffer(d2));
 * bufs2.push_back(autoboost::asio::buffer(d3));
...
//
//   unspecified buffer(autoboost::array<PodType, N>& array ...);
//
// and
//
//   unspecified buffer(autoboost::array<const PodType, N>& array ...);
//
...
buffer(autoboost::array<PodType, N>& data)
{
  typedef typename autoboost::asio::detail::buffer_types<PodType>::buffer_type
    buffer_type;
  typedef typename autoboost::asio::detail::buffer_types<PodType>::container_type
    container_type;
...
template <typename PodType, std::size_t N>
inline typename detail::buffer_types<PodType>::container_type
buffer(autoboost::array<PodType, N>& data, std::size_t max_size_in_bytes)
{
  typedef typename autoboost::asio::detail::buffer_types<PodType>::buffer_type
    buffer_type;
  typedef typename autoboost::asio::detail::buffer_types<PodType>::container_type
...
#else // defined(AUTOBOOST_ASIO_ENABLE_ARRAY_BUFFER_WORKAROUND)
 ...
 */
template <typename PodType, std::size_t N>
inline mutable_buffers_1 buffer(autoboost::array<PodType, N>& data)
{
  return mutable_buffers_1(
 ...
 */
template <typename PodType, std::size_t N>
inline const_buffers_1 buffer(autoboost::array<const PodType, N>& data)
{
  return const_buffers_1(
 ...
 */
template <typename PodType, std::size_t N>
inline const_buffers_1 buffer(const autoboost::array<PodType, N>& data)
{
  return const_buffers_1(
      const_buffer(data.data(), data.size() * sizeof(PodType)));
}
...
#if defined(AUTOBOOST_ASIO_HAS_STD_ARRAY) || defined(GENERATING_DOCUMENTATION)
...
#endif // defined(AUTOBOOST_ASIO_HAS_STD_ARRAY) || defined(GENERATING_DOCUMENTATION)
...
/** @defgroup buffer_copy autoboost::asio::buffer_copy
 *
 * @brief The autoboost::asio::buffer_copy function is used to copy bytes from a
...
#endif // AUTOBOOST_ASIO_BUFFER_HPP
