...
#ifndef AUTOBOOST_ASIO_SSL_DETAIL_ENGINE_HPP
#define AUTOBOOST_ASIO_SSL_DETAIL_ENGINE_HPP
...
public:
  ...
  AUTOBOOST_ASIO_DECL explicit engine(SSL_CTX* context);

  // Destructor.
  AUTOBOOST_ASIO_DECL ~engine();

  // Get the underlying implementation in the native type.
  AUTOBOOST_ASIO_DECL SSL* native_handle();
  ...
  AUTOBOOST_ASIO_DECL autoboost::system::error_code set_verify_callback(
      verify_callback_base* callback, autoboost::system::error_code& ec);

  // Perform an SSL handshake using either SSL_connect (client-side) or
  // SSL_accept (server-side).
  AUTOBOOST_ASIO_DECL want handshake(
      stream_base::handshake_type type, autoboost::system::error_code& ec);

  // Perform a graceful shutdown of the SSL session.
  AUTOBOOST_ASIO_DECL want shutdown(autoboost::system::error_code& ec);

  // Write bytes to the SSL session.
  AUTOBOOST_ASIO_DECL want write(const autoboost::asio::const_buffer& data,
      autoboost::system::error_code& ec, std::size_t& bytes_transferred);

  // Read bytes from the SSL session.
  AUTOBOOST_ASIO_DECL want read(const autoboost::asio::mutable_buffer& data,
      autoboost::system::error_code& ec, std::size_t& bytes_transferred);

  // Get output data to be written to the transport.
  AUTOBOOST_ASIO_DECL autoboost::asio::mutable_buffers_1 get_output(
      const autoboost::asio::mutable_buffer& data);

  // Put input data that was read from the transport.
  AUTOBOOST_ASIO_DECL autoboost::asio::const_buffer put_input(
      const autoboost::asio::const_buffer& data);

  // Map an error::eof code returned by the underlying transport according to
  // the type and state of the SSL session. Returns a const reference to the
  // error code object, suitable for passing to a completion handler.
  AUTOBOOST_ASIO_DECL const autoboost::system::error_code& map_error_code(
...
private:
  ...
  AUTOBOOST_ASIO_DECL static autoboost::asio::detail::static_mutex& accept_mutex();

  // Perform one operation. Returns >= 0 on success or error, want_read if the
  // operation needs more input, or want_write if it needs to write some output
  // before the operation can complete.
  AUTOBOOST_ASIO_DECL want perform(int (engine::* op)(void*, std::size_t),
  ...
  AUTOBOOST_ASIO_DECL int do_accept(void*, std::size_t);

  // Adapt the SSL_connect function to the signature needed for perform().
  AUTOBOOST_ASIO_DECL int do_connect(void*, std::size_t);

  // Adapt the SSL_shutdown function to the signature needed for perform().
  AUTOBOOST_ASIO_DECL int do_shutdown(void*, std::size_t);

  // Adapt the SSL_read function to the signature needed for perform().
  AUTOBOOST_ASIO_DECL int do_read(void* data, std::size_t length);

  // Adapt the SSL_write function to the signature needed for perform().
  AUTOBOOST_ASIO_DECL int do_write(void* data, std::size_t length);
...
#endif // AUTOBOOST_ASIO_SSL_DETAIL_ENGINE_HPP
