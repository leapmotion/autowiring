...
#ifndef AUTOBOOST_ASIO_SSL_OLD_DETAIL_OPENSSL_OPERATION_HPP
#define AUTOBOOST_ASIO_SSL_OLD_DETAIL_OPENSSL_OPERATION_HPP
...
public:
  ...
  openssl_operation(ssl_primitive_func primitive,
                    ...
                    autoboost::asio::io_service::strand& strand
                    )
  ...
  int start()
  {
    ...
    if (is_shut_down_sent && is_shut_down_received
      ...
      return handler_(autoboost::system::error_code(), 1);
      ...
      if (recv_buf_.get_data_len() > 0)
      {
        ...
        else if (written < 0)
        {
          if (!BIO_should_retry(ssl_bio_))
          {
            ...
            return handler_(autoboost::asio::error::no_recovery, 0);
          }
        }
...
private:
  typedef autoboost::function<int (const autoboost::system::error_code&, int)>
    int_handler_func;
  typedef autoboost::function<int (bool, int)> write_func;
  typedef autoboost::function<int ()> read_func;
  ...
  //
  int sync_user_handler(const autoboost::system::error_code& error, int rc)
  {
    if (!error)
      return rc;
  ...
  int async_user_handler(autoboost::system::error_code error, int rc)
  {
    ...
    {
      if (!error)
        error = autoboost::asio::error::no_recovery;
      rc = 0;
    }
  ...
  int  do_async_write(bool is_operation_done, int rc)
  {
    ...
    if ( len )
    {
      ...
      else if (!BIO_should_retry(ssl_bio_))
      {
        ...
        handler_(autoboost::asio::error::no_recovery, 0);
        return 0;
      }
    }
  ...
  void async_read_handler(const autoboost::system::error_code& error,
      size_t bytes_recvd)
  {
    if (!error)
    {
...
#endif // AUTOBOOST_ASIO_SSL_OLD_DETAIL_OPENSSL_OPERATION_HPP
