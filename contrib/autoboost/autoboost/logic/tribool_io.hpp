...
#ifndef AUTOBOOST_LOGIC_TRIBOOL_IO_HPP
#define AUTOBOOST_LOGIC_TRIBOOL_IO_HPP

#include <autoboost/logic/tribool.hpp>
...
class indeterminate_name : public std::locale::facet, private autoboost::noncopyable
{
public:
...
#ifndef AUTOBOOST_NO_STD_LOCALE
        if (AUTOBOOST_HAS_FACET(indeterminate_name<CharT>, out.getloc())) {
          const indeterminate_name<CharT>& facet =
            AUTOBOOST_USE_FACET(indeterminate_name<CharT>, out.getloc());
          out << facet.name();
        } else {
  ...
  if (in.flags() & std::ios_base::boolalpha) {
    ...
    if (cerberus) {
      ...
      const std::numpunct<CharT>& numpunct_facet =
        AUTOBOOST_USE_FACET(std::numpunct<CharT>, in.getloc());

      string_type falsename = numpunct_facet.falsename();
      string_type truename = numpunct_facet.truename();

      string_type othername;
      if (AUTOBOOST_HAS_FACET(indeterminate_name<CharT>, in.getloc())) {
        othername =
          AUTOBOOST_USE_FACET(indeterminate_name<CharT>, in.getloc()).name();
      } else {
...
#endif // AUTOBOOST_LOGIC_TRIBOOL_IO_HPP
