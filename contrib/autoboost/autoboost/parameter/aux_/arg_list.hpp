...
#include <autoboost/parameter/aux_/parameter_requirements.hpp>
...
struct empty_arg_list
{
    ...
    // Constructor taking AUTOBOOST_PARAMETER_MAX_ARITY empty_arg_list
    // arguments; this makes initialization
    empty_arg_list(
        AUTOBOOST_PP_ENUM_PARAMS(
            AUTOBOOST_PARAMETER_MAX_ARITY, void_ AUTOBOOST_PP_INTERCEPT
        ))
    {}
    ...
    typename result_of0<F>::type
    operator[](
        AUTOBOOST_PARAMETER_lazy_default_fallback<K,F> x) const
    {
        return x.compute_default();
    }
...
struct arg_list : Next
{
    ...
    arg_list( // A0& a0, A1& a1, ...
        AUTOBOOST_PP_ENUM_BINARY_PARAMS(AUTOBOOST_PARAMETER_MAX_ARITY, A, & a)
    )
      : Next( // a1, a2, ...
            AUTOBOOST_PP_ENUM_SHIFTED_PARAMS(AUTOBOOST_PARAMETER_MAX_ARITY, a)
          , void_reference()
        )
      , arg(a0)
    {}
...
#if !AUTOBOOST_WORKAROUND(__BORLANDC__, AUTOBOOST_TESTED_AT(0x564)) && !AUTOBOOST_WORKAROUND(__GNUC__, == 2)
# if AUTOBOOST_WORKAROUND(AUTOBOOST_MSVC, <= 1300)
    friend yes_tag operator*(arg_list, key_type*);
#  define AUTOBOOST_PARAMETER_CALL_HAS_KEY(next, key) (*(next*)0 * (key*)0)
# else
...
#  define AUTOBOOST_PARAMETER_CALL_HAS_KEY(next, key) next::has_key((key*)0)
# endif

    AUTOBOOST_MPL_ASSERT_MSG(
        sizeof(AUTOBOOST_PARAMETER_CALL_HAS_KEY(Next,key_type)) == sizeof(no_tag)
      , duplicate_keyword, (key_type)
    );

# undef AUTOBOOST_PARAMETER_CALL_HAS_KEY
#endif
    //
    ...
    {
        AUTOBOOST_MPL_ASSERT_NOT((holds_maybe));
        return arg.value;
    }
...
#if AUTOBOOST_WORKAROUND(AUTOBOOST_MSVC, <= 1300)  // ETI workaround
