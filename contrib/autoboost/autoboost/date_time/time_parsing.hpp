...
#include "autoboost/date_time/date_parsing.hpp"
...
namespace date_time {
  ...
  str_from_delimited_time_duration(const std::basic_string<char_type>& s)
  {
    ...
    for(tokenizer_iterator beg=tok.begin(); beg!=tok.end();++beg){
      switch(pos) {
      case 0: {
        hour = autoboost::lexical_cast<int>(*beg);
        break;
      }
      case 1: {
        min = autoboost::lexical_cast<unsigned short>(*beg);
        break;
      }
      case 2: {
        sec = autoboost::lexical_cast<unsigned short>(*beg);
        break;
      };
      case 3: {
        ...
        if(digits >= precision) {
          // drop excess digits
          fs = autoboost::lexical_cast<autoboost::int64_t>(beg->substr(0, precision));
        }
        else {
          fs = autoboost::lexical_cast<autoboost::int64_t>(*beg);
        }
#endif
        if(digits < precision){
        ...
        case 3:
          {
            ...
            if(digits >= precision) {
              // drop excess digits
              fs = autoboost::lexical_cast<autoboost::int64_t>(char_digits.substr(0, precision));
            }
            else if(digits == 0) {
              fs = 0; // lexical_cast doesn't like empty strings
            }
            else {
              fs = autoboost::lexical_cast<autoboost::int64_t>(char_digits);
            }
#endif
            if(digits < precision){
              // trailing zeros get dropped from the string,
