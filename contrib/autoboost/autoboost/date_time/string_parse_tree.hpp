#ifndef AUTOBOOST_DATE_TIME_STRING_PARSE_TREE___HPP__
#define AUTOBOOST_DATE_TIME_STRING_PARSE_TREE___HPP__
...
#include "autoboost/lexical_cast.hpp" //error without?
#include "autoboost/algorithm/string/case_conv.hpp"
#include <map>
#include <string>
#include <vector>
#include <algorithm>
...
struct string_parse_tree
{
  ...
  string_parse_tree(collection_type names, unsigned int starting_point=0)
  {
    ...
    while (index != names.size() ) {
      string_type s = autoboost::algorithm::to_lower_copy(names[index]);
