...
#include <autoboost/algorithm/string/erase.hpp>
#include <autoboost/algorithm/string/replace.hpp>
#include <autoboost/date_time/compiler_config.hpp>
#include <autoboost/date_time/date_facet.hpp>
#include <autoboost/date_time/string_convert.hpp>
#include <autoboost/date_time/special_defs.hpp>
#include <autoboost/date_time/time_resolution_traits.hpp> // absolute_value
...
namespace date_time {
  ...
  class AUTOBOOST_SYMBOL_VISIBLE time_facet :
   ...
   public:
    ...
    typedef autoboost::date_time::period<time_type,time_duration_type> period_type;
    typedef autoboost::date_time::date_facet<typename time_type::date_type, CharT, OutItrT> base_type;
                ...
                const time_type& time_arg) const
    {
      ...
      autoboost::algorithm::replace_all(local_format,
        autoboost::as_literal(formats_type::full_24_hour_time_format),
        autoboost::as_literal(formats_type::full_24_hour_time_expanded_format));
      autoboost::algorithm::replace_all(local_format,
        autoboost::as_literal(formats_type::short_24_hour_time_format),
        autoboost::as_literal(formats_type::short_24_hour_time_expanded_format));
      ...
      if (local_format.find(posix_zone_string_format) != string_type::npos) {
        if(time_arg.zone_abbrev().empty()) {
          ...
          autoboost::algorithm::erase_all(local_format, posix_zone_string_format);
        }
        else{
          autoboost::algorithm::replace_all(local_format,
      ...
      if (local_format.find(zone_name_format) != string_type::npos) {
        if(time_arg.zone_name().empty()) {
          ...
          ss << ' ' << zone_name_format;
          autoboost::algorithm::erase_all(local_format, ss.str());
        }
        else{
          autoboost::algorithm::replace_all(local_format,
                                        zone_name_format,
      ...
      if (local_format.find(zone_iso_extended_format) != string_type::npos) {
        if(time_arg.zone_name(true).empty()) {
          ...
          autoboost::algorithm::erase_all(local_format, zone_iso_extended_format);
        }
        else{
          autoboost::algorithm::replace_all(local_format,
      ...
      if (local_format.find(zone_iso_format) != string_type::npos) {
        if(time_arg.zone_abbrev(true).empty()) {
          ...
          autoboost::algorithm::erase_all(local_format, zone_iso_format);
        }
        else{
          autoboost::algorithm::replace_all(local_format,
                                        zone_iso_format,
      ...
      if (local_format.find(fractional_seconds_or_none_format) != string_type::npos) {
        ...
        if (frac_str.size()) {
          ...
          autoboost::algorithm::erase_all(local_format,
                                      fractional_seconds_or_none_format);
        }
      }
                ...
                const time_duration_type& time_dur_arg) const
    {
      ...
      if (time_dur_arg.is_negative()) {
        ...
        autoboost::algorithm::replace_all(format,
        ...
        autoboost::algorithm::erase_all(format, duration_sign_negative_only);
        // remove all the %+ in the string with '+'
        autoboost::algorithm::replace_all(format,
                                      duration_sign_always,
                                      positive_sign);
      ...
      if (format.find(unrestricted_hours_format) != string_type::npos) {
        ...
        autoboost::algorithm::replace_all(format, unrestricted_hours_format, hours_str);
      }
      ...
      if (format.find(hours_format) != string_type::npos) {
        if (hours_str.empty())
          hours_str = hours_as_string(time_dur_arg);
        AUTOBOOST_ASSERT(hours_str.length() <= 2);
        autoboost::algorithm::replace_all(format, hours_format, hours_str);
      }
      ...
      if (format.find(fractional_seconds_or_none_format) != string_type::npos) {
        ...
        if (frac_str.size()) {
          ...
          autoboost::algorithm::erase_all(format,
                                      fractional_seconds_or_none_format);
        }
      }
  ...
  protected:
    ...
    integral_as_string(IntT val, int width = 2)
    {
      ...
      char_type buff[34];
      ss << _i64toa(static_cast<autoboost::int64_t>(val), buff, 10);
#else
      ss << val;
#endif
      return ss.str();
  ...
  class AUTOBOOST_SYMBOL_VISIBLE time_input_facet :
    ...
    public:
      ...
      typedef autoboost::date_time::period<time_type,time_duration_type> period_type;
      typedef autoboost::date_time::date_input_facet<typename time_type::date_type, CharT, InItrT> base_type;
    ...
    protected:
      ...
      InItrT check_special_value(InItrT& sitr,InItrT& stream_end, temporal_type& tt, char_type c='\0') const
      {
        ...
        if(mr.current_match == match_results::PARSE_ERROR) {
          ...
          autoboost::throw_exception(std::ios_base::failure("Parse failed. No match found for '" + tmp + "'"));
          AUTOBOOST_DATE_TIME_UNREACHABLE_EXPRESSION(return sitr); // should never reach
        }
