...
#include <autoboost/date_time/posix_time/posix_time.hpp> //todo remove?
...
namespace local_time {
  ...
  struct AUTOBOOST_SYMBOL_VISIBLE ambiguous_result : public std::logic_error
  {
  ...
  struct AUTOBOOST_SYMBOL_VISIBLE time_label_invalid : public std::logic_error
  {
    time_label_invalid (std::string const& msg = std::string()) :
      std::logic_error(std::string("Time label given is invalid: " + msg)) {}
  };
  struct AUTOBOOST_SYMBOL_VISIBLE dst_not_valid: public std::logic_error
  {
   ...
   * required to be in the form of a autoboost::shared_ptr<time_zone_base>.
   */
  template<class utc_time_=posix_time::ptime,
           class tz_type=date_time::time_zone_base<utc_time_,char> >
  class AUTOBOOST_SYMBOL_VISIBLE local_date_time_base :  public date_time::base_time<utc_time_,
                                                            autoboost::posix_time::posix_time_system> {
  public:
    ...
    local_date_time_base(utc_time_type t,
                         autoboost::shared_ptr<tz_type> tz) :
    ...
    local_date_time_base(date_type d,
      ...
      zone_(tz)
    {
      if(tz != autoboost::shared_ptr<tz_type>() && tz->has_dst()){
        ...
        if(result == invalid_time_label){
          ...
          ss << "time given: " << d << ' ' << td;
          autoboost::throw_exception(time_label_invalid(ss.str()));
        }
          ...
          autoboost::throw_exception(dst_not_valid(ss.str()));
        }
      ...
      zone_(tz)
    {
      ...
      if(result == ambiguous) {
        if(calc_option == EXCEPTION_ON_ERROR){
          std::ostringstream ss;
          ss << "time given: " << d << ' ' << td;
          autoboost::throw_exception(ambiguous_result(ss.str()));
        }
        else{ // NADT on error
    ...
    explicit local_date_time_base(const autoboost::date_time::special_values sv,
                                  autoboost::shared_ptr<tz_type> tz = autoboost::shared_ptr<tz_type>()) :
      date_time::base_time<utc_time_type, time_system_type>(utc_time_type(sv)),
      zone_(tz)
    {}

    //! returns time zone associated with calling instance
    autoboost::shared_ptr<tz_type> zone() const
    {
      return zone_;
    }
    ...
    bool is_dst() const
    {
      if(zone_ != autoboost::shared_ptr<tz_type>() && zone_->has_dst() && !this->is_special()) {
    ...
    {
      if(zone_ != autoboost::shared_ptr<tz_type>()){
    ...
    local_date_time_base local_time_in(autoboost::shared_ptr<tz_type> new_tz,
    ...
    /*bool operator==(const date_time::base_time<autoboost::posix_time::ptime,autoboost::posix_time::posix_time_system>& rhs) const
    { // fails due to rhs.time_ being protected
      return date_time::base_time<autoboost::posix_time::ptime,autoboost::posix_time::posix_time_system>::operator==(rhs);
  ...
  private:
    autoboost::shared_ptr<tz_type> zone_;
    //bool is_dst_;

    /*! Adjust the passed in time to UTC?
     */
    utc_time_type construction_adjustment(utc_time_type t,
                                          autoboost::shared_ptr<tz_type> z,
                                          bool dst_flag)
    {
      if(z != autoboost::shared_ptr<tz_type>()) {
