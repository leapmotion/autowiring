...
#include <autoboost/date_time/local_time/dst_transition_day_rules.hpp>
#include <autoboost/date_time/posix_time/posix_time.hpp>
...
namespace local_time{
  ...
  typedef autoboost::date_time::dst_adjustment_offsets<autoboost::posix_time::time_duration> dst_adjustment_offsets;
   ...
   * A autoboost::local_time::bad_offset exception will be thrown for:<br>
   ...
   * A autoboost::local_time::bad_adjustment exception will be thrown for:<br>
  ...
  public:
    ...
    typedef autoboost::char_separator<char_type, std::char_traits<char_type> > char_separator_type;
      ...
      dst_calc_rules_()
    {
      ...
      if (it == end)
        AUTOBOOST_THROW_EXCEPTION(std::invalid_argument("Could not parse time zone name"));
      calc_zone(*it++);
      if(has_dst_)
      {
        if (it == end)
          AUTOBOOST_THROW_EXCEPTION(std::invalid_argument("Could not parse DST begin time"));
        string_type dst_begin = *it++;

        if (it == end)
          AUTOBOOST_THROW_EXCEPTION(std::invalid_argument("Could not parse DST end time"));
    ...
    virtual string_type to_posix_string() const
    {
      ...
      ss.fill('0');
      autoboost::shared_ptr<dst_calc_rule> no_rules;
      // std
      ss << std_zone_abbrev();
      // offset
  ...
  private:
    ...
    autoboost::shared_ptr<dst_calc_rule> dst_calc_rules_;
    ...
    void calc_zone(const string_type& obj){
      ...
      if(sit != obj_end){
        ...
        {
          autoboost::throw_exception(bad_offset(posix_time::to_simple_string(base_utc_offset_)));
        }
      }
      ...
      if(sit != obj_end){
        ...
        {
          autoboost::throw_exception(bad_adjustment(posix_time::to_simple_string(dst_offsets_.dst_adjust_)));
        }
      }
    ...
    void calc_rules(const string_type& start, const string_type& end){
      ...
      {
        autoboost::throw_exception(bad_offset(posix_time::to_simple_string(dst_offsets_.dst_start_offset_)));
      }
      ...
      {
        autoboost::throw_exception(bad_offset(posix_time::to_simple_string(dst_offsets_.dst_end_offset_)));
      }
    }
...
} } // namespace autoboost::local_time
