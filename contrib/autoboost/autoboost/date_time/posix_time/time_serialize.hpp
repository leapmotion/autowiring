...
#include "autoboost/date_time/gregorian/greg_serialize.hpp"
...
AUTOBOOST_SERIALIZATION_SPLIT_FREE(autoboost::posix_time::ptime)
AUTOBOOST_SERIALIZATION_SPLIT_FREE(autoboost::posix_time::time_duration)
AUTOBOOST_SERIALIZATION_SPLIT_FREE(autoboost::posix_time::time_period)
          ...
          unsigned int /*version*/)
{
  ...
  if(!d.is_special()) {
    ar & make_nvp("ptime_time_duration", td);
    pt = autoboost::posix_time::ptime(d,td);
  }
  else {
    pt = autoboost::posix_time::ptime(d.as_special());
  }
                                ...
                                const unsigned int /*file_version*/)
{
  ...
  new(pt) autoboost::posix_time::ptime(autoboost::posix_time::not_a_date_time);
}
...
template<class Archive>
void load(Archive & ar,
          autoboost::posix_time::time_period & tp,
          unsigned int /*version*/)
{
  ...
  ar & make_nvp("time_period_begin", beg);
  ar & make_nvp("time_period_end", end);
  tp = autoboost::posix_time::time_period(beg, end);
}
                                ...
                                const unsigned int /*file_version*/)
{
  ...
  posix_time::ptime beg(d,td);
  posix_time::ptime end(d,td);
  new(tp) autoboost::posix_time::time_period(beg,end);
}
