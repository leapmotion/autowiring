...
#include <autoboost/date_time/c_time.hpp>
#include <autoboost/date_time/time_resolution_traits.hpp> // absolute_value
#include <autoboost/date_time/gregorian/conversion.hpp>
...
namespace posix_time {
  ...
  inline
  std::tm to_tm(const autoboost::posix_time::ptime& t) {
    std::tm timetm = autoboost::gregorian::to_tm(t.date());
    autoboost::posix_time::time_duration td = t.time_of_day();
  ...
  inline
  std::tm to_tm(const autoboost::posix_time::time_duration& td) {
    std::tm timetm;
  ...
  ptime ptime_from_tm(const std::tm& timetm) {
    autoboost::gregorian::date d = autoboost::gregorian::date_from_tm(timetm);
    return ptime(d, time_duration(timetm.tm_hour, timetm.tm_min, timetm.tm_sec));
  }


#if defined(AUTOBOOST_HAS_FTIME)
   ...
   *       autoboost::date_time::winapi::file_time type.
   */
  template< typename TimeT, typename FileTimeT >
  inline
  TimeT from_ftime(const FileTimeT& ft)
  {
    return autoboost::date_time::time_from_ftime<TimeT>(ft);
  }

#endif // AUTOBOOST_HAS_FTIME

} } //namespace autoboost::posix_time
