#ifndef AB_DATE_TIME_TZ_DB_BASE_HPP__
#define AB_DATE_TIME_TZ_DB_BASE_HPP__
...
namespace autoboost {
  namespace date_time {
    ...
    public:
      ...
      typedef autoboost::date_time::dst_adjustment_offsets<time_duration_type> dst_adjustment_offsets;
      ...
      void load_from_file(const std::string& pathspec)
      {
        ...
        if(!ifs){
          autoboost::throw_exception(data_not_accessible(pathspec));
        }
      ...
      bool add_record(const string_type& region,
                      autoboost::shared_ptr<time_zone_base_type> tz)
      {
      ...
      autoboost::shared_ptr<time_zone_base_type>
      time_zone_from_region(const string_type& region) const
      {
        ...
        if(record == m_zone_map.end()){
          return autoboost::shared_ptr<time_zone_base_type>(); //null pointer
        }
    ...
    private:
      typedef std::map<string_type, autoboost::shared_ptr<time_zone_base_type> > map_type;
      map_type m_zone_map;
      ...
      void split_rule_spec(int& nth, int& d, int& m, string_type rule) const
      {
        ...
        if ( std::distance ( tokens.begin(), tokens.end ()) != 3 ) {
          ...
          msg << "Expecting 3 fields, got "
              ...
              << " fields in line: " << rule;
          autoboost::throw_exception(bad_field_count(msg.str()));
        }
      ...
      bool parse_string(string_type& s)
      {
        ...
        typedef autoboost::token_iterator_generator<autoboost::escaped_list_separator<char_type>, string_type::const_iterator, string_type >::type token_iter_type;

        token_iter_type i = autoboost::make_token_iterator<string_type>(s.begin(), s.end(),autoboost::escaped_list_separator<char_type>());
        ...
        if (result.size() != expected_fields) {
          ...
          autoboost::throw_exception(bad_field_count(msg.str()));
          AUTOBOOST_DATE_TIME_UNREACHABLE_EXPRESSION(return false); // should never reach
        }
        ...
        autoboost::shared_ptr<rule_type> rules;
        ...
        if(has_dst){
          ...
          rules =
            autoboost::shared_ptr<rule_type>(parse_rules(result[START_DATE_RULE],
                                                     result[END_DATE_RULE]));
        }
        string_type id(result[ID]);
        autoboost::shared_ptr<time_zone_base_type> zone(new time_zone_type(names, utc_offset, adjust, rules));
