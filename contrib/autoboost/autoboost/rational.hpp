...
#define AUTOBOOST_RATIONAL_HPP

#include <autoboost/config.hpp>      // for AUTOBOOST_NO_STDC_NAMESPACE, AUTOBOOST_MSVC, etc
...
#include <autoboost/operators.hpp>   // for autoboost::addable etc
#include <cstdlib>               // for std::abs
#include <autoboost/call_traits.hpp> // for autoboost::call_traits
#include <autoboost/detail/workaround.hpp> // for AUTOBOOST_WORKAROUND
#include <autoboost/assert.hpp>      // for AUTOBOOST_ASSERT
#include <autoboost/integer/common_factor_rt.hpp> // for autoboost::integer::gcd, lcm
#include <limits>                // for std::numeric_limits
#include <autoboost/static_assert.hpp>  // for AUTOBOOST_STATIC_ASSERT
...
#ifndef AUTOBOOST_CONTROL_RATIONAL_HAS_GCD
#define AUTOBOOST_CONTROL_RATIONAL_HAS_GCD  1
#endif

namespace autoboost {

#if AUTOBOOST_CONTROL_RATIONAL_HAS_GCD
...
#endif  // AUTOBOOST_CONTROL_RATIONAL_HAS_GCD
...
namespace rational_detail{
   ...
   {
      AUTOBOOST_STATIC_CONSTANT(bool, value = ((std::numeric_limits<FromInt>::is_specialized && std::numeric_limits<FromInt>::is_integer
...
class rational
{
    ...
    AUTOBOOST_STATIC_ASSERT( ::std::numeric_limits<IntType>::is_specialized );

    // Helper types
    typedef typename autoboost::call_traits<IntType>::param_type param_type;
...
public:
    ...
    template <class T>
    AUTOBOOST_CONSTEXPR rational(const T& n, typename enable_if_c<
    ...
    AUTOBOOST_CONSTEXPR explicit
       ...
       (AUTOBOOST_THROW_EXCEPTION(bad_rational("bad rational: denormalized conversion")), 0)){}
    ...
    >::type assign(const T& n, const U& d)
    {
       if(!is_safe_narrowing_conversion(n) || !is_safe_narrowing_conversion(d))
          AUTOBOOST_THROW_EXCEPTION(bad_rational());
       return *this = rational<IntType>(static_cast<IntType>(n), static_cast<IntType>(d));
    }
    ...
    template <class T>
    typename autoboost::enable_if_c<rational_detail::is_compatible_integer<T, IntType>::value, rational&>::type operator+= (const T& i)
    {
       num += i * den;
       return *this;
    }
    ...
    typename autoboost::enable_if_c<rational_detail::is_compatible_integer<T, IntType>::value, rational&>::type operator/= (const T& i)
    {
       ...
       if(i == zero) AUTOBOOST_THROW_EXCEPTION(bad_rational());
...
#pragma parse_mfunc_templ reset
    ...
    template <class T>
    typename autoboost::enable_if_c<rational_detail::is_compatible_integer<T, IntType>::value, bool>::type operator< (const T& i) const
    {
       // Avoid repeated construction
       int_type const  zero(0);

       // Break value into mixed-fraction form, w/ always-nonnegative remainder
       AUTOBOOST_ASSERT(this->den > zero);
    ...
    template <class T>
    AUTOBOOST_CONSTEXPR typename autoboost::enable_if_c<rational_detail::is_compatible_integer<T, IntType>::value, bool>::type operator== (const T& i) const
    {
...
private:
    ...
    //
    template <class T>
    AUTOBOOST_CONSTEXPR static typename autoboost::enable_if_c<(std::numeric_limits<T>::digits > std::numeric_limits<IntType>::digits) && (std::numeric_limits<T>::is_signed == false), bool>::type is_safe_narrowing_conversion(const T& val)
    {
    ...
    //
    template <class T>
    AUTOBOOST_CONSTEXPR static typename autoboost::enable_if_c<(std::numeric_limits<T>::digits > std::numeric_limits<IntType>::digits) && (std::numeric_limits<T>::is_signed == true) && (std::numeric_limits<IntType>::is_signed == true), bool>::type is_safe_narrowing_conversion(const T& val)
    {
    ...
    //
    template <class T>
    AUTOBOOST_CONSTEXPR static typename autoboost::enable_if_c<(std::numeric_limits<T>::digits > std::numeric_limits<IntType>::digits) && (std::numeric_limits<T>::is_signed == true) && (std::numeric_limits<IntType>::is_signed == false), bool>::type is_safe_narrowing_conversion(const T& val)
    {
    ...
    //
    template <class T>
    AUTOBOOST_CONSTEXPR static typename autoboost::enable_if_c<(std::numeric_limits<T>::digits <= std::numeric_limits<IntType>::digits) && (std::numeric_limits<T>::is_signed == false) && (std::numeric_limits<IntType>::is_signed == true), bool>::type is_safe_narrowing_conversion(const T&)
    {
       return true;
    }
    //
    ...
    //
    template <class T>
    AUTOBOOST_CONSTEXPR static typename autoboost::enable_if_c<(std::numeric_limits<T>::digits <= std::numeric_limits<IntType>::digits) && (std::numeric_limits<T>::is_signed == false) && (std::numeric_limits<IntType>::is_signed == false), bool>::type is_safe_narrowing_conversion(const T&)
    {
       return true;
    }
    //
    ...
    //
    template <class T>
    AUTOBOOST_CONSTEXPR static typename autoboost::enable_if_c<(std::numeric_limits<T>::digits <= std::numeric_limits<IntType>::digits) && (std::numeric_limits<T>::is_signed == true) && (std::numeric_limits<IntType>::is_signed == true), bool>::type is_safe_narrowing_conversion(const T&)
    {
       return true;
    }
};
...
inline typename autoboost::enable_if_c <
...
void rational<IntType>::normalize()
{
    ...
    if (den < zero)
       AUTOBOOST_THROW_EXCEPTION(bad_rational("bad rational: non-zero singular denominator"));

    AUTOBOOST_ASSERT( this->test_invariant() );
}
