...
namespace
    ...
    clone_msvc_exception( void * src, cpp_exception_type const & et )
        ...
        if( void * dst = malloc(ti.size) )
            ...
            return autoboost::shared_ptr<void>(dst,exception_object_deleter(et));
            }
        else
            throw std::bad_alloc();
        }
    ...
    cloned_exception:
        ...
        autoboost::exception_detail::clone_base const *
        clone() const
            {
        ...
        if( is_cpp_exception(record) )
            ...
            if( is_cpp_exception(record) && record->ExceptionInformation[2] )
                try
                    ...
                    result = autoboost::exception_detail::clone_current_exception_result::success;
                    }
                catch(
                std::bad_alloc & )
                    {
                    result = autoboost::exception_detail::clone_current_exception_result::bad_alloc;
                    }
                catch(
                ... )
                    {
                    result = autoboost::exception_detail::clone_current_exception_result::bad_exception;
                    }
            }
