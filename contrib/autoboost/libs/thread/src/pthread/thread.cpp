...
namespace autoboost
{
    namespace detail
    {
        ...
        {
            autoboost::detail::thread_exit_function_base* func;
            thread_exit_callback_node* next;

            thread_exit_callback_node(autoboost::detail::thread_exit_function_base* func_,
        ...
        autoboost::detail::thread_data_base* get_current_thread_data()
        {
            autoboost::call_once(current_thread_tls_init_flag,create_current_thread_tls_key);
            return (autoboost::detail::thread_data_base*)pthread_getspecific(current_thread_tls_key);
        }
...
//                 }
                ...
                autoboost::lock_guard<autoboost::mutex> lock(thread_info->data_mutex);
    ...
    #   endif
                    timespec now2 = autoboost::detail::timespec_now();
                    if (autoboost::detail::timespec_ge(now2, ts))
                    {
                      return;
                    }
                  }
      ...
      namespace hidden
      {
        void AUTOBOOST_THREAD_DECL sleep_for(const timespec& ts)
        {
            ...
            {
              autoboost::this_thread::no_interruption_point::hidden::sleep_for(ts);
            }
        }
        ...
        void AUTOBOOST_THREAD_DECL sleep_until(const timespec& ts)
        {
            ...
            {
              autoboost::this_thread::no_interruption_point::hidden::sleep_until(ts);
            }
        }
      } // hidden
    } // this_thread
...
#   endif
    ...
    unsigned thread::physical_concurrency() AUTOBOOST_NOEXCEPT
    {
        ...
        try {
            ...
            while ( getline(proc_cpuinfo, line) ) {
                ...
                autoboost::split(key_val, line, autoboost::is_any_of(":"));
                ...
                autoboost::trim(key);
                autoboost::trim(value);

                if (key == physical_id) {
                    current_core_entry.first = autoboost::lexical_cast<unsigned>(value);
                    continue;
                }

                if (key == core_id) {
                    current_core_entry.second = autoboost::lexical_cast<unsigned>(value);
    ...
    namespace detail
    {
        ...
        void add_new_tss_node(void const* key,
                              autoboost::shared_ptr<tss_cleanup_function> func,
                              void* tss_data)
        {
            detail::thread_data_base* const current_thread_data(get_or_make_current_thread_data());
