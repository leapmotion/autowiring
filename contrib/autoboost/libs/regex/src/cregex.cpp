...
RegEx::RegEx()
{
   pdata = new AUTOBOOST_REGEX_DETAIL_NS::RegExData();
}

RegEx::RegEx(const RegEx& o)
{
   pdata = new AUTOBOOST_REGEX_DETAIL_NS::RegExData(*(o.pdata));
}
...
{
   autoboost::uint_fast32_t f = icase ? regex::normal | regex::icase : regex::normal;
...
{
   pdata->t = AUTOBOOST_REGEX_DETAIL_NS::RegExData::type_pc;
   pdata->pbase = p;
   const char* end = p;
   while(*end)++end;
...
unsigned int RegEx::Grep(GrepCallback cb, const char* p, match_flag_type flags)
{
   ...
   unsigned int result = regex_grep(AUTOBOOST_REGEX_DETAIL_NS::pred1(cb, this), p, end, pdata->e, flags);
   if(result)
      pdata->update();
   return result;
}
...
unsigned int RegEx::Grep(std::vector<std::string>& v, const char* p, match_flag_type flags)
{
   ...
   unsigned int result = regex_grep(AUTOBOOST_REGEX_DETAIL_NS::pred2(v, this), p, end, pdata->e, flags);
   if(result)
      pdata->update();
   return result;
}
...
unsigned int RegEx::Grep(std::vector<std::size_t>& v, const char* p, match_flag_type flags)
{
   ...
   unsigned int result = regex_grep(AUTOBOOST_REGEX_DETAIL_NS::pred3(v, p, this), p, end, pdata->e, flags);
   if(result)
      pdata->update();
   return result;
}
...
void BuildFileList(std::list<std::string>* pl, const char* files, bool recurse)
{
   ...
   if(recurse)
   {
      ...
      char buf[MAX_PATH];
      AUTOBOOST_REGEX_DETAIL_NS::overflow_error_if_not_zero(AUTOBOOST_REGEX_DETAIL_NS::strcpy_s(buf, MAX_PATH, start.root()));
      if(*buf == 0)
      {
         AUTOBOOST_REGEX_DETAIL_NS::overflow_error_if_not_zero(AUTOBOOST_REGEX_DETAIL_NS::strcpy_s(buf, MAX_PATH, "."));
         AUTOBOOST_REGEX_DETAIL_NS::overflow_error_if_not_zero(AUTOBOOST_REGEX_DETAIL_NS::strcat_s(buf, MAX_PATH, directory_iterator::separator()));
         AUTOBOOST_REGEX_DETAIL_NS::overflow_error_if_not_zero(AUTOBOOST_REGEX_DETAIL_NS::strcat_s(buf, MAX_PATH, "*"));
      }
      else
      {
         AUTOBOOST_REGEX_DETAIL_NS::overflow_error_if_not_zero(AUTOBOOST_REGEX_DETAIL_NS::strcat_s(buf, MAX_PATH, directory_iterator::separator()));
...
unsigned int RegEx::GrepFiles(GrepFileCallback cb, const char* files, bool recurse, match_flag_type flags)
{
   ...
   while(start != end)
   {
      ...
      pdata->t = AUTOBOOST_REGEX_DETAIL_NS::RegExData::type_pf;
      pdata->fbase = map.begin();
      AUTOBOOST_REGEX_DETAIL_NS::pred4 pred(cb, this, (*start).c_str());
...
{
   std::string result;
   AUTOBOOST_REGEX_DETAIL_NS::string_out_iterator<std::string> i(result);
...
std::size_t RegEx::Position(int i)const
{
   switch(pdata->t)
   {
   case AUTOBOOST_REGEX_DETAIL_NS::RegExData::type_pc:
      return pdata->m[i].matched ? pdata->m[i].first - pdata->pbase : RegEx::npos;
#ifndef AUTOBOOST_REGEX_NO_FILEITER
   case AUTOBOOST_REGEX_DETAIL_NS::RegExData::type_pf:
      return pdata->fm[i].matched ? pdata->fm[i].first - pdata->fbase : RegEx::npos;
#endif
   case AUTOBOOST_REGEX_DETAIL_NS::RegExData::type_copy:
      {
