...
struct BOOST_PP_CAT(result_of_is_callable_, BOOST_PP_ITERATION()) {
    ...
    static const bool value = (
        sizeof(result_of_no_type) == sizeof(detail::result_of_is_private_type(
            (autoboost::declval<wrapper_t>()(BOOST_PP_ENUM_BINARY_PARAMS(BOOST_PP_ITERATION(), autoboost::declval<T, >() BOOST_PP_INTERCEPT)), result_of_weird_type())
        ))
    );
...
struct cpp0x_result_of_impl<F(BOOST_PP_ENUM_PARAMS(BOOST_PP_ITERATION(),T)), false>
{
  typedef decltype(
    autoboost::declval<F>()(
      BOOST_PP_ENUM_BINARY_PARAMS(BOOST_PP_ITERATION(), autoboost::declval<T, >() BOOST_PP_INTERCEPT)
    )
  ) type;
};
