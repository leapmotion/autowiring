...
public:
    ...
    shared_ptr( autoboost::detail::sp_nullptr_t ) BOOST_NOEXCEPT : px( 0 ), pn() // never throws
    {
    }
...
// ... except in C++0x, move disables the implicit copy
    ...
    BOOST_NOEXCEPT : px( 0 ), pn( r.pn, autoboost::detail::sp_nothrow_tag() )
    {
        if( !pn.empty() )
        {
            px = r.px;
...
// Move support
    ...
    shared_ptr & operator=( autoboost::detail::sp_nullptr_t ) BOOST_NOEXCEPT // never throws
    {
        this_type().swap(*this);
        return *this;
    }
    ...
    {
        BOOST_ASSERT( px != 0 );
        BOOST_ASSERT( i >= 0 && ( i < autoboost::detail::sp_extent< T >::value || autoboost::detail::sp_extent< T >::value == 0 ) );
...
#include <boost/smart_ptr/detail/operator_bool.hpp>
    ...
    void * _internal_get_deleter( autoboost::detail::sp_typeinfo const & ti ) const BOOST_NOEXCEPT
    {
...
template<class T> inline bool operator==( shared_ptr<T> const & p, autoboost::detail::sp_nullptr_t ) BOOST_NOEXCEPT
{
    return p.get() == 0;
}

template<class T> inline bool operator==( autoboost::detail::sp_nullptr_t, shared_ptr<T> const & p ) BOOST_NOEXCEPT
{
    return p.get() == 0;
}
...
template< class T > std::size_t hash_value( autoboost::shared_ptr<T> const & p ) BOOST_NOEXCEPT
{
