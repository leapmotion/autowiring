...
template<class T> inline bool operator==( scoped_array<T> const & p, autoboost::detail::sp_nullptr_t ) BOOST_NOEXCEPT
{
    return p.get() == 0;
}

template<class T> inline bool operator==( autoboost::detail::sp_nullptr_t, scoped_array<T> const & p ) BOOST_NOEXCEPT
{
    return p.get() == 0;
}
