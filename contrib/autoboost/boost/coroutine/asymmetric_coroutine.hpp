...
# endif
    ...
    {
        push_coroutine tmp( autoboost::move( other) );
        swap( tmp);
        return * this;
    }
...
# ifdef BOOST_MSVC
        ...
        impl_( 0)
    {
        ...
        impl_ = new ( internal_stack_ctx.sp) object_t(
                autoboost::forward< coroutine_fn >( fn), attrs, stack_ctx, internal_stack_ctx, stack_alloc); 
        BOOST_ASSERT( impl_);
        impl_->pull();
    }
...
# endif
        ...
        impl_( 0)
    {
        ...
        // placement new for internal coroutine
        impl_ = new ( internal_stack_ctx.sp) object_t(
                    autoboost::forward< Fn >( fn), attrs, stack_ctx, internal_stack_ctx, stack_alloc); 
        BOOST_ASSERT( impl_);
        impl_->pull();
    }
    ...
    {
        pull_coroutine tmp( autoboost::move( other) );
        swap( tmp);
        return * this;
    }
...
{ return autoboost::begin( c); }

template< typename R >
typename pull_coroutine< R >::iterator
end( pull_coroutine< R > & c)
{ return autoboost::end( c); }
