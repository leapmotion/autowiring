...
#include <boost/type_traits/is_floating_point.hpp> // for autoboost::is_floating_point;
#include <boost/type_traits/is_integral.hpp> // for autoboost::is_integral
#include <boost/type_traits/is_convertible.hpp> // for autoboost::is_convertible
#include <boost/type_traits/is_same.hpp>// for autoboost::is_same
#include <boost/type_traits/remove_cv.hpp>// for autoboost::remove_cv
// Boost Template meta programming:
#include <boost/mpl/if.hpp> // for autoboost::mpl::if_c.
#include <boost/mpl/and.hpp> // for autoboost::mpl::if_c.
#include <boost/mpl/or.hpp> // for autoboost::mpl::if_c.
#include <boost/mpl/not.hpp> // for autoboost::mpl::if_c.
  ...
  namespace math
  {
    namespace tools
    {
      ...
      struct promote_args
      {
         ...
         //
         BOOST_STATIC_ASSERT_MSG((0 == ::autoboost::is_same<type, long double>::value), "Sorry, but this platform does not have sufficient long double support for the special functions to be reliably implemented.");
#endif
      };
