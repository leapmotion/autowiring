...
prg_exec_monitor_main( int (*cpp_main)( int argc, char* argv[] ), int argc, char* argv[] )
{
    ...
    try {
        autoboost::unit_test::const_string p( std::getenv( "BOOST_TEST_CATCH_SYSTEM_ERRORS" ) );
        ::autoboost::execution_monitor ex_mon;

        ex_mon.p_catch_system_errors.value = p != "no";
        
        result = ex_mon.execute( 
            ::autoboost::unit_test::callback0<int>( cpp_main_caller( cpp_main, argc, argv ) ) );
        
        if( result == 0 )
            result = ::autoboost::exit_success;
        else if( result != ::autoboost::exit_success ) {
            std::cout << "\n**** error return code: " << result << std::endl;
            result = ::autoboost::exit_failure;
        }
    }
    catch( ::autoboost::execution_exception const& exex ) {
        std::cout << "\n**** exception(" << exex.code() << "): " << exex.what() << std::endl;
        result = ::autoboost::exit_exception_failure;
    }
    catch( ::autoboost::system_error const& ex ) {
    ...
    if( result != ::autoboost::exit_success ) {
        ...
        ::autoboost::unit_test::const_string p( std::getenv( "BOOST_PRG_MON_CONFIRM" ) );
        if( p != "no" ) { 
...
{
    return ::autoboost::prg_exec_monitor_main( &cpp_main, argc, argv );
}
